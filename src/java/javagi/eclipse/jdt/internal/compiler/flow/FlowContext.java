...
import javagi.eclipse.jdt.internal.compiler.ast.LabeledStatement;
import javagi.eclipse.jdt.internal.compiler.ast.Reference;
	...
	while (traversedContext != null) {
		...
		if (traversedContext instanceof ExceptionHandlingFlowContext) {
			...
			if ((caughtExceptions = exceptionContext.handledExceptions) != Binding.NO_EXCEPTIONS) {
				...
				for (int caughtIndex = 0, caughtCount = caughtExceptions.length;
				    ...
				    int state = caughtException == null 
				        ...
				        : Scope.compareTypes(scope.getTypeEnvironment(), raisedException, caughtException);
					switch (state) {
			...
			if (exceptionContext.isMethodContext) {
				if (raisedException.isUncheckedException(scope.getTypeEnvironment(), false))
					return;
				...
				if (exceptionContext.associatedNode instanceof AbstractMethodDeclaration){
					...
					if (method.isConstructor() && method.binding.declaringClass.isAnonymousType()){
						...
						exceptionContext.mergeUnhandledException(scope.getTypeEnvironment(), raisedException);
