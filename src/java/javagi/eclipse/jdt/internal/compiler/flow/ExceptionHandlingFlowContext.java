...
import javagi.eclipse.jdt.internal.compiler.ast.TryStatement;
import javagi.eclipse.jdt.internal.compiler.codegen.ObjectCache;
...
public ExceptionHandlingFlowContext(
	...
	for (int i = 0; i < count; i++) {
		...
		if (handledException.isUncheckedException(scope.getTypeEnvironment(), true)) {
...
public void complainIfUnusedExceptionHandlers(AbstractMethodDeclaration method) {
	...
	if (scope.compilerOptions().
		...
		for (int i = 0; i < docCommentReferencesLength; i++) {
			docCommentReferences[i] = method.javadoc.exceptionReferences[i].getResolvedType();
		}
	}
 ...
 */
public void mergeUnhandledException(javagi.compiler.TypeEnvironment env, TypeBinding newException){
	...
	for(int i = this.extendedExceptions.size()-1; i >= 0; i--){
		switch(Scope.compareTypes(env, newException, (TypeBinding)this.extendedExceptions.get(i))){
 ...
 * exception handlers.
 * @see javagi.eclipse.jdt.internal.compiler.flow.FlowContext#subroutine()
 */
@Override
