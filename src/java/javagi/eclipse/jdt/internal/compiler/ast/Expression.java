...
import javagi.eclipse.jdt.internal.compiler.lookup.WildcardBinding;
...
public abstract class Expression extends Statement {
	...
	protected TypeBinding resolvedType;
...
public final boolean checkCastTypesCompatibility(Scope scope, TypeBinding castType, TypeBinding expressionType, Expression expression) {
	...
	if (castType.isBaseType()) {
		if (expressionType.isBaseType()) {
			...
			if (expressionType.isCompatibleWith(scope, castType)
				...
				return true;
							...
							&& scope.environment().computeBoxingType(scope.getTypeEnvironment(), expressionType).isCompatibleWith(scope, castType)) { // unboxing - only widening match is allowed
			tagAsUnnecessaryCast(scope, castType);  
			return true;
		}
		return false;
						...
						&& scope.environment().computeBoxingType(scope.getTypeEnvironment(), expressionType).isCompatibleWith(scope, castType)) { // boxing - only widening match is allowed
		tagAsUnnecessaryCast(scope, castType);  
		return true;
	}
	...
	switch(expressionType.kind()) {
		...
		case Binding.ARRAY_TYPE :
			...
			switch (castType.kind()) {
				...
				case Binding.TYPE_PARAMETER : 
					...
					TypeBinding match = expressionType.findSuperTypeOriginatingFrom(scope.getTypeEnvironment(), castType);
					if (match == null) {
		...
		case Binding.INTERSECTION_TYPE :
			match = expressionType.findSuperTypeOriginatingFrom(scope.getTypeEnvironment(), castType);
			if (match != null) {
				return checkUnsafeCast(scope, castType, expressionType, match, false);
			}
			// recursively on the type variable upper bound
			...
			if (expressionType.isInterface()) {
				switch (castType.kind()) {
					...
					default :
						if (castType.isInterface()) {
							...
							match = interfaceType.findSuperTypeOriginatingFrom(scope.getTypeEnvironment(), castType);
							if (match != null) {
								return checkUnsafeCast(scope, castType, interfaceType, match, false);
							}
							tagAsNeedCheckCast();
							match = castType.findSuperTypeOriginatingFrom(scope.getTypeEnvironment(), interfaceType);
							if (match != null) {
							...
							if (use15specifics) {
								...
								if (interfaceType.hasIncompatibleSuperType(scope.getTypeEnvironment(), (ReferenceBinding)castType))
									return false;
							} else {
							...
							tagAsNeedCheckCast();
							match = castType.findSuperTypeOriginatingFrom(scope.getTypeEnvironment(), expressionType);
							if (match != null) {
							...
							if (use15specifics) {
								checkUnsafeCast(scope, castType, expressionType, null /*no match*/, true);
								// ensure there is no collision between both interfaces: i.e. I1 extends List<String>, I2 extends List<Object>
								if (((ReferenceBinding)castType).hasIncompatibleSuperType(scope.getTypeEnvironment(), (ReferenceBinding) expressionType)) {
									return false;
								}
							}
							return true;
				...
				switch (castType.kind()) {
					...
					default :
						if (castType.isInterface()) {
							// ( INTERFACE ) CLASS
						    /* cannot reject this in JavaGI
							ReferenceBinding refExprType = (ReferenceBinding) expressionType;
							match = refExprType.findSuperTypeOriginatingFrom(scope.getTypeEnvironment(), castType);
							if (match != null) {
								return checkUnsafeCast(scope, castType, expressionType, match, false);
							}
							...
							if (use15specifics) {
								...
								// ensure there is no collision between both interfaces: i.e. I1 extends List<String>, I2 extends List<Object>
								if (refExprType.hasIncompatibleSuperType(scope.getTypeEnvironment(), (ReferenceBinding) castType))
									return false;
							}	
					        */
						    tagAsNeedCheckCast();
...
public void computeConversion(Scope scope, TypeBinding runtimeType, TypeBinding compileTimeType) {
	...
	if (runtimeType != TypeBinding.NULL && runtimeType.isBaseType()) {
		if (!compileTimeType.isBaseType()) {
			TypeBinding unboxedType = scope.environment().computeBoxingType(scope.getTypeEnvironment(), compileTimeType);
		...
		TypeBinding boxedType = scope.environment().computeBoxingType(scope.getTypeEnvironment(), runtimeType);
		if (boxedType == runtimeType) // Object o = 12;
			boxedType = compileTimeType; 
		this.implicitConversion = TypeIds.BOXING | (boxedType.id << 4) + compileTimeType.id;
		scope.problemReporter().autoboxing(this, compileTimeType, scope.environment().computeBoxingType(scope.getTypeEnvironment(), boxedType));
		return;
	...
	if ((compileTimeTypeID = compileTimeType.id) == TypeIds.NoId) { // e.g. ? extends String  ==> String (103227)
		compileTimeTypeID = compileTimeType.erasure(scope).id == TypeIds.T_JavaLangString ? TypeIds.T_JavaLangString : TypeIds.T_JavaLangObject;
	}		
...
public TypeBinding resolveTypeExpecting(BlockScope scope, TypeBinding expectedType) {
	...
	if (!expressionType.isCompatibleWith(scope, expectedType)) {
...
public final Expression toTypeReference() {
...
public final TypeBinding setResolvedType(TypeBinding resolvedType) {
    this.resolvedType = resolvedType;
    return resolvedType;
}

public final TypeBinding getResolvedType() {
    return resolvedType;
}

public boolean isType() {
    return this instanceof NameReference && (((NameReference) this).bits & Binding.TYPE) != 0;
}
