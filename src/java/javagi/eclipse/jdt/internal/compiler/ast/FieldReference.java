...
//				&& originalType != this.binding.type // commented out for JavaGI
				&& runtimeTimeType.id != TypeIds.T_JavaLangObject
				&& (originalType.tagBits & TagBits.HasTypeVariable) != 0) {
	    	TypeBinding targetType = (!compileTimeType.isBaseType() && runtimeTimeType.isBaseType()) 
	    		? compileTimeType  // unboxing: checkcast before conversion
	    		: runtimeTimeType;
	    	TypeEnvironment env = this.binding.declaringClass.getTypeEnvironment();
	        this.genericCast = originalType.genericCast(env, targetType, scope.getTypeEnvironment());
...
public void generateAssignment(BlockScope currentScope, CodeStream codeStream, Assignment assignment, boolean valueRequired) {
	...
	FieldBinding originalBinding = this.codegenBinding.original();
    Coercion.generateWrapperInvocation(currentScope.getTypeEnvironment(), 
            assignment.expression, this.codegenBinding.type,
            originalBinding.type,
            originalBinding.declaringClass.getTypeEnvironment(),
            this.codegenBinding.declaringClass.getSubstitution(),
            codeStream);
	fieldStore(
		codeStream,
		this.codegenBinding,
	...
	if (valueRequired 
		...
		if (this.codegenBinding.declaringClass == null) { // array length
			...
			if (this.syntheticAccessors == null || this.syntheticAccessors[FieldReference.READ] == null) {
				...
				ctType = this.codegenBinding.type;
			} else {
				codeStream.invokestatic(this.syntheticAccessors[FieldReference.READ]);
				ctType = this.syntheticAccessors[FieldReference.READ].returnType;
				ctTypeIsPossiblyTyvarInst = this.syntheticAccessors[FieldReference.READ].original().returnType.isTypeVariable();
			}
		...
		if (isThisReceiver) {
			if (isStatic){
				...
				if (this.binding.original().declaringClass != this.receiverType.erasure(currentScope)) {
...
public void generateCompoundAssignment(BlockScope currentScope, CodeStream codeStream, Expression expression, int operator, int assignmentImplicitConversion, boolean valueRequired) {
	...
	if (isStatic) {
		if (this.syntheticAccessors == null || this.syntheticAccessors[FieldReference.READ] == null) {
			codeStream.getstatic(this.codegenBinding);
			ctType = this.codegenBinding.type;
			ctTypeIsPossiblyTyvarInst = this.codegenBinding.typeIsPossiblyTyvarInst();
		} else {
			codeStream.invokestatic(this.syntheticAccessors[FieldReference.READ]);
			ctType = this.syntheticAccessors[FieldReference.READ].returnType;
			ctTypeIsPossiblyTyvarInst = this.syntheticAccessors[FieldReference.READ].returnTypeIsPossiblyTyvarInst();
		}
	} else {
		codeStream.dup();
		if (this.syntheticAccessors == null || this.syntheticAccessors[FieldReference.READ] == null) {
		...
		if ((options.targetJDK >= ClassFileConstants.JDK1_2
			...
			this.codegenBinding =
				currentScope.enclosingSourceType().getUpdatedFieldBinding(
					this.codegenBinding,
					(ReferenceBinding) this.receiverType.erasure(currentScope));
		}
	}		
}
...
public TypeBinding resolveType(BlockScope scope) {
	...
	if (receiverCast) {
		...
		if (((CastExpression)this.receiver).expression.getResolvedType() == this.receiverType) { 
	...
	TypeBinding receiverErasure = this.receiverType.erasure(scope);
	if (receiverErasure instanceof ReferenceBinding) {
		if (receiverErasure.findSuperTypeOriginatingFrom(scope.getTypeEnvironment(), fieldBinding.declaringClass) == null) {
	...
	if (fieldType != null) {
		if ((this.bits & ASTNode.IsStrictlyAssigned) == 0) {
			fieldType = fieldType.capture(scope);	// perform capture conversion if read access
		}
		this.setResolvedType(fieldType);
