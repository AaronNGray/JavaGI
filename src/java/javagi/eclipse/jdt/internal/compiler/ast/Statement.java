...
public abstract class Statement extends ASTNode {
	...
	public final void generateArguments(MethodBinding binding, Expression[] arguments, BlockScope currentScope, CodeStream codeStream) {
	    generateArguments(binding, arguments, currentScope, codeStream, null);
	}
	
    public final void generateArguments(MethodBinding binding, Expression[] arguments, BlockScope currentScope, CodeStream codeStream, boolean[] unwrapArgI) {
        //System.out.println(binding);
        Translation.extraArgumentsForMethodCall(binding, currentScope, codeStream);
        if (arguments == null) {
            arguments = new Expression[]{};
        } 
        for (int i = 0; i < binding.parameters.length; i++) {
            generateArgument(binding, arguments, i, currentScope, codeStream);
            if (unwrapArgI != null && i < unwrapArgI.length && unwrapArgI[i]) Translation.unwrap(codeStream);
        }
    }
	 ...
	 */
	public final void generateArgument(MethodBinding binding, Expression[] arguments, int ix, BlockScope currentScope, CodeStream codeStream) {
	    if (! binding.isVarargs() || ix < binding.parameters.length - 1) {
	        if (ix >= arguments.length) return;
	        arguments[ix].generateCode(currentScope, codeStream, true);
	        Coercion.generateWrapperInvocation(currentScope.getTypeEnvironment(),
	                                           arguments[ix],
	                                           binding.parameters[ix],
	                                           binding.original().parameters[ix],
	                                           binding.original().getTypeEnvironment(),
	                                           TySubstJ.make(currentScope.environment(), binding.original(), binding),
	                                           codeStream);
	    } else {
            ...
            ArrayBinding codeGenVarArgsType = (ArrayBinding) binding.parameters[varArgIndex].erasure(currentScope);
            ...
            if (argLength > paramLength) {
                ...
                TypeBinding lastType = arguments[varArgIndex].getResolvedType();
                if (lastType == TypeBinding.NULL
                    || (varArgsType.dimensions() == lastType.dimensions()
                        && lastType.isCompatibleWith(currentScope, varArgsType))) {
