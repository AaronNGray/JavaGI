			...
			if (isEqualOperator) {
				switch ((left.implicitConversion & IMPLICIT_CONVERSION_MASK) >> 4) { // operand runtime type
					...
					case T_double :
						...
						break;
					default :
					    Translation.javaGIEquality(codeStream, falseLabel, false, left, right);
				}
			} else {
				switch ((left.implicitConversion & IMPLICIT_CONVERSION_MASK) >> 4) { // operand runtime type
					case T_int :
					...
					case T_double :
						...
						break;
					default :
					    Translation.javaGIEquality(codeStream, falseLabel, true, left, right);
				}				
			}
			// comparison is TRUE 
			codeStream.iconst_1();
				...
				if (trueLabel != null) {
					...
					switch ((left.implicitConversion & IMPLICIT_CONVERSION_MASK) >> 4) { // operand runtime type
						...
						case T_double :
							...
							break;
						default :
						    Translation.javaGIEquality(codeStream, trueLabel, true, left, right);
					}
				}
			} else {
				// implicit falling through the TRUE case
    ...
    public TypeBinding resolveType(BlockScope scope) {
		...
		if (use15specifics) {
			if (leftType != TypeBinding.NULL && leftType.isBaseType()) {
				if (!rightType.isBaseType()) {
					rightType = scope.environment().computeBoxingType(scope.getTypeEnvironment(), rightType);
				}
			} else {
				if (rightType != TypeBinding.NULL && rightType.isBaseType()) {
					leftType = scope.environment().computeBoxingType(scope.getTypeEnvironment(), leftType);
				}
			}
		}
		// both base type
		...
		if ((!leftType.isBaseType() || leftType == TypeBinding.NULL) // cannot compare: Object == (int)0
			...
			if (unnecessaryLeftCast || unnecessaryRightCast) {
				TypeBinding alternateLeftType = unnecessaryLeftCast ? ((CastExpression)left).expression.getResolvedType() : leftType;
				TypeBinding alternateRightType = unnecessaryRightCast ? ((CastExpression)right).expression.getResolvedType() : rightType;
