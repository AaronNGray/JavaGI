...
/** 
 * @see javagi.eclipse.jdt.internal.compiler.ast.AbstractMethodDeclaration#analyseCode(javagi.eclipse.jdt.internal.compiler.lookup.ClassScope, javagi.eclipse.jdt.internal.compiler.flow.InitializationFlowContext, javagi.eclipse.jdt.internal.compiler.flow.FlowInfo)
 ...
 *
 * @param classScope javagi.eclipse.jdt.internal.compiler.lookup.ClassScope
 * @param classFile javagi.eclipse.jdt.internal.compiler.codegen.ClassFile
 */
@Override
public void generateCode(ClassScope classScope, ClassFile classFile, MethodPatch patch) {
	...
	try {
		...
		this.internalGenerateCode(classScope, classFile, patch);
	} catch (AbortMethod e) {
		if (e.compilationResult == CodeStream.RESTART_IN_WIDE_MODE) {
			...
			try {
				...
				this.internalGenerateCode(classScope, classFile, patch); // restart method generation
...
private void internalGenerateCode(ClassScope classScope, ClassFile classFile, MethodPatch patch) {
	...
	if ((!this.binding.isNative()) && (!this.binding.isAbstract())) {
		...
		// update the super constructor call (if necessary)
		if (this.isJavaGIImplementationConstructor) {
	        ReferenceBinding sup = declaringClass.superclass();
	        MethodBinding constr = sup.findNoArgConstructor();
	        if (constr != null) {
	            this.constructorCall.binding = constr;
	            this.constructorCall.codegenBinding = constr;
	        }
	    }
		...
		if (declaringClass.isNestedType()){
			...
			this.scope.computeLocalVariablePositions(patch, 1 + enumOffset,  codeStream);
		}
		...
		for (FreshLocal l : patch.extraParameters()) {
		    TypeBinding argType = l.getType();
            if (argType  == TypeBinding.LONG || argType == TypeBinding.DOUBLE) {
                argSlotSize += 2;
            } else {
                argSlotSize++;
            }		    
		...
		initializerScope.computeLocalVariablePositions(DefaultMethodPatch.theInstance, argSlotSize, codeStream); // offset by the argument size (since not linked to method scope)
		...
		patch.generateExtraEntryCode(codeStream);
...
//SW: JavaGI support
//////////////////////////////////////////////////////////////////////////////////////////////////
boolean isJavaGIImplementationConstructor;
