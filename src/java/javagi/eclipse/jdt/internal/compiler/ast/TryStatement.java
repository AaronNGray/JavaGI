	...
	if (this.subRoutineStartLabel == null) {
		...
		if (this.catchArguments != null) {
			...
			for (int i = 0; i < catchCount; i++) {
				...
				FlowInfo catchInfo;
				if (this.caughtExceptionTypes[i].isUncheckedException(scope.getTypeEnvironment(), true)) {
					catchInfo =
...
public void resolve(BlockScope upperScope) {
	...
	if (this.catchBlocks != null) {
		...
		for (int i = 0; i < length; i++) {
			...
			for (int j = 0; j < i; j++) {
				if (this.caughtExceptionTypes[i].isCompatibleWith(upperScope, argumentTypes[j])) {
