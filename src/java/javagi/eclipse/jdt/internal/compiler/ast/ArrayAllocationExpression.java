...
public class ArrayAllocationExpression extends Expression {
    ...
    public void generateCode(BlockScope currentScope, 	CodeStream codeStream, boolean valueRequired) {
		...
		if (explicitDimCount == 1) {
			...
			codeStream.newArray((ArrayBinding)this.getResolvedType());
		} else {
			// Multi-dimensional array
			codeStream.multianewarray(this.getResolvedType(), explicitDimCount);
		}
		if (valueRequired) {
    ...
    public TypeBinding resolveType(BlockScope scope) {
		...
		if (referenceType != null) {
			...
			this.setResolvedType(scope.createArrayType(referenceType, this.dimensions.length));

			// check the initializer
			if (this.initializer != null) {
				if ((this.initializer.resolveTypeExpecting(scope, this.getResolvedType())) != null)
					this.initializer.binding = (ArrayBinding)this.getResolvedType();
			}
