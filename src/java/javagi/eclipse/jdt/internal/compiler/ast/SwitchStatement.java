...
public class SwitchStatement extends Statement {
    ...
    public FlowInfo analyseCode(
	    ...
	    try {
			...
			final TypeBinding resolvedTypeBinding = this.expression.getResolvedType();
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream) {
	    ...
	    try {
			...
			final TypeBinding resolvedType = this.expression.getResolvedType();
	    ...
	    try {
				...
				checkType: {
					if (!expressionType.isValidBinding()) {
						...
						if (expression.isConstantValueOfTypeAssignableToType(expressionType, TypeBinding.INT))
							break checkType;
						if (expressionType.isCompatibleWith(upperScope, TypeBinding.INT))
							break checkType;
			...
			if (isEnumSwitch && defaultCase == null 
				...
				if (constantCount == caseCount // ignore diagnosis if unresolved constants
					...
					FieldBinding[] enumFields = ((ReferenceBinding)expressionType.erasure(upperScope)).fields();
