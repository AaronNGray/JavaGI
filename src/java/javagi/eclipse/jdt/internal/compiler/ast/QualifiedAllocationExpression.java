...
public class QualifiedAllocationExpression extends AllocationExpression {
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
		...
		checkCapturedLocalInitializationIfNecessary(
			(ReferenceBinding)(this.anonymousType == null 
				? this.binding.declaringClass.erasure(currentScope)
    ...
    public void manageEnclosingInstanceAccessIfNecessary(BlockScope currentScope, FlowInfo flowInfo) {
		...
		ReferenceBinding allocatedTypeErasure = (ReferenceBinding) this.binding.declaringClass.erasure(currentScope);
    ...
    public TypeBinding resolveType(BlockScope scope) {
		...
		if (hasError) {
			if (receiverType instanceof ReferenceBinding) {
				...
				if (receiverType.isValidBinding()) {
					...
					this.binding = scope.findMethod(referenceReceiver, TypeConstants.INIT, pseudoArgs, this, scope.getTypeEnvironment());
				...
				if (this.anonymousType != null) {
					...
					return this.setResolvedType(this.anonymousType.binding());
				}
			}
			return this.setResolvedType(receiverType);
		}
		if (this.anonymousType == null) {
			...
			if ((this.binding = scope.getConstructor(allocationType, argumentTypes, this, scope.getTypeEnvironment())).isValidBinding()) {
				...
				if (this.type != null && !this.type.getResolvedType().isValidBinding()) {
			...
			if (enclosingInstanceType.isCompatibleWith(scope, expectedType) || scope.isBoxingCompatibleWith(enclosingInstanceType, expectedType)) {
		...
		if (superType.isTypeVariable()) {
			...
			return this.setResolvedType(superType);
		}
		...
		this.setResolvedType(this.anonymousType.binding()); // 1.2 change
		if ((this.getResolvedType().tagBits & TagBits.HierarchyHasProblems) != 0) {
			return null; // stop secondary errors
		}
		MethodBinding inheritedBinding = scope.getConstructor(anonymousSuperclass, argumentTypes, this, scope.getTypeEnvironment());
		...
		if (this.enclosingInstance != null) {
			...
			if (targetEnclosing == null) {
				...
				return this.getResolvedType();
			} else if (!enclosingInstanceType.isCompatibleWith(scope, targetEnclosing) && !scope.isBoxingCompatibleWith(enclosingInstanceType, targetEnclosing)) {
