...
import javagi.eclipse.jdt.internal.compiler.flow.InitializationFlowContext;
...
public abstract class AbstractMethodDeclaration
		...
		if (this.thrownExceptions != null
			...
			if (length == thrownExceptionLength) {
				for (int i = 0; i < length; i++) {
					this.thrownExceptions[i].setResolvedType(this.binding.thrownExceptions[i]);
				}
			} else {
				int bindingIndex = 0;
				for (int i = 0; i < thrownExceptionLength && bindingIndex < length; i++) {
					...
					if (thrownException instanceof SingleTypeReference) {
						...
						if (CharOperation.equals(thrownExceptionTypeName, bindingCompoundName[lengthName - 1])) {
							thrownException.setResolvedType(thrownExceptionBinding);
							bindingIndex++;
						}
					} else {
	...
	public void generateCode(ClassScope classScope, ClassFile classFile, MethodPatch patch) {
		...
		try {
			...
			this.generateCode(classFile, patch);
		} catch (AbortMethod e) {
			...
			if (e.compilationResult == CodeStream.RESTART_IN_WIDE_MODE) {
				...
				try {
					...
					this.generateCode(classFile, patch); // restart method generation
	...
	public void generateCode(ClassFile classFile, MethodPatch patch) {
	    int modifiers = patch.patchModifiers(this.binding.modifiers);
		classFile.generateMethodInfoHeader(this.binding, modifiers);
		...
		if ((!this.binding.isNative()) && (!this.binding.isAbstract())) {
			...
			int initialOffset = this.binding.isStatic() ? 0 : 1;		    
			this.scope.computeLocalVariablePositions(patch, initialOffset, codeStream);
    ...
    public boolean isStaticInterfaceMethod() {
        return this.binding != null && this.binding.declaringClass.isInterface() && this.isStatic();
    }

    public boolean isStaticInterfaceOrImplementationMethod() {
        return this.binding != null && (this.binding.declaringClass.isInterface() ||  this.binding.declaringClass.isImplementation()) && this.isStatic();
    }
    public boolean isReceiverMethod() {
        return this.binding.declaringClass.isReceiver();
    }
    
    @Override
    public boolean referencesLocalVariables() {
        if (this.statements == null) return false;
        for (Statement stmt : this.statements) {
            if (stmt.referencesLocalVariables()) return true;
        }
        return false;
    }
}
