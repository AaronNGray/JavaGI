...
public class ExplicitConstructorCall extends Statement implements InvocationSite {
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream) {
		...
		try {
			...
			if (targetType.erasure(currentScope).id == TypeIds.T_JavaLangEnum || targetType.isEnum()) {
	...
	void manageEnclosingInstanceAccessIfNecessary(BlockScope currentScope, FlowInfo flowInfo) {
		ReferenceBinding superTypeErasure = (ReferenceBinding) this.binding.declaringClass.erasure(currentScope);
    ...
    public void resolve(BlockScope scope) {
		...
		try {
			...
			ReferenceBinding receiverType = scope.enclosingReceiverType(true);
			boolean rcvHasError = false;
			if (this.accessMode != ExplicitConstructorCall.This) {
				receiverType = receiverType.superclass();
				TypeReference superclassRef = scope.referenceType().superclass;
				if (superclassRef != null && superclassRef.getResolvedType() != null && !superclassRef.getResolvedType().isValidBinding()) {
					rcvHasError = true;
				}
			}
			if (receiverType != null) {
				// prevent (explicit) super constructor invocation from within enum
				if (this.accessMode == ExplicitConstructorCall.Super && receiverType.erasure(scope).id == TypeIds.T_JavaLangEnum) {
			...
			if (this.arguments != null) {
				...
				if (argHasError) {
					...
					this.binding = scope.findMethod(receiverType, TypeConstants.INIT, pseudoArgs, this, scope.getTypeEnvironment());
			...
			} else if (receiverType != null && receiverType.erasure(scope).id == TypeIds.T_JavaLangEnum) {
			...
			if ((this.binding = scope.getConstructor(receiverType, argumentTypes, this, scope.getTypeEnvironment())).isValidBinding()) {
