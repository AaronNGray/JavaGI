...
public class SingleNameReference extends NameReference implements OperatorIds {
        ...
        if (isCompound) { // check the variable part is initialized if blank
            ...
            case Binding.FIELD: // reading a field
                ...
                if ((fieldBinding = (FieldBinding) this.binding).isBlankFinal()
                        && currentScope
                                .needBlankFinalFieldInitializationCheck(fieldBinding)) {
            ...
            case Binding.LOCAL: // reading a local variable
                ...
                if (!flowInfo
                    ...
                    currentScope.problemReporter().uninitializedLocalVariable(
                            localBinding, this);
                    // we could improve error msg here telling
                    // "cannot use compound assignment on final local variable"
                }
                if (isReachable) {
        ...
        case Binding.LOCAL: // assigning to a local variable
            ...
            if (localBinding.isFinal()) {
                if ((this.bits & ASTNode.DepthMASK) == 0) {
                    // tolerate assignment to final local in unreachable code
                    // (45674)
                    if ((isReachable && isCompound)
                            || !localBinding.isBlankFinal()) {
                        currentScope.problemReporter()
                ...
                currentScope.problemReporter().parameterAssignment(
                        localBinding, this);
            }
    ...
    public TypeBinding checkFieldAccess(BlockScope scope) {
        ...
        if (fieldBinding.isStatic()) {
            ...
            if (declaringClass.isEnum()) {
                ...
                if (this.constant == Constant.NotAConstant
                        ...
                        && (sourceType == declaringClass || sourceType.superclass == declaringClass) // enum
                                                                                                     // constant
                                                                                                     // body
            ...
            if (scope.compilerOptions().getSeverity(
                ...
                scope.problemReporter().unqualifiedFieldAccess(this,
                        fieldBinding);
            }
        ...
        if ((this.bits & ASTNode.IsStrictlyAssigned) == 0
            ...
            scope.problemReporter().forwardReference(this, 0,
                    methodScope.enclosingSourceType());
    ...
    /**
     * @see javagi.eclipse.jdt.internal.compiler.ast.Expression#computeConversion(javagi.eclipse.jdt.internal.compiler.lookup.Scope,
     *      javagi.eclipse.jdt.internal.compiler.lookup.TypeBinding,
     *      javagi.eclipse.jdt.internal.compiler.lookup.TypeBinding)
     */
    @Override
    public void computeConversion(Scope scope, TypeBinding runtimeTimeType, TypeBinding compileTimeType) {
        if (runtimeTimeType == null || compileTimeType == null)
            return;			
        this.genericCast = SingleNameReference.computeConversion(this, scope, runtimeTimeType, compileTimeType, this.binding);
    ...
    public void generateAssignment(BlockScope currentScope,
        ...
        if (assignment.expression.isCompactableOperation()) {
            ...
            if ((operation.left instanceof SingleNameReference)
                ...
                // i = i + value, then use the variable on the right hand side,
                // since it has the correct implicit conversion
                variableReference
                        .generateCompoundAssignment(
                                currentScope,
                                codeStream,
                                this.syntheticAccessors == null ? null
                                        : this.syntheticAccessors[SingleNameReference.WRITE],
                                operation.right, operator,
                                operation.implicitConversion, valueRequired);
                if (valueRequired) {
                    codeStream
                            .generateImplicitConversion(assignment.implicitConversion);
                }
                return;
            }
            if ((operation.right instanceof SingleNameReference)
                    && ((operator == OperatorIds.PLUS) || (operator == OperatorIds.MULTIPLY)) // only
                                                                                              // commutative
                                                                                              // operations
                    && ((variableReference = (SingleNameReference) operation.right).binding == this.binding)
                    && (operation.left.constant != Constant.NotAConstant) // exclude
                                                                          // non
                                                                          // constant
                                                                          // expressions,
                                                                          // since
                                                                          ...
                                                                          // have
                                                                          // side-effect
                    && (((operation.left.implicitConversion & TypeIds.IMPLICIT_CONVERSION_MASK) >> 4) != TypeIds.T_JavaLangString) // exclude
                                                                                                                                   // string
                                                                                                                                   // concatenation
                                                                                                                                   // which
                                                                                                                                   // would
                                                                                                                                   // occur
                                                                                                                                   // backwards
                    && (((operation.right.implicitConversion & TypeIds.IMPLICIT_CONVERSION_MASK) >> 4) != TypeIds.T_JavaLangString)) { // exclude
                                                                                                                                       // string
                                                                                                                                       // concatenation
                                                                                                                                       // which
                                                                                                                                       // would
                                                                                                                                       // occur
                                                                                                                                       // backwards
                // i = value + i, then use the variable on the right hand side,
                // since it has the correct implicit conversion
                variableReference
                        .generateCompoundAssignment(
                                currentScope,
        ...
        case Binding.FIELD: // assigning to a field
            ...
            if (!(fieldBinding = (FieldBinding) this.codegenBinding).isStatic()) { // need
                                                                                   // a
                                                                                   // receiver?
                if ((this.bits & ASTNode.DepthMASK) != 0) {
                    ReferenceBinding targetType = currentScope
                            .enclosingSourceType()
                            .enclosingTypeAt(
                                    (this.bits & ASTNode.DepthMASK) >> ASTNode.DepthSHIFT);
                    Object[] emulationPath = currentScope.getEmulationPath(
                            targetType, true /* only exact match */, false/*
                                                                           * consider
                                                                           * enclosing
                                                                           * arg
                                                                           */);
                    codeStream.generateOuterAccess(emulationPath, this,
                            targetType, currentScope);
                } else {
            ...
            Coercion.generateWrapperInvocation(currentScope.getTypeEnvironment(), 
                                               assignment.expression, fieldBinding.type, 
                                               fieldBinding.original().type,
                                               fieldBinding.original().declaringClass.getTypeEnvironment(),
                                               fieldBinding.declaringClass.getSubstitution(),
                                               codeStream);
            fieldStore(
                    codeStream,
                    fieldBinding,
        ...
        case Binding.LOCAL: // assigning to a local variable
            ...
            if (localBinding.resolvedPosition != -1) {
                assignment.expression.generateCode(currentScope, codeStream,
                        true);
                Coercion.generateWrapperInvocation(currentScope.getTypeEnvironment(), 
                        assignment.expression, localBinding.type, 
                        null, //localBinding.originalType(currentScope.getTypeEnvironment()),
                        null,
                        null,
                        codeStream);
            } else {
                if (assignment.expression.constant != Constant.NotAConstant) {
                    // assigning an unused local to a constant value = no actual
                    ...
                    assignment.expression.generateCode(currentScope,
                            codeStream, true);
                    /*
                     * Even though the value may not be required, we force it to
                     * be produced, and discard it later on if it was actually
                     * not necessary, so as to provide the same behavior as
                     * JDK1.2beta3.
                     */
                    if (valueRequired) {
                        codeStream
                                .generateImplicitConversion(assignment.implicitConversion); // implicit
                                                                                            // conversion
                    } else {
                        if ((localBinding.type == TypeBinding.LONG)
                                || (localBinding.type == TypeBinding.DOUBLE)) {
                            codeStream.pop2();
                        } else {
                            codeStream.pop();
                        }
            ...
            if (localBinding.type.isArrayType()
                    && (assignment.expression.getResolvedType() == TypeBinding.NULL // arrayLoc
                                                                                    // =
                                                                                    // null
                    ...
                    && (((CastExpression) assignment.expression)
                            .innermostCastedExpression().getResolvedType() == TypeBinding.NULL)))) {
                codeStream.checkcast(localBinding.type);
            }

            // normal local assignment (since cannot store in outer local which
            // are final locations)
            codeStream.store(localBinding, valueRequired);
            if ((this.bits & ASTNode.FirstAssignmentToLocal) != 0) { // for
                                                                     // local
                                                                     // variable
                                                                     // debug
                                                                     // attributes
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream,
        ...
        int pc = codeStream.position;
        TypeBinding ctType = null;
        boolean ctTypeIsPossiblyTyvarInst = true;
        if (this.constant != Constant.NotAConstant) {
            if (valueRequired) {
                codeStream.generateConstant(this.constant,
            ...
            case Binding.FIELD: // reading a field
                ...
                if (fieldConstant != Constant.NotAConstant) {
                    ...
                    if (valueRequired) {
                        codeStream.generateConstant(fieldConstant,
                                this.implicitConversion);
                    }
                    codeStream.recordPositionsFrom(pc, this.sourceStart);
                    return;
                ...
                if (fieldBinding.isStatic()) {
                    if (!valueRequired
                            // if no valueRequired, still need possible
                            // side-effects of <clinit> invocation, if field
                            // belongs to different class
                            && ((FieldBinding) this.binding).original().declaringClass == this.actualReceiverType
                                    .erasure(currentScope)
                    ...
                    if ((this.syntheticAccessors == null)
                        ...
                        ctType = fieldBinding.type;
                        ctTypeIsPossiblyTyvarInst = fieldBinding.typeIsPossiblyTyvarInst();
                    } else {
                        codeStream
                                .invokestatic(this.syntheticAccessors[SingleNameReference.READ]);
                        ctType = this.syntheticAccessors[SingleNameReference.READ].returnType;
                        ctTypeIsPossiblyTyvarInst = this.syntheticAccessors[SingleNameReference.READ].returnTypeIsPossiblyTyvarInst();
                    }
                } else {
                    if (!valueRequired
                            && (this.implicitConversion & TypeIds.UNBOXING) == 0
                    ...
                    if ((this.bits & ASTNode.DepthMASK) != 0) {
                        ReferenceBinding targetType = currentScope
                                ...
                                .enclosingTypeAt(
                                        (this.bits & ASTNode.DepthMASK) >> ASTNode.DepthSHIFT);
                        Object[] emulationPath = currentScope
                                .getEmulationPath(targetType, true /*
                                                                    * only exact
                                                                    * match
                                                                    */, false/*
                                                                              * consider
            ...
            case Binding.LOCAL: // reading a local
                ...
                // outer local?
                if ((this.bits & ASTNode.DepthMASK) != 0) {
                    // outer local can be reached either through a synthetic arg
                    // or a synthetic field
                    VariableBinding[] path = currentScope
                            .getEmulationPath(localBinding);
                    codeStream.generateOuterAccess(path, this, localBinding,
                            currentScope);
                } else {
                    // regular local variable read
                    codeStream.load(localBinding);
                    ctType = localBinding.type;
                    ctTypeIsPossiblyTyvarInst = false;
                }
                break;
            default: // type
                codeStream.recordPositionsFrom(pc, this.sourceStart);
        ...
        if (this.genericCast != null) {
            ...
            Translation.javaGICast(currentScope, codeStream, this.genericCast, ctType, ctTypeIsPossiblyTyvarInst);
        ...
        if (valueRequired) {
            ...
            if (isUnboxing)
                codeStream.generateImplicitConversion(this.implicitConversion);
            switch (isUnboxing ? postConversionType(currentScope).id : this
                    .getResolvedType().id) {
            case T_long:
            case T_double:
                codeStream.pop2();
     ...
     * only one reference to the variable, which carries both synthetic
     * read/write accessors. The APIs with an extra argument is used whenever
     * there are two references to the same variable which are optimized in one
     * access: e.g "a = a + 1" optimized into "a++".
     */
    @Override
    ...
    /*
     * The APIs with an extra argument is used whenever there are two references
     * to the same variable which are optimized in one access: e.g "a = a + 1"
     * optimized into "a++".
     */
    public void generateCompoundAssignment(BlockScope currentScope,
            CodeStream codeStream, MethodBinding writeAccessor,
        ...
        case Binding.FIELD: // assigning to a field
            ...
            break;
        case Binding.LOCAL: // assigning to a local variable (cannot assign to
                            // outer local)
            LocalVariableBinding localBinding = (LocalVariableBinding) this.codegenBinding;
            // using incr bytecode if possible
            switch (localBinding.type.id) {
            case T_JavaLangString:
                codeStream.generateStringConcatenationAppend(currentScope,
                        this, expression);
                if (valueRequired) {
                    codeStream.dup();
                }
            ...
            case T_int:
                ...
                if (((assignConstant = expression.constant) != Constant.NotAConstant)
                        && (assignConstant.typeID() != TypeIds.T_float) // only
                                                                        // for
                                                                        // integral
                                                                        // types
                    ...
                    case PLUS:
                        ...
                        codeStream.iinc(localBinding.resolvedPosition,
                                increment);
                        if (valueRequired) {
                            codeStream.load(localBinding);
                        }
        ...
        case T_undefined:
            ...
            // java.lang.Object.
            codeStream.generateStringConcatenationAppend(currentScope, null,
                    expression);
            // no need for generic cast on previous #getfield since using Object
        ...
        switch (this.bits & ASTNode.RestrictiveFlagMASK) {
        case Binding.FIELD: // assigning to a field
            fieldStore(codeStream, (FieldBinding) this.codegenBinding,
                    writeAccessor, valueRequired);
            // no need for generic cast as value got dupped
            return;
        case Binding.LOCAL: // assigning to a local variable
            LocalVariableBinding localBinding = (LocalVariableBinding) this.codegenBinding;
    ...
    public void generatePostIncrement(BlockScope currentScope,
            ...
            boolean valueRequired) {
        TypeBinding ctType;
        boolean ctTypeIsPossiblyTyvarInst;
        switch (this.bits & ASTNode.RestrictiveFlagMASK) {
        case Binding.FIELD: // assigning to a field
            FieldBinding fieldBinding;
            if ((fieldBinding = (FieldBinding) this.codegenBinding).isStatic()) {
            ...
            codeStream
                    .generateImplicitConversion(postIncrement.preAssignImplicitConversion);
            fieldStore(
                    codeStream,
                    fieldBinding,
                    this.syntheticAccessors == null ? null
    ...
    /**
     * Returns the local variable referenced by this node. Can be a direct
     * reference (SingleNameReference) or thru a cast expression etc...
     */
    @Override
    ...
    public void manageEnclosingInstanceAccessIfNecessary(
            BlockScope currentScope, FlowInfo flowInfo) {
        if ((flowInfo.tagBits & FlowInfo.UNREACHABLE) == 0) {
            // If inlinable field, forget the access emulation, the code gen
            // will directly target it
            if (((this.bits & ASTNode.DepthMASK) == 0)
            ...
            if ((this.bits & ASTNode.RestrictiveFlagMASK) == Binding.LOCAL) {
                currentScope
                        .emulateOuterAccess((LocalVariableBinding) this.binding);
            }
        }
    }
    ...
    public void manageSyntheticAccessIfNecessary(BlockScope currentScope,
        ...
        if ((this.bits & Binding.FIELD) != 0) {
            ...
            if (((this.bits & ASTNode.DepthMASK) != 0)
                    ...
                    && codegenField.declaringClass.getPackage() != currentScope
                            .enclosingSourceType().getPackage()))) {
                if (this.syntheticAccessors == null)
                    this.syntheticAccessors = new MethodBinding[2];
                this.syntheticAccessors[isReadAccess ? SingleNameReference.READ
                        : SingleNameReference.WRITE] = ((SourceTypeBinding) currentScope
                        .enclosingSourceType()
                        .enclosingTypeAt(
                                (this.bits & ASTNode.DepthMASK) >> ASTNode.DepthSHIFT))
                        .addSyntheticMethod(codegenField, isReadAccess);
                currentScope.problemReporter().needToEmulateFieldAccess(
                        codegenField, this, isReadAccess);
                return;
            }
            ...
            if (fieldBinding.declaringClass != this.actualReceiverType
                ...
                if ((options.targetJDK >= ClassFileConstants.JDK1_2
                        && (options.complianceLevel >= ClassFileConstants.JDK1_4 || !fieldBinding
                                .isStatic()) && fieldBinding.declaringClass.id != TypeIds.T_JavaLangObject) // no
                                                                                                            // change
                                                                                                            // for
                                                                                                            // Object
                                                                                                            // fields
    ...
    public TypeBinding postConversionType(Scope scope) {
        ...
        if ((this.implicitConversion & TypeIds.BOXING) != 0) {
            convertedType = scope.environment().computeBoxingType(
                    scope.getTypeEnvironment(), convertedType);
        }
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (this.actualReceiverType != null) {
            this.binding = scope.getField(this.actualReceiverType, this.token,
                    this);
        } else {
            this.actualReceiverType = scope.enclosingReceiverType();
            this.binding = scope.getBinding(this.token, this.bits
                    & ASTNode.RestrictiveFlagMASK, this, true /* resolve */);
        }
        this.codegenBinding = this.binding;
        if (this.binding.isValidBinding()) {
            switch (this.bits & ASTNode.RestrictiveFlagMASK) {
            case Binding.VARIABLE: // =========only variable============
            case Binding.VARIABLE | Binding.TYPE: // ====both variable and
                                                  // type============
                if (this.binding instanceof VariableBinding) {
                    ...
                    if (this.binding instanceof LocalVariableBinding) {
                        ...
                        this.constant = (this.bits & ASTNode.IsStrictlyAssigned) == 0 ? variable
                                .constant()
                    ...
                    if (variableType != null) {
                        this
                                .setResolvedType(variableType = (((this.bits & ASTNode.IsStrictlyAssigned) == 0) ? variableType
                                        .capture(scope)
                                        : variableType));
                        if ((variableType.tagBits & TagBits.HasMissingType) != 0) {
                            if ((this.bits & Binding.LOCAL) == 0) {
                                // only complain if field reference (for local,
                                // its type got flagged already)
                                scope.problemReporter().invalidType(this,
                                        variableType);
                            }
                            return null;
        ...
        // error scenarii
        return this.setResolvedType(this.reportError(scope));
    }
    ...
    public static TypeBinding computeConversion(ASTNode location, Scope scope,
            TypeBinding runtimeTimeType, TypeBinding compileTimeType,
            Binding binding) {
...
//                  && originalType != field.type // commented out for JavaGI
                    && runtimeTimeType.id != TypeIds.T_JavaLangObject
                    && (originalType.tagBits & TagBits.HasTypeVariable) != 0) {
                TypeBinding targetType = (!compileTimeType.isBaseType() && runtimeTimeType.isBaseType()) 
                    ? compileTimeType  // unboxing: checkcast before conversion
                    : runtimeTimeType;
                TypeEnvironment env = field.declaringClass.getTypeEnvironment();
                res = originalType.genericCast(env, scope.boxing(targetType), scope.getTypeEnvironment());
                checkGenericCast(location, res, scope);
            }   
        } else if (binding instanceof LocalVariableBinding) {
            LocalVariableBinding local = (LocalVariableBinding) binding;
            if (local.type.isTypeVariable()) {
                TypeVariableBinding x = (TypeVariableBinding) local.type;
                TypeEnvironment env = null;
                if (x.declaringElement instanceof SourceTypeBinding) {
                    env = ((SourceTypeBinding) x.declaringElement).getTypeEnvironment();
                } else if (x.declaringElement instanceof MethodBinding) {
                    env = ((MethodBinding) x.declaringElement).getTypeEnvironment();
                } else {
                    throw new GICompilerBug("Unexpected declaringElement for local variable " + x.debugName() + ": " + x.declaringElement);
                }
                TypeBinding targetType = (!compileTimeType.isBaseType() && runtimeTimeType.isBaseType()) 
                   ? compileTimeType  // unboxing: checkcast before conversion
                   : runtimeTimeType;
                res = x.genericCast(env, targetType, scope.getTypeEnvironment());
                checkGenericCast(location, res, scope);
            }
        }
        return res;
    ...
    private static void checkGenericCast(ASTNode location, TypeBinding genericCast, Scope scope) {
        if (genericCast instanceof ReferenceBinding) {
            ReferenceBinding referenceCast = (ReferenceBinding) genericCast;
            if (!referenceCast.canBeSeenBy(scope)) {
                scope.problemReporter().invalidType(
                        location,
                        new ProblemReferenceBinding(CharOperation.splitOn('.',
                                referenceCast.shortReadableName()),
    ...
    public boolean resolvedTypeIsPossiblyTyvarInst() {
        if (binding != null && binding.isValidBinding()) {
            if (binding instanceof FieldBinding) {
                return ((FieldBinding) binding).typeIsPossiblyTyvarInst();
            } else if (binding instanceof LocalVariableBinding) {
                return false;
            }
        }
