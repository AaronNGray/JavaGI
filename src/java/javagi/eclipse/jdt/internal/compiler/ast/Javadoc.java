...
import javagi.eclipse.jdt.internal.compiler.parser.JavadocTagConstants;
...
public class Javadoc extends ASTNode {
	...
	public ASTNode getNodeStartingAt(int start) {
		...
		if (this.seeReferences != null) {
			...
			for (int i=0; i<length; i++) {
				javagi.eclipse.jdt.internal.compiler.ast.Expression expression = this.seeReferences[i];
			...
			if (methDecl != null && (methDecl.isConstructor() || overriding) && !superRef) {
				if (this.seeReferences[i] instanceof JavadocMessageSend) {
					...
					if (messageSend.binding != null && messageSend.binding.isValidBinding() && messageSend.actualReceiverType instanceof ReferenceBinding) {
						...
						if ((methodReceiverType.isSuperclassOf(methScope.getTypeEnvironment(), methDecl.binding.declaringClass) || (methodReceiverType.isInterface() && methDecl.binding.declaringClass.implementsInterface(methScope.getTypeEnvironment(), methodReceiverType, true))) &&
								CharOperation.equals(messageSend.selector, methDecl.selector) &&
								(methDecl.binding.returnType.isCompatibleWith(methScope, messageSend.binding.returnType))) {
					...
					// if binding is valid then look if we have a reference to an overriden method/constructor
					if (allocationExpr.binding != null && allocationExpr.binding.isValidBinding()) {
						if (methDecl.binding.declaringClass.isCompatibleWith(methScope, allocationExpr.getResolvedType())) {
							if (allocationExpr.arguments == null && methDecl.arguments == null) {
								...
								superRef = methDecl.binding.areParametersCompatibleWith(methScope.getTypeEnvironment(), allocationExpr.binding.parameters);
							}
						}
					}
				}
	...
	private void resolveReference(Expression reference, Scope scope) {
		...
		if (reference instanceof JavadocFieldReference) {
			...
			if (fieldRef.methodBinding != null) {
				...
				if (fieldRef.tagValue == JavadocTagConstants.TAG_VALUE_VALUE) {
					...
					if (scope.enclosingSourceType().isCompatibleWith(scope.getTypeEnvironment(), fieldRef.receiverType)) {
						fieldRef.bits |= ASTNode.SuperAccess;
					}
					fieldRef.methodBinding = scope.findMethod((ReferenceBinding)fieldRef.receiverType, fieldRef.token, new TypeBinding[0], fieldRef, scope.getTypeEnvironment());
				}
			}
		...
		// Verify type references
		if (!hasProblems && (reference instanceof JavadocSingleTypeReference || reference instanceof JavadocQualifiedTypeReference) && reference.getResolvedType() instanceof ReferenceBinding) {
			ReferenceBinding resolvedType = (ReferenceBinding) reference.getResolvedType();
		...
		// Verify that constructor reference are not used for @value tags
			...
			// Verify type references
			if (!hasProblems && alloc.binding != null && alloc.binding.isValidBinding() && alloc.getResolvedType() instanceof ReferenceBinding) {
				ReferenceBinding resolvedType = (ReferenceBinding) alloc.getResolvedType();
		...
		if (reference.getResolvedType() != null && reference.getResolvedType().isTypeVariable()) {
	...
	private void resolveThrowsTags(MethodScope methScope, boolean reportMissing) {
		...
		if (throwsTagsLength == 0) {
			if (reportMissing) {
				for (int i = 0; i < boundExceptionLength; i++) {
					...
					if (exceptionBinding != null && exceptionBinding.isValidBinding()) { // flag only valid class name
						int j=i;
						while (j<thrownExceptionLength && exceptionBinding != md.thrownExceptions[j].getResolvedType()) j++;
			...
			for (int i = 0; i < throwsTagsLength; i++) {
				...
				TypeBinding typeBinding = typeRef.getResolvedType();
			...
			for (int i = 0; i < boundExceptionLength; i++) {
				ReferenceBinding exceptionBinding = md.binding.thrownExceptions[i];
				if (exceptionBinding != null) exceptionBinding = (ReferenceBinding) exceptionBinding.erasure(methScope);
				boolean found = false;
				for (int j = 0; j < maxRef && !found; j++) {
					if (typeReferences[j] != null) {
						TypeBinding typeBinding = typeReferences[j].getResolvedType();
				...
				if (!found && reportMissing) {
					if (exceptionBinding != null && exceptionBinding.isValidBinding()) { // flag only valid class name
						int k=i;
						while (k<thrownExceptionLength && exceptionBinding != md.thrownExceptions[k].getResolvedType()) k++;
			...
			for (int i = 0; i < maxRef; i++) {
				...
				if (typeRef != null) {
					...
					for (int j = 0; j<thrownExceptionLength && !compatible; j++) {
						TypeBinding exceptionBinding = md.thrownExceptions[j].getResolvedType();
						if (exceptionBinding != null) {
							compatible = typeRef.getResolvedType().isCompatibleWith(methScope, exceptionBinding);
						}
					}
			
					//  If not compatible only complain on unchecked exception
					if (!compatible && !typeRef.getResolvedType().isUncheckedException(methScope.getTypeEnvironment(), false)) {
