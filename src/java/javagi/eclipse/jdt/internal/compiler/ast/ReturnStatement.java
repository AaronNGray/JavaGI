...
public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {	// here requires to generate a sequence of finally blocks invocations depending corresponding
	...
	if (saveValueNeeded) {
		...
		if (((this.bits & ASTNode.IsSynchronized) == 0) && this.expression != null && this.expression.getResolvedType() == TypeBinding.BOOLEAN) {
...
public void generateCode(BlockScope currentScope, CodeStream codeStream) {
	...
	if (needValueStore()) {
		...
		generateCoercion(currentScope, codeStream);
...
public void resolve(BlockScope scope) {
	...
	if (this.expression.isConstantValueOfTypeAssignableToType(expressionType, methodType)
			|| expressionType.isCompatibleWith(scope, methodType)) {

		this.expression.computeConversion(scope, methodType, expressionType);
		if (expressionType.needsUncheckedConversion(scope.getTypeEnvironment(), methodType)) {
		...
		return;
						...
						|| (expressionType.isBaseType()  // narrowing then boxing ?
								...
								&& this.expression.isConstantValueOfTypeAssignableToType(expressionType, scope.environment().computeBoxingType(scope.getTypeEnvironment(), methodType)))) {
		this.expression.computeConversion(scope, methodType, expressionType);
		if (this.expression instanceof CastExpression 
				&& (this.expression.bits & (ASTNode.UnnecessaryCast|ASTNode.DisableUnnecessaryCastCheck)) == 0) {
			CastExpression.checkNeedForAssignedCast(scope, methodType, (CastExpression) this.expression);
...
//SW: JavaGI support
//////////////////////////////////////////////////////////////////////////////////////////////////
private void generateCoercion(BlockScope currentScope, CodeStream codeStream) {
    AbstractMethodDeclaration method = currentScope.methodScope().referenceMethod();
    if (method == null) return;
    MethodBinding binding = method.binding;
    if (binding == null) return;
    Coercion.generateWrapperInvocation(
            currentScope.getTypeEnvironment(),
            this.expression,
            binding.returnType,
            binding.original().returnType,
...
public boolean referencesLocalVariables() {
    return this.expression.referencesLocalVariables();
}
}
