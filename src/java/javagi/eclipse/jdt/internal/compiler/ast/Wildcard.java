...
public class Wildcard extends SingleTypeReference {
    ...
    public char [][] getParameterizedTypeName() {
		switch (this.kind) {
			...
			case Wildcard.IMPLEMENTS :
                return new char[][] { CharOperation.concat(WILDCARD_NAME, WILDCARD_IMPLEMENTS, CharOperation.concatWith(this.bound.getParameterizedTypeName(), '.')) };
			default: // SUPER
    ...
    public char [][] getTypeName() {
		switch (this.kind) {
            ...
            case Wildcard.IMPLEMENTS :
                return new char[][] { CharOperation.concat(WILDCARD_NAME, WILDCARD_IMPLEMENTS, CharOperation.concatWith(this.bound.getTypeName(), '.')) };
			default: // SUPER
	...
	private TypeBinding internalResolveType(Scope scope, ReferenceBinding genericType, int rank) {
		...
		return this.setResolvedType(wildcard);
	}
    ...
    public StringBuffer printExpression(int indent, StringBuffer output){
		switch (this.kind) {
			...
			case Wildcard.EXTENDS :
				...
				break;
            case Wildcard.IMPLEMENTS :
                output.append(WILDCARD_NAME).append(WILDCARD_IMPLEMENTS);
                this.bound.printExpression(0, output);
                break;
			default: // SUPER
