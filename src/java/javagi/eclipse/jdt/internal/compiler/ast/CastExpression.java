...
public static void checkNeedForAssignedCast(BlockScope scope, TypeBinding expectedType, CastExpression rhs) {
	...
	TypeBinding castedExpressionType = rhs.expression.getResolvedType();
	//	int i = (byte) n; // cast still had side effect
	// double d = (float) n; // cast to float is unnecessary
	if (castedExpressionType == null || rhs.getResolvedType().isBaseType()) return; 
	//if (castedExpressionType.id == T_null) return; // tolerate null expression cast
	if (castedExpressionType.isCompatibleWith(scope, expectedType)) {
...
public static void checkNeedForCastCast(BlockScope scope, CastExpression enclosingCast) {
	...
	alternateCast.setResolvedType(enclosingCast.getResolvedType());
	if (!alternateCast.checkCastTypesCompatibility(scope, enclosingCast.getResolvedType(), nestedCast.expression.getResolvedType(), null /* no expr to avoid side-effects*/)) return;
...
public static void checkNeedForEnclosingInstanceCast(BlockScope scope, Expression enclosingInstance, TypeBinding enclosingInstanceType, TypeBinding memberType) {
	...
	TypeBinding castedExpressionType = ((CastExpression)enclosingInstance).expression.getResolvedType();
...
public static void checkNeedForArgumentCast(BlockScope scope, int operator, int operatorSignature, Expression expression, int expressionTypeId) {
	...
	if ((expression.bits & ASTNode.UnnecessaryCast) == 0 && expression.getResolvedType().isBaseType()) {
		// narrowing conversion on base type may change value, thus necessary
		return;
	} else  {
		TypeBinding alternateLeftType = ((CastExpression)expression).expression.getResolvedType();
...
public static void checkNeedForArgumentCasts(BlockScope scope, Expression receiver, TypeBinding receiverType, MethodBinding binding, Expression[] arguments, TypeBinding[] argumentTypes, final InvocationSite invocationSite) {
	...
	for (int i = 0; i < length; i++) {
		...
		if (argument instanceof CastExpression) {
			// narrowing conversion on base type may change value, thus necessary
			if ((argument.bits & ASTNode.UnnecessaryCast) == 0 && argument.getResolvedType().isBaseType()) {
				continue;
			}		
			TypeBinding castedExpressionType = ((CastExpression)argument).expression.getResolvedType();
			if (castedExpressionType == null) return; // cannot do better
			// obvious identity cast
...
public static void checkNeedForArgumentCasts(BlockScope scope, int operator, int operatorSignature, Expression left, int leftTypeId, boolean leftIsCast, Expression right, int rightTypeId, boolean rightIsCast) {
	...
	if (leftIsCast) {
		if ((left.bits & ASTNode.UnnecessaryCast) == 0 && left.getResolvedType().isBaseType()) {
			// narrowing conversion on base type may change value, thus necessary
			leftIsCast = false;
		} else  {
			TypeBinding alternateLeftType = ((CastExpression)left).expression.getResolvedType();
			if (alternateLeftType == null) return; // cannot do better
	...
	if (rightIsCast) {
		if ((right.bits & ASTNode.UnnecessaryCast) == 0 && right.getResolvedType().isBaseType()) {
			// narrowing conversion on base type may change value, thus necessary
			rightIsCast = false;
		} else {
			TypeBinding alternateRightType = ((CastExpression)right).expression.getResolvedType();
...
private static void checkAlternateBinding(BlockScope scope, Expression receiver, TypeBinding receiverType, MethodBinding binding, Expression[] arguments, TypeBinding[] originalArgumentTypes, TypeBinding[] alternateArgumentTypes, final InvocationSite invocationSite) {
		InvocationSite fakeInvocationSite = new InvocationSite(){	
            ...
            @Override
            public void markAsJavaGICallSite() { /* ignore */ }
            @Override 
		...
		if (binding.isConstructor()) {
			bindingIfNoCast = scope.getConstructor((ReferenceBinding)receiverType, alternateArgumentTypes, fakeInvocationSite, scope.getTypeEnvironment());
		} else {
			bindingIfNoCast = receiver.isImplicitThis()
				? scope.getImplicitMethod(binding.selector, alternateArgumentTypes, fakeInvocationSite, scope.getTypeEnvironment())
				: scope.getMethod(receiverType, binding.selector, alternateArgumentTypes, fakeInvocationSite, scope.getTypeEnvironment()); 	
		}
		if (bindingIfNoCast == binding) {
			...
			if (binding.isVarargs()) {
				...
				if (paramLength == argumentLength) {
					...
					if (lastArgType.isCompatibleWith(scope, varargsType.elementsType())
							&& lastArgType.isCompatibleWith(scope, varargsType)) {
						return;
					}
				}
			}
...
public boolean checkUnsafeCast(Scope scope, TypeBinding castType, TypeBinding expressionType, TypeBinding match, boolean isNarrowing) {
	if (match == castType) {
		if (!isNarrowing && match == this.getResolvedType().leafComponentType()) { // do not tag as unnecessary when recursing through upper bounds
	...
	if (match != null) {
		if (isNarrowing
				? match.isProvablyDistinct(scope.getTypeEnvironment(), expressionType)
				: castType.isProvablyDistinct(scope.getTypeEnvironment(), match)) {
			return false;
		}
	}
			...
			if (castType.isBoundParameterizedType()) {
				...
				switch (match.kind()) {
					case Binding.PARAMETERIZED_TYPE :
						if (isNarrowing) {
							// [JLS 5.5] T <: S
							if (expressionType.isRawType() || !expressionType.isEquivalentTo(scope.getTypeEnvironment(), match)) {
								this.bits |= ASTNode.UnsafeCast;
								return true;
							}
							...
							if (paramMatch.arguments == null || length > paramMatch.arguments.length) {
								...
								nextAlternateArgument: for (int i = 0; i < length; i++) {
									...
									ParameterizedTypeBinding alternateCastType = environment.createParameterizedType((ReferenceBinding)castType.erasure(scope.getTypeEnvironment()), alternateArguments, castType.enclosingType());
									if (alternateCastType.findSuperTypeOriginatingFrom(scope.getTypeEnvironment(), expressionType) == match) {
										this.bits |= ASTNode.UnsafeCast;
										break;
									}
								}
							...
							// [JLS 5.5] T >: S
							if (!match.isEquivalentTo(scope.getTypeEnvironment(), castType)) {
								this.bits |= ASTNode.UnsafeCast;
								return true;
							}
						}
...
public void generateCode(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
	...
	if (this.constant != Constant.NotAConstant) {
		if (valueRequired || needRuntimeCheckcast) { // Added for: 1F1W9IG: IVJCOM:WINNT - Compiler omits casting check
			...
			if (needRuntimeCheckcast) {
			    Translation.javaGICast(currentScope, codeStream, this.getResolvedType(), 
			                           TypeBinding.wellKnownType(currentScope, this.constant.typeID()), false);
				// codeStream.checkcast(this.getResolvedType());
			}
			if (!valueRequired) {
	...
	if (needRuntimeCheckcast && this.expression.postConversionType(currentScope) != this.getResolvedType().erasure(currentScope)) { // no need to issue a checkcast if already done as genericCast
	    Translation.javaGICast(currentScope, codeStream, this.getResolvedType(), this.expression.getResolvedType(),
	                           this.expression.resolvedTypeIsPossiblyTyvarInst());
		// codeStream.checkcast(this.getResolvedType());
	}
	if (valueRequired) {
...
/**
 * @see javagi.eclipse.jdt.internal.compiler.ast.Expression#localVariableBinding()
 */
@Override
...
/**
 * @see javagi.eclipse.jdt.internal.compiler.ast.Expression#optimizedBooleanConstant()
 */
@Override
public Constant optimizedBooleanConstant() {
	switch(this.getResolvedType().id) {
		case T_boolean :
...
public TypeBinding resolveType(BlockScope scope) {
	...
	if ((this.type instanceof TypeReference) || (this.type instanceof NameReference)
		...
		TypeBinding castType = this.setResolvedType(this.type.resolveType(scope));
		...
		if (castType != null) {
				...
				if (isLegal) {
					...
					if ((this.bits & ASTNode.UnsafeCast) != 0) { // unsafe cast
						...
						if ((this.bits & (ASTNode.UnnecessaryCast|ASTNode.DisableUnnecessaryCastCheck)) == ASTNode.UnnecessaryCast) { // unnecessary cast 
							if (!isIndirectlyUsed(scope.getTypeEnvironment())) // used for generic type inference or boxing ?
			...
			this.setResolvedType(castType.capture(scope));
			if (exprContainCast) {
 ...
 */
private boolean isIndirectlyUsed(javagi.compiler.TypeEnvironment env) {
	if (this.expression instanceof MessageSend) {
		...
		if (method instanceof ParameterizedGenericMethodBinding
			...
			if (this.expectedType == null) 
				return true;
			if (this.getResolvedType() != this.expectedType)
				return true;
		}
	}
	if (this.expectedType != null && this.getResolvedType().isBaseType() && !this.getResolvedType().isCompatibleWith(env, this.expectedType)) {
...
/**
 * @see javagi.eclipse.jdt.internal.compiler.ast.Expression#tagAsNeedCheckCast()
 */
@Override
