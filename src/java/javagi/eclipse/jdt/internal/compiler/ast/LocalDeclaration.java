    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream) {
		...
		generateInit: {
			...
			Coercion.generateWrapperInvocation(currentScope.getTypeEnvironment(), 
			                                   initialization, 
			                                   binding.type,
			                                   null, // binding.originalType(currentScope.getTypeEnvironment()),
			                                   null,
			                                   null,
			                                   codeStream);
			...
			if (binding.type.isArrayType() 
				&& (initialization.getResolvedType() == TypeBinding.NULL	// arrayLoc = null
					|| ((initialization instanceof CastExpression)	// arrayLoc = (type[])null
						&& (((CastExpression)initialization).innermostCastedExpression().getResolvedType() == TypeBinding.NULL)))){
			...
			if (initialization instanceof ArrayInitializer) {
				...
				if (initializationType != null) {
					...
					if (initialization.isConstantValueOfTypeAssignableToType(initializationType, variableType)
						...
						|| initializationType.isCompatibleWith(scope, variableType)) {
						this.initialization.computeConversion(scope, variableType, initializationType);
						if (initializationType.needsUncheckedConversion(scope.getTypeEnvironment(), variableType)) {
						...
						if (this.initialization instanceof CastExpression 
							...
							CastExpression.checkNeedForAssignedCast(scope, variableType, (CastExpression) this.initialization);
										...
										|| (initializationType.isBaseType()  // narrowing then boxing ?
												...
												&& initialization.isConstantValueOfTypeAssignableToType(initializationType, scope.environment().computeBoxingType(scope.getTypeEnvironment(), variableType)))) {
						this.initialization.computeConversion(scope, variableType, initializationType);
						if (this.initialization instanceof CastExpression 
								&& (this.initialization.bits & ASTNode.UnnecessaryCast) == 0) {
							CastExpression.checkNeedForAssignedCast(scope, variableType, (CastExpression) this.initialization);
