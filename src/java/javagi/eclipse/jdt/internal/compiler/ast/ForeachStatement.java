...
import javagi.eclipse.jdt.internal.compiler.flow.LoopingFlowContext;
...
public class ForeachStatement extends Statement {
			...
			case GENERIC_ITERABLE :
				...
				MethodBinding iteratorMethodBinding =
					new MethodBinding(
							...
							(ReferenceBinding) this.iteratorReceiverType.erasure(currentScope));
				if (! implementsIterableByJavaGI) {
				    ...
				    Translation.invokeRetroactiveMethod(iteratorMethodBinding, scope, codeStream);
				}
				codeStream.store(this.indexVariable, false);
				codeStream.addVariable(this.indexVariable);
				break;
    ...
    public void resolve(BlockScope upperScope) {
		...
		TypeBinding elementType = this.elementVariable.type.getResolvedType();
		...
		if (elementType != null && collectionType != null) {
			if (collectionType.isArrayType()) { // for(E e : E[])
				...
				if (!collectionElementType.isCompatibleWith(upperScope, elementType)
				...
				if (elementType.isBaseType()) {
					if (!collectionElementType.isBaseType()) {
						compileTimeTypeID = scope.environment().computeBoxingType(scope.getTypeEnvironment(), collectionElementType).id;
					...
					if (collectionElementType.isBaseType()) {
						int boxedID = scope.environment().computeBoxingType(scope.getTypeEnvironment(), collectionElementType).id;
			    ...
			    ReferenceBinding iterableType = ((ReferenceBinding)collectionType).findSuperTypeOriginatingFrom(scope.getTypeEnvironment(), T_JavaLangIterable, false /*Iterable is not a class*/);
			    checkIterable: {
			    	if (iterableType == null) {
			    	    // JavaGI
			    	    iterableType = Entailment.entailsJavaLangIterable(scope, scope.getTypeEnvironment(), collectionType);
			    	    if (iterableType == null) {
			    	        break checkIterable;
			    	    } else {
			    	        implementsIterableByJavaGI = true;
			    	    }
			    	}
			    	
					this.iteratorReceiverType = collectionType.erasure(upperScope);
					if (((ReferenceBinding)iteratorReceiverType).findSuperTypeOriginatingFrom(scope.getTypeEnvironment(), T_JavaLangIterable, false) == null) {
    ...
    //SW: JavaGI support
    //////////////////////////////////////////////////////////////////////////////////////////////////
	private boolean implementsIterableByJavaGI = false;
}
