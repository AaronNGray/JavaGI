...
import javagi.eclipse.jdt.internal.compiler.impl.IntConstant;
...
public FlowInfo analyseCode(
	...
	if (this.constantExpression != null) {
		if (this.constantExpression.constant == Constant.NotAConstant 
				&& !this.constantExpression.getResolvedType().isEnum()) {
 ...
 * @see javagi.eclipse.jdt.internal.compiler.ast.Statement#resolveCase(javagi.eclipse.jdt.internal.compiler.lookup.BlockScope, javagi.eclipse.jdt.internal.compiler.lookup.TypeBinding, javagi.eclipse.jdt.internal.compiler.ast.SwitchStatement)
 */
@Override
public Constant resolveCase(BlockScope scope, TypeBinding switchExpressionType, SwitchStatement switchStatement) {
	...
	if (this.constantExpression.isConstantValueOfTypeAssignableToType(caseType, switchExpressionType)
			|| caseType.isCompatibleWith(scope, switchExpressionType)) {
		if (caseType.isEnum()) {
			...
			return this.constantExpression.constant;
					...
					|| (caseType.isBaseType()  // narrowing then boxing ?
							...
							&& this.constantExpression.isConstantValueOfTypeAssignableToType(caseType, scope.environment().computeBoxingType(scope.getTypeEnvironment(), switchExpressionType)))) {
