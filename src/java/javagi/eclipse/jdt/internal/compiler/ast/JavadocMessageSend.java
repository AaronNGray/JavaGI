...
public class JavadocMessageSend extends MessageSend {
	...
	private TypeBinding internalResolveType(Scope scope) {
		...
		this.actualReceiverType = scope.environment().convertToRawType(this.receiver.getResolvedType(), true /*force the conversion of enclosing types*/);
		ReferenceBinding enclosingType = scope.enclosingReceiverType();
		if (enclosingType==null ? false : enclosingType.isCompatibleWith(scope, this.actualReceiverType)) {
		...
		this.binding = scope.getMethod(this.actualReceiverType, this.selector, argumentTypes, this, scope.getTypeEnvironment());
		if (!this.binding.isValidBinding()) {
			...
			while (!methodBinding.isValidBinding() && (enclosingTypeBinding.isMemberType() || enclosingTypeBinding.isLocalType())) {
				...
				methodBinding = scope.getMethod(enclosingTypeBinding, this.selector, argumentTypes, this, scope.getTypeEnvironment());
			}
			if (methodBinding.isValidBinding()) {
				...
				while (!contructorBinding.isValidBinding() && (enclosingTypeBinding.isMemberType() || enclosingTypeBinding.isLocalType())) {
					...
					if (CharOperation.equals(this.selector, enclosingTypeBinding.shortReadableName())) {
						contructorBinding = scope.getConstructor((ReferenceBinding)enclosingTypeBinding, argumentTypes, this, scope.getTypeEnvironment());
					}
				}
		...
		if (!this.binding.isValidBinding()) {
			...
			return this.setResolvedType(this.binding == null ? null : this.binding.returnType);
		} else if (hasArgsTypeVar) {
			...
			int length = argumentTypes.length;
			for (int i=0; i<length; i++) {
				if (this.binding.parameters[i].erasure(scope) != argumentTypes[i].erasure(scope)) {
					MethodBinding problem = new ProblemMethodBinding(this.binding, this.selector, argumentTypes, ProblemReasons.NotFound);
					scope.problemReporter().javadocInvalidMethod(this, problem, scope.getDeclarationModifiers());
					break;
				}
		...
		return this.setResolvedType(this.binding.returnType);
	}
	
	/* (non-Javadoc)
	 * @see javagi.eclipse.jdt.internal.compiler.lookup.InvocationSite#isSuperAccess()
	 */
	@Override
