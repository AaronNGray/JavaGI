...
public void computeConversion(Scope scope, TypeBinding runtimeTimeType, TypeBinding compileTimeType) {
	...
	if (field != null) {
		...
		if (originalBinding != field 
	    	...
	    	TypeBinding typeCast = originalType.genericCast(env, targetType, scope.getTypeEnvironment());
...
public void generateAssignment(BlockScope currentScope, CodeStream codeStream, Assignment assignment, boolean valueRequired) {
	...
	TypeEnvironment env = currentScope.getTypeEnvironment();
    Coercion.generateWrapperInvocation(env, 
            assignment.expression, this.getResolvedType(), 
            lastFieldBinding.original().type, 
            lastFieldBinding.original().declaringClass.getTypeEnvironment(),
            lastFieldBinding.declaringClass.getSubstitution(),
            codeStream);
				...
				if (valueRequired  
					...
					if (lastFieldBinding.declaringClass == null) { // array length
						...
						if (accessor == null) {
							...
							ctType = lastFieldBinding.type;
							ctTypeIsPossiblyTyvarInst = lastFieldBinding.typeIsPossiblyTyvarInst();
						} else {
							codeStream.invokestatic(accessor);
							ctType = accessor.returnType;
							ctTypeIsPossiblyTyvarInst = accessor.returnTypeIsPossiblyTyvarInst();
						}
						if (requiredGenericCast != null) {
						      // FIXME: should be optimized
					        Translation.javaGICast(currentScope, codeStream, requiredGenericCast, ctType, ctTypeIsPossiblyTyvarInst);
	...
	switch (this.bits & ASTNode.RestrictiveFlagMASK) {
		...
		case Binding.LOCAL : // reading the first local variable
			...
			if (localConstant != Constant.NotAConstant) {
				...
				ctType = TypeBinding.wellKnownType(currentScope, localConstant.typeID());
				...
				if ((this.bits & ASTNode.DepthMASK) != 0) {
					...
					ctType = null;
					ctTypeIsPossiblyTyvarInst = true;
				} else {
				...
				if (fieldConstant != Constant.NotAConstant) {
					...
					if (needValue || (i > 0 && complyTo14) || lastGenericCast != null) {
						...
						if (lastGenericCast != null) {
						      // FIXME: should be optimized
					        Translation.javaGICast(currentScope, codeStream, lastGenericCast, ctType, ctTypeIsPossiblyTyvarInst);
						...
						if (this.codegenBinding == lastFieldBinding) {
							if (lastFieldBinding.isStatic()){
								...
								if (((FieldBinding)this.binding).original().declaringClass != this.actualReceiverType.erasure(currentScope)) {
									MethodBinding accessor = this.syntheticReadAccessors == null ? null : this.syntheticReadAccessors[i]; 
									if (accessor == null) {
										codeStream.getstatic(lastFieldBinding);
									} else {
...
public TypeBinding getOtherFieldBindings(BlockScope scope) {
	...
	while (index < length) {
		...
		field = scope.getField(type.capture(scope), token, this);
		...
		if (field.isValidBinding()) {
			...
			if (previousField != null) {
				...
				TypeBinding receiverErasure = type.erasure(scope);
				if (receiverErasure instanceof ReferenceBinding) {
					if (receiverErasure.findSuperTypeOriginatingFrom(scope.getTypeEnvironment(), field.declaringClass) == null) {
			    ...
			    if ((originalBinding.type.tagBits &  TagBits.HasTypeVariable) != 0 && fieldReceiverType.id != TypeIds.T_JavaLangObject) {
			        TypeEnvironment env = previousField.declaringClass.getTypeEnvironment();
			    	setGenericCast(index-1, originalBinding.type.genericCast(env, fieldReceiverType, scope.getTypeEnvironment())); // type cannot be base-type even in boxing case
			    }
		    }
			// only last field is actually a write access if any
		...
		if ((options.targetJDK >= ClassFileConstants.JDK1_2
		    ...
		    setCodegenBinding(
		            ...
		            currentScope.enclosingSourceType().getUpdatedFieldBinding(
		                    ...
		                    (ReferenceBinding)lastReceiverType.erasure(currentScope)));
		}
	}			
}
		...
		switch (this.bits & ASTNode.RestrictiveFlagMASK) {
			...
			case Binding.TYPE | Binding.VARIABLE :
				if (this.binding instanceof LocalVariableBinding) {
					...
					this.setResolvedType(getOtherFieldBindings(scope));
					if (this.getResolvedType() != null 
							&& (this.getResolvedType().tagBits & TagBits.HasMissingType) != 0) {
						FieldBinding lastField = this.otherBindings[this.otherBindings.length - 1];
						scope.problemReporter().invalidField(this, new ProblemFieldBinding(lastField.declaringClass, lastField.name, ProblemReasons.NotFound), this.tokens.length, this.getResolvedType().leafComponentType());
						return null;
					}
...
@Override
public void computeJavaGIConversion(Scope scope) {
    if (binding != null && binding.isValidBinding()) {
        TypeBinding t = null;
        if (binding instanceof FieldBinding) {
            t = ((FieldBinding) binding).type;
        } else if (binding instanceof LocalVariableBinding) {
            t = ((LocalVariableBinding) binding).type;
        }
        if (t != null) {
            setGenericCast(0, SingleNameReference.computeConversion(this, scope, t , t, binding));
        }
    }
}
