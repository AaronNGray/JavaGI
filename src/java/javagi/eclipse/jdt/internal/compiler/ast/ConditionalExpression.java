    ...
    public void generateCode(
		...
		if (needTruePart) {
			...
			if (needFalsePart) {
				...
				if (valueRequired) {
					codeStream.decrStackSize(this.getResolvedType() == TypeBinding.LONG || this.getResolvedType() == TypeBinding.DOUBLE ? 2 : 1);
				}
			}
		}
		if (needFalsePart) {
			...
			if (valueRequired) {
				codeStream.recordExpressionType(this.getResolvedType());
			}
			if (needTruePart) {
				// End of if statement
				endifLabel.place();
...
public int nullStatus(FlowInfo flowInfo) {
    ...
    public TypeBinding resolveType(BlockScope scope) {
			...
			if (valueIfTrueType.isBaseType()) {
				if (valueIfFalseType.isBaseType()) {
					...
					if (valueIfTrueType == TypeBinding.NULL) {  // bool ? null : 12 --> Integer
						valueIfFalseType = env.computeBoxingType(scope.getTypeEnvironment(), valueIfFalseType); // boxing
					} else if (valueIfFalseType == TypeBinding.NULL) {  // bool ? 12 : null --> Integer
						valueIfTrueType = env.computeBoxingType(scope.getTypeEnvironment(), valueIfTrueType); // boxing
					}
				} else {
					// bool ? baseType : nonBaseType
					TypeBinding unboxedIfFalseType = valueIfFalseType.isBaseType() ? valueIfFalseType : env.computeBoxingType(scope.getTypeEnvironment(), valueIfFalseType);
					if (valueIfTrueType.isNumericType() && unboxedIfFalseType.isNumericType()) {
						...
						valueIfFalseType = env.computeBoxingType(scope.getTypeEnvironment(), valueIfFalseType); // unboxing
					}
				}
			} else if (valueIfFalseType.isBaseType()) {
					// bool ? nonBaseType : baseType
					TypeBinding unboxedIfTrueType = valueIfTrueType.isBaseType() ? valueIfTrueType : env.computeBoxingType(scope.getTypeEnvironment(), valueIfTrueType);
					if (unboxedIfTrueType.isNumericType() && valueIfFalseType.isNumericType()) {
						...
						valueIfTrueType = env.computeBoxingType(scope.getTypeEnvironment(), valueIfTrueType); // unboxing
					}					
			} else {
					// bool ? nonBaseType : nonBaseType
					TypeBinding unboxedIfTrueType = env.computeBoxingType(scope.getTypeEnvironment(), valueIfTrueType);
					TypeBinding unboxedIfFalseType = env.computeBoxingType(scope.getTypeEnvironment(), valueIfFalseType);
		...
		if (valueIfTrueType == valueIfFalseType) { // harmed the implicit conversion 
			...
			return this.setResolvedType(valueIfTrueType);
		}
		...
		if (valueIfTrueType.isNumericType() && valueIfFalseType.isNumericType()) {
			...
			if ((valueIfTrueType == TypeBinding.BYTE && valueIfFalseType == TypeBinding.SHORT)
				...
				return this.setResolvedType(TypeBinding.SHORT);
			}
			...
			if ((valueIfFalseType == TypeBinding.BYTE
				...
				valueIfFalse.computeConversion(scope, valueIfFalseType, originalValueIfFalseType);
				return this.setResolvedType(valueIfFalseType);
			}
			...
			if (BaseTypeBinding.isNarrowing(valueIfTrueType.id, T_int)
				...
				return this.setResolvedType(TypeBinding.INT);
			}
			// long
			if (BaseTypeBinding.isNarrowing(valueIfTrueType.id, T_long)
				...
				return this.setResolvedType(TypeBinding.LONG);
			}
			// float
			if (BaseTypeBinding.isNarrowing(valueIfTrueType.id, T_float)
				...
				return this.setResolvedType(TypeBinding.FLOAT);
			}
			// double
			...
			return this.setResolvedType(TypeBinding.DOUBLE);
		}
		// Type references (null null is already tested)
		if (valueIfTrueType.isBaseType() && valueIfTrueType != TypeBinding.NULL) {
			if (use15specifics) {
				valueIfTrueType = env.computeBoxingType(scope.getTypeEnvironment(), valueIfTrueType);
			} else {
		...
		if (valueIfFalseType.isBaseType() && valueIfFalseType != TypeBinding.NULL) {
			if (use15specifics) {
				valueIfFalseType = env.computeBoxingType(scope.getTypeEnvironment(), valueIfFalseType);
			} else {
				scope.problemReporter().conditionalArgumentsIncompatibleTypes(this, valueIfTrueType, valueIfFalseType);
				return null;
			}
		...
		if (use15specifics) {
			...
			if (valueIfTrueType == TypeBinding.NULL) {
				...
				commonType = scope.lowerUpperBound(scope.getTypeEnvironment(), new TypeBinding[] { valueIfTrueType, valueIfFalseType });
			}
			if (commonType != null) {
				...
				return this.setResolvedType(commonType.capture(scope));
			}
		} else {
			// < 1.5 : one operand must be convertible to the other
			if (valueIfFalseType.isCompatibleWith(scope, valueIfTrueType)) {
				valueIfTrue.computeConversion(scope, valueIfTrueType, originalValueIfTrueType);
				valueIfFalse.computeConversion(scope, valueIfTrueType, originalValueIfFalseType);
				return this.setResolvedType(valueIfTrueType);
			} else if (valueIfTrueType.isCompatibleWith(scope, valueIfFalseType)) {
				valueIfTrue.computeConversion(scope, valueIfFalseType, originalValueIfTrueType);
				valueIfFalse.computeConversion(scope, valueIfFalseType, originalValueIfFalseType);
				return this.setResolvedType(valueIfFalseType);
			}
