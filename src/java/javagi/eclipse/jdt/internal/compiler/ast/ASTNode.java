...
public abstract class ASTNode implements TypeConstants, TypeIds {
	...
	private static int checkInvocationArgument(BlockScope scope, Expression argument, TypeBinding parameterType, TypeBinding argumentType, TypeBinding originalParameterType) {
		...
		if (argumentType != checkedParameterType && argumentType.needsUncheckedConversion(scope.getTypeEnvironment(), checkedParameterType)) {
					...
					if (lastArgType == TypeBinding.NULL) {
						...
						if (varargsType.dimensions < dimensions) {
							scope.problemReporter().varargsArgumentNeedCast(method, lastArgType, invocationSite);
										...
										&& lastArgType.leafComponentType().erasure(scope.getTypeEnvironment()) != 
										    varargsType.leafComponentType.erasure(scope.getTypeEnvironment())
										&& lastArgType.isCompatibleWith(scope, varargsType.elementsType())
	...
	public final boolean isFieldUseDeprecated(FieldBinding field, Scope scope, boolean isStrictlyAssigned) {
		...
		if ((field.modifiers & ExtraCompilerModifiers.AccRestrictedAccess) != 0) {
			AccessRestriction restriction =
				scope.environment().getAccessRestriction(field.declaringClass.erasure(scope.getTypeEnvironment()));
			if (restriction != null) {
	...
	*/
	public final boolean isMethodUseDeprecated(MethodBinding method, Scope scope,
		...
		if (isExplicitUse && (method.modifiers & ExtraCompilerModifiers.AccRestrictedAccess) != 0) {
			...
			AccessRestriction restriction =
				scope.environment().getAccessRestriction(method.declaringClass.erasure(scope.getTypeEnvironment()));
			if (restriction != null) {
	...
	*/
	public final boolean isTypeUseDeprecated(TypeBinding type, Scope scope) {
		...
		if ((this.bits & ASTNode.InsideJavadoc) == 0 &&
			...
			((ReferenceBinding)refType.erasure(env)).modifiers |= ExtraCompilerModifiers.AccLocallyUsed;
		}

		if (refType.hasRestrictedAccess()) {
			AccessRestriction restriction = scope.environment().getAccessRestriction(type.erasure(env));
			if (restriction != null) {
...
public static void resolveDeprecatedAnnotations(BlockScope scope, Annotation[] annotations, Binding recipient) {
	...
	public String formatLocation(Scope scope) {
	    return formatLocation(scope.referenceContext().compilationResult());
	}
	
	public String formatLocation(CompilationResult unitResult) {
	    int problemStartPosition = sourceStart;
	    int[] lineEnds;
	    ...
	    int columnNumber = problemStartPosition >= 0
	      ...
	      : 0;
	    String fileName = new String(unitResult.getFileName());
