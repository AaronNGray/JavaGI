...
public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, 
	...
	if (this.getResolvedType().id != T_JavaLangString) {
    ...
    public TypeBinding resolveType(BlockScope scope) {
		...
		if (use15specifics) {
			if (!lhsType.isBaseType() && expressionType.id != T_JavaLangString && expressionType.id != T_null) {
				TypeBinding unboxedType = env.computeBoxingType(scope.getTypeEnvironment(), lhsType);
			...
			if (!expressionType.isBaseType() && lhsType.id != T_JavaLangString  && lhsType.id != T_null) {
				expressionType = env.computeBoxingType(scope.getTypeEnvironment(), expressionType);
			}
		}
		...
		if (lhs instanceof QualifiedNameReference) {
		    ((QualifiedNameReference) lhs).computeJavaGIConversion(scope);
		}
		return this.setResolvedType(originalLhsType);
	}
