...
public void generateCode(BlockScope currentScope, CodeStream codeStream) {
	...
	if (this.initialization != null
		...
		FieldBinding originalBinding = binding.original();
		Coercion.generateWrapperInvocation(currentScope.getTypeEnvironment(), 
		                                   ...
		                                   binding.declaringClass.getSubstitution(),
		                                   codeStream);
		// store into field
		if (isStatic) {
...
public void resolve(MethodScope initializationScope) {
	...
	if (this.type != null ) { // enum constants have no declared type
		this.type.setResolvedType(this.binding.type); // update binding for type reference
	}
		...
		if (this.initialization == null) {
			...
			if (this.initialization instanceof ArrayInitializer) {
				...
				if (this.initialization.isConstantValueOfTypeAssignableToType(initializationType, fieldType)
						...
						|| initializationType.isCompatibleWith(initializationScope, fieldType)) {
					this.initialization.computeConversion(initializationScope, fieldType, initializationType);
					if (initializationType.needsUncheckedConversion(initializationScope.getTypeEnvironment(), fieldType)) {
					...
					if (this.initialization instanceof CastExpression 
						...
						CastExpression.checkNeedForAssignedCast(initializationScope, fieldType, (CastExpression) this.initialization);
									...
									|| (initializationType.isBaseType()  // narrowing then boxing ?
											...
											&& initialization.isConstantValueOfTypeAssignableToType(initializationType, initializationScope.environment().computeBoxingType(initializationScope.getTypeEnvironment(), fieldType)))) {
					this.initialization.computeConversion(initializationScope, fieldType, initializationType);
					if (this.initialization instanceof CastExpression 
							&& (this.initialization.bits & ASTNode.UnnecessaryCast) == 0) {
						CastExpression.checkNeedForAssignedCast(initializationScope, fieldType, (CastExpression) this.initialization);
