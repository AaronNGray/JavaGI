...
public class ArrayInitializer extends Expression {
    ...
    public TypeBinding resolveTypeExpecting(BlockScope scope, TypeBinding expectedType) {
		...
		if (expectedType instanceof ArrayBinding) {
			...
			this.setResolvedType(this.binding = (ArrayBinding) expectedType);
			...
			for (int i = 0, length = this.expressions.length; i < length; i++) {
				...
				if ((expression.isConstantValueOfTypeAssignableToType(expressionType, elementType)
						...
						|| expressionType.isCompatibleWith(scope, elementType)) {
					expression.computeConversion(scope, elementType, expressionType);
									...
									|| (expressionType.isBaseType()  // narrowing then boxing ?
											...
											&& expression.isConstantValueOfTypeAssignableToType(expressionType, scope.environment().computeBoxingType(scope.getTypeEnvironment(), elementType)))) {
					expression.computeConversion(scope, elementType, expressionType);
				} else {
		...
		if (leafElementType != null) {
			this.setResolvedType(scope.createArrayType(leafElementType, dim));
			if (expectedType != null)
				scope.problemReporter().typeMismatchError(this.getResolvedType(), expectedType, this, null);
		}
		return null;
	}
