...
public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
	...
	checkCapturedLocalInitializationIfNecessary((ReferenceBinding)this.binding.declaringClass.erasure(currentScope.getTypeEnvironment()), currentScope, flowInfo);
...
public void manageEnclosingInstanceAccessIfNecessary(BlockScope currentScope, FlowInfo flowInfo) {
	...
	ReferenceBinding allocatedTypeErasure = (ReferenceBinding) binding.declaringClass.erasure(currentScope.getTypeEnvironment());
...
public TypeBinding resolveType(BlockScope scope) {
	...
	if (this.type == null) {
		...
		this.setResolvedType(scope.enclosingReceiverType());
	} else {
		this.setResolvedType(this.type.resolveType(scope, true /* check bounds*/));
			...
			if (this.type instanceof ParameterizedQualifiedTypeReference) { // disallow new X<String>.Y<Integer>()
				...
				for (int i = qRef.typeArguments.length - 2; i >= 0; i--) {
					if (qRef.typeArguments[i] != null) {
						scope.problemReporter().illegalQualifiedParameterizedTypeAllocation(this.type, this.getResolvedType());
						break;
					}
				}
			}
	...
	if (arguments != null) {
		...
		if (argHasError) {
			if (this.getResolvedType() instanceof ReferenceBinding) {
				// record a best guess, for clients who need hint about possible contructor match
				TypeBinding[] pseudoArgs = new TypeBinding[length];
				for (int i = length; --i >= 0;) {
					pseudoArgs[i] = argumentTypes[i] == null ? TypeBinding.NULL : argumentTypes[i]; // replace args with errors with null type
				}
				this.binding = scope.findMethod((ReferenceBinding) this.getResolvedType(), TypeConstants.INIT, pseudoArgs, this, scope.getTypeEnvironment());
	...
	if (this.getResolvedType() == null || !this.getResolvedType().isValidBinding()) {
		return null;
	}

	// null type denotes fake allocation for enum constant inits
	if (this.type != null && !this.getResolvedType().canBeInstantiated()) {
		scope.problemReporter().cannotInstantiate(type, this.getResolvedType());
		return this.getResolvedType();
	}
	ReferenceBinding allocationType = (ReferenceBinding) this.getResolvedType();
	if (!(binding = scope.getConstructor(allocationType, argumentTypes, this, scope.getTypeEnvironment())).isValidBinding()) {
