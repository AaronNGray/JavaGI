...
public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, 
	...
	if (this.getResolvedType().id == TypeIds.T_JavaLangString) {
...
void nonRecursiveResolveTypeUpwards(BlockScope scope) {
	...
	TypeBinding leftType = this.left.getResolvedType();
	...
	if (use15specifics) {
		if (!leftType.isBaseType() && rightTypeID != TypeIds.T_JavaLangString && rightTypeID != TypeIds.T_null) {
			leftTypeID = scope.environment().computeBoxingType(scope.getTypeEnvironment(), leftType).id;
		}
		if (!rightType.isBaseType() && leftTypeID != TypeIds.T_JavaLangString && leftTypeID != TypeIds.T_null) {
			rightTypeID = scope.environment().computeBoxingType(scope.getTypeEnvironment(), rightType).id;
		}
	}
	if (leftTypeID > 15
	...
	switch (operatorSignature & 0xF) { // record the current ReturnTypeID
		...
		case T_boolean :
			this.setResolvedType(TypeBinding.BOOLEAN);
			break;
		case T_byte :
			this.setResolvedType(TypeBinding.BYTE);
			break;
		case T_char :
			this.setResolvedType(TypeBinding.CHAR);
			break;
		case T_double :
			this.setResolvedType(TypeBinding.DOUBLE);
			break;
		case T_float :
			this.setResolvedType(TypeBinding.FLOAT);
			break;
		case T_int :
			this.setResolvedType(TypeBinding.INT);
			break;
		case T_long :
			this.setResolvedType(TypeBinding.LONG);
			break;
		case T_JavaLangString :
			this.setResolvedType(scope.getJavaLangString());
			break;
		default : //error........
			this.constant = Constant.NotAConstant;
			scope.problemReporter().invalidOperator(this, leftType, rightType);
