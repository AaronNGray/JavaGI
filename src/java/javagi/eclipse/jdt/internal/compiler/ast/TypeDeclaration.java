...
import javagi.compiler.ClassPatch;
...
import javagi.eclipse.jdt.core.compiler.*;
import javagi.eclipse.jdt.internal.compiler.*;
...
import javagi.eclipse.jdt.internal.compiler.problem.*;
 ...
 * 
 * @see javagi.eclipse.jdt.internal.compiler.codegen.ConstantPool#resetForClinit(int, int)
 */
...
public ConstructorDeclaration createDefaultConstructor(boolean needExplicitConstructorCall, boolean needToInsert) {
    return createDefaultConstructor(needExplicitConstructorCall, needToInsert, false);
}

public ConstructorDeclaration createDefaultConstructor(boolean needExplicitConstructorCall, boolean needToInsert, boolean isJavaGIImplementationConstructor) {
	...
	constructor.isJavaGIImplementationConstructor = isJavaGIImplementationConstructor;
...
public MethodBinding createDefaultConstructorWithBinding(MethodBinding inheritedConstructorBinding) {
	...
	constructor.binding = new MethodBinding(
			...
			inheritedConstructorBinding.constraints,
...
public void generateCode(ClassFile enclosingClassFile) {
	if ((this.bits & ASTNode.HasBeenGenerated) != 0)
		return;
	if (isImplementationReceiver() || isReceiver()) {
	    return;
	}
	...
	try {
		...
		ClassPatch classPatch = Translation.patchClass(this);
		this.binding.applyPatch(classPatch);
		classFile.initialize(this.binding(), classPatch, enclosingClassFile, false);
		...
		// generate all fiels
		classFile.addFieldInfos(classPatch);
		...
		if (this.methods != null) {
			for (int i = 0, max = this.methods.length; i < max; i++) {
			    if (this.methods[i].isStaticInterfaceMethod()) continue;
			    MethodPatch p = Translation.patchMethod(this, this.methods[i], 0, classFile.codeStream);
				this.methods[i].generateCode(this.scope, classFile, p);
				classFile.codeStream.resetPatch();
			}
		}
        if (isImplementation() && this.memberTypes != null) {
            for (int i = 0, max = this.memberTypes.length; i < max; i++) {
                TypeDeclaration memberType = this.memberTypes[i];
                if (memberType.ignoreFurtherInvestigation) continue;
                if (memberType.methods != null) {
                    for (AbstractMethodDeclaration m : memberType.methods) {
                        MethodPatch p = Translation.patchMethod(this, m, memberType.receiverIndex(), classFile.codeStream);
                        m.binding.declaringClass = this.binding();
                        m.generateCode(this.scope, classFile, p);
                        classFile.codeStream.resetPatch();
                    }
                }
            }
        }
        classPatch.addExtraMethods(classFile);
...
public final static int kind(int flags) {
	switch (flags & (ClassFileConstants.AccInterface|ClassFileConstants.AccAnnotation|ClassFileConstants.AccEnum
	                 |ClassFileConstants.AccReceiver|ClassFileConstants.AccImplementationReceiver
	                 |ClassFileConstants.AccImplementation)) {
		...
		case ClassFileConstants.AccReceiver :
		    return TypeDeclaration.RECEIVER_DECL;
	    case ClassFileConstants.AccImplementationReceiver :
	            return TypeDeclaration.IMPLEMENTATION_RECEIVER_DECL;
        case ClassFileConstants.AccImplementation :
            return TypeDeclaration.IMPLEMENTATION_DECL;		    
		default : 
...
public void manageEnclosingInstanceAccessIfNecessary(BlockScope currentScope, FlowInfo flowInfo) {
	...
	if (nestedType.isAnonymousType()) {
		ReferenceBinding superclassBinding = (ReferenceBinding)nestedType.superclass.erasure(currentScope);
...
//					if (nestedEnclosing.findSuperTypeErasingTo(nestedEnclosing.enclosingType()) == null) { // only if not inheriting
					SyntheticArgumentBinding syntheticEnclosingInstanceArgument = nestedEnclosing.getSyntheticArgument(currentScope.getTypeEnvironment(), nestedEnclosing.enclosingType(), true);
...
public StringBuffer printHeader(int indent, StringBuffer output) {
	...
	switch (kind(this.modifiers)) {
		...
		case TypeDeclaration.ANNOTATION_TYPE_DECL :
			output.append("@interface "); //$NON-NLS-1$
			break;
		case TypeDeclaration.RECEIVER_DECL :
		case TypeDeclaration.IMPLEMENTATION_RECEIVER_DECL :
		    output.append("receiver ");
		    break;
		case TypeDeclaration.IMPLEMENTATION_DECL :
		    output.append("implementation");
		    break;
	}
	int k = kind(this.modifiers);
	if (k != TypeDeclaration.IMPLEMENTATION_DECL && k != TypeDeclaration.IMPLEMENTATION_RECEIVER_DECL) output.append(this.name);
	...
	if (this.implParameters != null) {
        output.append("[");//$NON-NLS-1$
        for (int i = 0; i < this.implParameters.length; i++) {
            if (i > 0) output.append( ", "); //$NON-NLS-1$
            this.implParameters[i].print(0, output);
        }
        if (this.implConstraints != null) {
            output.append(" where ");//$NON-NLS-1$
            for (int i = 0; i < this.implConstraints.length; i++) {
                if (i > 0) output.append( ", "); //$NON-NLS-1$
                this.implConstraints[i].print(0, output);
            }           
        }
        output.append("]");//$NON-NLS-1$	    
	}
	if (this.interfaceType != null) {
	    output.append(" ");
	    this.interfaceType.print(0, output);
	}
	if (this.receiverType != null) {
	    this.receiverType.print(0, output);
	}
	if (this.implTypes != null) {
	    output.append(" [");
        for (int i = 0; i < this.implTypes.length; i++) {
            if (i > 0) output.append( ", "); //$NON-NLS-1$
            this.implTypes[i].print(0, output);
        }
	    output.append("]");
	}
	if (k == TypeDeclaration.IMPLEMENTATION_DECL && explicitImplementationName) {
	    output.append(" as ");
    ...
    if (this.constraints != null && this.constraints.length > 0) {
        ...
        for (int i = 0; i < this.constraints.length; i++) {
            if (i > 0) output.append(", "); //$NON-NLS-1$
            this.constraints[i].print(0, output);
        }
    }
	return output;
}
...
public void resolve() {
	...
	try {
		...
		boolean needSerialVersion = 
						...
						&& sourceType.isClass()
						&& sourceType.findSuperTypeOriginatingFrom(scope.getTypeEnvironment(), TypeIds.T_JavaIoExternalizable, false /*Externalizable is not a class*/) == null
						&& sourceType.findSuperTypeOriginatingFrom(scope.getTypeEnvironment(), TypeIds.T_JavaIoSerializable, false /*Serializable is not a class*/) != null;
		...
		if (sourceType.findSuperTypeOriginatingFrom(scope.getTypeEnvironment(), TypeIds.T_JavaLangThrowable, true) != null) {
			...
			checkEnclosedInGeneric : do {
				...
				if (current.isLocalType()) {
					NestedTypeBinding nestedType = (NestedTypeBinding) current.erasure(sourceType.getTypeEnvironment());
	...
	TypeChecker.resolve(this, this.scope);
}
...
public static final int RECEIVER_DECL = 5; // a receiver declaration inside an interface
public static final int IMPLEMENTATION_DECL = 6;
public static final int IMPLEMENTATION_RECEIVER_DECL = 7;
public Constraint[] constraints;       // for classes, interfaces, and implementations
public TypeParameter[] implParameters; // for interfaces only
public Constraint[] implConstraints;   // for interfaces only
public TypeReference[] implTypes;      // for implementation declarations only
public TypeReference interfaceType;    // for implementation declarations only
public char[] receiverVariable;        // for receiver declarations inside interfaces only
public TypeReference receiverType;     // for receiver declarations inside implementations only
private int originalSuperInterfaceCount = -1;

private TypeParameter[] allTypeParameters;
public Constraint extendsConstraint;
public boolean explicitImplementationName;
public TypeReference extendsImplementation;

public TypeParameter[] allTypeParameters() {
    if (allTypeParameters == null) {
        TypeParameter[] a1 = typeParameters == null ? new TypeParameter[0] : typeParameters;
        TypeParameter[] a2 = implParameters == null ? new TypeParameter[0] : implParameters;
        allTypeParameters = new TypeParameter[a1.length + a2.length];
        System.arraycopy(a1, 0, allTypeParameters, 0, a1.length);
        System.arraycopy(a2, 0, allTypeParameters, a1.length, a2.length);
    }
    return allTypeParameters;
}

public void fixSuperInterfaces() {
    if (kind(this.modifiers) != INTERFACE_DECL || implParameters.length > 1 || implConstraints == null || 
        implConstraints.length == 0) return;
    javagi.compiler.GILog.Parsing().jdebug("Fixing super interfaces of interface " + new String(name));
    int n = superInterfaces == null ? 0 : superInterfaces.length;
    int m = implConstraints.length;
    TypeReference[] ts = new TypeReference[n + m];
    if (superInterfaces != null) System.arraycopy(superInterfaces, 0, ts, 0, n);
    for (int i = 0; i < m; i++) {
        Constraint c = implConstraints[i];
        ts[i+n] = c.constrainingType;
    }
    this.superInterfaces = ts;
    this.originalSuperInterfaceCount = n;
}

public boolean isClass() {
    return (kind(this.modifiers) == CLASS_DECL);
}
public boolean isInterface() {
    return (kind(this.modifiers) == INTERFACE_DECL);
}
public boolean isReceiver() {
    return (kind(this.modifiers) == RECEIVER_DECL);
}
public boolean isImplementation() {
    return (kind(this.modifiers) == IMPLEMENTATION_DECL);
}
public boolean isImplementationReceiver() {
    return (kind(this.modifiers) == IMPLEMENTATION_RECEIVER_DECL);
}
public boolean isMultiHeadedInterface() {
    return isInterface() && implParameters.length > 1;
}
public boolean isAbstract() {
    return (this.modifiers & ClassFileConstants.AccAbstract) != 0;
}

public TypeReference[] implTypes() {
    if (isImplementation()) {
        return implTypes;
    } else if (isImplementationReceiver()) {
        return enclosingType.implTypes;
    } else {
        throw new GICompilerBug("implTypes() called on " + this);
    }
}
...
public TypeReference interfaceType() {
    if (isImplementation()) {
        ...
        return enclosingType.interfaceType;
    } else {
        throw new GICompilerBug("interfaceType() called on " + this);
    }
}  

public TypeDeclaration[] receivers() {
    if (memberTypes == null) {
        return new TypeDeclaration[0];
    }
    List<TypeDeclaration> l = new ArrayList<TypeDeclaration>();
    for (TypeDeclaration t : memberTypes) {
        if (t.isImplementationReceiver() || t.isReceiver()) {
            l.add(t);
        }
    }
    return l.toArray(new TypeDeclaration[l.size()]);
}

public void setBinding(SourceTypeBinding binding) {
    this.binding = binding;
    if (this.originalSuperInterfaceCount >= 0) {
        this.binding.originalSuperInterfaceCount = this.originalSuperInterfaceCount;
    } else {
        this.binding.originalSuperInterfaceCount = this.superInterfaces == null ? 0 : this.superInterfaces.length;
    }
}
...
public String formatLocation() {
    return formatLocation(this.scope);
}
...
public int receiverIndex() {
    SourceTypeBinding impl = enclosingType.binding();
    for (int i = 0; i < impl.memberTypes.length; i++) {
        ReferenceBinding recv = impl.memberTypes[i];
        if (recv == binding()) return i;
    }
    throw new GICompilerBug("No receiver index found for " + this.debugName());
}
}
