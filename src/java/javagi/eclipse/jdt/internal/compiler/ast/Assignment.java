...
void checkAssignment(BlockScope scope, TypeBinding lhsType, TypeBinding rhsType) {
	...
	} else if (rhsType.needsUncheckedConversion(scope.getTypeEnvironment(), lhsType)) {
...
public TypeBinding resolveType(BlockScope scope) {
	...
	if (lhsType != null) {
		this.setResolvedType(lhsType.capture(scope));
	}
	...
	if ((this.expression.isConstantValueOfTypeAssignableToType(rhsType, lhsType)
			...
			|| rhsType.isCompatibleWith(scope, lhsType)) {
        ...
        ((Reference) lhs).computeJavaGIConversion(scope);
		...
		return this.getResolvedType();
						...
						|| (rhsType.isBaseType()  // narrowing then boxing ?
								...
								&& !lhsType.isBaseType()
								&& this.expression.isConstantValueOfTypeAssignableToType(rhsType, scope.environment().computeBoxingType(scope.getTypeEnvironment(), lhsType)))) {
		this.expression.computeConversion(scope, lhsType, rhsType);
		if (this.expression instanceof CastExpression 
				&& (this.expression.bits & ASTNode.UnnecessaryCast) == 0) {
			CastExpression.checkNeedForAssignedCast(scope, lhsType, (CastExpression) this.expression);
...
/**
 * @see javagi.eclipse.jdt.internal.compiler.ast.Expression#resolveTypeExpecting(javagi.eclipse.jdt.internal.compiler.lookup.BlockScope, javagi.eclipse.jdt.internal.compiler.lookup.TypeBinding)
 */
@Override
public TypeBinding resolveTypeExpecting(BlockScope scope, TypeBinding expectedType) {
	...
	TypeBinding lhsType = this.getResolvedType(); 
	TypeBinding rhsType = this.expression.getResolvedType();
...
public boolean resolvedTypeIsPossiblyTyvarInst() {
    return lhs.resolvedTypeIsPossiblyTyvarInst();
}
}
