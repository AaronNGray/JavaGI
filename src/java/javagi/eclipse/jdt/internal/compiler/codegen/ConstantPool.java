...
public int literalIndexForType(final char[] constantPoolName) {
    ...
    if ((index = classCache.putIfAbsent(constantPoolName, this.currentIndex)) < 0) {
        GILog.ConstantPool().jdebug("Entering new type into constant pool: %s", new String(constantPoolName));
        // The entry doesn't exit yet
        this.currentIndex++;
        if ((index = -index) > 0xFFFF){
            this.classFile.referenceBinding.scope.problemReporter().noMoreAvailableSpaceInConstantPool(this.classFile.referenceBinding.scope.referenceType());
 ...
 * @see javagi.eclipse.jdt.internal.compiler.ast.TypeDeclaration#addClinit()
 */
