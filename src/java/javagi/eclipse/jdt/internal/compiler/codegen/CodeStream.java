 ...
 *******************************************************************************/
package javagi.eclipse.jdt.internal.compiler.codegen;

import javagi.compiler.MethodPatch;
...
import javagi.eclipse.jdt.internal.compiler.ast.AllocationExpression;
import javagi.eclipse.jdt.internal.compiler.ast.ExplicitConstructorCall;
import javagi.eclipse.jdt.internal.compiler.ast.Expression;
import javagi.eclipse.jdt.internal.compiler.ast.OperatorIds;
...
import javagi.eclipse.jdt.internal.compiler.problem.AbortMethod;
import javagi.eclipse.jdt.internal.compiler.util.Util;

public class CodeStream {
	public static final boolean DEBUG = GILog.CodeGen().isFinest();
...
public void aaload() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\taaload"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
...
public void aastore() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\taastore"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 3;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void aconst_null() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\taconst_null"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
...
// Handle substitution of local variables. We only allow reference variables to
// be substituted, so it's enough to modify the generation of aload and astore
// instructions
public void aload(int iArg) {
    int newIndex = substitueLocalReferenceVariable(iArg);
    _aload(newIndex);
}
public void aload_0() {
    aload(0);
...
public void _aload(int iArg) {
    switch (iArg) {
    case 0: _aload_0(); return;
    case 1: _aload_1(); return;
    case 2: _aload_2(); return;
    case 3: _aload_3(); return;
    }
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\taload:"+iArg); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void _aload_0() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\taload_0"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax) {
		stackMax = stackDepth;
...
private void _aload_1() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\taload_1"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
private void _aload_2() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\taload_2"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
private void _aload_3() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\taload_3"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void anewarray(TypeBinding typeBinding) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tanewarray: " + typeBinding); //$NON-NLS-1$
	countLabels = 0;
...
public void areturn() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tareturn"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
...
public void arrayAtPut(TypeBinding t, boolean valueRequired) {
    arrayAtPut(t.id, valueRequired);
}
...
public void arraylength() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tarraylength"); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
...
//Handle substitution of local variables.
...
public void _astore(int iArg) {
    switch (iArg) {
    case 0: _astore_0(); return;
    case 1: _astore_1(); return;
    case 2: _astore_2(); return;
    case 3: _astore_3(); return;
    }
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tastore:"+iArg); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (maxLocals <= iArg) {
		maxLocals = iArg + 1;
...
private void _astore_0() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tastore_0"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (maxLocals == 0) {
		maxLocals = 1;
...
private void _astore_1() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tastore_1"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (maxLocals <= 1) {
		maxLocals = 2;
...
private void _astore_2() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tastore_2"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (maxLocals <= 2) {
		maxLocals = 3;
...
private void _astore_3() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tastore_3"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (maxLocals <= 3) {
		maxLocals = 4;
...
public void athrow() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tathrow"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void baload() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tbaload"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void bastore() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tbastore"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 3;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void bipush(byte b) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tbipush "+b); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void caload() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tcaload"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void castore() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tcastore"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 3;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void checkcast(TypeBinding typeBinding) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tcheckcast:"+typeBinding.debugName()); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset + 2 >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void checkcast(char[] constantPoolName) {
    if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tcheckcast:"+new String(constantPoolName)); //$NON-NLS-1$
    countLabels = 0;
    if (classFileOffset + 2 >= bCodeStream.length) {
        resizeByteArray();
    }
    position++;
    bCodeStream[classFileOffset++] = Opcodes.OPC_checkcast;
    writeUnsignedShort(constantPool.literalIndexForType(constantPoolName));
}
public void d2f() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\td2f"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void d2i() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\td2i"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void d2l() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\td2l"); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void dadd() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdadd"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void daload() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdaload"); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void dastore() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdastore"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 4;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void dcmpg() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdcmpg"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 3;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void dcmpl() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdcmpl"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 3;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void dconst_0() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdconst_0"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth += 2;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void dconst_1() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdconst_1"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth += 2;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void ddiv() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tddiv"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void dload(int iArg) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdload:"+iArg); //$NON-NLS-1$
	countLabels = 0;
	stackDepth += 2;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void dload_0() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdload_0"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth += 2;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void dload_1() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdload_1"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth += 2;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void dload_2() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdload_2"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth += 2;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void dload_3() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdload_3"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth += 2;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void dmul() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdmul"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void dneg() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdneg"); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void drem() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdrem"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void dreturn() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdreturn"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	// the stackDepth should be equal to 0 
	if (classFileOffset >= bCodeStream.length) {
...
public void dstore(int iArg) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdstore:"+iArg); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (maxLocals <= iArg + 1) {
		maxLocals = iArg + 2;
...
public void dstore_0() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdstore_0"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (maxLocals < 2) {
		maxLocals = 2;
...
public void dstore_1() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdstore_1"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (maxLocals < 3) {
		maxLocals = 3;
...
public void dstore_2() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdstore_2"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (maxLocals < 4) {
		maxLocals = 4;
...
public void dstore_3() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdstore_3"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (maxLocals < 5) {
		maxLocals = 5;
...
public void dsub() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdsub"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void dup() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdup"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax) {
		stackMax = stackDepth;
...
public void dup_x1() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdup_x1"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void dup_x2() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdup_x2"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void dup2() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdup2"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth += 2;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void dup2_x1() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdup2_x1"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth += 2;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void dup2_x2() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tdup2_x2"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth += 2;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void f2d() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tf2d"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void f2i() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tf2i"); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void f2l() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tf2l"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void fadd() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfadd"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void faload() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfaload"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void fcmpg() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfcmpg"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void fcmpl() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfcmpl"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void fconst_0() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfconst_0"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void fconst_1() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfconst_1"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void fconst_2() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfconst_2"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void fdiv() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfdiv"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void fload(int iArg) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfload:"+iArg); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (maxLocals <= iArg) {
		maxLocals = iArg + 1;
...
public void fload_0() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfload_0"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (maxLocals == 0) {
		maxLocals = 1;
...
public void fload_1() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfload_1"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (maxLocals <= 1) {
		maxLocals = 2;
...
public void fload_2() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfload_2"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (maxLocals <= 2) {
		maxLocals = 3;
...
public void fload_3() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfload_3"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (maxLocals <= 3) {
		maxLocals = 4;
...
public void fmul() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfmul"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void fneg() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfneg"); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void frem() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfrem"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void freturn() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfreturn"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	// the stackDepth should be equal to 0 
	if (classFileOffset >= bCodeStream.length) {
...
public void fstore(int iArg) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfstore:"+iArg); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (maxLocals <= iArg) {
		maxLocals = iArg + 1;
...
public void fstore_0() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfstore_0"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (maxLocals == 0) {
		maxLocals = 1;
...
public void fstore_1() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfstore_1"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (maxLocals <= 1) {
		maxLocals = 2;
...
public void fstore_2() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfstore_2"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (maxLocals <= 2) {
		maxLocals = 3;
...
public void fstore_3() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfstore_3"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (maxLocals <= 3) {
		maxLocals = 4;
...
public void fsub() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tfsub"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
    ...
    switch (unboxedTypeID) {
        case TypeIds.T_byte :
            if (this.targetLevel >= ClassFileConstants.JDK1_5) {
    			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokestatic java.lang.Byte.valueOf(byte)"); //$NON-NLS-1$
               ...
               // new Byte( byte )
    			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokespecial java.lang.Byte(byte)"); //$NON-NLS-1$
        ...
        case TypeIds.T_short :
            if ( this.targetLevel >= ClassFileConstants.JDK1_5 ) {
    			...
    			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokestatic java.lang.Short.valueOf(short)"); //$NON-NLS-1$
                this.invoke(
                    Opcodes.OPC_invokestatic,
                    1, // argCount
                    1, // return type size
                ...
                // new Short(short)
            	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokespecial java.lang.Short(short)"); //$NON-NLS-1$
            	newWrapperFor(unboxedTypeID);                
                dup_x1();
                swap();             
                this.invoke(
        ...
        case TypeIds.T_char :
            if ( this.targetLevel >= ClassFileConstants.JDK1_5 ) {
            	...
            	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokestatic java.lang.Character.valueOf(char)"); //$NON-NLS-1$
                this.invoke(
                    Opcodes.OPC_invokestatic,
                    1, // argCount
                    1, // return type size
                ...
                // new Char( char )
            	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokespecial java.lang.Character(char)"); //$NON-NLS-1$
                newWrapperFor(unboxedTypeID);
                dup_x1();
                swap();
                this.invoke(
        ...
        case TypeIds.T_int :             
            if (this.targetLevel >= ClassFileConstants.JDK1_5) {
            	...
            	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokestatic java.lang.Integer.valueOf(int)"); //$NON-NLS-1$
                this.invoke(
                    Opcodes.OPC_invokestatic,
                    1, // argCount
                    1, // return type size
                ...
                // new Integer(int)
            	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokespecial java.lang.Integer(int)"); //$NON-NLS-1$
                newWrapperFor(unboxedTypeID);
                dup_x1();
                swap();             
                this.invoke(
        ...
        case TypeIds.T_long :
            if (this.targetLevel >= ClassFileConstants.JDK1_5) { 
            	...
            	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokestatic java.lang.Long.valueOf(long)"); //$NON-NLS-1$
                this.invoke(
                    Opcodes.OPC_invokestatic,
                    2, // argCount
                    1, // return type size
                ...
                // new Long( long )
            	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokespecial java.lang.Long(long)"); //$NON-NLS-1$
                newWrapperFor(unboxedTypeID);
                dup_x2();
                dup_x2();
                pop();
        ...
        case TypeIds.T_float :
            if ( this.targetLevel >= ClassFileConstants.JDK1_5 ) {
            	...
            	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokestatic java.lang.Float.valueOf(float)"); //$NON-NLS-1$
                this.invoke(
                    Opcodes.OPC_invokestatic,
                    1, // argCount
                    1, // return type size
                ...
                // new Float(float)
            	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokespecial java.lang.Float(float)"); //$NON-NLS-1$
                newWrapperFor(unboxedTypeID);
                dup_x1();
                swap();             
                this.invoke(
        ...
        case TypeIds.T_double :
            if ( this.targetLevel >= ClassFileConstants.JDK1_5 ) { 
            	...
            	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokestatic java.lang.Double.valueOf(double)"); //$NON-NLS-1$
                this.invoke(
                    Opcodes.OPC_invokestatic,
                    2, // argCount
                    1, // return type size
                ...
                // new Double( double )
            	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokespecial java.lang.Double(double)"); //$NON-NLS-1$
            	newWrapperFor(unboxedTypeID);                
                dup_x2();
                dup_x2();
                pop();
        ...
        case TypeIds.T_boolean :
            if ( this.targetLevel >= ClassFileConstants.JDK1_5 ) {
            	...
            	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokestatic java.lang.Boolean.valueOf(boolean)"); //$NON-NLS-1$
                this.invoke(
                    Opcodes.OPC_invokestatic,
                    1, // argCount
                    1, // return type size
                ...
                // new Boolean(boolean)
            	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokespecial java.lang.Boolean(boolean)"); //$NON-NLS-1$
                newWrapperFor(unboxedTypeID);
                dup_x1();
                swap();             
                this.invoke(
...
public void generateFieldAccess(byte opcode, int returnTypeSize, char[] declaringClass, char[] name, char[] signature) {
	countLabels = 0;
	switch(opcode) {
...
public void generateOuterAccess(Object[] mappingSequence, ASTNode invocationSite, Binding target, Scope scope) {
	...
	if (mappingSequence == BlockScope.EmulationPathToImplicitThis) {
		this._aload_0();  // no substitution by patch!
		return;
...
public void generateReturn(TypeBinding t) {
    switch(t.id) {
    case TypeIds.T_void:
        this.return_();
        break;
...
public void generateSyntheticBodyForConstructorAccess(SyntheticMethodBinding accessBinding) {
	...
	if (declaringClass.erasure(((ReferenceBinding) declaringClass).getTypeEnvironment()).id == TypeIds.T_JavaLangEnum || declaringClass.isEnum()) {
...
public void generateSyntheticBodyForMethodAccess(SyntheticMethodBinding accessMethod) {
	...
	switch (targetMethod.returnType.id) {
		...
		case TypeIds.T_double :
			...
			break;
		default :
			TypeBinding accessErasure = accessMethod.returnType.erasure(accessMethod.getTypeEnvironment());
			TypeBinding match = targetMethod.returnType.findSuperTypeOriginatingFrom(targetMethod.getTypeEnvironment(), accessErasure);
			if (match == null) {
...
public void generateSyntheticEnclosingInstanceValues(
	...
	ReferenceBinding checkedTargetType = targetType.isAnonymousType() ? (ReferenceBinding)targetType.superclass().erasure(currentScope) : targetType;
...
public void getfield(ReferenceBinding declaringClass, char[] name, TypeBinding type) {
    int returnTypeSize = 1;
    if ((type.id == TypeIds.T_double) || (type.id == TypeIds.T_long)) {
        returnTypeSize = 2;
    }
    generateFieldAccess(
...
public void getfield(FieldBinding fieldBinding) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tgetfield:"+fieldBinding); //$NON-NLS-1$
	getfield(fieldBinding.declaringClass,
			 fieldBinding.name,
			 fieldBinding.type);
}
...
public void getstatic(FieldBinding fieldBinding) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tgetstatic:"+fieldBinding); //$NON-NLS-1$
	int returnTypeSize = 1;
...
public void getTYPE(int baseTypeID) {
	...
	switch (baseTypeID) {
		case TypeIds.T_byte :
			...
			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tgetstatic: java.lang.Byte.TYPE"); //$NON-NLS-1$
			generateFieldAccess(
					Opcodes.OPC_getstatic,
					1,
					ConstantPool.JavaLangByteConstantPoolName,
		...
		case TypeIds.T_short :
			...
			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tgetstatic: java.lang.Short.TYPE"); //$NON-NLS-1$
			generateFieldAccess(
					Opcodes.OPC_getstatic,
					1,
					ConstantPool.JavaLangShortConstantPoolName,
		...
		case TypeIds.T_char :
			...
			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tgetstatic: java.lang.Character.TYPE"); //$NON-NLS-1$
			generateFieldAccess(
					Opcodes.OPC_getstatic,
					1,
					ConstantPool.JavaLangCharacterConstantPoolName,
		...
		case TypeIds.T_int :
			...
			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tgetstatic: java.lang.Integer.TYPE"); //$NON-NLS-1$
			generateFieldAccess(
					Opcodes.OPC_getstatic,
					1,
					ConstantPool.JavaLangIntegerConstantPoolName,
		...
		case TypeIds.T_long :
			...
			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tgetstatic: java.lang.Long.TYPE"); //$NON-NLS-1$
			generateFieldAccess(
					Opcodes.OPC_getstatic,
					1,
					ConstantPool.JavaLangLongConstantPoolName,
		...
		case TypeIds.T_float :
			...
			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tgetstatic: java.lang.Float.TYPE"); //$NON-NLS-1$
			generateFieldAccess(
					Opcodes.OPC_getstatic,
					1,
					ConstantPool.JavaLangFloatConstantPoolName,
		...
		case TypeIds.T_double :
			...
			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tgetstatic: java.lang.Double.TYPE"); //$NON-NLS-1$
			generateFieldAccess(
					Opcodes.OPC_getstatic,
					1,
					ConstantPool.JavaLangDoubleConstantPoolName,
		...
		case TypeIds.T_boolean :
			...
			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tgetstatic: java.lang.Boolean.TYPE"); //$NON-NLS-1$
			generateFieldAccess(
					Opcodes.OPC_getstatic,
					1,
					ConstantPool.JavaLangBooleanConstantPoolName,
		...
		case TypeIds.T_void :
			...
			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tgetstatic: java.lang.Void.TYPE"); //$NON-NLS-1$
			generateFieldAccess(
					Opcodes.OPC_getstatic,
					1,
...
public void goto_(BranchLabel label) {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tgoto:"+label); //$NON-NLS-1$
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
	...
	if (DEBUG && chained) {
		if (DEBUG) {
			if (this.lastAbruptCompletion == this.position) {
				GILog.CodeGen().jfinest("\t\t\t\t<branch chaining - goto eliminated : "+this.position+","+label+">");//$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$				
			} else {
				GILog.CodeGen().jfinest("\t\t\t\t<branch chaining - goto issued : "+this.position+","+label+">");//$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$
			}
		}		
	}
	/*
...
public void goto_w(BranchLabel label) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tgotow:"+label); //$NON-NLS-1$
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
	position++;
...
public void i2b() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\ti2b"); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void i2c() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\ti2c"); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void i2d() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\ti2d"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void i2f() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\ti2f"); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void i2l() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\ti2l"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void i2s() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\ti2s"); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void iadd() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tiadd"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void iaload() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tiaload"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void iand() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tiand"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void iastore() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tiastore"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 3;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void iconst_0() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\ticonst_0"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void iconst_1() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\ticonst_1"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void iconst_2() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\ticonst_2"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void iconst_3() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\ticonst_3"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void iconst_4() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\ticonst_4"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void iconst_5() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\ticonst_5"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void iconst_m1() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\ticonst_m1"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void idiv() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tidiv"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void if_acmpeq(BranchLabel lbl) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tif_acmpeq:"+lbl); //$NON-NLS-1$
	countLabels = 0;
	stackDepth-=2;
	if (this.wideMode) {
...
public void if_acmpne(BranchLabel lbl) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tif_acmpne:"+lbl); //$NON-NLS-1$
	countLabels = 0;
	stackDepth-=2;
	if (this.wideMode) {
...
public void if_icmpeq(BranchLabel lbl) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tif_cmpeq:"+lbl); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (this.wideMode) {
...
public void if_icmpge(BranchLabel lbl) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tif_icmpge:"+lbl); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (this.wideMode) {
...
public void if_icmpgt(BranchLabel lbl) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tif_icmpgt:"+lbl); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (this.wideMode) {
...
public void if_icmple(BranchLabel lbl) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tif_icmple:"+lbl); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (this.wideMode) {
...
public void if_icmplt(BranchLabel lbl) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tif_icmplt:"+lbl); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (this.wideMode) {
...
public void if_icmpne(BranchLabel lbl) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tif_icmpne:"+lbl); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (this.wideMode) {
...
public void ifeq(BranchLabel lbl) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tifeq:"+lbl); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (this.wideMode) {
...
public void ifge(BranchLabel lbl) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tifge:"+lbl); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (this.wideMode) {
...
public void ifgt(BranchLabel lbl) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tifgt:"+lbl); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (this.wideMode) {
...
public void ifle(BranchLabel lbl) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tifle:"+lbl); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (this.wideMode) {
...
public void iflt(BranchLabel lbl) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tiflt:"+lbl); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (this.wideMode) {
...
public void ifne(BranchLabel lbl) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tifne:"+lbl); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (this.wideMode) {
...
public void ifnonnull(BranchLabel lbl) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tifnonnull:"+lbl); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (this.wideMode) {
...
public void ifnull(BranchLabel lbl) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tifnull:"+lbl); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (this.wideMode) {
...
final public void iinc(int index, int value) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tiinc:"+index+","+value); //$NON-NLS-1$ //$NON-NLS-2$
	countLabels = 0;
...
public void iload(int iArg) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tiload:"+iArg); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (maxLocals <= iArg) {
		maxLocals = iArg + 1;
...
public void iload_0() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tiload_0"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (maxLocals <= 0) {
		maxLocals = 1;
...
public void iload_1() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tiload_1"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (maxLocals <= 1) {
		maxLocals = 2;
...
public void iload_2() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tiload_2"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (maxLocals <= 2) {
		maxLocals = 3;
...
public void iload_3() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tiload_3"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (maxLocals <= 3) {
		maxLocals = 4;
...
public void imul() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\timul"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void ineg() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tineg"); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void initializeMaxLocals(MethodBinding methodBinding) {
	...
	if (methodBinding.extraParameters != null) {
	    for (TypeBinding t : methodBinding.extraParameters) {
            if (t == TypeBinding.LONG || t == TypeBinding.DOUBLE) {
                this.maxLocals += 2;
            } else {
                this.maxLocals++;
            }	        
...
public void instance_of(TypeBinding typeBinding) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinstance_of:"+typeBinding); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset + 2 >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void invoke(int opcode, int argsSize, int returnTypeSize, char[] declaringClass, char[] selector, char[] signature) {
	countLabels = 0;
...
public void invokeClassForName() {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokestatic: java.lang.Class.forName(Ljava.lang.String;)Ljava.lang.Class;"); //$NON-NLS-1$
	this.invoke(
		Opcodes.OPC_invokestatic,
		1, // argCount
		1, // return type size
...
public void invokeEnumOrdinal(char[] enumTypeConstantPoolName) {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokevirtual: "+new String(enumTypeConstantPoolName)+".ordinal()"); //$NON-NLS-1$ //$NON-NLS-2$
	this.invoke(
			Opcodes.OPC_invokevirtual,
			0, // argCount
			1, // return type size
...
public void invokeinterface(MethodBinding methodBinding) {
    invokeinterface(methodBinding.constantPoolDeclaringClass().constantPoolName(), 
                    methodBinding.selector,
                    methodBinding.signature(classFile), methodBinding.parameters,
                    methodBinding.returnType);
}
public void invokeinterface(char[] declaringClass, char[] name, char[] signature, TypeBinding[] parameters,
                            TypeBinding returnType) {
	if (DEBUG) {	    
	    GILog.CodeGen().jfinest(position + "\t\tinvokeinterface: %s, %s, %s, %s, %s",
	                            new String(declaringClass), new String(name), new String(signature),
	                            Util.toString(parameters), returnType.debugName());
	}
	countLabels = 0;
	...
	for (int i = parameters.length - 1; i >= 0; i--)
		if (((id = parameters[i].id) == TypeIds.T_double) || (id == TypeIds.T_long))
			argCount += 2;
		else
			argCount += 1;
	bCodeStream[classFileOffset++] = (byte) argCount;
	...
	// the number of bytes.
	bCodeStream[classFileOffset++] = 0;
	if (((id = returnType.id) == TypeIds.T_double) || (id == TypeIds.T_long)) {
...
public void invokeJavaLangAssertionErrorConstructor(int typeBindingID) {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokespecial: java.lang.AssertionError.<init>(typeBindingID)V"); //$NON-NLS-1$
	int argCount = 1;
	char[] signature = null;
	switch (typeBindingID) {
		case TypeIds.T_int :
...
public void invokeJavaLangAssertionErrorDefaultConstructor() {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokespecial: java.lang.AssertionError.<init>()V"); //$NON-NLS-1$
	this.invoke(
			Opcodes.OPC_invokespecial,
			0, // argCount
			0, // return type size
...
public void invokeJavaLangClassDesiredAssertionStatus() {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokevirtual: java.lang.Class.desiredAssertionStatus()Z;"); //$NON-NLS-1$
	this.invoke(
			Opcodes.OPC_invokevirtual,
			0, // argCount
			1, // return type size
...
public void invokeJavaLangEnumvalueOf(ReferenceBinding binding) {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokestatic: java.lang.Enum.valueOf(Ljava/lang/Class;Ljava/lang/String;)Ljava/lang/Enum;"); //$NON-NLS-1$
	this.invoke(
			Opcodes.OPC_invokestatic,
			2, // argCount
			1, // return type size
...
public void invokeJavaLangErrorConstructor() {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokespecial: java.lang.Error<init>(Ljava.lang.String;)V"); //$NON-NLS-1$
	this.invoke(
			Opcodes.OPC_invokespecial,
			1, // argCount
			0, // return type size
...
public void invokeJavaUtilIteratorHasNext() {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokeinterface: java.util.Iterator.hasNext()Z"); //$NON-NLS-1$
	this.invoke(
...
public void invokeJavaUtilIteratorNext() {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokeinterface: java.util.Iterator.next()java.lang.Object"); //$NON-NLS-1$
	this.invoke(
			Opcodes.OPC_invokeinterface,
			0, // argCount
			1, // return type size
...
public void invokeNoClassDefFoundErrorStringConstructor() {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokespecial: java.lang.NoClassDefFoundError.<init>(Ljava.lang.String;)V"); //$NON-NLS-1$
	this.invoke(
			Opcodes.OPC_invokespecial,
			1, // argCount
			0, // return type size
...
public void invokeObjectGetClass() {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokevirtual: java.lang.Object.getClass()Ljava.lang.Class;"); //$NON-NLS-1$
	this.invoke(
			Opcodes.OPC_invokevirtual,
			0, // argCount
			1, // return type size
...
public void invokespecial(MethodBinding methodBinding) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokespecial:"+methodBinding); //$NON-NLS-1$
	countLabels = 0;
	// initialized to 1 to take into account this immediately
	int argCount = 1;
	int id;
...
public void invokestatic(MethodBinding methodBinding) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokestatic:"+methodBinding); //$NON-NLS-1$
...
public void invokeStringConcatenationAppendForType(int typeID) {
	if (DEBUG) {
		if (this.targetLevel >= ClassFileConstants.JDK1_5) {
			GILog.CodeGen().jfinest(position + "\t\tinvokevirtual: java.lang.StringBuilder.append(...)"); //$NON-NLS-1$
		} else {
			GILog.CodeGen().jfinest(position + "\t\tinvokevirtual: java.lang.StringBuffer.append(...)"); //$NON-NLS-1$
		}
	}
	int argCount = 1;
	int returnType = 1;
...
public void invokeStringConcatenationDefaultConstructor() {
	...
	if (DEBUG) {
		if (this.targetLevel >= ClassFileConstants.JDK1_5) {
			GILog.CodeGen().jfinest(position + "\t\tinvokespecial: java.lang.StringBuilder.<init>()V"); //$NON-NLS-1$
		} else {
			GILog.CodeGen().jfinest(position + "\t\tinvokespecial: java.lang.StringBuffer.<init>()V"); //$NON-NLS-1$
		}
	}
...
public void invokeStringConcatenationStringConstructor() {
	if (DEBUG) {
		if (this.targetLevel >= ClassFileConstants.JDK1_5) {
			GILog.CodeGen().jfinest(position + "\t\tjava.lang.StringBuilder.<init>(Ljava.lang.String;)V"); //$NON-NLS-1$
		} else {
			GILog.CodeGen().jfinest(position + "\t\tjava.lang.StringBuffer.<init>(Ljava.lang.String;)V"); //$NON-NLS-1$
		}
	}
	char[] declaringClass = ConstantPool.JavaLangStringBufferConstantPoolName;
	if (this.targetLevel >= ClassFileConstants.JDK1_5) {
...
public void invokeStringConcatenationToString() {
	if (DEBUG) {
		if (this.targetLevel >= ClassFileConstants.JDK1_5) {
			GILog.CodeGen().jfinest(position + "\t\tinvokevirtual: StringBuilder.toString()Ljava.lang.String;"); //$NON-NLS-1$
		} else {
			GILog.CodeGen().jfinest(position + "\t\tinvokevirtual: StringBuffer.toString()Ljava.lang.String;"); //$NON-NLS-1$
		}
	}
	char[] declaringClass = ConstantPool.JavaLangStringBufferConstantPoolName;
	if (this.targetLevel >= ClassFileConstants.JDK1_5) {
...
public void invokeStringIntern() {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokevirtual: java.lang.String.intern()"); //$NON-NLS-1$
	this.invoke(
			Opcodes.OPC_invokevirtual,
			0, // argCount
			1, // return type size
...
public void invokeStringValueOf(int typeID) {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokestatic: java.lang.String.valueOf(...)"); //$NON-NLS-1$
	int argCount = 1;
	char[] signature = null;
	switch (typeID) {
		case TypeIds.T_int :
...
public void invokeSystemArraycopy() {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokevirtual: java.lang.System.arraycopy(Ljava/lang/Object;ILjava/lang/Object;II)V"); //$NON-NLS-1$
	this.invoke(
			Opcodes.OPC_invokestatic,
			5, // argCount
			0, // return type size
...
public void invokeThrowableGetMessage() {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokevirtual: java.lang.Throwable.getMessage()Ljava.lang.String;"); //$NON-NLS-1$
	this.invoke(
			Opcodes.OPC_invokevirtual,
			0, // argCount
			1, // return type size
...
public void invokevirtual(MethodBinding methodBinding) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tinvokevirtual:"+methodBinding); //$NON-NLS-1$
	countLabels = 0;
	// initialized to 1 to take into account this  immediately
	int argCount = 1;
	int id;
...
public void ior() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tior"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void irem() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tirem"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void ireturn() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tireturn"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	// the stackDepth should be equal to 0 
	if (classFileOffset >= bCodeStream.length) {
...
public void ishl() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tishl"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void ishr() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tishr"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void istore(int iArg) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tistore:"+iArg); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (maxLocals <= iArg) {
		maxLocals = iArg + 1;
...
public void istore_0() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tistore_0"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (maxLocals == 0) {
		maxLocals = 1;
...
public void istore_1() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tistore_1"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (maxLocals <= 1) {
		maxLocals = 2;
...
public void istore_2() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tistore_2"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (maxLocals <= 2) {
		maxLocals = 3;
...
public void istore_3() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tistore_3"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (maxLocals <= 3) {
		maxLocals = 4;
...
public void isub() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tisub"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void iushr() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tiushr"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void ixor() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tixor"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
final public void jsr(BranchLabel lbl) {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tjsr"+lbl); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
...
final public void jsr_w(BranchLabel lbl) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tjsr_w"+lbl); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void l2d() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tl2d"); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void l2f() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tl2f"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void l2i() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tl2i"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void ladd() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tladd"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void laload() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlaload"); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void land() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tland"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void lastore() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlastore"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 4;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void lcmp() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlcmp"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 3;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void lconst_0() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlconst_0"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth += 2;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void lconst_1() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlconst_1"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth += 2;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void ldc(float constant) {
	...
	if (stackDepth > stackMax)
		stackMax = stackDepth;
	if (index > 255) {
		if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tldc_w:"+constant); //$NON-NLS-1$
		// Generate a ldc_w
		if (classFileOffset + 2 >= bCodeStream.length) {
			resizeByteArray();
		}
		...
		if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tldc:"+constant); //$NON-NLS-1$
		// Generate a ldc
		if (classFileOffset + 1 >= bCodeStream.length) {
			resizeByteArray();
		}
...
public void ldc(char[] constantPoolName) {
    int index = constantPool.literalIndexForType(constantPoolName);
    ldcTypeIndex(index);
}
private void ldcTypeIndex(int index) {
    countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
	if (index > 255) {
		if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tldc_w:"+ index); //$NON-NLS-1$
		// Generate a ldc_w
		if (classFileOffset + 2 >= bCodeStream.length) {
			resizeByteArray();
		}
		...
		if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tldw:"+ index); //$NON-NLS-1$
		// Generate a ldc
		if (classFileOffset + 1 >= bCodeStream.length) {
			resizeByteArray();
		}
...
public void ldc2_w(double constant) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tldc2_w:"+constant); //$NON-NLS-1$
	countLabels = 0;
	int index = constantPool.literalIndex(constant);
	stackDepth += 2;
	if (stackDepth > stackMax)
...
public void ldcForIndex(int index, char[] constant) {
	...
	if (index > 255) {
		// Generate a ldc_w
		if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tldc_w:"+ new String(constant)); //$NON-NLS-1$
		if (classFileOffset + 2 >= bCodeStream.length) {
			resizeByteArray();
		}
		position++;
		...
		// Generate a ldc
		if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tldc:"+ new String(constant)); //$NON-NLS-1$
		if (classFileOffset + 1 >= bCodeStream.length) {
			resizeByteArray();
		}
		position += 2;
...
public void ldiv() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tldiv"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void lload(int iArg) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlload:"+iArg); //$NON-NLS-1$
	countLabels = 0;
	stackDepth += 2;
	if (maxLocals <= iArg + 1) {
		maxLocals = iArg + 2;
...
public void lload_0() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlload_0"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth += 2;
	if (maxLocals < 2) {
		maxLocals = 2;
...
public void lload_1() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlload_1"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth += 2;
	if (maxLocals < 3) {
		maxLocals = 3;
...
public void lload_2() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlload_2"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth += 2;
	if (maxLocals < 4) {
		maxLocals = 4;
...
public void lload_3() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlload_3"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth += 2;
	if (maxLocals < 5) {
		maxLocals = 5;
...
public void lmul() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlmul"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void lneg() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlneg"); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void lookupswitch(CaseLabel defaultLabel, int[] keys, int[] sortedIndexes, CaseLabel[] casesLabel) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlookupswitch"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
...
public void lor() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlor"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void lrem() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlrem"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void lreturn() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlreturn"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	// the stackDepth should be equal to 0 
	if (classFileOffset >= bCodeStream.length) {
...
public void lshl() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlshl"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void lshr() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlshr"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void lstore(int iArg) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlstore:"+iArg); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (maxLocals <= iArg + 1) {
		maxLocals = iArg + 2;
...
public void lstore_0() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlstore_0"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (maxLocals < 2) {
		maxLocals = 2;
...
public void lstore_1() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlstore_1"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (maxLocals < 3) {
		maxLocals = 3;
...
public void lstore_2() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlstore_2"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (maxLocals < 4) {
		maxLocals = 4;
...
public void lstore_3() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlstore_3"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (maxLocals < 5) {
		maxLocals = 5;
...
public void lsub() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlsub"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void lushr() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlushr"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void lxor() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tlxor"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void monitorenter() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tmonitorenter"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void monitorexit() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tmonitorexit"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void multianewarray(TypeBinding typeBinding, int dimensions) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tmultinewarray:"+typeBinding+","+dimensions); //$NON-NLS-1$ //$NON-NLS-2$
	countLabels = 0;
...
private void new_(int constantPoolIndex) {
    if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tnew:"+constantPoolIndex); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
	...
	writeUnsignedShort(constantPoolIndex);
}
public void new_(TypeBinding typeBinding) {
    new_(constantPool.literalIndexForType(typeBinding));
}
public void new_(char[] constantPoolTypeName) {
    new_(constantPool.literalIndexForType(constantPoolTypeName));
}
public void newarray(int array_Type) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tnewarray:"+array_Type); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset + 1 >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void newArray(ArrayBinding arrayBinding) {
	...
	newArrayOfType(component);
}
public void newArrayOfType(TypeBinding component) {
	switch (component.id) {
		case TypeIds.T_int :
			this.newarray(ClassFileConstants.INT_ARRAY);
			break;
...
public void newJavaLangAssertionError() {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tnew: java.lang.AssertionError"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void newJavaLangError() {
	// new: java.lang.Error
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tnew: java.lang.Error"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void newNoClassDefFoundError() {
	...
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tnew: java.lang.NoClassDefFoundError"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void newStringContatenation() {
	...
	if (DEBUG) {
		if (this.targetLevel >= ClassFileConstants.JDK1_5) {
			GILog.CodeGen().jfinest(position + "\t\tnew: java.lang.StringBuilder"); //$NON-NLS-1$
		} else {
			GILog.CodeGen().jfinest(position + "\t\tnew: java.lang.StringBuffer"); //$NON-NLS-1$
		}
	}
	countLabels = 0;
	stackDepth++;
...
public void newWrapperFor(int typeID) {
	...
	switch (typeID) {
		case TypeIds.T_int : // new: java.lang.Integer
			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tnew: java.lang.Integer"); //$NON-NLS-1$
		...
		case TypeIds.T_boolean : // new: java.lang.Boolean
			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tnew: java.lang.Boolean"); //$NON-NLS-1$
		...
		case TypeIds.T_byte : // new: java.lang.Byte
			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tnew: java.lang.Byte"); //$NON-NLS-1$
		...
		case TypeIds.T_char : // new: java.lang.Character
			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tnew: java.lang.Character"); //$NON-NLS-1$
		...
		case TypeIds.T_float : // new: java.lang.Float
			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tnew: java.lang.Float"); //$NON-NLS-1$
		...
		case TypeIds.T_double : // new: java.lang.Double
			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tnew: java.lang.Double"); //$NON-NLS-1$
		...
		case TypeIds.T_short : // new: java.lang.Short
			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tnew: java.lang.Short"); //$NON-NLS-1$
		...
		case TypeIds.T_long : // new: java.lang.Long
			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tnew: java.lang.Long"); //$NON-NLS-1$
		...
		case TypeIds.T_void : // new: java.lang.Void
			if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tnew: java.lang.Void"); //$NON-NLS-1$
			writeUnsignedShort(constantPool.literalIndexForType(ConstantPool.JavaLangVoidConstantPoolName));
	}
}
public void nop() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tnop"); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void pop() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tpop"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void pop2() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tpop2"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 2;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void putfield(FieldBinding fieldBinding) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tputfield:"+fieldBinding); //$NON-NLS-1$
	putfield(fieldBinding.declaringClass,
             fieldBinding.name,
             fieldBinding.type);
}
public void putfield(ReferenceBinding declaringClass, char[] name, TypeBinding type) {
	int returnTypeSize = 1;
	if ((type.id == TypeIds.T_double) || (type.id == TypeIds.T_long)) {
		returnTypeSize = 2;
	}
...
public void putstatic(FieldBinding fieldBinding) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tputstatic:"+fieldBinding); //$NON-NLS-1$
	int returnTypeSize = 1;
	if ((fieldBinding.type.id == TypeIds.T_double) || (fieldBinding.type.id == TypeIds.T_long)) {
		returnTypeSize = 2;
	}
...
/**
 * @param anExceptionLabel javagi.eclipse.jdt.internal.compiler.codegen.ExceptionLabel
 */
...
/**
 * @param referenceMethod javagi.eclipse.jdt.internal.compiler.ast.AbstractMethodDeclaration
 * @param targetClassFile javagi.eclipse.jdt.internal.compiler.codegen.ClassFile
 */
...
final public void ret(int index) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tret:"+index); //$NON-NLS-1$
	countLabels = 0;
	if (index > 255) { // Widen
		if (classFileOffset + 3 >= bCodeStream.length) {
			resizeByteArray();
...
public void return_() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\treturn"); //$NON-NLS-1$
	countLabels = 0;
	// the stackDepth should be equal to 0 
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void saload() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tsaload"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void sastore() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tsastore"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth -= 3;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
...
public void sipush(int s) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tsipush:"+s); //$NON-NLS-1$
	countLabels = 0;
	stackDepth++;
	if (stackDepth > stackMax)
		stackMax = stackDepth;
...
public void swap() {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\tswap"); //$NON-NLS-1$
	countLabels = 0;
	if (classFileOffset >= bCodeStream.length) {
		resizeByteArray();
	}
...
public void tableswitch(CaseLabel defaultLabel, int low, int high, int[] keys, int[] sortedIndexes, CaseLabel[] casesLabel) {
	if (DEBUG) GILog.CodeGen().jfinest(position + "\t\ttableswitch"); //$NON-NLS-1$
	countLabels = 0;
	stackDepth--;
...
public void setPatch(MethodPatch patch) {
    this.patch = patch;
}
public MethodPatch getPatch() {
    return patch;
}
...
private MethodPatch patch;
private int substitueLocalReferenceVariable(int i) {
    if (getPatch() == null) {
        GILog.CodeGen().jfinest("No method patch for code stream");
        return i;
    }
    return getPatch().substituteLocalReferenceVariable(i);
}
}
