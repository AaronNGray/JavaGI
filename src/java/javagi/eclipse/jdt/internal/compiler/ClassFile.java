 ...
 *******************************************************************************/
package javagi.eclipse.jdt.internal.compiler;
...
import javagi.compiler.DefaultClassPatch;
import javagi.compiler.DefaultMethodPatch;
import javagi.compiler.SyntheticTypeBinding;
import javagi.compiler.TypeEnvironment;
import javagi.compiler.Version;
...
import javagi.eclipse.jdt.internal.compiler.ast.AnnotationMethodDeclaration;
import javagi.eclipse.jdt.internal.compiler.ast.Argument;
import javagi.eclipse.jdt.internal.compiler.ast.ArrayInitializer;
import javagi.eclipse.jdt.internal.compiler.ast.ClassLiteralAccess;
...
import javagi.eclipse.jdt.internal.compiler.ast.MemberValuePair;
import javagi.eclipse.jdt.internal.compiler.ast.MethodDeclaration;
import javagi.eclipse.jdt.internal.compiler.ast.NormalAnnotation;
import javagi.eclipse.jdt.internal.compiler.ast.QualifiedNameReference;
import javagi.eclipse.jdt.internal.compiler.ast.SingleMemberAnnotation;
import javagi.eclipse.jdt.internal.compiler.ast.SingleNameReference;
...
import javagi.eclipse.jdt.internal.compiler.codegen.AttributeNamesConstants;
...
import javagi.eclipse.jdt.internal.compiler.codegen.ExceptionLabel;
import javagi.eclipse.jdt.internal.compiler.codegen.Opcodes;
import javagi.eclipse.jdt.internal.compiler.codegen.StackMapFrame;
import javagi.eclipse.jdt.internal.compiler.codegen.StackMapFrameCodeStream;
import javagi.eclipse.jdt.internal.compiler.codegen.VerificationTypeInfo;
import javagi.eclipse.jdt.internal.compiler.codegen.StackMapFrameCodeStream.ExceptionMarker;
import javagi.eclipse.jdt.internal.compiler.codegen.StackMapFrameCodeStream.StackDepthMarker;
import javagi.eclipse.jdt.internal.compiler.codegen.StackMapFrameCodeStream.StackMarker;
...
import javagi.eclipse.jdt.internal.compiler.impl.StringConstant;
...
import javagi.eclipse.jdt.internal.compiler.lookup.LocalTypeBinding;
...
import javagi.eclipse.jdt.internal.compiler.lookup.SyntheticArgumentBinding;
...
import javagi.eclipse.jdt.internal.compiler.problem.ShouldNotImplement;
...
public class ClassFile implements TypeConstants, TypeIds {
	 ...
	 *
	 * @param typeDeclaration javagi.eclipse.jdt.internal.compiler.ast.TypeDeclaration
	 * @param unitResult javagi.eclipse.jdt.internal.compiler.CompilationUnitResult
	 */
	public static void createProblemType(TypeDeclaration typeDeclaration, CompilationResult unitResult) {
		SourceTypeBinding typeBinding = typeDeclaration.binding();
		ClassFile classFile = ClassFile.getNewInstance(typeBinding);
		classFile.initialize(typeBinding, DefaultClassPatch.theInstance, null, true);
		...
		if ((fields != null) && (fields != Binding.NO_FIELDS)) {
			classFile.addFieldInfos(DefaultClassPatch.theInstance);
		} else {
	 ...
	 *
	 * @param method javagi.eclipse.jdt.internal.compiler.ast.AbstractMethodDeclaration
	 * @param methodBinding javagi.eclipse.jdt.internal.compiler.nameloopkup.MethodBinding
	 */
	 ...
	 * - a field info for each synthetic field (e.g. this$0)
	 */
	public void addFieldInfos(ClassPatch patch) {
	 ...
	 *
	 * @param problems javagi.eclipse.jdt.internal.compiler.problem.Problem[]
	 */
	 ...
	 *
	 * @param methodBinding javagi.eclipse.jdt.internal.compiler.nameloopkup.SyntheticAccessMethodBinding
	 */
...
//						stackMapFrameCodeStream.addExceptionMarker(handlerPC, exceptionLabel.exceptionType);
	 ...
	 * - and debug attributes if necessary.
	 *
	 * @param binding javagi.eclipse.jdt.internal.compiler.lookup.SyntheticAccessMethodBinding
	 * @param codeAttributeOffset <CODE>int</CODE>
	 */
...
//							stackMapFrameCodeStream.addExceptionMarker(handlerPC, exceptionLabel.exceptionType);
	...
	private void generateAnnotation(Annotation annotation, int currentOffset) {
		...
		TypeBinding annotationTypeBinding = annotation.getResolvedType();
	...
	private void generateElementValue(
		...
		TypeBinding defaultValueBinding = defaultValue.getResolvedType();
	 ...
	 *
	 * @param methodBinding javagi.eclipse.jdt.internal.compiler.lookup.MethodBinding
	...
	public void generateMethodInfoHeader(MethodBinding methodBinding, int accessFlags) {
        ...
        generateMethodInfoHeader(accessFlags, methodBinding.selector, methodBinding.signature(this));
	}
	
	public void generateMethodInfoHeader(int accessFlags, char[] selector, char[] signature) {
		...
		contents[contentsOffset++] = (byte) (accessFlags >> 8);
		contents[contentsOffset++] = (byte) accessFlags;
		int nameIndex = constantPool.literalIndex(selector);
		contents[contentsOffset++] = (byte) (nameIndex >> 8);
		contents[contentsOffset++] = (byte) nameIndex;
		int descriptorIndex = constantPool.literalIndex(signature);
		contents[contentsOffset++] = (byte) (descriptorIndex >> 8);
		contents[contentsOffset++] = (byte) descriptorIndex;
	}
	...
	public void initialize(SourceTypeBinding aType, ClassPatch patch, ClassFile parentClassFile, boolean createProblemType) {
		...
		if (aType.isInterface()) {
		    ...
		    ReferenceBinding sup = aType.superclass(); // getPatchedSuperClass(patch);
			superclassNameIndex =
				(sup == null ? 0 : constantPool.literalIndexForType(sup));
		}
		contents[contentsOffset++] = (byte) (superclassNameIndex >> 8);
		contents[contentsOffset++] = (byte) superclassNameIndex;
		ReferenceBinding[] superInterfacesBinding = aType.superInterfaces();
		String[] extraSuperInterfaces = patch.extraSuperInterfaces(); 
		int interfacesCount = superInterfacesBinding.length + extraSuperInterfaces.length;
		contents[contentsOffset++] = (byte) (interfacesCount >> 8);
		contents[contentsOffset++] = (byte) interfacesCount;
		for (int i = 0; i < superInterfacesBinding.length; i++) {
		...
		for (int i = 0; i < extraSuperInterfaces.length; i++) {
		    int interfaceIndex = constantPool.literalIndexForType(extraSuperInterfaces[i].toCharArray());
	        contents[contentsOffset++] = (byte) (interfaceIndex >> 8);
	        contents[contentsOffset++] = (byte) interfaceIndex;
		}
		...
		constantPool.literalIndex("compiled with " + Version.versionString);
	}
	...
	private boolean isRuntimeInvisible(Annotation annotation) {
		final TypeBinding annotationBinding = annotation.getResolvedType();
	 ...
	 * @return javagi.eclipse.jdt.internal.compiler.codegen.ClassFile
	 */
	...
	public void recordInnerClasses(TypeBinding binding) {
		...
		this.innerClassesBindings.add(innerClass.erasure(innerClass.getTypeEnvironment()));
		ReferenceBinding enclosingType = innerClass.enclosingType();
		while (enclosingType != null
				&& enclosingType.isNestedType()) {
			this.innerClassesBindings.add(enclosingType.erasure(enclosingType.getTypeEnvironment()));
	...
	private VerificationTypeInfo retrieveLocal(int currentPC, int resolvedPosition) {
		...
		if (codeStream.getPatch() != null) {
		    MethodPatch p = codeStream.getPatch();
		    for (FreshLocal l : p.extraParameters()) {
		        if (l.getResolvedPosition() == resolvedPosition) {
		            return new VerificationTypeInfo(l.getType());
		        }
		    }
            for (FreshLocal l : p.extraLocals()) {
                if (l.getResolvedPosition() == resolvedPosition) {
                    return new VerificationTypeInfo(l.getType());
                }
            }		    
