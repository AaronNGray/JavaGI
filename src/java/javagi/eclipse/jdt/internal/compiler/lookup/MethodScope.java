...
private void checkAndSetModifiersForMethod(MethodBinding methodBinding) {
	...
	if (declaringClass.isInterface() || declaringClass.isReceiver()) {
		if ((realModifiers & ~(ClassFileConstants.AccPublic | ClassFileConstants.AccAbstract)) != 0 &&
		    (declaringClass.modifiers & ClassFileConstants.AccAnnotation) != 0) {
		    problemReporter().illegalModifierForAnnotationMember((AbstractMethodDeclaration) referenceContext);
		} else if ((realModifiers & ~(ClassFileConstants.AccPublic | ClassFileConstants.AccAbstract)) != 0 &&
		            declaringClass.isReceiver()) {
		    problemReporter().illegalModifierForReceiverMethod((AbstractMethodDeclaration) referenceContext);
		} else if ((realModifiers & ~(ClassFileConstants.AccPublic | ClassFileConstants.AccAbstract | ClassFileConstants.AccStatic)) != 0) {
	...
	if ((modifiers & ClassFileConstants.AccAbstract) != 0) {
		...
		if (!methodBinding.declaringClass.isAbstract()) {
		    ReferenceBinding t = methodBinding.declaringClass;
		    boolean ok = false;
		    if (t.isImplementationReceiver()) {
		        t = t.enclosingType();
		    }
		    if (t.isImplementation()) {
		        SourceTypeBinding stb = (SourceTypeBinding) t;
		        if (stb.implTypes().length == 1) {
		            ReferenceBinding implType = stb.implTypes()[0];
		            if (implType.isAbstract()) {
		                if (stb.interfaceType().hasBinaryMethod()) {
		                    problemReporter().javaGIProblem((AbstractMethodDeclaration) referenceContext, 
                            "Abstract methods in single-headed do not make much sense and are thus not supported.");
		                } else {
		                    stb.markAsHavingAbstractMethod();
		                }
		                ok = true;
		            }
		        } else {
                    problemReporter().javaGIProblem((AbstractMethodDeclaration) referenceContext, 
                            "Abstract methods are currently not supported in implementations of multi-headed interfaces. " +
                            "The feature should be easy to implement, though.");
                    ok = true;
                }
		    }
		    if (! ok) {
 ...
 */
public void computeLocalVariablePositions(MethodPatch patch, int initOffset, CodeStream codeStream) {
	this.offset = initOffset;
	this.maxOffset = initOffset;

	// manage extra arguments
	for (FreshLocal local : patch.extraParameters()) {
	    local.setResolvedPosition(this.offset);
	    TypeBinding type = local.getType();
        if ((type == TypeBinding.LONG) || (type == TypeBinding.DOUBLE)) {
             this.offset += 2;
         } else {
             this.offset++;
         }
         ...
         if (this.offset > 0xFF) { // no more than 255 words of arguments
             this.problemReporter().javaGIProblem(this.referenceMethod(), "too many method parameters");
         }
	}
	// manage arguments	
	...
	// manage extra locals
    for (FreshLocal local : patch.extraLocals()) {
        local.setResolvedPosition(this.offset);
        TypeBinding type = local.getType();
        if ((type == TypeBinding.LONG) || (type == TypeBinding.DOUBLE)) {
             this.offset += 2;
         ...
         // check for too many arguments/local variables
         if (this.offset > 0xFF) { // no more than 255 words of arguments
             this.problemReporter().javaGIProblem(this.referenceMethod(), "too many method locals");
         }
    }
...
MethodBinding createMethod(AbstractMethodDeclaration method) {
	...
	if (method.isConstructor()) {
		...
		method.binding = new MethodBinding(modifiers, null, null, null, declaringClass);
		checkAndSetModifiersForConstructor(method.binding);
	} else {
		if (declaringClass.isInterface()) // interface or annotation type
			modifiers |= ClassFileConstants.AccPublic | ClassFileConstants.AccAbstract;
		method.binding =
			new MethodBinding(modifiers, method.selector, null, null, null, null, declaringClass);
	...
	Constraint[] constraints = method.constraints;
	if (constraints == null || constraints.length == 0) {
	    method.binding.constraints = Binding.NO_CONSTRAINTS;
	} else {
	    method.binding.constraints = new ConstraintBinding[constraints.length];
	    for (int i = 0; i < constraints.length; i++) {
            method.binding.constraints[i] = constraints[i].resolveConstraint(this);
        }
	    for (Constraint c : method.constraints) {
	      c.checkBounds(this);
	    }
	}
...
public String shortString() {
    AbstractMethodDeclaration m = referenceMethod();
    if (m != null) {
        MethodBinding b = m.binding;
        if (b != null)
            return "--- Method Scope --- "  //$NON-NLS-1$
                   + new String(b.getSignature());
        else
            return "--- Method Scope --- Binding not initialized.\n" + m.toString(); //$NON-NLS-1$
    }
    return "--- Method Scope --- Reference method not initialized.\n" + referenceContext.toString(); //$NON-NLS-1$
}

public final Map<TypeVariableBinding, FreshLocal> evidence = new HashMap<TypeVariableBinding, FreshLocal>();
}
