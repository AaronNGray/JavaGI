...
public boolean areMethodsCompatible(MethodBinding current, MethodBinding inherited) {
	return isParameterSubsignature(current, inherited) && areReturnTypesCompatible(current.getTypeEnvironment(), current, inherited);
}
public boolean areParametersEqual(TypeEnvironment env, MethodBinding one, MethodBinding two) {
	...
	for (int i = 0; i < length; i++)
		if (!areTypesEqual(env, oneArgs[i], twoArgs[i])) return false;
	return true;
}
boolean areReturnTypesCompatible(TypeEnvironment env, MethodBinding one, MethodBinding two) {
	if (one.returnType == two.returnType) return true;

	if (areTypesEqual(env, one.returnType, two.returnType)) return true;
...
boolean areReturnTypesCompatible0(TypeEnvironment env, MethodBinding one, MethodBinding two) {
	...
	if (!one.declaringClass.isInterface()) {
		if (one.declaringClass.id == TypeIds.T_JavaLangObject)
			return two.returnType.isCompatibleWithKernel(env, one.returnType); // interface methods inherit from Object
		return one.returnType.isCompatibleWithKernel(env, two.returnType);
	}
	...
	if (one.declaringClass.implementsInterface(env, two.declaringClass, true))
		return one.returnType.isCompatibleWithKernel(env, two.returnType);
	if (two.declaringClass.implementsInterface(env, one.declaringClass, true))
		return two.returnType.isCompatibleWithKernel(env, one.returnType);

	// unrelated interfaces... one must be a subtype of the other
	return one.returnType.isCompatibleWithKernel(env, two.returnType)
		|| two.returnType.isCompatibleWithKernel(env, one.returnType);
}
...
final void checkAbstractMethod(MethodBinding abstractMethod) {
	if (mustImplementAbstractMethod(abstractMethod.declaringClass, abstractMethod)) {
...
//					currentMethod.modifiers |= CompilerModifiers.AccImplementing;
			...
			if (!areReturnTypesCompatible(currentMethod.getTypeEnvironment(), currentMethod, inheritedMethod)
				...
				if (!currentMethod.isViewedAsDeprecated() || options.reportDeprecationInsideDeprecatedCode) {
					...
					if (declaringClass.isInterface())
						for (int j = length; --j >= 0;) {
						    TypeEnvironment env = methods[j].declaringClass.getTypeEnvironment();
							if (i != j && methods[j].declaringClass.implementsInterface(env, declaringClass, false))
								continue nextMethod;
						}
...
void checkExceptions(MethodBinding newMethod, MethodBinding inheritedMethod) {
	...
	for (int i = newExceptions.length; --i >= 0;) {
		...
		if (j == -1)
			if (!newException.isUncheckedException(newMethod.getTypeEnvironment(), false)
				...
				if (!inheritedInterfaces.includes(inheritedInterface) && inheritedInterface.isValidBinding()) {
					if (interfacesToCheck.includes(inheritedInterface)) {
						...
						for (int r = 0, rl = refs.length; r < rl; r++) {
							if (refs[r].getResolvedType() == inheritedInterface) {
...
void checkInheritedMethods(TypeEnvironment[] envs, MethodBinding[] methods, int length) {
	if (length > 1) {
		...
		if (overriddenInheritedMethods != null) {
			...
			if (index > 1 && !checkInheritedReturnTypes(envs, closestMethods, index))
				return;
		} else if (!checkInheritedReturnTypes(envs, methods, length)) {
			return;
		}
	}
	...
	if (concreteMethod == null) {
		if (!this.type.isAbstract()) {
			for (int i = length; --i >= 0;) {
				if (mustImplementAbstractMethod(methods[i].declaringClass, methods[i])) {
					TypeDeclaration typeDeclaration = this.type.scope.referenceContext;
					if (typeDeclaration != null) {
...
boolean checkInheritedReturnTypes(TypeEnvironment[] envs, MethodBinding[] methods, int length) {
	...
	int index = length;
	while (--index > 0 && areReturnTypesCompatible(envs[index], first, methods[index])){/*empty*/}
	if (index == 0) 
		return true;
...
void checkMethods() {
	...
	nextSelector : for (int s = methodSelectors.length; --s >= 0;) {
		...
		TypeEnvironment[] envs = new TypeEnvironment[inherited.length];
		if (current != null) {
				...
				for (int j = 0, length2 = inherited.length; j < length2; j++) {
					...
					if (inheritedMethod != null) {
						if (isParameterSubsignature(currentMethod, inheritedMethod)) {
							...
							envs[index] = currentMethod.getTypeEnvironment();
		...
		for (int i = 0, length = inherited.length; i < length; i++) {
			...
			if (index == -1) continue;
			if (index > 0)
				checkInheritedMethods(envs, matchingInherited, index + 1); // pass in the length of matching
...
void computeInheritedMethods() {
	...
	computeInheritedMethods(this.type.getTypeEnvironment(), superclass, type.superInterfaces());
...
*/
void computeInheritedMethods(TypeEnvironment env, ReferenceBinding superclass, ReferenceBinding[] superInterfaces) {
	...
	SimpleSet skip = findSuperinterfaceCollisions(env, superclass, superInterfaces);
	for (int i = 0; i < nextPosition; i++) {
		superType = interfacesToVisit[i];
...
int[] findOverriddenInheritedMethods(MethodBinding[] methods, int length) {
	...
	nextMethod : for (; i < length; i++) {
		...
		for (int j = i + 1; j < length; j++) {
			...
			if (declaringClass.implementsInterface(declaringClass.getTypeEnvironment(), declaringClass2, true)) {
				if (toSkip == null)
					toSkip = new int[length];
				toSkip[j] = -1;
			} else if (declaringClass2.implementsInterface(declaringClass2.getTypeEnvironment(), declaringClass, true)) {
				if (toSkip == null)
					toSkip = new int[length];
				toSkip[i] = -1;
				continue nextMethod;
...
boolean isInterfaceMethodImplemented(MethodBinding inheritedMethod, MethodBinding existingMethod, ReferenceBinding superType) {
	...
	return areParametersEqual(existingMethod.getTypeEnvironment(), existingMethod, inheritedMethod) && existingMethod.declaringClass.implementsInterface(existingMethod.declaringClass.getTypeEnvironment(), superType, true);
}

public boolean isMethodSubsignature(MethodBinding method, MethodBinding inheritedMethod) {
	return javagi.eclipse.jdt.core.compiler.CharOperation.equals(method.selector, inheritedMethod.selector)
...
boolean isParameterSubsignature(TypeEnvironment env, MethodBinding method, MethodBinding inheritedMethod) {
    return areParametersEqual(env, method, inheritedMethod);
}

final boolean isParameterSubsignature(MethodBinding method, MethodBinding inheritedMethod) {
...
final boolean mustImplementAbstractMethod(ReferenceBinding declaringClass, MethodBinding abstractMethod) {
    for (ConstraintBinding c : abstractMethod.constraints()) {
        if (Entailment.isUnsatisfiable(abstractMethod.getTypeEnvironment(), c)) {
            return false;
        }
    }
    ...
    TypeEnvironment env = this.type.getTypeEnvironment();
	...
	if (declaringClass.isClass()) {
		...
		if (this.type.implementsInterface(env, declaringClass, false)) {
			if (this.type.isAbstract()) return false; // leave it for the subclasses
			if (!superclass.implementsInterface(env, declaringClass, true)) // only if a superclass does not also implement the interface
				return true;
		}
		while (superclass.isAbstract() && !superclass.implementsInterface(env, declaringClass, false))
