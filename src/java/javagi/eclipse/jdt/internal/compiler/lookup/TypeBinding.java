 ...
 */
public final TypeBinding capture(Scope scope) {
	return capture(scope.getTypeEnvironment());
}
public TypeBinding capture(TypeEnvironment env) {
    return this;
}
/**
...
public final TypeBinding erasure(Scope scope) {
    return erasure(scope.getTypeEnvironment());
}
 ...
 *
 */
public ReferenceBinding findSuperTypeOriginatingFrom(TypeEnvironment env, int wellKnownOriginalID, boolean originalIsClass) {
    ...
    if (reference.id == wellKnownOriginalID || (original(env).id == wellKnownOriginalID)) return reference;
    ...
    if (originalIsClass) {
		while ((currentType = currentType.superclass(env)) != null) { 
			if (currentType.id == wellKnownOriginalID)
				return currentType;
			if (currentType.original(env).id == wellKnownOriginalID) 
				return currentType;
		}    
		return null;
    }
 ...
 */
public TypeBinding findSuperTypeOriginatingFrom(TypeEnvironment env, TypeBinding otherType) {
	...
	switch(kind()) {
		case Binding.ARRAY_TYPE :
			...
			TypeBinding leafSuperType = arrayType.leafComponentType.findSuperTypeOriginatingFrom(env, otherType.leafComponentType());
		...
		case Binding.TYPE_PARAMETER :
		    if (isCapture()) {
		    	...
		    	if (captureBound instanceof ArrayBinding) {
		    		TypeBinding match = captureBound.findSuperTypeOriginatingFrom(env, otherType);
		...
		case Binding.INTERSECTION_TYPE:
			...
			otherType = otherType.original(env);
		    if (this == otherType)
		    	return this;
		    if (original(env) == otherType)
		    	return this;
		    ...
		    if (!otherType.isInterface()) {
				while ((currentType = currentType.superclass(env)) != null) {
					if (currentType == otherType)
						return currentType;
					if (currentType.original(env) == otherType)
						return currentType;
				}
				return null;
		    }
...
/**
 * Returns a pair (t, b) where t is the type to use for generic cast, and
 * b is true iff we have to perform unwrapping before doing the cast.
 * The pair is null if no generic cast should be performed.
 */
public final TypeBinding genericCast(TypeEnvironment thisEnv, TypeBinding otherType, TypeEnvironment otherEnv) {
    /*
	if (this == otherType) {
	    GILog.Erasure().jfine("No generic cast required from %s (under %s) to %s (under %s)", 
                              this.debugName(), thisEnv, otherType.debugName(), otherEnv);
		return null;
	}
	*/
    TypeBinding thisErasure = this.erasure(thisEnv);
	TypeBinding otherErasure = otherType.erasure(otherEnv);
	if (otherErasure == thisErasure) {
       GILog.Erasure().jdebug("No generic cast required from %s (under %s) to %s (under %s): erasure of from-type = %s, erasure of to-type = %s", 
                              this.debugName(), thisEnv, otherType.debugName(), otherEnv,
                              thisErasure.debugName(), otherErasure.debugName());
       return null;
	}
	GILog.Erasure().jdebug("Creating generic cast %s from %s (under %s) to %s (under %s)", otherErasure.debugName(),
	                       this.debugName(), thisEnv, otherType.debugName(), otherEnv);
	return otherErasure;
}
 ...
 */
public final boolean isCompatibleWith(Scope scope, TypeBinding right) {
    return isCompatibleWith(scope.getTypeEnvironment(), right);
}
public final boolean isCompatibleWith(TypeEnvironment env, TypeBinding right) {
    return javagi.compiler.Subtyping.isSubtype(env, this, right);
}
public abstract boolean isCompatibleWithKernel(TypeEnvironment env, TypeBinding right);
 ...
 */
public boolean isParameterizedWithOwnVariables(TypeEnvironment env) {
	...
	if (paramType.arguments == null)
		return false;
	TypeVariableBinding[] variables = this.erasure(env).typeVariables();
	...
	if (enclosing != null && enclosing.erasure(env).isGenericType()
			&& !enclosing.isParameterizedWithOwnVariables(env)) {
		return false;
	}
	return true;
}

private boolean isProvableDistinctSubType(TypeEnvironment env, TypeBinding otherType) {
	if (otherType.isInterface()) {
		if (this.isInterface())
			return false;
		if (this.isArrayType()
				|| ((this instanceof ReferenceBinding) && ((ReferenceBinding) this).isFinal())
				|| (this.isTypeVariable() && ((TypeVariableBinding)this).superclass(env).isFinal())) { 
			return !this.isCompatibleWith(env, otherType);
		}
		return false;
	} else {
		if (this.isInterface()) {
		    ...
		    switch(otherType.kind()) {
		    	case Binding.PARAMETERIZED_TYPE :
		            ...
		            if (!paramType.isStatic()) { // static member types do not compare their enclosing
		            	...
		            	if (enclosing != null) {
		            		...
		            		if ((otherEnclosing.tagBits & TagBits.HasDirectWildcard) == 0) {
		            			...
		            			if (!enclosing.isEquivalentTo(env, otherParamType.enclosingType())) return true;
		            		}
		            	}
		            }
		            ...
		            for (int i = 0; i < length; i++) {
		            	if (paramType.arguments[i].isProvablyDistinctTypeArgument(env, otherArguments[i], paramType, i))
		            		return true;
		            }
		            return false;
		            ...
		            if (!paramType.isStatic()) { // static member types do not compare their enclosing
		            	...
		            	if (enclosing != null) {
		            		...
		            		if ((otherEnclosing.tagBits & TagBits.HasDirectWildcard) == 0) {
		            			...
		            			if (!enclosing.isEquivalentTo(env, otherGenericType.enclosingType())) return true;
		            		}
		            	}
		            }
 ...
 */
private boolean isProvablyDistinctTypeArgument(javagi.compiler.TypeEnvironment env, TypeBinding otherArgument, final ParameterizedTypeBinding paramType, final int rank) {
	...
	if (lowerBound1 != null) {
		if (lowerBound2 != null) {
			...
			return !lowerBound1.isCompatibleWith(env, upperBound2);
		} else {
			if (lowerBound1.isTypeVariable() || otherArgument.isTypeVariable()) {
				return false;
			}
			return !lowerBound1.isCompatibleWith(env, otherArgument);
		}
	} else if (upperBound1 != null) {
		if (lowerBound2 != null) {
			return !lowerBound2.isCompatibleWith(env, upperBound1);
		} else if (upperBound2 != null) {
			return upperBound1.isProvableDistinctSubType(env, upperBound2) 
							&& upperBound2.isProvableDistinctSubType(env, upperBound1);
		} else {
			return otherArgument.isProvableDistinctSubType(env, upperBound1);
		}
	} else {
		if (lowerBound2 != null) {
			...
			return !lowerBound2.isCompatibleWith(env, this);
		} else if (upperBound2 != null) {
			return this.isProvableDistinctSubType(env, upperBound2);
		} else {
...
public boolean isReifiable() {
	...
	do {
		...
		if (current.isLocalType()) {
			NestedTypeBinding nestedType = (NestedTypeBinding) current.erasure(current.getTypeEnvironment());
...
// JLS3: 4.5.1.1
public final boolean isTypeArgumentContainedBy(TypeEnvironment env, TypeBinding otherType) {
    if (this.isCapture()) {
        CaptureBinding cp = (CaptureBinding) this;
        ConstraintBinding[] cs = ReferenceBinding.implConstraintsFromTypeVariables(cp);
        env = JTypeEnvironment.create(null, env, cs, new TypeVariableBinding[]{cp});
    }
	if (this == otherType)
		return true;
	switch (otherType.kind()) {
		...
		case Binding.INTERSECTION_TYPE:
			...
			if (otherWildcard.otherBounds != null && otherWildcard.boundKind != Wildcard.IMPLEMENTS)
			...
			switch (otherWildcard.boundKind) {
				case Wildcard.EXTENDS:
					...
					if (upperBound == null)
						return false;
					TypeBinding match = upperBound.findSuperTypeOriginatingFrom(env, otherBound);
					...
					return upperBound.isCompatibleWithKernel(env, otherBound);
				...
				case Wildcard.SUPER:
					...
					if (lowerBound == null)
						return false;
					match = otherBound.findSuperTypeOriginatingFrom(env, lowerBound);
					if (match != null && (match = match.leafComponentType()).isRawType()) {
						return match == lowerBound.leafComponentType(); // forbide: Collection <=  ? super Collection<?>
																												// forbide: Collection[] <=  ? super Collection<?>[]
					}
					return otherBound.isCompatibleWithKernel(env, lowerBound);
		
				case Wildcard.IMPLEMENTS:
				    if (! Entailment.entails(env, ConstraintBinding.newImplConstraint(this, otherWildcard.bound))) return false;
				    if (otherWildcard.otherBounds != null) {
				        for (TypeBinding t : otherWildcard.otherBounds) {
				            if (! Entailment.entails(env, ConstraintBinding.newImplConstraint(this, t))) return false;
				        }
				    }
				    return true;
		...
		case Binding.PARAMETERIZED_TYPE:
			...
			if (!paramType.isStatic()) { // static member types do not compare their enclosing
				...
				if (enclosing != null) {
					...
					if ((otherEnclosing.tagBits & TagBits.HasDirectWildcard) == 0) {
						...
						if (!enclosing.isEquivalentTo(env, otherParamType.enclosingType()))
							return false;
					}
				}
			}
			...
			nextArgument: for (int i = 0; i < length; i++) {
				...
				switch (kind) {
					case Binding.PARAMETERIZED_TYPE:
						if (argument.isTypeArgumentContainedBy(env, otherArgument)) // recurse
							continue nextArgument;
						break;
					case Binding.WILDCARD_TYPE:
					case Binding.INTERSECTION_TYPE:
 ...
 */
public boolean needsUncheckedConversion(TypeEnvironment env, TypeBinding targetType) {
	...
	TypeBinding match = currentType.findSuperTypeOriginatingFrom(env, targetType);
 ...
 */
public TypeBinding original(TypeEnvironment env) {
	switch(kind()) {
		case Binding.PARAMETERIZED_TYPE :
		case Binding.RAW_TYPE :
		case Binding.ARRAY_TYPE : 
...
private TypeVariableBinding[] allTypeVariables;
public final TypeVariableBinding[] allTypeVariables() {
    if (allTypeVariables == null) {
        TypeVariableBinding[] a1 = typeVariables() == null ? Binding.NO_TYPE_VARIABLES : typeVariables();
        TypeVariableBinding[] a2 = implTypeVariables() == null ? Binding.NO_TYPE_VARIABLES : implTypeVariables();
        allTypeVariables = new TypeVariableBinding[a1.length + a2.length];
        System.arraycopy(a1, 0, allTypeVariables, 0, a1.length);
        System.arraycopy(a2, 0, allTypeVariables, a1.length, a2.length);
    }
...
public final Set<TypeVariableBinding> freeTypeVariables() {
    Set<TypeVariableBinding> s = new HashSet<TypeVariableBinding>();
    freeTypeVariables(s);
    return s;
}
...
public final TypeBinding[] typeArguments() {
    if (this.kind() != Binding.PARAMETERIZED_TYPE)
    ...
    if (paramType.arguments == null) {
        ...
        return paramType.arguments;
    }
}

public boolean isJavaLangObject() {
    return (this.id == TypeIds.T_JavaLangObject);
}
