...
public final Binding getBinding(char[][] compoundName, InvocationSite invocationSite) {
	...
	foundField : if (binding instanceof ReferenceBinding) {
		while (currentIndex < length) {
			...
			char[] nextName = compoundName[currentIndex++];
			TypeBinding receiverType = typeBinding.capture(this);
...
public Object[] getEmulationPath(ReferenceBinding targetEnclosingType, boolean onlyExactMatch, boolean denyEnclosingArgInConstructorCall) {
	...
	if (!currentMethodScope.isStatic && !currentMethodScope.isConstructorCall) {
		if (sourceType == targetEnclosingType || (!onlyExactMatch && sourceType.findSuperTypeOriginatingFrom(currentMethodScope.getTypeEnvironment(), targetEnclosingType) != null)) {
	...
	if (insideConstructor) {
		SyntheticArgumentBinding syntheticArg;
		if ((syntheticArg = ((NestedTypeBinding) sourceType).getSyntheticArgument(currentMethodScope.getTypeEnvironment(), targetEnclosingType, onlyExactMatch)) != null) {
			// reject allocation and super constructor call
			if (denyEnclosingArgInConstructorCall
					&& currentMethodScope.isConstructorCall 
					&& (sourceType == targetEnclosingType || (!onlyExactMatch && sourceType.findSuperTypeOriginatingFrom(currentMethodScope.getTypeEnvironment(), targetEnclosingType) != null))) {
				return BlockScope.NoEnclosingInstanceInConstructorCall;
			}
	...
	if (sourceType.isAnonymousType()) {
		...
		if (enclosingType.isNestedType()) {
			...
			SyntheticArgumentBinding enclosingArgument = nestedEnclosingType.getSyntheticArgument(currentMethodScope.getTypeEnvironment(), nestedEnclosingType.enclosingType(), onlyExactMatch);
			if (enclosingArgument != null) {
				FieldBinding syntheticField = sourceType.getSyntheticField(enclosingArgument);
				if (syntheticField != null) {
					if (syntheticField.type == targetEnclosingType || (!onlyExactMatch && ((ReferenceBinding)syntheticField.type).findSuperTypeOriginatingFrom(currentMethodScope.getTypeEnvironment(), targetEnclosingType) != null))
	...
	if (insideConstructor) {
		path[0] = ((NestedTypeBinding) sourceType).getSyntheticArgument(currentMethodScope.getTypeEnvironment(), currentType, onlyExactMatch);
	} else {
		if (currentMethodScope.isConstructorCall){
			return BlockScope.NoEnclosingInstanceInConstructorCall;
		}
	...
	if (path[0] != null) { // keep accumulating
		...
		while ((currentEnclosingType = currentType.enclosingType()) != null) {
			...
			if (currentType == targetEnclosingType
				|| (!onlyExactMatch && currentType.findSuperTypeOriginatingFrom(currentMethodScope.getTypeEnvironment(), targetEnclosingType) != null))	break;
		...
		if (currentType == targetEnclosingType
			|| (!onlyExactMatch && currentType.findSuperTypeOriginatingFrom(currentMethodScope.getTypeEnvironment(), targetEnclosingType) != null)) {
			return path;
		}
	}
	return null;
...
public final boolean needBlankFinalFieldInitializationCheck(FieldBinding binding) {
	...
	while (methodScope != null) {
		...
		if (!enclosingType.erasure(methodScope.getTypeEnvironment()).isAnonymousType()) {
