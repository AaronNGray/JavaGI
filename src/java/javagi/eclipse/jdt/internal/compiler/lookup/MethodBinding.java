...
import javagi.compiler.FreshLocal;
...
import javagi.eclipse.jdt.internal.compiler.codegen.ConstantPool;
...
protected MethodBinding() {
    ...
    this.constraints = Binding.NO_CONSTRAINTS;
}
public MethodBinding(int modifiers, char[] selector, TypeBinding returnType, TypeBinding[] parameters, ReferenceBinding[] thrownExceptions, ConstraintBinding[] constraints, ReferenceBinding declaringClass) {
...
public MethodBinding(int modifiers, TypeBinding[] parameters, ReferenceBinding[] thrownExceptions, ConstraintBinding[] constraints, ReferenceBinding declaringClass) {
	this(modifiers, TypeConstants.INIT, TypeBinding.VOID, parameters, thrownExceptions, constraints, declaringClass);
}
...
public MethodBinding(MethodBinding initialMethodBinding, ReferenceBinding declaringClass) {
	...
	this.constraints = initialMethodBinding.constraints;
	this.declaringClass = declaringClass;
...
public MethodBinding(MethodBinding initialMethodBinding) {
    ...
    this.thrownExceptions = initialMethodBinding.thrownExceptions;
    this.constraints = initialMethodBinding.constraints;
    this.declaringClass = initialMethodBinding.declaringClass;
}
*/
...
public final boolean areParameterErasuresEqual(MethodBinding method) {
	...
	for (int i = 0; i < length; i++)
		if (parameters[i] != args[i] && parameters[i].erasure(getTypeEnvironment()) != args[i].erasure(method.getTypeEnvironment()))
			return false;
	return true;
}
/*
 ...
 */
public final boolean areParametersCompatibleWith(TypeEnvironment env, TypeBinding[] arguments) {
	...
	if (isVarargs()) {
		...
		if (paramLength == argLength) { // accept X[] but not X or X[][]
			...
			if (varArgType != lastArgument && !lastArgument.isCompatibleWith(env, varArgType))
				return false;
		} else if (paramLength < argLength) { // all remainig argument types must be compatible with the elementsType of varArgType
			TypeBinding varArgType = ((ArrayBinding) parameters[lastIndex]).elementsType();
			for (int i = lastIndex; i < argLength; i++)
				if (varArgType != arguments[i] && !arguments[i].isCompatibleWith(env, varArgType))
					return false;
	...
	for (int i = 0; i < lastIndex; i++)
		if (parameters[i] != arguments[i] && !arguments[i].isCompatibleWith(env, parameters[i]))
			return false;
	return true;
}
...
public final boolean areTypeVariableErasuresEqual(MethodBinding method) {
	...
	for (int i = 0; i < length; i++)
		if (this.typeVariables[i] != vars[i] && this.typeVariables[i].erasure(this.getTypeEnvironment()) != vars[i].erasure(method.getTypeEnvironment()))
			return false;
	return true;
}
...
public final boolean canBeSeenBy(InvocationSite invocationSite, Scope scope) {
	...
	if (isPrivate()) {
		...
		ReferenceBinding outerDeclaringClass = (ReferenceBinding)declaringClass.erasure(scope);
...
public final boolean canBeSeenBy(TypeBinding receiverType, InvocationSite invocationSite, Scope scope) {
	...
	if (isProtected()) {
		...
		TypeBinding receiverErasure = receiverType.erasure(scope);		
		ReferenceBinding declaringErasure = (ReferenceBinding) declaringClass.erasure(scope);
		int depth = 0;
		do {
			if (currentType.findSuperTypeOriginatingFrom(scope.getTypeEnvironment(), declaringErasure) != null) {
				...
				if (currentType == receiverErasure || receiverErasure.findSuperTypeOriginatingFrom(scope.getTypeEnvironment(), currentType) != null) {
					if (depth > 0) invocationSite.setDepth(depth);
					return true;
				}
			}
	...
	if (isPrivate()) {
		...
		receiverCheck: {
			if (receiverType != declaringClass) {
				...
				if (receiverType.isTypeVariable() && ((TypeVariableBinding) receiverType).isErasureBoundTo(scope.getTypeEnvironment(), declaringClass.erasure(scope)))
					break receiverCheck;
				return false;
			}
		}
	...
	} while ((currentType = currentType.superclass(scope.getTypeEnvironment())) != null);
	return false;
}
...
MethodBinding computeSubstitutedMethod(MethodBinding method, LookupEnvironment env) {
	...
	for (int i = 0; i < length; i++)
		if (!this.typeVariables[i].isInterchangeableWith(getTypeEnvironment(), vars[i], substitute))
			return null;
	return substitute;
}
 ...
 */
public final char[] genericSignature() {
	...
	sig.append('(');
	for (int i = 0, length = this.extraParameters.length; i < length; i++) {
	    sig.append(this.extraParameters[i].genericTypeSignature());
	}
...
public char[] readableName() /* foo(int, Thread) */ {
	...
	if (parameters != Binding.NO_PARAMETERS) {
		for (int i = 0, length = parameters.length; i < length; i++) {
			...
			char[] cs = parameters[i].sourceName();
			if (cs != null) {
			    buffer.append(cs);
			} else {
...
*/
public final char[] getSignature() /* (ILjava/lang/Thread;)Ljava/lang/Object; */ {
	if (signature != null)
		return signature;
	...
	// SW: JavaGI
    for (int i = 0; i < this.extraParameters.length; i++) {
        buffer.append(this.extraParameters[i].signature());
    }
...
public final char[] signature(ClassFile classFile) {
    return signature_(classFile, null);
}
public final char[] signature(ClassFile classFile, TypeBinding[] moreExtraParameters) {
    char[] old = signature;
    signature = null;
    char[] res = signature_(classFile, moreExtraParameters);
    signature = old;
    return res;
}
private char[] signature_(ClassFile classFile, TypeBinding[] moreExtraParameters) {
	if (signature != null) {
	...
	if (moreExtraParameters != null) {
        for (int i = 0; i < moreExtraParameters.length; i++) {
            TypeBinding extraParameter = moreExtraParameters[i];
            TypeBinding leafExtraParameterType = extraParameter.leafComponentType();
            if (leafExtraParameterType.isNestedType()) {
                this.tagBits |= TagBits.ContainsNestedTypesInSignature;
                classFile.recordInnerClasses(leafExtraParameterType);
            }
            buffer.append(extraParameter.signature());
        }
	}
    for (int i = 0; i < this.extraParameters.length; i++) {
        TypeBinding extraParameter = this.extraParameters[i];
        TypeBinding leafExtraParameterType = extraParameter.leafComponentType();
        if (leafExtraParameterType.isNestedType()) {
            this.tagBits |= TagBits.ContainsNestedTypesInSignature;
            classFile.recordInnerClasses(leafExtraParameterType);
...
public AbstractMethodDeclaration sourceMethod() {
	...
	if (sourceType.scope == null || sourceType.scope.referenceContext.methods == null) return null;
...
public String toString() {
	...
	if (constraints != null) {
	    ...
	    for (int i = 0; i < constraints.length; i++) {
            ConstraintBinding c = constraints[i];
            output.append(c.debugName());
            if (i != constraints.length - 1) output.append(", ");
        }
	}
...
public TypeBinding[] extraParameters = Binding.NO_PARAMETERS;
public ConstraintBinding[] constraints;
public MethodBinding implementedInterfaceMethod; // only relevant for methods in implementations

public ConstraintBinding[] constraints() {
    return constraints;
}

public void applyPatch(MethodPatch patch) {
    FreshLocal[] locals = patch.extraParameters();
    this.extraParameters = new TypeBinding[locals.length];
    for (int i = 0; i < locals.length; i++) {
        this.extraParameters[i] = locals[i].getType();
    }
}
private javagi.compiler.TypeEnvironment typeEnvironment;
public javagi.compiler.TypeEnvironment getTypeEnvironment() {
    if (typeEnvironment == null) {
        GILog.TypeEnv().jdebug("computing type environment for " + this.debugName());
//        if (this instanceof ParameterizedMethodBinding)
//            throw new GICompilerBug("cannot compute type environment for method " + this + "(" + this.getClass() + ")");
        TypeEnvironment env = this.declaringClass.getTypeEnvironment();
        ConstraintBinding[] cs1 = this.constraints();
        if (cs1 == null) throw new NullPointerException();
        ConstraintBinding[] cs2 = ReferenceBinding.implConstraintsFromTypeVariables(this.typeVariables());
        ConstraintBinding[] cs = new ConstraintBinding[cs1.length + cs2.length];
        System.arraycopy(cs1, 0, cs, 0, cs1.length);
        System.arraycopy(cs2, 0, cs, cs1.length, cs2.length);
        typeEnvironment = JTypeEnvironment.create(original().sourceMethod(), env, cs, this.typeVariables());
        GILog.TypeEnv().jdebug("type environment for " + this.debugName() + ": " + typeEnvironment);
    }
    return typeEnvironment;
}
public boolean isJavaGIProblem() {
    return false;
}
...
public boolean returnTypeIsPossiblyTyvarInst() {
    MethodBinding m = original();
    if (m == null) return true;
    return m.returnType.isTypeVariable();
}
}
