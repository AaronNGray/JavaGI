...
public TypeBinding erasure(TypeEnvironment tenv) {
    TypeBinding erasedType = this.leafComponentType.erasure(tenv);
...
*/
@Override
public boolean isCompatibleWithKernel(TypeEnvironment env, TypeBinding otherType) {
	if (this == otherType)
		return true;
	...
	switch (otherType.kind()) {
		case Binding.ARRAY_TYPE :
			...
			if (dimensions == otherArray.dimensions)
			    // only use kernel subtyping for comparing the component types
			    // in order to avoid the need for coercions for whole arrays
				return leafComponentType.isCompatibleWithKernel(env, otherArray.leafComponentType);
			if (dimensions < otherArray.dimensions)
...
public void freeTypeVariables(Set<TypeVariableBinding> set) {
    if (this.leafComponentType != null) this.leafComponentType.freeTypeVariables(set);
}
@Override
public boolean containsWildcards() {
    if (this.leafComponentType != null) return this.leafComponentType.containsWildcards();
    else return false;
}
}
