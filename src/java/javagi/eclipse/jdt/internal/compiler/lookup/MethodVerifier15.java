...
@Override
public boolean areMethodsCompatible(MethodBinding current, MethodBinding inherited) {
	// use the original methods to test compatibility, but do not check visibility, etc
	current = current.original();
	inherited = inherited.original();

	TypeBinding match = current.declaringClass.findSuperTypeOriginatingFrom(current.getTypeEnvironment(), inherited.declaringClass);
	if (!(match instanceof ReferenceBinding))
		return false; // method's declaringClass does not inherit from inheritedMethod's

	if (match != inherited.declaringClass) {
		MethodBinding[] superMethods = ((ReferenceBinding) match).getMethods(inherited.selector);
		for (int i = 0, length = superMethods.length; i < length; i++)
			if (superMethods[i].original() == inherited)
				return isParameterSubsignature(current, superMethods[i]);
	}

	return isParameterSubsignature(current, inherited);
}

@Override
public boolean areParametersEqual(TypeEnvironment env, MethodBinding one, MethodBinding two) {
	...
	if (one.declaringClass.isInterface()) {
		for (int i = 0; i < length; i++)
			if (!areTypesEqual(env, oneArgs[i], twoArgs[i]))
				return false;
	} else {
		...
		foundRAW: for (i = 0; i < length; i++) {
			if (!areTypesEqual(env, oneArgs[i], twoArgs[i])) {
				if (oneArgs[i].leafComponentType().isRawType()) {
					if (oneArgs[i].dimensions() == twoArgs[i].dimensions() && oneArgs[i].leafComponentType().isEquivalentTo(env, twoArgs[i].leafComponentType())) {
...
@Override
boolean areReturnTypesCompatible(TypeEnvironment env, MethodBinding one, MethodBinding two) {
	if (one.returnType == two.returnType) return true;
	return areReturnTypesCompatible0(env, one, two);
}

@Override
boolean areTypesEqual(TypeEnvironment env, TypeBinding one, TypeBinding two) {
	if (one == two) return true;

	// need to consider X<?> and X<? extends Object> as the same 'type'
	if (one.isParameterizedType() && two.isParameterizedType())
		return one.isEquivalentTo(env, two) && two.isEquivalentTo(env, one);
	...
	for (int i = 0, l = abstractMethods.length; i < l; i++) {
		...
		if (originalInherited.declaringClass.isInterface()) {
			if ((concreteMethod.declaringClass == this.type.superclass && this.type.superclass.isParameterizedType())
				|| this.type.superclass.erasure(this.type.getTypeEnvironment()).findSuperTypeOriginatingFrom(this.type.getTypeEnvironment(), originalInherited.declaringClass) == null)
	...
	if (!detectNameClash(currentMethod, inheritedMethod)) { // check up the hierarchy for skipped inherited methods
		...
		for (int i = 0; i < length; i++)
			if (currentParams[i] != inheritedParams[i])
				if (currentParams[i].isBaseType() != inheritedParams[i].isBaseType() || !inheritedParams[i].isCompatibleWith(currentMethod.getTypeEnvironment(), currentParams[i]))
...
void checkInheritedMethods(MethodBinding inheritedMethod, MethodBinding otherInheritedMethod) {
	...
	if (inheritedMethod.declaringClass.erasure(inheritedMethod.getTypeEnvironment()) == 
	    otherInheritedMethod.declaringClass.erasure(otherInheritedMethod.getTypeEnvironment())) {
...
@Override
void checkInheritedMethods(TypeEnvironment[] envs, MethodBinding[] methods, int length) {
	int count = length;
	...
	super.checkInheritedMethods(envs, methods, length);
}

@Override
boolean checkInheritedReturnTypes(TypeEnvironment[] envs, MethodBinding[] methods, int length) {
	// assumes length > 1
	...
	for (int i = 0, l = methods[0].isAbstract() ? length - 2 : 0; i <= l;) {
		MethodBinding method = methods[i++];
		TypeEnvironment env = envs[i];
		nextMethod : for (int j = i; j < length; j++) {
			if (!areReturnTypesCompatible(env, method, methods[j])) {
				if (this.type.isInterface()) {
				    ...
				    TypeEnvironment classEnv = this.type.getTypeEnvironment();
					if (method.declaringClass.isClass() || !this.type.implementsInterface(classEnv, method.declaringClass, false))
						if (methods[j].declaringClass.isClass() || !this.type.implementsInterface(classEnv, methods[j].declaringClass, false))
				...
				if (isUnsafeReturnTypeOverride(env, method, methods[j])) {
...
void checkMethods() {
	...
	nextSelector : for (int s = methodSelectors.length; --s >= 0;) {
		...
		Pair<MethodBinding, TypeEnvironment>[] matchingInherited = new Pair[inherited.length];
		...
		if (current != null) {
			for (int i = 0, length1 = current.length; i < length1; i++) {
				...
				for (int j = 0; j < inheritedLength; j++) {
					...
					if (inheritedMethod != null) {
						if (foundMatch[j] == null && isSubstituteParameterSubsignature(currentMethod, inheritedMethod)) {
							matchingInherited[++index] = new Pair<MethodBinding, TypeEnvironment>(inheritedMethod, currentMethod.getTypeEnvironment());
				...
				if (index >= 0) {
				    ...
				    MethodBinding[] ms = new MethodBinding[matchingInherited.length];
				    int len = index + 1;
				    for (int j = 0; j < len; j++) {
                        ms[j] = matchingInherited[j].fst;
                    }
					checkAgainstInheritedMethods(currentMethod, ms, len, nonMatchingInherited); // pass in the length of matching
		...
		for (int i = 0; i < inheritedLength; i++) {
			...
			if (matchMethod == null) {
				matchingInherited[++index] = new Pair<MethodBinding, TypeEnvironment>(inheritedMethod, inheritedMethod.getTypeEnvironment());
			}
				...
				if (otherInheritedMethod != null) {
					if (inheritedMethod.declaringClass != otherInheritedMethod.declaringClass
						&& isSubstituteParameterSubsignature(inheritedMethod, otherInheritedMethod)) {
							...
							matchingInherited[++index] = new Pair<MethodBinding, TypeEnvironment>(otherInheritedMethod, inheritedMethod.getTypeEnvironment());
							skip[j] = true;
			...
			if (index > 0) {
			    TypeEnvironment[] envs = new TypeEnvironment[matchingInherited.length];
                MethodBinding[] ms = new MethodBinding[matchingInherited.length];
                int len = index + 1;
                for (int j = 0; j < len; j++) {
                    ms[j] = matchingInherited[j].fst;
                    envs[j] = matchingInherited[j].snd;
                }
				checkInheritedMethods(envs, ms, len); // pass in the length of matching
			}
			else if (mustImplementAbstractMethods && matchingInherited[0].fst.isAbstract())
				checkAbstractMethod(matchingInherited[0].fst);
	...
	nextSelector : for (int s = methodSelectors.length; --s >= 0;) {
		...
		for (int i = 0, length = inherited.length; i < length; i++) {
			...
			if (index > 0) {
				...
				int count = index + 1;
				while (--count > 0 && areReturnTypesCompatible(inheritedMethod.getTypeEnvironment(), first, matchingInherited[count])){/*empty*/}
...
MethodBinding computeSubstituteMethod(MethodBinding inheritedMethod, MethodBinding currentMethod) {
    ...
    // JavaGI: instantiate implementing type
    final TypeBinding implementingType = this.type;
    ReferenceBinding declaringClass = inheritedMethod.declaringClass;
    if (declaringClass.isInterface()) {
        final TypeVariableBinding implTypeVariable = declaringClass.implTypeVariables()[0];
        Substitution subst = new Substitution() {
            @Override
            public LookupEnvironment environment() {
                return MethodVerifier15.this.type.scope.environment();
            }
            @Override
            ...
            public TypeBinding substitute(TypeVariableBinding typeVariable) {
                if (typeVariable.equals(implTypeVariable)) {
        ...
        inheritedMethod = new ParameterizedMethodBinding(declaringClass, subst, inheritedMethod);
    }
	...
	for (int i = 0; i < inheritedLength; i++) {
		...
		} else if (inheritedTypeVariable.boundCheck(substitute, currentMethod.getTypeEnvironment(), argument) != TypeConstants.OK) {
...
boolean detectInheritedNameClash(MethodBinding inherited, MethodBinding otherInherited) {
	if (!inherited.areParameterErasuresEqual(otherInherited) || 
	    inherited.returnType.erasure(inherited.getTypeEnvironment()) != otherInherited.returnType.erasure(otherInherited.getTypeEnvironment()))
		return false;
	// skip it if otherInherited is defined by a subtype of inherited's declaringClass
	if (inherited.declaringClass.erasure(inherited.getTypeEnvironment()) != otherInherited.declaringClass.erasure(otherInherited.getTypeEnvironment()))
		if (inherited.declaringClass.findSuperTypeOriginatingFrom(inherited.getTypeEnvironment(), otherInherited.declaringClass) != null)
			return false;
...
boolean detectNameClash(MethodBinding current, MethodBinding inherited) {
	...
	if (!current.areParameterErasuresEqual(original) || current.returnType.erasure(current.getTypeEnvironment()) != original.returnType.erasure(original.getTypeEnvironment()))
		return false;
...
@Override
SimpleSet findSuperinterfaceCollisions(TypeEnvironment env, ReferenceBinding superclass, ReferenceBinding[] superInterfaces) {
	ReferenceBinding[] interfacesToVisit = null;
	int nextPosition = 0;
	...
	for (int i = 0; i < nextPosition; i++) {
		...
		if (current.isValidBinding()) {
			TypeBinding erasure = current.erasure(env);
			for (int j = i + 1; j < nextPosition; j++) {
				ReferenceBinding next = interfacesToVisit[j];
				if (next.isValidBinding() && next.erasure(env) == erasure) {
					if (copy == null)
...
public boolean isMethodSubsignature(MethodBinding method, MethodBinding inheritedMethod) {
	if (!javagi.eclipse.jdt.core.compiler.CharOperation.equals(method.selector, inheritedMethod.selector))
		return false;
	...
	TypeBinding match = method.declaringClass.findSuperTypeOriginatingFrom(method.getTypeEnvironment(), inheritedMethod.declaringClass);
	if ((match instanceof ReferenceBinding) && match != inheritedMethod.declaringClass) {
		...
		for (int i = 0, length = superMethods.length; i < length; i++)
			if (superMethods[i].original() == inheritedMethod.original())
				return isParameterSubsignature(method.getTypeEnvironment(), method, superMethods[i]);
	}

	return isParameterSubsignature(method.getTypeEnvironment(), method, inheritedMethod);
}
@Override
boolean isParameterSubsignature(TypeEnvironment env, MethodBinding method, MethodBinding inheritedMethod) {
	MethodBinding substitute = computeSubstituteMethod(inheritedMethod, method);
	return substitute != null && isSubstituteParameterSubsignature(env, method, substitute);
}
...
boolean isSubstituteParameterSubsignature(MethodBinding method, MethodBinding substituteMethod) {
    return isSubstituteParameterSubsignature(method.getTypeEnvironment(), method, substituteMethod);
}

boolean isSubstituteParameterSubsignature(TypeEnvironment env, MethodBinding method, MethodBinding substituteMethod) {
	if (!areParametersEqual(env, method, substituteMethod)) {
...
boolean isUnsafeReturnTypeOverride(MethodBinding currentMethod, MethodBinding inheritedMethod) {
    return isUnsafeReturnTypeOverride(currentMethod.getTypeEnvironment(), currentMethod, inheritedMethod);
}

boolean isUnsafeReturnTypeOverride(TypeEnvironment env, MethodBinding currentMethod, MethodBinding inheritedMethod) {
	// JLS 3 8.4.5: more are accepted, with an unchecked conversion
	if (currentMethod.returnType == inheritedMethod.returnType.erasure(inheritedMethod.getTypeEnvironment())) {
		TypeBinding[] currentParams = currentMethod.parameters;
		TypeBinding[] inheritedParams = inheritedMethod.parameters;
		for (int i = 0, l = currentParams.length; i < l; i++)
			if (!areTypesEqual(env, currentParams[i], inheritedParams[i]))
				return true;
	}
	if (currentMethod.typeVariables == Binding.NO_TYPE_VARIABLES
		&& inheritedMethod.original().typeVariables != Binding.NO_TYPE_VARIABLES
		&& currentMethod.returnType.erasure(currentMethod.getTypeEnvironment()).findSuperTypeOriginatingFrom(currentMethod.getTypeEnvironment(), inheritedMethod.returnType.erasure(inheritedMethod.getTypeEnvironment())) != null) {
			return true;
	}
	return false;
}
...
void verify(SourceTypeBinding someType) {
	...
	for (int i = someType.typeVariables.length; --i >= 0;) {
		...
		if (superclass.kind() == Binding.TYPE_PARAMETER)
			superclass = (ReferenceBinding) superclass.erasure(someType.getTypeEnvironment());
		...
		for (int j = itsInterfaces.length; --j >= 0;) {
			superInterfaces[j] = itsInterfaces[j].kind() == Binding.TYPE_PARAMETER
				? (ReferenceBinding) itsInterfaces[j].erasure(someType.getTypeEnvironment())
				: itsInterfaces[j];
		}
		computeInheritedMethods(someType.getTypeEnvironment(), superclass, superInterfaces);
