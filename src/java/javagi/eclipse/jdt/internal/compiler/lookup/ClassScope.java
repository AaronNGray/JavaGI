...
import javagi.compiler.FreshField;
...
import javagi.eclipse.jdt.internal.compiler.ast.AbstractVariableDeclaration;
...
import javagi.eclipse.jdt.internal.compiler.ast.QualifiedAllocationExpression;
import javagi.eclipse.jdt.internal.compiler.ast.SingleTypeReference;
...
import javagi.eclipse.jdt.internal.compiler.env.AccessRestriction;
...
public class ClassScope extends Scope {
	...
	public ClassScope(Scope parent, TypeDeclaration context, boolean isSynthetic) {
	    this(parent, context);
	...
	void buildAnonymousTypeBinding(SourceTypeBinding enclosingType, ReferenceBinding supertype) {
		...
		if (supertype.isInterface()) {
			...
			if (typeReference != null) { // no check for enum constant body
				if (supertype.erasure(this).id == TypeIds.T_JavaLangEnum) {
	...
	void buildFieldsAndMethods() {
		...
		for (int i = 0, length = memberTypes.length; i < length; i++) {
		    if (memberTypes[i] instanceof SourceTypeBinding) {
		        SourceTypeBinding stb = (SourceTypeBinding) memberTypes[i];
		        if (! stb.isSynthetic) stb.scope.buildFieldsAndMethods();
		    }
		}
	}
	...
	void buildLocalTypeBinding(SourceTypeBinding enclosingType) {
	    javagi.compiler.GILog.NameResolve().jfine("buildLocalTypeBinding for enclosing type " + enclosingType.debugName());
	...
	private void buildTypeVariables() {
		...
		TypeParameter[] implParameters = this.referenceContext.implParameters;
		//System.out.println(compilerOptions().sourceLevel < ClassFileConstants.JDK1_5);
	    // do not construct type variables if source < 1.5
		if ((typeParameters == null && implParameters == null) || compilerOptions().sourceLevel < ClassFileConstants.JDK1_5) {
		...
		if (TypeDeclaration.kind(this.referenceContext.modifiers) == TypeDeclaration.INTERFACE_DECL) {
		    TypeParameter[] ps = typeParameters == null ? new TypeParameter[0] : typeParameters;
		    TypeParameter[] is = implParameters == null ? new TypeParameter[0] : implParameters;
		    TypeParameter[] allParameters = new TypeParameter[ps.length + is.length];
		    System.arraycopy(ps, 0, allParameters, 0, ps.length);
		    System.arraycopy(is, 0, allParameters, ps.length, is.length);
		    TypeVariableBinding[] allVariables = createTypeVariables(allParameters, sourceType);
		    sourceType.typeVariables = ps.length == 0 ? Binding.NO_TYPE_VARIABLES : new TypeVariableBinding[ps.length];
		    System.arraycopy(allVariables, 0, sourceType.typeVariables, 0, ps.length);
		    sourceType.implTypeVariables = is.length == 0 ? Binding.NO_TYPE_VARIABLES : new TypeVariableBinding[is.length];
		    System.arraycopy(allVariables, ps.length, sourceType.implTypeVariables, 0, is.length);
		} else {
...
//					realModifiers = modifiers & ExtraCompilerModifiers.AccJustFlag;
		...
		} else if ((realModifiers & ClassFileConstants.AccReceiver) != 0) {
		    // nothing todo, the parser ensures that a receiver specification never carries modifiers
		} else {
	...
	public void checkParameterizedSuperTypeCollisions() {
		...
		nextVariable : for (int i = 0, paramLength = typeParameters == null ? 0 : typeParameters.length; i < paramLength; i++) {
			...
			if (boundRefs != null) {
				...
				for (int j = 0, boundLength = boundRefs.length; j < boundLength; j++) {
					...
					TypeBinding superType = typeRef.getResolvedType();
		...
		if (memberTypes != null && memberTypes != Binding.NO_MEMBER_TYPES)
			for (int i = 0, size = memberTypes.length; i < size; i++) {
			    if (memberTypes[i] instanceof SourceTypeBinding) {
			        SourceTypeBinding stb = (SourceTypeBinding) memberTypes[i];
				    if (! stb.isSynthetic) stb.scope.checkParameterizedSuperTypeCollisions();
			    }
			}
	}
		...
		if (memberTypes != null && memberTypes != Binding.NO_MEMBER_TYPES)
			for (int i = 0, size = memberTypes.length; i < size; i++) {
			    if (memberTypes[i] instanceof SourceTypeBinding) {
			        SourceTypeBinding stb = (SourceTypeBinding) memberTypes[i];
			        if (! stb.isSynthetic) {
			            stb.scope.checkParameterizedTypeBounds();
			        }
			    }
			}
		
		// SW: checks for JavaGI
		Constraint[] constraints = this.referenceContext.constraints;
		if (constraints != null)
		    for (Constraint c : constraints)
		        c.checkBounds(this);
		        
		Constraint[] implConstraints = this.referenceContext.implConstraints;
		if (implConstraints != null)
		    for (Constraint c : implConstraints)
		        c.checkBounds(this);
		        
		TypeParameter[] implParameters = this.referenceContext.implParameters;
		if (implParameters != null)
		    for (TypeParameter p : implParameters)
		        p.checkBounds(this);
		
		TypeReference[] implTypes = this.referenceContext.implTypes;
		if (implTypes != null)
		    for (TypeReference t : implTypes)
		        t.checkBounds(this);
		
		TypeReference interfaceType = this.referenceContext.interfaceType;
		if (interfaceType != null)
		    interfaceType.checkBounds(this);
		
		TypeReference receiverType = this.referenceContext.receiverType;
        if (receiverType != null)
            receiverType.checkBounds(this);
	}
	...
	*/
	private boolean connectSuperclass() {
		...
		if (superclass != null) { // is null if a cycle was detected cycle or a problem
			if (!superclass.isClass() && (superclass.tagBits & TagBits.HasMissingType) == 0) {
			...
			} else if (superclass.erasure((TypeEnvironment)null).id == TypeIds.T_JavaLangEnum) { // it's ok to ignore the type environment here
				problemReporter().cannotExtendEnum(sourceType, superclassRef, superclass);
					...
					|| !superclassRef.getResolvedType().isValidBinding()) {
				sourceType.superclass = superclass;
				sourceType.tagBits |= TagBits.HierarchyHasProblems; // propagate if missing supertype
				return superclassRef.getResolvedType().isValidBinding(); // reported some error against the source type ?
			} else {
	...
	private boolean connectEnumSuperclass() {
		...
		if (refTypeVariables[0].boundCheck(superType, this.getTypeEnvironment(), sourceType) != TypeConstants.OK) {
	...
	*/
	private boolean connectSuperInterfaces() {
		...
		nextInterface : for (int i = 0; i < length; i++) {
			...
			if ((superInterface.tagBits & TagBits.HierarchyHasProblems) != 0
					|| !superInterfaceRef.getResolvedType().isValidBinding()) {
				sourceType.tagBits |= TagBits.HierarchyHasProblems; // propagate if missing supertype
				noProblems &= superInterfaceRef.getResolvedType().isValidBinding();
			} 			
			// only want to reach here when no errors are reported
	...
	void connectTypeHierarchy() {
		SourceTypeBinding sourceType = this.referenceContext.binding();
		javagi.compiler.GILog.NameResolve().jfine("connectTypeHierarchy called for " + sourceType.debugName());
		if ((sourceType.tagBits & TagBits.BeginHierarchyCheck) == 0) {
			sourceType.tagBits |= TagBits.BeginHierarchyCheck;
			boolean noProblems = connectSuperclass();
			noProblems &= connectSuperInterfaces();
			sourceType.tagBits |= TagBits.EndHierarchyCheck;
			noProblems &= connectTypeVariables(this.referenceContext.allTypeParameters(), false);
		...
		connectMemberTypes();
	      // reorder receivers
		if (this.referenceContext.binding().isToplevel()) {
		    WellFormedness.sortReceivers(this.referenceContext.binding());
		}
		...
		try {
			...
			javagi.compiler.GILog.NameResolve().jfine("finished connectTypeHierarchy for " + sourceType.debugName());
		}
	}
	...
	private boolean detectHierarchyCycle(SourceTypeBinding sourceType, ReferenceBinding superType, TypeReference reference) {
		...
		if (superType.isHierarchyBeingConnected()) {
			javagi.eclipse.jdt.internal.compiler.ast.TypeReference ref = ((SourceTypeBinding) superType).scope.superTypeReference;
			// https://bugs.eclipse.org/bugs/show_bug.cgi?id=133071
			// https://bugs.eclipse.org/bugs/show_bug.cgi?id=121734
			if (ref != null && (ref.getResolvedType() == null || ((ReferenceBinding) ref.getResolvedType()).isHierarchyBeingConnected())) {
				problemReporter().hierarchyCircularity(sourceType, superType, reference);
				sourceType.tagBits |= TagBits.HierarchyHasProblems;
				superType.tagBits |= TagBits.HierarchyHasProblems;
				return true;
	...
	*/
    ...
    public String toString() {
		if (this.referenceContext != null)
			return "--- Class Scope --- "  //$NON-NLS-1$
							+ this.referenceContext.binding().toString();
		return "--- Class Scope --- Binding not initialized" ; //$NON-NLS-1$
	}
    ...
    public void checkFinitaryClosure() {
        javagi.compiler.WellFormedness.checkFinitaryClosure(this);
    }
    
    public void checkImplementationOk() {
        if (! this.referenceType().binding().isImplementation()) return;
        javagi.compiler.WellFormedness.checkImplementation(this);
    }
    
    private void connectJavaGI() {
        SourceTypeBinding sourceType = this.referenceContext.binding();
        javagi.compiler.GILog.jdebug("initializing source type " + sourceType.debugName() + " for JavaGI");
        if (sourceType.scope.referenceContext.implConstraints != null) {
            Constraint[] cs = sourceType.scope.referenceContext.implConstraints;
            sourceType.implConstraints = new ConstraintBinding[cs.length];
            for (int i = 0; i < cs.length; i++) {
                sourceType.implConstraints[i] = cs[i].resolveConstraint(this);
            }
        } else {
            sourceType.implConstraints = Binding.NO_CONSTRAINTS;
        }
        
        if (sourceType.scope.referenceContext.implParameters != null) {
            TypeParameter[] xs = sourceType.scope.referenceContext.implParameters;
            sourceType.implTypeVariables = new TypeVariableBinding[xs.length];
            for (int i = 0; i < xs.length; i++) {
                sourceType.implTypeVariables[i] = xs[i].binding;
                if (sourceType.implTypeVariables[i] == null) {
                    throw new GICompilerBug("type " + sourceType + " has implParameters but some of them are null"); 
                }
                sourceType.implTypeVariables[i].isImplTypeVariable = true;
            }
        } else {
            sourceType.implTypeVariables = new TypeVariableBinding[]{new FreshTypeVariableBinding()};
        }
        
        if (sourceType.scope.referenceContext.constraints != null) {
            Constraint[] cs = sourceType.scope.referenceContext.constraints;
            sourceType.constraints = new ConstraintBinding[cs.length];
            for (int i = 0; i < cs.length; i++) {
                sourceType.constraints[i] = cs[i].resolveConstraint(this);
            }    
        } else {
            sourceType.constraints = Binding.NO_CONSTRAINTS;
        }
        if (sourceType.scope.referenceContext.implTypes != null) {
            TypeReference[] ts = sourceType.scope.referenceContext.implTypes;
            sourceType.implTypes = new ReferenceBinding[ts.length];
            for (int i = 0; i < ts.length; i++) {
                sourceType.implTypes[i] = (ReferenceBinding) ts[i].resolveType(this);
                if (sourceType.implTypes[i] != null) {
                    compilationUnitScope().recordTypeReference(sourceType.implTypes[i]);
                } else {
                    sourceType.implTypes[i] = ts[i].asUnresolvedReferenceBinding();
                }
            }
        } else {
            sourceType.implTypes = Binding.NO_IMPL_TYPES;
        }
        if (sourceType.scope.referenceContext.interfaceType != null) {
            sourceType.interfaceType = (ReferenceBinding) sourceType.scope.referenceContext.interfaceType.resolveType(this);
            if (sourceType.interfaceType != null) {
                compilationUnitScope().recordTypeReference(sourceType.interfaceType);
            } else {
                sourceType.interfaceType = sourceType.scope.referenceContext.interfaceType.asUnresolvedReferenceBinding();
            }
        }
        if (sourceType.scope.referenceContext.receiverType != null) {
            sourceType.receiverType = (ReferenceBinding) sourceType.scope.referenceContext.receiverType.resolveType(this);
            if (sourceType.receiverType != null) {
                compilationUnitScope().recordTypeReference(sourceType.receiverType);
            } else {
                sourceType.receiverType = sourceType.scope.referenceContext.receiverType.asUnresolvedReferenceBinding();
            }
        } else if (sourceType.scope.referenceContext.receiverVariable != null) {
            sourceType.receiverType = this.getType(sourceType.scope.referenceContext.receiverVariable, true);
            if (sourceType.receiverType != null) {
                compilationUnitScope().recordTypeReference(sourceType.receiverType);
            } else {
                sourceType.receiverType = sourceType.scope.referenceContext.receiverType.asUnresolvedReferenceBinding();
        ...
        javagi.compiler.GILog.jdebug("finished initializing source type " + sourceType.debugName() + " for JavaGI");
    }
    ...
    public String shortString() {
        if (this.referenceContext != null)
            return "--- Class Scope --- "  //$NON-NLS-1$
                            + this.referenceContext.binding().debugName();
        return "--- Class Scope --- Binding not initialized" ; //$NON-NLS-1$
    }
    
    public final Map<TypeVariableBinding, FreshField> evidence = new HashMap<TypeVariableBinding, FreshField>();

    public void connectSuperImplementations() {
        SourceTypeBinding sourceType = this.referenceContext.binding();
        if (sourceType.scope.referenceContext.extendsConstraint != null) {
            ConstraintBinding c = sourceType.scope.referenceContext.extendsConstraint.resolveConstraint(this);
            ReferenceBinding impl = Entailment.findSuperImplementation(this.referenceContext,
                                                                       this,
                                                                       sourceType.implTypes, 
                                                                       sourceType.interfaceType, 
                                                                       c);
            sourceType.superImplementation = impl;
        } else if (sourceType.scope.referenceContext.extendsImplementation != null) {
            TypeReference t = sourceType.scope.referenceContext.extendsImplementation;
            TypeBinding sup = t.resolveType(this);
            if (sup.isImplementation()) {
                sourceType.superImplementation = (ReferenceBinding) sup;
            } else {
                problemReporter().javaGIProblem(t, "%s is not the name of an implemetation", t);
            }
        }
    }
