...
import javagi.eclipse.jdt.internal.compiler.env.IDependent;
import javagi.eclipse.jdt.internal.compiler.util.SimpleLookupTable;
...
public final boolean canBeSeenBy(ReferenceBinding receiverType, ReferenceBinding invocationType) {
    TypeEnvironment env = null; // don't care about proper type environment for erasure
	if (isPublic()) return true;
	...
	if (isProtected()) {
		...
		TypeBinding currentType = invocationType.erasure(env);
		TypeBinding declaringClass = enclosingType().erasure(env); // protected types always have an enclosing one
		...
		//int depth = 0;
		do {
			if (currentType.findSuperTypeOriginatingFrom(env, declaringClass) != null) return true;
			//depth++;
	...
	if (isPrivate()) {
		...
		receiverCheck: {
			if (!(receiverType == this || receiverType == enclosingType())) {
				...
				if (receiverType.isTypeVariable()) {
					...
					if (typeVariable.isErasureBoundTo(env, this.erasure(env)) || typeVariable.isErasureBoundTo(env, enclosingType().erasure(env)))
						break receiverCheck;
				}
				return false;
			}
		...
		if (invocationType != this) {
			...
			ReferenceBinding outerDeclaringClass = (ReferenceBinding)this.erasure(env);
	...
	} while ((currentType = currentType.superclass()) != null); // FIXME type environment?
	return false;
}
...
public final boolean canBeSeenBy(Scope scope) {
	...
	if (isProtected()) {
		...
		TypeBinding currentType = invocationType.erasure(scope);
		TypeBinding declaringClass = enclosingType().erasure(scope); // protected types always have an enclosing one
		if (declaringClass == null) return false; // could be null if incorrect top-level protected type
		// int depth = 0;
		do {
			if (declaringClass == invocationType) return true;
			if (currentType.findSuperTypeOriginatingFrom(scope.getTypeEnvironment(), declaringClass) != null) return true;
			// depth++;
			currentType = currentType.enclosingType();
		} while (currentType != null);
		return false;
	...
	if (isPrivate()) {
		...
		ReferenceBinding outerDeclaringClass = (ReferenceBinding)this.erasure(scope);
		temp = outerDeclaringClass.enclosingType();
		while (temp != null) {
			outerDeclaringClass = temp;
			temp = temp.enclosingType();
...
/**
 * @see javagi.eclipse.jdt.internal.compiler.lookup.Binding#getAnnotationTagBits()
 */
@Override
...
/**
 * @see javagi.eclipse.jdt.internal.compiler.env.IDependent#getFileName()
 */
...
public ReferenceBinding getMemberType(char[] typeName) {
	...
	for (int i = memberTypes.length; --i >= 0;)
		if (! memberTypes[i].isSynthetic() && CharOperation.equals(memberTypes[i].sourceName, typeName))
...
public TypeVariableBinding getTypeVariable(char[] variableName) {
	TypeVariableBinding[] typeVariables = allTypeVariables();
 ...
 */
public boolean hasIncompatibleSuperType(TypeEnvironment env, ReferenceBinding otherType) {
	...
	TypeBinding match;
	do {
		match = otherType.findSuperTypeOriginatingFrom(env, currentType);
		if (match != null && match.isProvablyDistinct(env, currentType))
			return true;
	...
	} while ((currentType = currentType.superclass(env)) != null);
...
*/
// FIXME: should we change this method for JavaGI?
public final boolean implementsInterface(TypeEnvironment env, ReferenceBinding anInterface, boolean searchHierarchy) {
	if (this == anInterface)
		return true;
	...
	ReferenceBinding currentType = this;
	do {
		ReferenceBinding[] itsInterfaces = currentType.superInterfaces(env);
	...
	} while (searchHierarchy && (currentType = currentType.superclass(env)) != null); 

	for (int i = 0; i < nextPosition; i++) {
		currentType = interfacesToVisit[i];
		if (currentType.isEquivalentTo(env, anInterface))
			return true;
...
boolean implementsMethod(MethodBinding method) {
	...
	while (type != null) {
		...
		type = type.superclass(); // FIXME type environment?
	}
	return false;
}
...
public boolean isClass() {
	return (this.modifiers & (ClassFileConstants.AccInterface | ClassFileConstants.AccAnnotation | ClassFileConstants.AccEnum |
	                          ClassFileConstants.AccReceiver | ClassFileConstants.AccImplementation | 
	                          ClassFileConstants.AccImplementationReceiver)) == 0;
}
 ...
 */
@Override
public final boolean isCompatibleWithKernel(javagi.compiler.TypeEnvironment env, TypeBinding otherType) {
	if (otherType == this)
		return true;
	...
	// System.out.println("putting (" + this.debugName() + ", " + otherType.debugName() + ") into the cache");
	if (isCompatibleWith0(env, otherType)) {
 ...
 * Answer true if the receiver type can be assigned to the argument type (right)
 */
private boolean isCompatibleWith0(javagi.compiler.TypeEnvironment env, TypeBinding otherType) {
    // javagi.compiler.Subtyping.sanityCheck(env, this, otherType);
	if (otherType == this)
		return true;
	if (otherType.id == TypeIds.T_JavaLangObject)
		return true;
	...
	// bound
	if (this.isEquivalentTo(env, otherType))
		return true;
	switch (otherType.kind()) {
		...
		case Binding.TYPE_PARAMETER :
			...
			if (otherType.isCapture()) {
				...
				if ((otherLowerBound = otherCapture.lowerBound) != null) {
					...
					return this.isCompatibleWith(env, otherLowerBound);
				}
			}
		...
		case Binding.RAW_TYPE :
			switch (this.kind()) {
				...
				case Binding.PARAMETERIZED_TYPE :
				case Binding.RAW_TYPE :
					if (this.erasure(env) == otherType.erasure(env))
			...
			if (otherReferenceType.isInterface()) // could be annotation type
				return implementsInterface(env, otherReferenceType, true);
			if (this.isInterface())  // Explicit conversion from an interface
										// to a class is not allowed
				return false;
			return otherReferenceType.isSuperclassOf(env, this);
		default :
			return false;
	}
}
 ...
 */
public boolean isSuperclassOf(TypeEnvironment env, ReferenceBinding otherType) {
	while ((otherType = otherType.superclass(env)) != null) {
		if (otherType.isEquivalentTo(env, this)) return true;
	}
	return false;
}

/**
 * @see javagi.eclipse.jdt.internal.compiler.lookup.TypeBinding#isThrowable()
 */
@Override
public boolean isThrowable(TypeEnvironment env) {
	ReferenceBinding current = this;
	do {
		switch (current.id) {
	...
	} while ((current = current.superclass(env)) != null);
	return false;
}
...
public boolean isReceiver() {
    return (this.modifiers & ClassFileConstants.AccReceiver) != 0;
}
@Override
public boolean isImplementation() {
    return (this.modifiers & ClassFileConstants.AccImplementation) != 0;
}

@Override
public boolean isImplementationReceiver() {
    return (this.modifiers & ClassFileConstants.AccImplementationReceiver) != 0;
}

public final TypeEnvironment getTypeEnvironment() {
    if (typeEnvironment == null) {
        GILog.TypeEnv().jdebug("computing type environment for " + this.debugName());
        ReferenceBinding current = this;
        List<ConstraintBinding[]> constraintList = new ArrayList<ConstraintBinding[]>();
        Set<TypeVariableBinding> vars = new HashSet<TypeVariableBinding>();
        ConstraintBinding[] cons = null;
        do {
            GILog.TypeEnv().jfinest("current = " + current.debugName());
//            if (! (current instanceof SourceTypeBinding || current instanceof BinaryTypeBinding))
//                throw new GICompilerBug("cannot compute type environment for " + current);

            cons = current.constraints();
            checkConstraintArray(cons);
            constraintList.add(0, cons);
            
            cons = implConstraintsFromTypeVariables(current.typeVariables());
            constraintList.add(0, cons);
            java.util.Collections.addAll(vars, current.typeVariables());
            
            if (current.isInterface() && ! current.isAnnotationType()) {
            
                cons = current.implConstraints();
                checkConstraintArray(cons);
                constraintList.add(0, cons);
                
                cons = implConstraintsFromTypeVariables(current.implTypeVariables());
                checkConstraintArray(cons);
                constraintList.add(0, cons);
                
                java.util.Collections.addAll(vars, current.implTypeVariables());
            }
        } while ((current = current.enclosingType()) != null);
        typeEnvironment = javagi.compiler.JTypeEnvironment.create(this, constraintList, vars);
        GILog.TypeEnv().jdebug("type environment for " + this.debugName() + ": " + typeEnvironment);
    }
    return this.typeEnvironment;
}

private void checkConstraintArray(ConstraintBinding[] cons) {
    if (cons == null) throw new NullPointerException();
    for (ConstraintBinding c : cons) {
        if (c == null) throw new NullPointerException();
    }    
}

static ConstraintBinding[] implConstraintsFromTypeVariables(TypeVariableBinding... xs) {
    if (xs == null || xs.length == 0) return Binding.NO_CONSTRAINTS;
    List<ConstraintBinding> list = new ArrayList<ConstraintBinding>();
    for (TypeVariableBinding x : xs) {
        TypeBinding[] left = new TypeBinding[]{x};
        if (x.boundKind == TypeParameter.IMPLEMENTS_BOUND) {
            list.add(new ConstraintBinding(Constraint.IMPLEMENTS_CONSTRAINT,
                                           left, x.upperBound(true)));
            for (TypeBinding t : x.otherUpperBounds(true)) {
                list.add(new ConstraintBinding(Constraint.IMPLEMENTS_CONSTRAINT,
                                               left, t));                    
            }
        }
    }
    return list.toArray(new ConstraintBinding[list.size()]);
}
...
public final ReferenceBinding[] superInterfaces(TypeEnvironment env) {
    return superInterfaces(env, false);
}

public ReferenceBinding[] superInterfaces(TypeEnvironment env, boolean includeImplements) {
    ReferenceBinding[] res = superInterfaces();
    /*
    if (includeImplements) {
        ReferenceBinding[] byImpl = Entailment.superInterfacesByImplementation(env, this);
        ReferenceBinding[] oldRes = res;
        res = new ReferenceBinding[oldRes.length + byImpl.length];
        System.arraycopy(oldRes, 0, res, 0, oldRes.length);
        System.arraycopy(byImpl, 0, res, oldRes.length, byImpl.length);
    }
    */
    return res;
}

public void setCompoundName(char[][] name) {
    this.compoundName = name;
    this.constantPoolName = null;
    this.signature = null;
}
...
public int originalSuperInterfaceCount() {
    return 0;
}

public ReferenceBinding superImplementation() {
    return null;
}
public ReferenceBinding[] receivers() {
    return Binding.NO_RECEIVERS;
}

public MethodBinding findNoArgConstructor() {
    for (MethodBinding m : methods()) {
        if (m.isConstructor() && (m.parameters == null || m.parameters.length == 0)) {
            return m;
        }
    }
    return null;
...
public MethodBinding getMatchingMethod(MethodBinding inheritedMethod, CompilationUnitScope scope) {
    MethodBinding[] candidates = getMethods(inheritedMethod.selector);
    MethodVerifier verif = scope.environment().methodVerifier();
    for (MethodBinding cand : candidates) {
        if (verif.isParameterSubsignature(cand, inheritedMethod)) {
            return cand;
        }
    }
    return null;
...
public boolean hasBinaryMethod() {
    if (! isInterface()) return false;
    TypeVariableBinding[] xs = implTypeVariables();
    return hasBinaryMethod(xs);
}

public boolean hasBinaryMethod(TypeVariableBinding[] xs) {
    if (! (isInterface() || isReceiver())) return false;    
    for (MethodBinding m : methods()) {
        TypeBinding[] args = m.parameters;
        for (TypeBinding t : args) {
            for (TypeVariableBinding x : xs) {
    ...
    for (ReferenceBinding member : memberTypes()) {
        if (member.hasBinaryMethod(xs)) return true;
    }
    return false;
}

public MethodBinding[] getMethodsInclReceivers(char[] selector) {
