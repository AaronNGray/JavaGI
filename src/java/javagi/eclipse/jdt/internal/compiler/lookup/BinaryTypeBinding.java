...
/**
 * @see javagi.eclipse.jdt.internal.compiler.lookup.ReferenceBinding#availableMethods()
 */
@Override
...
void cachePartsFrom(IBinaryType binaryType, boolean needFieldsAndMethods) {
	...
	if (typeSignature == null) {
		...
		// attempt to find the superclass if it exists in the cache (otherwise - resolve it when requested)
		this.superclass = (ReferenceBinding) environment.getTypeFromTypeSignature(getTypeEnvironment(), wrapper, Binding.NO_TYPE_VARIABLES, this, missingTypeNames);
		this.tagBits |= TagBits.HasUnresolvedSuperclass;
		...
		if (!wrapper.atEnd()) {
			...
			do {
				types.add(environment.getTypeFromTypeSignature(getTypeEnvironment(), wrapper, Binding.NO_TYPE_VARIABLES, this, missingTypeNames));
			} while (!wrapper.atEnd());
		...
		if (size > 0) {
			...
			for (int i = 0; i < size; i++) {
				...
				TypeBinding type = fieldSignature == null 
					...
					: environment.getTypeFromTypeSignature(getTypeEnvironment(), new SignatureWrapper(fieldSignature), Binding.NO_TYPE_VARIABLES, this, missingTypeNames);
				FieldBinding field = 
					new FieldBinding(
						binaryField.getName(), 
						type, 
...
private MethodBinding createMethod(IBinaryMethod method, long sourceLevel, char[][][] missingTypeNames) {
	...
	if (methodSignature == null) { // no generics
		...
		if (wrapper.signature[wrapper.start] == '(') {
			...
			if (wrapper.signature[wrapper.start] == ')') {
				...
				while (wrapper.signature[wrapper.start] != ')')
					types.add(environment.getTypeFromTypeSignature(getTypeEnvironment(), wrapper, typeVars, this, missingTypeNames));
				wrapper.start++; // skip ')'
		...
		returnType = environment.getTypeFromTypeSignature(getTypeEnvironment(), wrapper, typeVars, this, missingTypeNames);
	...
	MethodBinding result = method.isConstructor()
		? new MethodBinding(methodModifiers, parameters, exceptions, Binding.NO_CONSTRAINTS, this)
		: new MethodBinding(methodModifiers, method.getSelector(), returnType, parameters, exceptions, 
		                    Binding.NO_CONSTRAINTS, this);
	if (this.environment.globalOptions.storeAnnotations)
			...
			switch(typeSignature[i]) {
				...
				default:
					if (pendingVariable) {
						...
						variables.add(new TypeVariableBinding(variableName, TypeParameter.EXTENDS_BOUND, this, rank++));
					}
			}
		}
	}
...
private void initializeTypeVariable(TypeVariableBinding variable, TypeVariableBinding[] existingVariables, SignatureWrapper wrapper, char[][][] missingTypeNames) {
	...
	if (wrapper.signature[wrapper.start] == ':') {
		...
		type = (ReferenceBinding) environment.getTypeFromTypeSignature(getTypeEnvironment(), wrapper, existingVariables, this, missingTypeNames);
		firstBound = type;
	}
	...
	if (wrapper.signature[wrapper.start] == ':') {
		...
		do {
			wrapper.start++; // skip ':'
			types.add(environment.getTypeFromTypeSignature(getTypeEnvironment(), wrapper, existingVariables, this, missingTypeNames));
		} while (wrapper.signature[wrapper.start] == ':');
...
// SW: JavaGI support
private TypeVariableBinding[] implTypeVariables; // only relevant for interfaces
private ConstraintBinding[] implConstraints;     // for interfaces only
private ConstraintBinding[] constraints;

@Override
public ConstraintBinding[] constraints() {
    javagi.compiler.GILog.jfine("constraints() not yet supported for types loaded from .class files");
    return Binding.NO_CONSTRAINTS;
}

@Override
public ConstraintBinding[] implConstraints() {
    javagi.compiler.GILog.jfine("implConstraints() not yet supported for types loaded from .class files");
    return Binding.NO_CONSTRAINTS;
}

@Override
public TypeBinding[] implTypes() {
    throw new javagi.compiler.GICompilerBug("implTypes() not yet supported for types loaded from .class files");
}

@Override
public TypeBinding interfaceType() {
    throw new javagi.compiler.GICompilerBug("interfaceType() not yet supported for types loaded from .class files");
}

@Override
public TypeVariableBinding[] implTypeVariables() {
    javagi.compiler.GILog.jfine("implTypeVariables() not yet supported for types loaded from .class files");
    return new TypeVariableBinding[]{new FreshTypeVariableBinding()};
}
}
