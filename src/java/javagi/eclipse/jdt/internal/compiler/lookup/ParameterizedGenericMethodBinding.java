...
public class ParameterizedGenericMethodBinding extends ParameterizedMethodBinding implements Substitution {
	 ...
	 */	
	public static MethodBinding computeCompatibleMethod(MethodBinding originalMethod, TypeBinding[] arguments, Scope scope, InvocationSite invocationSite, TypeEnvironment tenv) {
		...
		computeSubstitutes: {
			...
			methodSubstitute = inferFromArgumentTypes(scope, tenv, originalMethod, arguments, parameters, inferenceContext);
			...
			if (inferenceContext.hasUnresolvedTypeArgument()) {
				...
				methodSubstitute = methodSubstitute.inferFromExpectedType(scope, tenv, inferenceContext);
				if (methodSubstitute == null) 
					return null;
			}
		}
		...
		if (!methodSubstitute.isRaw) {
			for (int i = 0, length = typeVariables.length; i < length; i++) {
			    ...
			    /*
			    System.out.println("\n----");
	            System.out.println(typeVariable.debugName() + ".boundCheck(" + methodSubstitute.debugName() + ", ???, " + substitute.debugName() + ")");
	            System.out.println("tenv = " + tenv);
	            System.out.println("originalMethod.getTypeEnvironment() = " + originalMethod.getTypeEnvironment());
	            System.out.println(typeVariable + " substituted: " + methodSubstitute.substitute(typeVariable).debugName());
	            */
			    // sanity checks fail for the following tests if we use "originalMethod.getTypeEnvironment()" 
	            // in the next call to boundCheck:
			    // - root/java5/sun/generics/inference/6273455/T6273455.java
			    // - root/java5/sun/generics/6207386/Test.java
			    // int boundCheckRes = typeVariable.boundCheck(methodSubstitute, originalMethod.getTypeEnvironment() /* or tenv ?? */, substitute);
			    
			    // sanity checks fail for the following tests if we use "tenv" in the next call to boundCheck:
			    // - root/java5/sun/boxing/BoxedForeach.java
			    // - root/java5/sun/generics/inference/5021635/T6299211.java
	            
	            // we use tenv, which is the type environment of the call
			    int boundCheckRes = typeVariable.boundCheck(methodSubstitute, tenv, substitute); 
			    
				switch (boundCheckRes) {
	 ...
	 */
	private static ParameterizedGenericMethodBinding inferFromArgumentTypes(Scope scope, TypeEnvironment tenv, MethodBinding originalMethod, TypeBinding[] arguments, TypeBinding[] parameters, InferenceContext inferenceContext) {
		...
		if (!resolveSubstituteConstraints(scope, tenv, originalVariables , inferenceContext, false/*ignore Ti<:Uk*/)) 
	...
	private static boolean resolveSubstituteConstraints(Scope scope, TypeEnvironment tenv, TypeVariableBinding[] typeVariables, InferenceContext inferenceContext, boolean considerEXTENDSConstraints) {
...
//							}
		...
		if (inferenceContext.hasUnresolvedTypeArgument()) {
			...
			nextTypeParameter: 
				for (int i = 0; i < varLength; i++) {
					...
					TypeBinding mostSpecificSubstitute = scope.lowerUpperBound(scope.getTypeEnvironment(), bounds);
		...
		if (considerEXTENDSConstraints && inferenceContext.hasUnresolvedTypeArgument()) {
			...
			nextTypeParameter: 
				for (int i = 0; i < varLength; i++) {
					...
					// JavaGI FAILURE
					// sanity checks fail for the following tests if we use "tenv" as the type environment parameter of the next call to greaterLowerBound
					// - root/java5/sun/generics/inference/5021635/T6299211.java 
					TypeBinding[] glb = Scope.greaterLowerBound(tenv, bounds);
		
	                // sanity checks fail for the following tests if we use "scope.getTypeEnvironment()" as the type environment parameter of the next call to greaterLowerBound
                    // - root/java5/sun/generics/inference/5021635/T6299211.java 
					// - root/java5/sun/boxing/BoxedForeach.java 
					// TypeBinding[] glb = Scope.greaterLowerBound(scope.getTypeEnvironment(), bounds);
					
					// sanity checks fail for the following tests if we use "inferenceContext.genericMethod.getTypeEnvironment()" as the type environment parameter of the next call to greaterLowerBound
					// - root/java5/sun/generics/inference/6302954/X.java
					// TypeBinding[] glb = Scope.greaterLowerBound(inferenceContext.genericMethod.getTypeEnvironment(), bounds);
	...
	public ParameterizedGenericMethodBinding(MethodBinding originalMethod, RawTypeBinding rawType, LookupEnvironment environment) {
		...
		for (int i = 0; i < length; i++) {
			rawArguments[i] =  environment.convertToRawType(originalVariables[i].erasure(originalMethod.getTypeEnvironment()), false /*do not force conversion of enclosing types*/);
		}
	    this.isRaw = true;
	...
	public ParameterizedGenericMethodBinding(MethodBinding originalMethod, TypeBinding[] typeArguments, LookupEnvironment environment) {
	    ...
	    this.constraints = Scope.substitute(this, originalMethod.constraints);
	    // error case where exception type variable would have been substituted by a non-reference type (207573)
	    if (this.thrownExceptions == null) this.thrownExceptions = Binding.NO_EXCEPTIONS;	    
	 ...
	 */
	private ParameterizedGenericMethodBinding inferFromExpectedType(Scope scope, TypeEnvironment tenv, InferenceContext inferenceContext) {
		...
		computeSubstitutes: {
			...
			for (int i = 0; i < varLength; i++) {
				...
				for (int j = 0, max = originalVariable.superInterfaces(tenv).length; j < max; j++) {
					TypeBinding substitutedBound = Scope.substitute(this, originalVariable.superInterfaces(tenv)[j]);
					argument.collectSubstitutes(scope, substitutedBound, inferenceContext, TypeConstants.CONSTRAINT_SUPER);
					if (inferenceContext.status == InferenceContext.FAILED) return null; // impossible substitution
					// JLS 15.12.2.8 claims reverse inference shouldn't occur, however it improves inference
					if (argAlreadyInferred) {
			...
			if (!resolveSubstituteConstraints(scope, tenv, originalVariables, inferenceContext, true/*consider Ti<:Uk*/)) 
	...
	/**
	 * @see javagi.eclipse.jdt.internal.compiler.lookup.MethodBinding#tiebreakMethod()
	 */
	@Override
    public MethodBinding tiebreakMethod() {
		if (this.tiebreakMethod == null) {
			TypeVariableBinding[] originalVariables = this.originalMethod.typeVariables;
			TypeEnvironment env = this.originalMethod.getTypeEnvironment();
			int length = originalVariables.length;
			...
			for (int i = 0; i < length; i++) {
				...
				if (originalVariable.boundsCount() <= 1) {
					...
					newArguments[i] = this.environment.convertToRawType(
							...
							this.environment.createWildcard(null, 0, originalVariable.superclass(env), originalVariable.superInterfaces(env), Wildcard.EXTENDS), false /*do not force conversion of enclosing types*/);
				}
			}
