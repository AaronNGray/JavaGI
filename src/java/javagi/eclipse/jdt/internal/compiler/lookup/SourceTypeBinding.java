...
import javagi.compiler.TypePool;
...
public SourceTypeBinding(char[][] compoundName, PackageBinding fPackage, ClassScope scope) {
	...
	this.declaration = scope.referenceContext;
...
public SyntheticMethodBinding addSyntheticEnumMethod(char[] selector) {
	...
	if (accessors == null) {
		...
		accessMethod.constraints = Binding.NO_CONSTRAINTS;
...
public SyntheticMethodBinding addSyntheticBridgeMethod(MethodBinding inheritedMethodToBridge, MethodBinding targetMethod) {
	...
	if (inheritedMethodToBridge.returnType.erasure(inheritedMethodToBridge.getTypeEnvironment()) == 
	    targetMethod.returnType.erasure(targetMethod.getTypeEnvironment())
		...
		while (synthMethods.hasNext()) {
			...
			if (synthetic instanceof MethodBinding) {
				...
				if (CharOperation.equals(inheritedMethodToBridge.selector, method.selector)
					&& inheritedMethodToBridge.returnType.erasure(inheritedMethodToBridge.getTypeEnvironment()) == 
					    method.returnType.erasure(method.getTypeEnvironment())
...
void faultInTypesForFieldsAndMethods() {
	...
	for (int i = 0, length = this.memberTypes.length; i < length; i++) {
	    if (memberTypes[i] instanceof SourceTypeBinding) {
            ...
            if (! stb.isSynthetic) stb.faultInTypesForFieldsAndMethods();
	    }
	}
}
...
/**
 * @see javagi.eclipse.jdt.internal.compiler.lookup.TypeBinding#genericTypeSignature()
 */
@Override
...
public MethodBinding[] getMethodsInclReceivers(char[] selector) {
    MethodBinding[] ms = getMethods(selector);
    if (! this.isImplementation()) {
        return ms;
    } else {
        List<MethodBinding> list = new ArrayList<MethodBinding>();
        for (MethodBinding m : ms) list.add(m);
        for (ReferenceBinding r : this.receivers()) {
            for (MethodBinding m : r.getMethods(selector)) list.add(m);
        }
	...
	if (!onlyExactMatch){
		...
		while (accessFields.hasNext()) {
			...
			if (CharOperation.prefixEquals(TypeConstants.SYNTHETIC_ENCLOSING_INSTANCE_PREFIX, field.name)
				&& field.type.findSuperTypeOriginatingFrom(getTypeEnvironment(), targetEnclosingType) != null)
					return field;
		}
	}
	return null;
...
/**
 * @see javagi.eclipse.jdt.internal.compiler.lookup.Binding#initializeDeprecatedAnnotationTagBits()
 */
@Override
	...
	switch(otherType.kind()) {
		...
		case Binding.PARAMETERIZED_TYPE :
			...
			for (int i = 0; i < length; i++)
				if (!this.typeVariables[i].isTypeArgumentContainedBy(env, otherArguments[i]))
					return false;
			return true;

		case Binding.RAW_TYPE :
	        return otherType.erasure(env) == this;
	}
	return false;
}
@Override
		...
		for (int i = 0, length = this.methods.length; i < length; i++) {
			...
			nextSibling: for (int j = i + 1; j < length; j++) {
				...
				if (complyTo15 && method.returnType != null && method2.returnType != null) {
					...
					if (equalParams && equalTypeVars) {
					...
					} else if (method.returnType.erasure(method.getTypeEnvironment()) == subMethod.returnType.erasure(subMethod.getTypeEnvironment()) && (equalParams || method.areParameterErasuresEqual(method2))) {
						...
						int index = pLength;
						for (; --index >= 0;) {
							if (params1[index] != params2[index].erasure(method2.getTypeEnvironment()))
								break;
						...
						if (index >= 0 && index < pLength) {
							for (index = pLength; --index >= 0;)
								if (params1[index].erasure(method.getTypeEnvironment()) != params2[index])
									break;
						}
						if (index >= 0)
							continue nextSibling;
...
public MethodBinding resolveTypesFor(MethodBinding method) {
	...
	if (exceptionTypes != null) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (resolvedExceptionType.findSuperTypeOriginatingFrom(method.getTypeEnvironment(), TypeIds.T_JavaLangThrowable, true) == null) {
...
public String toString() {
	...
	else if (isReceiver() || isImplementationReceiver()) buffer.append("receiver "); //$NON-NLS-1$
	else if (isImplementation()) buffer.append("implementation");
...
public TypeVariableBinding[] implTypeVariables; // only relevant for interfaces
public ConstraintBinding[] implConstraints;    // for interfaces only
public ConstraintBinding[] constraints;
public ReferenceBinding[] implTypes;                // for implementations only
private List<scala.Tuple2<MethodBinding, MethodBinding>> implicitImplMethods = 
     new ArrayList<scala.Tuple2<MethodBinding, MethodBinding>>();     /* for implementations only,
                                                                          the first method is the one in the interface,
                                                                          the second the one in the implementing type */
public ReferenceBinding superImplementation;   // for implementations only
public ReferenceBinding interfaceType;         // for implementations only
public TypeBinding receiverType;               // for receivers only
public int originalSuperInterfaceCount = -1;

// store the lookup environment and the declaration so that we can access it even if cleanup nullifies this.scope
public final LookupEnvironment lookupEnvironment;
public final TypeDeclaration declaration;

public boolean incompleteImplementation;
...
@Override
public ConstraintBinding[] constraints() {
    if (isEnum()) return Binding.NO_CONSTRAINTS;
...
@Override
public ReferenceBinding interfaceType() {
...
public int originalSuperInterfaceCount() {
    return this.originalSuperInterfaceCount;
}

public ConstraintBinding[] allConstraints() {
    if (typeVariables == null || typeVariables.length == 0) {
        return constraints();
    } else {
        List<ConstraintBinding> l = new ArrayList<ConstraintBinding>();
        for (TypeVariableBinding x : typeVariables) {
            ConstraintBinding[] cs = x.boundsAsConstraints();
            for (int i = 0; i < cs.length; i++) {
                l.add(cs[i]);
            }
        }
        ConstraintBinding[] cs = constraints();
        ConstraintBinding[] res = new ConstraintBinding[l.size() + cs.length];
        res = l.toArray(res);
        System.arraycopy(cs, 0, res, l.size(), cs.length);
        return res;
    }
}

public void addImplicitImplMethod(scala.Tuple2<MethodBinding, MethodBinding> p) {
    this.implicitImplMethods.add(p);
}

public scala.Tuple2<MethodBinding, MethodBinding>[] getImplicitImplMethods() {
    return (scala.Tuple2<MethodBinding, MethodBinding>[]) this.implicitImplMethods.toArray(new scala.Tuple2[this.implicitImplMethods.size()]);
}

@Override
public String debugName() {
    if (! isImplementation() || interfaceType == null || implTypes == null) return super.debugName();
    StringBuffer sb = new StringBuffer();
    sb.append(interfaceType().debugName());
    sb.append('[');
    for (int i = 0; i < implTypes.length; i++) {
        sb.append(implTypes[i].debugName());
        if (i != implTypes.length - 1) sb.append(',');
    }
    sb.append(']');
    return sb.toString();
}
...
private ReferenceBinding getPatchedSuperClass(ClassPatch patch) {
    ReferenceBinding patchedSuper = patch.newSuperClass();
    if (patchedSuper != null && this.superclass != null && 
        !patchedSuper.isJavaLangObject() && ! this.superclass.isJavaLangObject()) {
        throw new GICompilerBug("cannot patch the super class of class " + this + 
                                " because it already has a super class");
    }
    if (patchedSuper != null && !patchedSuper.isJavaLangObject()) return patchedSuper;
    return this.superclass;
}

public void applyPatch(ClassPatch classPatch) {
    ReferenceBinding old = this.superclass;
    this.superclass = getPatchedSuperClass(classPatch);
}
...
public void markAsHavingAbstractMethod() {
