...
public class NestedTypeBinding extends SourceTypeBinding {
	...
	public static SourceTypeBinding newDummyReceiver(ReferenceBinding r, SourceTypeBinding enclosingType, boolean isInterfaceReceiver) {
	    //this(new char[][]{"DUMMY".toCharArray()}, enclosingType.scope, enclosingType);
	    TypeDeclaration encT = enclosingType.scope.referenceContext;
	    TypeDeclaration t = new TypeDeclaration(encT.compilationResult);
	    ClassScope scope = new ClassScope(enclosingType.scope, t);
	    t.name = r.sourceName;
	    t.initializerScope = new MethodScope(scope, t, false);
	    t.staticInitializerScope =  new MethodScope(scope, t, true);
	    SourceTypeBinding stb = new NestedTypeBinding(computeName(r, enclosingType), scope, enclosingType);
	    stb.typeVariables = Binding.NO_TYPE_VARIABLES;
	    stb.implTypeVariables = Binding.NO_TYPE_VARIABLES;
	    stb.constraints = Binding.NO_CONSTRAINTS;
	    stb.implConstraints = Binding.NO_CONSTRAINTS;
	    stb.superInterfaces = Binding.NO_SUPERINTERFACES;
	    stb.memberTypes = Binding.NO_MEMBER_TYPES;
	    int recvMod = isInterfaceReceiver ? ClassFileConstants.AccReceiver : ClassFileConstants.AccImplementationReceiver;
	    stb.modifiers = recvMod | ClassFileConstants.AccPrivate;
	    stb.isSynthetic = true;
	    return stb;
	}
	
	private static char[][] computeName(ReferenceBinding r, SourceTypeBinding stb) {
	    if (r.compoundName != null) return r.compoundName;
	    char[][] res = new char[stb.compoundName.length + 1][];
	    System.arraycopy(stb.compoundName, 0, res, 0, stb.compoundName.length);
	    res[stb.compoundName.length] = r.sourceName;
	    return res;
	}
	...
	*/
	public SyntheticArgumentBinding getSyntheticArgument(TypeEnvironment env, ReferenceBinding targetEnclosingType, boolean onlyExactMatch) {
		...
		if (!onlyExactMatch){
			for (int i = enclosingInstances.length; --i >= 0;)
				if (enclosingInstances[i].actualOuterLocalVariable == null)
					if (enclosingInstances[i].type.findSuperTypeOriginatingFrom(env, targetEnclosingType) != null)
						return enclosingInstances[i];
		}
		return null;
	}
