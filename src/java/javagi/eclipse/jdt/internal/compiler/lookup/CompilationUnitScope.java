...
void buildTypeBindings(AccessRestriction accessRestriction) {
	...
	nextType: for (int i = 0; i < typeLength; i++) {
		...
		if ((typeDecl.modifiers & ClassFileConstants.AccPublic) != 0 && !typeDecl.isImplementation()) {
			char[] mainTypeName;
...
void checkParameterizedTypes() {
	...
	for (int i = 0, length = topLevelTypes.length; i < length; i++) {
		...
		scope.checkFinitaryClosure(); // SW: added
		scope.checkImplementationOk();
	}
}
/*
 * INTERNAL USE-ONLY
...
public ReferenceBinding[] getReferencedTypes() {
    int n = referencedTypes.size + referencedSuperTypes.size;
    ReferenceBinding[] res = new ReferenceBinding[n];
    ...
    for (int i = 0; i < referencedSuperTypes.size; i++) {
        ReferenceBinding type = (ReferenceBinding) referencedSuperTypes.elementAt(i);
        res[referencedTypes.size + i] = type;
    }
    return res;
}
...
public boolean doesImport(char[][] pkgName, char[] simpleName) { 
    for (ImportBinding i : imports) {
        if (i.doesImport(pkgName, simpleName)) return true;
    }
    return false;
}
...
public void connectSuperImplementations() {
    for (int i = 0, length = topLevelTypes.length; i < length; i++)
        topLevelTypes[i].scope.connectSuperImplementations();    
}
}
