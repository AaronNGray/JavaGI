...
import javagi.compiler.JTypeEnvironment;
import javagi.compiler.Loadable;
import javagi.compiler.MethodLookup;
import javagi.compiler.TypeEnvironment;
import javagi.compiler.WithError;
...
import javagi.eclipse.jdt.internal.compiler.util.HashtableOfObject;
import javagi.eclipse.jdt.internal.compiler.util.ObjectVector;
import javagi.eclipse.jdt.internal.compiler.util.SimpleSet;
...
public abstract class Scope implements TypeConstants, TypeIds {
	...
	*/
	public static int compareTypes(TypeEnvironment env, TypeBinding left, TypeBinding right) {
		if (left.isCompatibleWith(env, right))
			return Scope.EQUAL_OR_MORE_SPECIFIC;
		if (right.isCompatibleWith(env, left))
   ...
   // 5.1.10
	public static ReferenceBinding[] greaterLowerBound(TypeEnvironment env, ReferenceBinding[] types) {
		...
		for (int i = 0; i < length; i++) {
			...
			for (int j = 0; j < length; j++) {
				...
				if (iType.isCompatibleWith(env, jType)) { // if Vi <: Vj, Vj is removed
	...
	// 5.1.10
	public static TypeBinding[] greaterLowerBound(javagi.compiler.TypeEnvironment env, TypeBinding[] types) {
		if (types == null) return null;
		int length = types.length;
		if (length == 0) return null;
	...
	public TypeBinding boxing(TypeBinding type) {
		if (type.isBaseType())
			return environment().computeBoxingType(this.getTypeEnvironment(), type);
		return type;
	}
	 ...
	 */
	public final MethodBinding computeCompatibleMethod(MethodBinding method, TypeBinding[] arguments, InvocationSite invocationSite, TypeEnvironment env) {
		...
		if (typeVariables != Binding.NO_TYPE_VARIABLES) { // generic method
			...
			for (int i = 0; i < argLength; i++) {
				...
				if (arguments[i].isBaseType() != param.isBaseType()) {
					...
					newArgs[i] = environment().computeBoxingType(env, arguments[i]);
				}	
			}
			if (newArgs != null)
				arguments = newArgs;
			method = ParameterizedGenericMethodBinding.computeCompatibleMethod(method, arguments, this, invocationSite, env);
		...
		if (parameterCompatibilityLevel(env, method, arguments) > NOT_COMPATIBLE)
			return method;
	...
	protected boolean connectTypeVariables(TypeParameter[] typeParameters, boolean checkForErasedCandidateCollisions) {
		...
		nextVariable: for (int i = 0, paramLength = typeParameters.length; i < paramLength; i++) {
			...
			if (superType == null) {
				...
				typeRef.setResolvedType(superType); // hold onto the problem type
				firstBound: {
			...
			if (boundRefs != null) {
				nextBound: for (int j = 0, boundLength = boundRefs.length; j < boundLength; j++) {
					...
					if (superType == null) {
						...
						boolean didAlreadyComplain = !typeRef.getResolvedType().isValidBinding();
						...
						ReferenceBinding superRefType = (ReferenceBinding) superType;
						for (int index = typeVariable.superInterfaces().length; --index >= 0;) { // FIXME type environment?
	...
	public TypeVariableBinding[] createTypeVariables(TypeParameter[] typeParameters, Binding declaringElement) {
		...
		for (int i = 0; i < length; i++) {
			TypeParameter typeParameter = typeParameters[i];
			TypeVariableBinding parameterBinding = new TypeVariableBinding(typeParameter.name, typeParameter.boundKind, declaringElement, i);
...
//				}
	...
	*/
	public final ReferenceBinding enclosingReceiverType() {
	    return enclosingReceiverType(false);
	}
	public final ReferenceBinding enclosingReceiverType(boolean inJavaGIDictionaryConstructor) {
		Scope scope = this;
		do {
			if (scope instanceof ClassScope) {
				ReferenceBinding r = environment().convertToParameterizedType(((ClassScope) scope).referenceContext.binding());
				return javaGIEnclosing(r, inJavaGIDictionaryConstructor);
			}
			scope = scope.parent;
		} while (scope != null);
		return null;
	...
	*/
	public final SourceTypeBinding enclosingSourceType() {
		...
		do {
			if (scope instanceof ClassScope) {
				SourceTypeBinding stb = ((ClassScope) scope).referenceContext.binding();
				return stb;
			}
			scope = scope.parent;
		} while (scope != null);
	...
	protected MethodBinding findDefaultAbstractMethod(
		...
		while (currentType != null) {
			...
			currentType = currentType.superclass(); // FIXME type environment?
		}
		...
		if (foundSize > startFoundSize) {
			...
			for (int i = startFoundSize; i < foundSize; i++) {
				...
				MethodBinding compatibleMethod = computeCompatibleMethod(methodBinding, argumentTypes, invocationSite, env);
				if (compatibleMethod != null) {
					if (compatibleMethod.isValidBinding()) {
						if (concreteMatch != null && concreteMatch.declaringClass.findSuperTypeOriginatingFrom(env, compatibleMethod.declaringClass) != null)
							if (environment().methodVerifier().isParameterSubsignature(env, concreteMatch, compatibleMethod))
		...
		if (compilerOptions().complianceLevel >= ClassFileConstants.JDK1_4)
			return mostSpecificMethodBinding(candidates, candidatesCount, argumentTypes, invocationSite, env, receiverType);
		return mostSpecificInterfaceMethodBinding(candidates, candidatesCount, invocationSite, env);
	}
	...
	// Internal use only
	public MethodBinding findExactMethod(ReferenceBinding receiverType, char[] selector, TypeBinding[] argumentTypes, InvocationSite invocationSite, TypeEnvironment env) {
		CompilationUnitScope unitScope = compilationUnitScope();
		...
		if (exactMethod != null && exactMethod.typeVariables == Binding.NO_TYPE_VARIABLES && !exactMethod.isBridge()) {
			...
			if (receiverType.isInterface() || exactMethod.canBeSeenBy(receiverType, invocationSite, this)) {
				...
				if (invocationSite.genericTypeArguments() != null) {
					exactMethod = computeCompatibleMethod(exactMethod, argumentTypes, invocationSite, env);
				}
				return fixPositiveSelfType(exactMethod, receiverType);
			}
		}
		return null;
	}
	...
	*/
	public FieldBinding findField(TypeBinding receiverType, char[] fieldName, InvocationSite invocationSite, boolean needResolve) {
		...
		checkArrayField: {
			...
			switch (receiverType.kind()) {
				...
				case Binding.TYPE_PARAMETER : // capture
					TypeBinding receiverErasure = receiverType.erasure(this);
		...
		while (keepLooking) {
			ReferenceBinding[] itsInterfaces = currentType.superInterfaces(getTypeEnvironment(), true);
			...
			if ((currentType = currentType.superclass(getTypeEnvironment())) == null)
				break;
	...
	public MethodBinding findMethod(ReferenceBinding receiverType, char[] selector, TypeBinding[] argumentTypes, InvocationSite invocationSite, TypeEnvironment env) {
	   MethodBinding res = findMethod(receiverType, selector, argumentTypes, invocationSite, env, false);
	   if (res == null) res = new ProblemMethodBinding(selector, argumentTypes, ProblemReasons.NotFound);
	   if (res.isValidBinding()) {
	       return fixPositiveSelfType(res, receiverType);
	   }
	   MethodBinding javaGIRes = MethodLookup.findMethod(receiverType, selector, argumentTypes, invocationSite, env, this, (ProblemMethodBinding) res);
	   if (javaGIRes != res && javaGIRes.isValidBinding()) {
	       invocationSite.markAsJavaGICallSite();
	       javaGIRes = checkConstraints(javaGIRes, selector, argumentTypes, env);
	   }
	   return javaGIRes;
	}

	// Internal use only - use findMethod()
	private MethodBinding findMethod(ReferenceBinding receiverType, char[] selector, TypeBinding[] argumentTypes, InvocationSite invocationSite, TypeEnvironment env, boolean inStaticContext) {
	    MethodBinding method = findMethod0(receiverType, selector, argumentTypes, invocationSite, env, inStaticContext);
	    return checkConstraints(method, selector, argumentTypes, env);
	}
	
	private MethodBinding findMethod0(ReferenceBinding receiverType, char[] selector, TypeBinding[] argumentTypes, InvocationSite invocationSite, TypeEnvironment env, boolean inStaticContext) {
		...
		while (currentType != null) {
			unitScope.recordTypeReference(currentType);
			MethodBinding[] currentMethods = currentType.getMethodsInclReceivers(selector);
				...
				if (isCompliant14 && (receiverTypeIsInterface || found.size > 0)) {
					nextMethod: for (int i = 0, l = currentLength; i < l; i++) { // currentLength can be modified inside the loop
						...
						for (int j = 0, max = found.size; j < max; j++) {
							...
							if (verifier.isParameterSubsignature(env, matchingMethod.original(), currentMethod.original())) {
								if (isCompliant15) {
			...
			currentType = currentType.superclass(env);
		}
					...
					if (compatibleMethod.isValidBinding()) {
						if (foundSize == 1 && compatibleMethod.canBeSeenBy(receiverType, invocationSite, this)) {
							...
							if (searchForDefaultAbstractMethod)
								return findDefaultAbstractMethod(receiverType, selector, argumentTypes, invocationSite, env, classHierarchyStart, found, compatibleMethod);
		...
		if (candidatesCount == 0) {
			...
			MethodBinding interfaceMethod =
				findDefaultAbstractMethod(receiverType, selector, argumentTypes, invocationSite, env, classHierarchyStart, found, null);
		...
		if (receiverTypeIsInterface) {
			...
			switch (visiblesCount) {
				...
				case 1 :
					if (searchForDefaultAbstractMethod)
						return findDefaultAbstractMethod(receiverType, selector, argumentTypes, invocationSite, env, classHierarchyStart, found, candidates[0]);
					unitScope.recordTypeReferences(candidates[0].thrownExceptions);
					return candidates[0];
				default :
					break;
		...
		if (complianceLevel <= ClassFileConstants.JDK1_3) {
			...
			return !declaringClass.isInterface()
				? mostSpecificClassMethodBinding(candidates, visiblesCount, invocationSite, env)
				: mostSpecificInterfaceMethodBinding(candidates, visiblesCount, invocationSite, env);
		}
		...
		if (inStaticContext) {
			...
			if (staticCount > 1)
				return mostSpecificMethodBinding(staticCandidates, staticCount, argumentTypes, invocationSite, env, receiverType);
		}

		MethodBinding mostSpecificMethod = mostSpecificMethodBinding(candidates, visiblesCount, argumentTypes, invocationSite, env, receiverType);
		if (searchForDefaultAbstractMethod) { // search interfaces for a better match
			if (mostSpecificMethod.isValidBinding())
				...
				return findDefaultAbstractMethod(receiverType, selector, argumentTypes, invocationSite, env, classHierarchyStart, found, mostSpecificMethod);
			// see if there is a match in the interfaces - see LookupTest#84
			MethodBinding interfaceMethod = findDefaultAbstractMethod(receiverType, selector, argumentTypes, invocationSite, env, classHierarchyStart, found, null);
	...
	public MethodBinding findMethodForArray(
		...
		methodBinding = findMethod(object, selector, argumentTypes, invocationSite, env);
		if (methodBinding == null)
	...
	protected void findMethodInSuperInterfaces(ReferenceBinding currentType, char[] selector, ObjectVector found) {
		ReferenceBinding[] itsInterfaces = currentType.superInterfaces(getTypeEnvironment()); 
		if (itsInterfaces != null && itsInterfaces != Binding.NO_SUPERINTERFACES) {
	...
	public MethodBinding getConstructor(ReferenceBinding receiverType, TypeBinding[] argumentTypes, InvocationSite invocationSite, TypeEnvironment tenv) {
		CompilationUnitScope unitScope = compilationUnitScope();
		LookupEnvironment env = unitScope.environment;
		try {
			env.missingClassFileLocation = invocationSite;
			...
			if (methodBinding != null && methodBinding.canBeSeenBy(invocationSite, this)) {
			    ...
			    if (invocationSite.genericTypeArguments() != null)
			    	methodBinding = computeCompatibleMethod(methodBinding, argumentTypes, invocationSite, tenv);				
				return methodBinding;
			}
			...
			for (int i = 0, length = methods.length; i < length; i++) {
				MethodBinding compatibleMethod = computeCompatibleMethod(methods[i], argumentTypes, invocationSite, tenv);
				if (compatibleMethod != null) {
					if (compatibleMethod.isValidBinding())
			...
			return mostSpecificMethodBinding(visible, visibleIndex, argumentTypes, invocationSite, tenv, receiverType);
		} catch (AbortCompilation e) {
			e.updateContext(invocationSite, referenceCompilationUnit().compilationResult);
			throw e;
		} finally {
	 ...
	 */
	public MethodBinding getImplicitMethod(char[] selector, TypeBinding[] argumentTypes, InvocationSite invocationSite, TypeEnvironment env) {
			...
			switch (scope.kind) {
				...
				case CLASS_SCOPE :
					...
					if (!insideTypeAnnotation) {
						...
						MethodBinding methodBinding = classScope.findExactMethod(receiverType, selector, argumentTypes, invocationSite, env);
						if (methodBinding == null) {
							methodBinding = classScope.findMethod(receiverType, selector, argumentTypes, invocationSite, env);
						}
		...
		if (insideStaticContext && options.sourceLevel >= ClassFileConstants.JDK1_5) {
			...
			if (imports != null) {
					...
					if (importBinding.isStatic()) {
						...
						if (importBinding.onDemand) {
							if (!skipOnDemand && resolvedImport instanceof ReferenceBinding)
								...
								possible = findMethod((ReferenceBinding) resolvedImport, selector, argumentTypes, invocationSite, env, true);
						} else {
							if (resolvedImport instanceof MethodBinding) {
								...
								if (CharOperation.equals(staticMethod.selector, selector))
									// answers closest approximation, may not check argumentTypes or visibility
									possible = findMethod(staticMethod.declaringClass, selector, argumentTypes, invocationSite, env, true);
								...
								if (CharOperation.equals(staticField.name, selector)) {
									...
									if (referencedType != null)
										// answers closest approximation, may not check argumentTypes or visibility
										possible = findMethod((ReferenceBinding) referencedType, selector, argumentTypes, invocationSite, env, true);
								}
							}
						}
						if (possible != null && possible != foundProblem) {
							if (!possible.isValidBinding()) {
								...
								MethodBinding compatibleMethod = computeCompatibleMethod(possible, argumentTypes, invocationSite, env);
								if (compatibleMethod != null) {
									if (compatibleMethod.isValidBinding()) {
				...
				if (visible != null) {
					...
					foundMethod = mostSpecificMethodBinding(temp, temp.length, argumentTypes, invocationSite, env, null);
				}
			}
		}
	...
	*/
	...
	public final MethodBinding getMethod(TypeBinding receiverType, char[] selector, TypeBinding[] argumentTypes, InvocationSite invocationSite, TypeEnvironment tenv) {
		CompilationUnitScope unitScope = compilationUnitScope();
		LookupEnvironment env = unitScope.environment;
		try {
			env.missingClassFileLocation = invocationSite;
			switch (receiverType.kind()) {
				...
				case Binding.ARRAY_TYPE :
					unitScope.recordTypeReference(receiverType);
					return findMethodForArray((ArrayBinding) receiverType, selector, argumentTypes, invocationSite, tenv);
			}
			unitScope.recordTypeReference(receiverType);
			...
			// retrieve an exact visible match (if possible)
			MethodBinding methodBinding = findExactMethod(currentType, selector, argumentTypes, invocationSite, tenv);
			if (methodBinding != null) return methodBinding;
 
			methodBinding = findMethod(currentType, selector, argumentTypes, invocationSite, tenv);
	...
	*/
	...
	public final TypeBinding getType(char[] name, boolean ignoreStaticContextRestriction) {
		// Would like to remove this test and require senders to specially handle base types
		TypeBinding binding = getBaseType(name);
		if (binding != null) return binding;
		return (ReferenceBinding) getTypeOrPackage(name, Binding.TYPE, true, ignoreStaticContextRestriction);
	}
	...
	*/
	final Binding getTypeOrPackage(char[] name, int mask, boolean needResolve) {
	    return getTypeOrPackage(name, mask, needResolve, false);
	}
	
	final Binding getTypeOrPackage(char[] name, int mask, boolean needResolve, boolean ignoreStaticContextRestriction) {
	    if (javagi.compiler.GILog.NameResolve().isFinest()) {
	        javagi.compiler.GILog.NameResolve().jfinest("searching for type or package " + new String(name) + " in scope " + 
                    getClass().getName() + '@' + Integer.toHexString(hashCode()) +
                    " (referenceContext: " + this.referenceContext() + ")");
	        // javagi.compiler.GILog.printStackTrace();
	    }
	    Binding res = getTypeOrPackage0(name, mask, needResolve, ignoreStaticContextRestriction);
	    if (javagi.compiler.GILog.NameResolve().isFinest()) {
	        javagi.compiler.GILog.NameResolve().jfinest("result of searching for type or package " + new String(name) + " in scope " + 
                    getClass().getName() + '@' + Integer.toHexString(hashCode()) + ": " + res.debugName() + 
                    (res.isValidBinding() ? "" : " (Problem, see javagi.eclipse.jdt.internal.compiler.lookup.ProblemReasons: "
                            + res.problemId() + ")"));
	    }
	    return res;
	}
	
	private final Binding getTypeOrPackage0(char[] name, int mask, boolean needResolve, boolean ignoreStaticContextRestriction) {
		Scope scope = this;
			...
			boolean inheritedHasPrecedence = compilerOptions().complianceLevel >= ClassFileConstants.JDK1_4;
				...
				switch (scope.kind) {
					...
					case CLASS_SCOPE :
							...
							if (memberType != null) { // skip it if we did not find anything
								...
								if (memberType.isValidBinding()) {
									if (sourceType == memberType.enclosingType() || inheritedHasPrecedence) {
										if (insideStaticContext && !memberType.isStatic() && sourceType.isGenericType() && !ignoreStaticContextRestriction) {
						...
						TypeVariableBinding typeVariable = sourceType.getTypeVariable(name);
						if (typeVariable != null) {
						    // SW: changes for JavaGI
						    if (this.kind == METHOD_SCOPE) {
						        MethodScope ms = (MethodScope) this;
						        ReferenceContext rc = ms.referenceContext();
						        // in JavaGI, a static interface method can see the type variables of its directly
	                            // enclosing interface or implementation (see tests javagi/parser/StaticInterfaceMethod[1,2].java)
						        if (rc instanceof AbstractMethodDeclaration &&  this.parent == scope &&
						            ((AbstractMethodDeclaration) rc).isStaticInterfaceOrImplementationMethod()) {
						                return typeVariable;
						        }
						        // in JavaGI, the magic receiver field can see the implementation type variables of its interface
						        if (rc instanceof TypeDeclaration && this.parent != null && this.parent.parent == scope) {
						            TypeDeclaration typeDecl = (TypeDeclaration) rc;
						            if (typeDecl.isReceiver() && sourceType.isInterface()) {
						                for (TypeVariableBinding x : sourceType.implTypeVariables())
						                    if (x == typeVariable) return typeVariable;
						            }
						        }
						        // in JavaGI, methods inside receivers can see the type variable if their interfaces
						        if (rc instanceof AbstractMethodDeclaration && this.parent != null && this.parent.parent == scope) {
						            if (((AbstractMethodDeclaration) rc).isReceiverMethod()) return typeVariable;
						        }
						    }
							if (insideStaticContext && !ignoreStaticContextRestriction) {// do not consider this type modifiers: access is legite within same type
		...
		if (javagi.compiler.GILog.NameResolve().isFinest()) {
                  javagi.compiler.GILog.NameResolve().jfinest("reached compilation unit scope: " + unitScope.shortString() + " foundType=" + foundType +
                                                              "top level members");
		}
		...
		if (mecs != null) {
			nextCandidate: for (int k = 0, max = mecs.length; k < max; k++) {
				...
				if (value instanceof TypeBinding[]) {
					...
					problemReporter().superinterfacesCollide(invalidInvocations[0].erasure(this), typeRef, invalidInvocations[0], invalidInvocations[1]);
	...
	protected boolean isAcceptableMethod(TypeEnvironment env, MethodBinding one, MethodBinding two) {
				...
				if (oneParam == twoParam) {
					if (twoParam.leafComponentType().isRawType()) {
						...
						if (oneParam == one.original().parameters[i]
								&&  twoParam.leafComponentType().erasure(env) != two.original().parameters[i].leafComponentType().erasure(env)) {
							return false;
						}
					}
				} else if (oneParam.isCompatibleWith(env, twoParam)) {
					if (oneParam.leafComponentType().isRawType()) {
						// A#RAW is not more specific than a rawified A<T>
						if (oneParam.needsUncheckedConversion(env, two.declaringClass.isRawType() ? twoParam : two.original().parameters[i]))
							return false;
					}
				} else {
					if (i == oneParamsLength - 1 && one.isVarargs() && two.isVarargs()) {
						TypeBinding eType = ((ArrayBinding) twoParam).elementsType();
						if (oneParam == eType || oneParam.isCompatibleWith(env, eType))
		...
		if (one.isVarargs() && two.isVarargs()) {
			...
			for (int i = (oneParamsLength > twoParamsLength ? twoParamsLength : oneParamsLength) - 2; i >= 0; i--)
				if (oneParams[i] != twoParams[i] && !oneParams[i].isCompatibleWith(env, twoParams[i]))
					return false;
			if (parameterCompatibilityLevel(env, one, twoParams) == NOT_COMPATIBLE
				&& parameterCompatibilityLevel(env, two, oneParams) == VARARGS_COMPATIBLE)
					return true; 
		}
		return false;
	}
	...
	public boolean isBoxingCompatibleWith(TypeBinding expressionType, TypeBinding targetType) {
		...
		TypeBinding convertedType = environment.computeBoxingType(this.getTypeEnvironment(), expressionType);
		return convertedType == targetType || convertedType.isCompatibleWith(this.getTypeEnvironment(), targetType);
	}
	...
	*/
	...
	private TypeBinding leastContainingInvocation(TypeEnvironment env, TypeBinding mec, Object invocationData, List lubStack) {
			...
			switch (invocation.kind()) {
				case Binding.GENERIC_TYPE :
					...
					for (int j = 0; j < argLength; j++) {
						TypeBinding bestArgument = leastContainingTypeArgument(env, bestArguments[j], invocationVariables[j], (ReferenceBinding) mec, j, lubStack);
				...
				case Binding.PARAMETERIZED_TYPE :
					...
					for (int j = 0; j < argLength; j++) {
						TypeBinding bestArgument = leastContainingTypeArgument(env, bestArguments[j], parameterizedType.arguments[j], (ReferenceBinding) mec, j, lubStack);
						if (bestArgument == null) return null;
						bestArguments[j] = bestArgument;
					}
					break;
		...
		TypeBinding least = environment().createParameterizedType((ReferenceBinding) mec.erasure(env), bestArguments, mec.enclosingType());
		return dim == 0 ? least : environment().createArrayType(least, dim);
	}
	
	// JLS 15.12.2
	private TypeBinding leastContainingTypeArgument(TypeEnvironment env, TypeBinding u, TypeBinding v, ReferenceBinding genericType, int rank, List lubStack) {
		if (u == null) return v;
		if (u == v) return u;
		if (v.isWildcard()) {
			...
			if (u.isWildcard()) {
				...
				// FIXME: consider implements bounds for JavaGI
					...
					case Wildcard.EXTENDS :
						switch(wildV.boundKind) {
							// ? extends U, ? extends V
							case Wildcard.EXTENDS :  
								TypeBinding lub = lowerUpperBound(env, new TypeBinding[]{wildU.bound,wildV.bound}, lubStack);
					...
					case Wildcard.SUPER : 
						...
						if (wildU.boundKind == Wildcard.SUPER) {
							TypeBinding[] glb = greaterLowerBound(env, new TypeBinding[]{wildU.bound,wildV.bound});
				...
				switch (wildV.boundKind) {
					// U, ? extends V
					case Wildcard.EXTENDS :
						TypeBinding lub = lowerUpperBound(env, new TypeBinding[]{u,wildV.bound}, lubStack);
						if (lub == null) return null;
						// int is returned to denote cycle detected in lub computation - stop recursion by answering unbound wildcard
						if (lub == TypeBinding.INT) return environment().createWildcard(genericType, rank, null, null /*no extra bound*/, Wildcard.UNBOUND);
						return environment().createWildcard(genericType, rank, lub, null /*no extra bound*/, Wildcard.EXTENDS);	
					// U, ? super V
					case Wildcard.SUPER :
						TypeBinding[] glb = greaterLowerBound(env, new TypeBinding[]{u,wildV.bound});
						if (glb == null) return null;
						return environment().createWildcard(genericType, rank, glb[0], null /*no extra bound*/, Wildcard.SUPER);	// TODO (philippe) need to capture entire bounds
			...
			switch (wildU.boundKind) {
				// U, ? extends V
				case Wildcard.EXTENDS :
					TypeBinding lub = lowerUpperBound(env, new TypeBinding[]{wildU.bound, v}, lubStack);
					if (lub == null) return null;
					// int is returned to denote cycle detected in lub computation - stop recursion by answering unbound wildcard
					if (lub == TypeBinding.INT) return environment().createWildcard(genericType, rank, null, null /*no extra bound*/, Wildcard.UNBOUND);
					return environment().createWildcard(genericType, rank, lub, null /*no extra bound*/, Wildcard.EXTENDS);	
				// U, ? super V
				case Wildcard.SUPER :
					TypeBinding[] glb = greaterLowerBound(env, new TypeBinding[]{wildU.bound, v});
					if (glb == null) return null;
					return environment().createWildcard(genericType, rank, glb[0], null /*no extra bound*/, Wildcard.SUPER); // TODO (philippe) need to capture entire bounds		
				case Wildcard.UNBOUND :
			}
		}
		TypeBinding lub = lowerUpperBound(env, new TypeBinding[]{u,v}, lubStack);
		if (lub == null) return null;
		// int is returned to denote cycle detected in lub computation - stop recursion by answering unbound wildcard
		if (lub == TypeBinding.INT) return environment().createWildcard(genericType, rank, null, null /*no extra bound*/, Wildcard.UNBOUND);
		return environment().createWildcard(genericType, rank, lub, null /*no extra bound*/, Wildcard.EXTENDS);
	 ...
	 */
	public TypeBinding lowerUpperBound(TypeEnvironment env, TypeBinding[] types) {
		int typeLength = types.length;
		if (typeLength == 1) {
			TypeBinding type = types[0];
			return type == null ? TypeBinding.VOID : type;
		}		
		return lowerUpperBound(env, types, new ArrayList(1));
	}
	
	// 15.12.2
	private TypeBinding lowerUpperBound(TypeEnvironment env, TypeBinding[] types, List lubStack) {
		...
		nextLubCheck: for (int i = 0; i < stackLength; i++) {
			...
			nextTypeCheck:	for (int j = 0; j < typeLength; j++) {
				...
				for (int k = 0; k < lubTypeLength; k++) {
					...
					if (lubType == type || lubType.isEquivalentTo(env, type)) continue nextTypeCheck; // type found, jump to next one 
				}
		...
		for (int i = 0; i < length; i++) {
			...
			mec = leastContainingInvocation(env, mec, invocations.get(mec), lubStack);
	...
	protected TypeBinding[] minimalErasedCandidates(TypeBinding[] types, Map allInvocations) {
		...
		switch(leafType.kind()) {
			...
			case Binding.RAW_TYPE :
			case Binding.ARRAY_TYPE : 
				firstErasure = firstType.erasure(this);
				break;
			default :
		...
		for (int i = 0; i < max; i++) {
			...
			if (currentType.isCapture()) {
				...
				if (firstBound != null && firstBound.isArrayType()) {
					...
					if (!typesToVisit.contains(superType)) {
						typesToVisit.add(superType);
						max++;
						TypeBinding superTypeErasure = (firstBound.isTypeVariable() || firstBound.isWildcard() /*&& !itsInterface.isCapture()*/) ? superType : superType.erasure(this);
			...
			if (itsInterfaces != null) { // can be null during code assist operations that use LookupEnvironment.completeTypeBindings(parsedUnit, buildFieldsAndMethods)
				for (int j = 0, count = itsInterfaces.length; j < count; j++) {
					...
					if (!typesToVisit.contains(superType)) {
						typesToVisit.add(superType);
						max++;
						TypeBinding superTypeErasure = (itsInterface.isTypeVariable() || itsInterface.isWildcard() /*&& !itsInterface.isCapture()*/) ? superType : superType.erasure(this);
						if (superTypeErasure != superType) {
							allInvocations.put(superTypeErasure, superType);
						}						
					}
			...
			TypeBinding itsSuperclass = currentType.superclass(getTypeEnvironment());
			if (itsSuperclass != null) {
				...
				if (!typesToVisit.contains(superType)) {
					typesToVisit.add(superType);
					max++;
					TypeBinding superTypeErasure = (itsSuperclass.isTypeVariable() || itsSuperclass.isWildcard() /*&& !itsSuperclass.isCapture()*/) ? superType : superType.erasure(this);
					if (superTypeErasure != superType) {
						allInvocations.put(superTypeErasure, superType);
					}
				}
		...
		for (Iterator iter = typesToVisit.iterator(); iter.hasNext();) {
			...
			erasedSuperTypes[rank++] = (leafType.isTypeVariable() || leafType.isWildcard() /*&& !leafType.isCapture()*/) ? type : type.erasure(this);
		}
		...
		nextOtherType: for (int i = indexOfFirst+1; i < length; i++) {
			...
			if (otherType.isArrayType()) {
				nextSuperType: for (int j = 0; j < superLength; j++) {
					...
					TypeBinding match;
					if ((match = otherType.findSuperTypeOriginatingFrom(getTypeEnvironment(), erasedSuperType)) == null) {
			...
			nextSuperType: for (int j = 0; j < superLength; j++) {
				...
				if (erasedSuperType == otherType || erasedSuperType.id == T_JavaLangObject && otherType.isInterface()) {
					...
					if (erasedSuperType.isArrayType()) {
						...
						match = otherType.findSuperTypeOriginatingFrom(getTypeEnvironment(), erasedSuperType);
					}
			...
			nextType: for (int i = 0; i < superLength; i++) {
				...
				nextOtherType: for (int j = 0; j < superLength; j++) {
					...
					if (erasedSuperType instanceof ReferenceBinding) {
						...
						if (erasedSuperType.findSuperTypeOriginatingFrom(getTypeEnvironment(), otherType) != null) {
	...
	*/
	protected final MethodBinding mostSpecificClassMethodBinding(MethodBinding[] visible, int visibleSize, InvocationSite invocationSite, TypeEnvironment env) {
		...
		nextVisible : for (int i = 0; i < visibleSize; i++) {
			...
			for (int j = 0; j < visibleSize; j++) {
				if (i == j) continue;
				if (!visible[j].areParametersCompatibleWith(env, method.parameters))
	...
	*/
	protected final MethodBinding mostSpecificInterfaceMethodBinding(MethodBinding[] visible, int visibleSize, InvocationSite invocationSite, TypeEnvironment env) {
		nextVisible : for (int i = 0; i < visibleSize; i++) {
			MethodBinding method = visible[i];
			for (int j = 0; j < visibleSize; j++) {
				if (i == j) continue;
	...
	public final MethodBinding mostSpecificMethodBinding(MethodBinding[] visible, int visibleSize, TypeBinding[] argumentTypes, InvocationSite invocationSite, TypeEnvironment env, ReferenceBinding receiverType) {
		int[] compatibilityLevels = new int[visibleSize];
		for (int i = 0; i < visibleSize; i++)
			compatibilityLevels[i] = parameterCompatibilityLevel(env, visible[i], argumentTypes);
		...
		for (int level = 0, max = VARARGS_COMPATIBLE; level <= max; level++) {
			nextVisible : for (int i = 0; i < visibleSize; i++) {
				...
				for (int j = 0; j < visibleSize; j++) {
					...
					MethodBinding acceptable = computeCompatibleMethod(methodToTest, tiebreakMethod.parameters, invocationSite, env);
					...
					if (acceptable == null || !acceptable.isValidBinding())
						continue nextVisible;
					if (!isAcceptableMethod(env, tiebreakMethod, acceptable))
						continue nextVisible;
		...
		if (receiverType != null)
			receiverType = receiverType instanceof CaptureBinding ? receiverType : (ReferenceBinding) receiverType.erasure(env);
			...
			if (current != null) {
				...
				for (int j = 0; j < visibleSize; j++) {
					...
					if (!original.isAbstract()) {
						...
						TypeBinding superType = original.declaringClass.findSuperTypeOriginatingFrom(env, original2.declaringClass.erasure(env));
						if (superType == null)
						...
						if (current.hasSubstitutedParameters() || original.typeVariables != Binding.NO_TYPE_VARIABLES) {
							...
							if (!environment().methodVerifier().isParameterSubsignature(env, original, original2)) // HERE seems to be the problem, may be use the version of isParameterSubsignature without the env parameter?
								continue nextSpecific; // current does not override next
						}
					} else if (receiverType != null) { // should not be null if original isAbstract, but be safe
						TypeBinding superType = receiverType.findSuperTypeOriginatingFrom(env, original.declaringClass.erasure(env));
						...
						superType = receiverType.findSuperTypeOriginatingFrom(env, original2.declaringClass.erasure(env));
						...
						if (original2 == null || !original.areParameterErasuresEqual(original2))
							continue nextSpecific; // current does not override next
						if (!original.returnType.isCompatibleWith(env, original2.returnType) &&
								!original.returnType.erasure(env).isCompatibleWith(env, original2.returnType.erasure(env))) {
							// 15.12.2
								...
								if (next.thrownExceptions == Binding.NO_EXCEPTIONS) {
									...
									nextException : for (int t = 0; t < mostSpecificLength; t++) {
										...
										for (int s = 0; s < nextLength; s++) {
											...
											if (exception.isCompatibleWith(env, nextException)) {
												temp.add(exception);
												continue nextException;
											} else if (nextException.isCompatibleWith(env, exception)) {
	...
	public int parameterCompatibilityLevel(TypeEnvironment tenv, MethodBinding method, TypeBinding[] arguments) {
		TypeBinding[] parameters = method.parameters;
		int paramLength = parameters.length;
		int argLength = arguments.length;
		...
		if (compilerOptions().sourceLevel < ClassFileConstants.JDK1_5) {
			...
			for (int i = 0; i < argLength; i++) {
				...
				if (arg != param && !arg.isCompatibleWith(tenv, param))
					return NOT_COMPATIBLE;
			}
			return COMPATIBLE;
		}
		...
		if (method.isVarargs()) {
			...
			if (paramLength == argLength) { // accept X or X[] but not X[][]
				...
				if (param != arg) {
					level = parameterCompatibilityLevel(tenv, arg, param, env);
					if (level == NOT_COMPATIBLE) {
						// expect X[], is it called with X
						param = ((ArrayBinding) param).elementsType();
						if (parameterCompatibilityLevel(tenv, arg, param, env) == NOT_COMPATIBLE)
							return NOT_COMPATIBLE;
				...
				if (paramLength < argLength) { // all remaining argument types must be compatible with the elementsType of varArgType
					...
					for (int i = lastIndex; i < argLength; i++) {
						TypeBinding arg = arguments[i];
						if (param != arg && parameterCompatibilityLevel(tenv, arg, param, env) == NOT_COMPATIBLE)
							return NOT_COMPATIBLE;
					}
		...
		for (int i = 0; i < lastIndex; i++) {
			...
			TypeBinding arg = arguments[i];
			if (arg != param) {
				int newLevel = parameterCompatibilityLevel(tenv, arg, param, env);
	...
	private int parameterCompatibilityLevel(TypeEnvironment tenv, TypeBinding arg, TypeBinding param, LookupEnvironment env) {
		// only called if env.options.sourceLevel >= ClassFileConstants.JDK1_5
		if (arg.isCompatibleWith(tenv, param))
			return COMPATIBLE;
		if (arg.isBaseType() != param.isBaseType()) {
			TypeBinding convertedType = env.computeBoxingType(tenv, arg);
			if (convertedType == param || convertedType.isCompatibleWith(tenv, param))
	...
	public final TypeEnvironment getTypeEnvironment() {
	    Scope current = this;
	    do {
            switch(current.kind) {
            case METHOD_SCOPE :
                MethodScope ms = (MethodScope) current;
                if (ms.referenceMethod() == null) break;
                MethodBinding m = ms.referenceMethod().binding;
                if (m == null) {
                    GILog.TypeEnv().jwarn("binding for " + ms.referenceMethod() + " is null");
                    return JTypeEnvironment.empty(ms.referenceMethod(), environment());
                }
                return m.getTypeEnvironment();
            case CLASS_SCOPE :
                ClassScope cs = (ClassScope) current;
                SourceTypeBinding stb = cs.referenceContext.binding();
                return stb.getTypeEnvironment();
            default :
                break;
            }
        } while ((current = current.parent) != null);
	    return JTypeEnvironment.empty(null, environment());
	}

	public static ConstraintBinding substitute(Substitution subst, ConstraintBinding c) {
	    TypeBinding[] ts = Scope.substitute(subst, c.constrainedTypes);
	    TypeBinding t = Scope.substitute(subst, c.constrainingType);
	    return new ConstraintBinding(c.getConstraintKind(), ts, t);
	}
	
	public static ConstraintBinding[] substitute(Substitution subst, ConstraintBinding[] cs) {
	    if (cs == null) return null;
	    ConstraintBinding[] res = new ConstraintBinding[cs.length];
	    for (int i = 0; i < cs.length; i++) {
		res[i] = Scope.substitute(subst, cs[i]);
	    }
	    return res;
	}
	
	// invoked if method was found through Java's typing rules. If receiverType is
	// an interface type, then the methods return type may be the self type of the interface.
	// In this case, replace it with receiverType
    private MethodBinding fixPositiveSelfType(MethodBinding method, 
                                              final ReferenceBinding receiverType) {
        if (method.returnType.isImplTypeVariable()) {
            final TypeVariableBinding selfType = (TypeVariableBinding) method.returnType;
            Substitution subst = new Substitution() {
                ...
                public TypeBinding substitute(TypeVariableBinding typeVariable) {
                    if (typeVariable.equals(selfType)) {
                        return receiverType;
                    } else {
                        return typeVariable;
                    }
            ...
            MethodBinding m = new ParameterizedMethodBinding(method.declaringClass, subst, method);
            return m;
        } else {
            return method;
        }
    ...
    private ReferenceBinding javaGIEnclosing(ReferenceBinding r, boolean inJavaGIDictionaryConstructor) {
        if (r.isImplementationReceiver() && !inJavaGIDictionaryConstructor) {
            TypeBinding res = r.receiverType();
            if (res == null) throw new NullPointerException();
            return (ReferenceBinding) res;
        } else if (r.isImplementation() && !inJavaGIDictionaryConstructor) {
            TypeBinding[] implTypes = r.implTypes();
            if (implTypes == null || implTypes[0] == null) throw new NullPointerException();
            return (ReferenceBinding) implTypes[0];
        } else {
            return r;
        }
    }
    
    private MethodBinding checkConstraints(MethodBinding method, char[] selector, TypeBinding[] argumentTypes, TypeEnvironment env) {
        if (method == null || ! method.isValidBinding()) return method;
        for (ConstraintBinding c : method.constraints()) {
            if (!Entailment.entails(env, c)) {
                ProblemMethodBinding pmb = new ProblemMethodBinding(method, selector, argumentTypes, ProblemReasons.ParameterizedMethodTypeMismatch);
                pmb.reportJavaGIProblem("Constraint ``%s'' not satisfied for this call.", c);
                return pmb;
            }
        }
        return method;
    ...
    public final Loadable findEvidence(TypeVariableBinding x) {
        Scope scope = this;
        do {
            if (scope instanceof ClassScope) {
                ClassScope cs = (ClassScope) scope;
                if (cs.evidence.containsKey(x)) return cs.evidence.get(x);
            } else if (scope instanceof MethodScope) {
                MethodScope ms = (MethodScope) scope;
                if (ms.evidence.containsKey(x)) return ms.evidence.get(x);
            }
            scope = scope.parent;
        } while (scope != null);
        throw new GICompilerBug("No evidence found for type variable " + x.debugName());
    }
