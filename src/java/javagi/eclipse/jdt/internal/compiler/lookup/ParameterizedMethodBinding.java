...
public class ParameterizedMethodBinding extends MethodBinding {
	...
	public ParameterizedMethodBinding(final ParameterizedTypeBinding parameterizedDeclaringClass, MethodBinding originalMethod) {
	    this(parameterizedDeclaringClass, parameterizedDeclaringClass, originalMethod);
	}
	 
	public ParameterizedMethodBinding(final ReferenceBinding declaringClass, 
		...
		if (length == 0) {
			...
			if (!isStatic) substitution = subst;
		} else {
			...
			for (int i = 0; i < length; i++) { // copy original type variable to relocate
				...
				substitutedVariables[i] = new TypeVariableBinding(originalVariable.sourceName, originalVariable.boundKind, this, originalVariable.rank);
			}
			...
			substitution = new Substitution() {
				public LookupEnvironment environment() { 
					return subst.environment(); 
				}
				public boolean isRawSubstitution() {
					return !isStatic && subst.isRawSubstitution();
				}
				public TypeBinding substitute(TypeVariableBinding typeVariable) {
			        ...
			        if (!isStatic)
						return subst.substitute(typeVariable);
			        return typeVariable;
				}
			};
			...
			for (int i = 0; i < length; i++) {
				...
				switch (substitutedSuperclass.kind()) {
					case Binding.ARRAY_TYPE :
						substitutedVariable.superclass = subst.environment().getResolvedType(TypeConstants.JAVA_LANG_OBJECT, null);
		...
		if (substitution != null) {
			...
			this.constraints = Scope.substitute(substitution, this.constraints);
		...
		if (length == 0) {
			...
			for (int i = 0; i < length; i++) { // copy original type variable to relocate
				...
				substitutedVariables[i] = new TypeVariableBinding(
		                ...
		                alternateParamaterNames == null ?
		                                originalVariable.boundKind :
		                                TypeParameter.EXTENDS_BOUND,								
							this,
	...
	public ParameterizedMethodBinding(int modifiers, 
	                                  char[] selector, 
	...
	public static ParameterizedMethodBinding instantiateGetClass(TypeBinding receiverType, MethodBinding originalMethod, Scope scope) {
		...
		TypeBinding rawType = environment.convertToRawType(receiverType.erasure(scope), false /*do not force conversion of enclosing types*/);
