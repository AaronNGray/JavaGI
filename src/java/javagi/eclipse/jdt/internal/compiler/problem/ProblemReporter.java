...
import javagi.eclipse.jdt.core.compiler.IProblem;
...
public void abstractMethodMustBeImplemented(SourceTypeBinding type, MethodBinding abstractMethod) {
    ...
    ConstraintBinding[] cs = abstractMethod.constraints();
    if (cs.length == 0) {
        constraintString.append(" (no constraints)");
    } else {
        constraintString.append(" (constraints: ");
        for (int i = 0; i < cs.length; i++) {
            constraintString.append(cs[i].debugName());
            if (i != cs.length - 1) constraintString.append(", ");
        }
	...
	if (type.isEnum() && type.isLocalType()) {
		...
		this.handle(
			...
			new String[] { 
			        ...
			        typesAsString(abstractMethod.isVarargs(), abstractMethod.parameters, false) + constraintString, 
			        new String(decl.name), 
			},
			new String[] { 
			        new String(abstractMethod.selector),
			        typesAsString(abstractMethod.isVarargs(), abstractMethod.parameters, true) + constraintString, 
			        new String(decl.name), 
			},
			decl.sourceStart(),
			decl.sourceEnd());
...
public void disallowedTargetForAnnotation(Annotation annotation) {
	this.handle(
		...
		new String[] {new String(annotation.getResolvedType().readableName())},
		new String[] {new String(annotation.getResolvedType().shortReadableName())},
...
public void duplicateMethodInType(SourceTypeBinding type, AbstractMethodDeclaration methodDecl, boolean equalParameters) {
    ...
    if (equalParameters) {
        ...
        for (int i = 0; i < length; i++)  {
            erasures[i] = method.parameters[i].erasure(methodDecl.scope);
        }
		this.handle(
...
public void finalVariableBound(TypeVariableBinding typeVariable, TypeReference typeRef) {
	...
	this.handle(
		...
		new String[] { new String(typeVariable.sourceName), new String(typeRef.getResolvedType().readableName())},
		new String[] { new String(typeVariable.sourceName), new String(typeRef.getResolvedType().shortReadableName())},
		severity,
		typeRef.sourceStart,
		typeRef.sourceEnd);
}
...
public void illegalInstanceOfGenericType(TypeBinding checkedType, ASTNode location) {
	...
	if (checkedType.isTypeVariable()) {
		...
		IProblem.IllegalInstanceofTypeParameter,
			new String[] { new String(checkedType.readableName()), new String(checkedType.erasure(env).readableName())},
			new String[] { new String(checkedType.shortReadableName()), new String(checkedType.erasure(env).shortReadableName())},
			location.sourceStart,
			location.sourceEnd);
		return;
	}
	this.handle(
		...
		new String[] { new String(checkedType.readableName()), new String(checkedType.erasure(env).sourceName())},
		new String[] { new String(checkedType.shortReadableName()), new String(checkedType.erasure(env).sourceName())},
		location.sourceStart,
		location.sourceEnd);
}
...
// SW:
public void illegalModifierForReceiverMethod(AbstractMethodDeclaration methodDecl) {
    this.handle(
        IProblem.IllegalModifierForReceiverMethod,
        new String[] {
            new String(methodDecl.binding.declaringClass.readableName()),
            new String(methodDecl.selector),
            typesAsString(methodDecl.binding.isVarargs(), methodDecl.binding.parameters, false),
...
public void invalidMethod(MessageSend messageSend, MethodBinding method) {
	...
	// support for JavaGI
	if (method.isJavaGIProblem()) {
	    ProblemMethodBinding pmb = (ProblemMethodBinding) method;
	    javaGIProblem((InvocationSite) messageSend, pmb.getJavaGIMessage(), pmb.getJavaGIMessageArguments());
	    return;
	}
...
public void javadocDuplicatedThrowsClassName(TypeReference typeReference, int modifiers) {
	...
	if (severity == ProblemSeverities.Ignore) return;
	if (javadocVisibility(this.options.reportInvalidJavadocTagsVisibility, modifiers)) {
		String[] arguments = new String[] {String.valueOf(typeReference.getResolvedType().sourceName())};
		this.handle(
...
public void javadocMissingThrowsTag(TypeReference typeRef, int modifiers){
	...
	boolean report = (this.options.getSeverity(CompilerOptions.MissingJavadocTags) != ProblemSeverities.Ignore)
					&& (!overriding || this.options.reportMissingJavadocTagsOverriding);
	if (report && javadocVisibility(this.options.reportMissingJavadocTagsVisibility, modifiers)) {
		String[] arguments = new String[] { String.valueOf(typeRef.getResolvedType().sourceName()) };
		this.handle(
...
public void missingValueForAnnotationMember(Annotation annotation, char[] memberName) {
	...
	this.handle(
		...
		new String[] {new String(annotation.getResolvedType().readableName()), memberString },
		new String[] {new String(annotation.getResolvedType().shortReadableName()), memberString},
		annotation.sourceStart,
		annotation.sourceEnd);
}
...
public void noAdditionalBoundAfterTypeVariable(TypeReference boundReference) {
	this.handle(
		...
		new String[] { new String(boundReference.getResolvedType().readableName()) },
		new String[] { new String(boundReference.getResolvedType().shortReadableName()) },
...
public void rawTypeReference(ASTNode location, TypeBinding type) {
    ...
    this.handle(
		...
		new String[] {new String(type.readableName()), new String(type.erasure(env).readableName()), },
		new String[] {new String(type.shortReadableName()),new String(type.erasure(env).shortReadableName()),},
		location.sourceStart,
		nodeSourceEnd(null, location));
}
...
public void unnecessaryCast(CastExpression castExpression) {
	...
	if (severity == ProblemSeverities.Ignore) return;
	TypeBinding castedExpressionType = castExpression.expression.getResolvedType();
	this.handle(
		IProblem.UnnecessaryCast,
		new String[]{ new String(castedExpressionType.readableName()), new String(castExpression.type.getResolvedType().readableName())},
		new String[]{ new String(castedExpressionType.shortReadableName()), new String(castExpression.type.getResolvedType().shortReadableName())},
		severity,
...
public void unnecessaryInstanceof(InstanceOfExpression instanceofExpression, TypeBinding checkType) {
	...
	if (severity == ProblemSeverities.Ignore) return;
	TypeBinding expressionType = instanceofExpression.expression.getResolvedType();
	this.handle(
...
public void unsafeCast(CastExpression castExpression, Scope scope) {
	...
	if (severity == ProblemSeverities.Ignore) return;
	TypeBinding castedExpressionType = castExpression.expression.getResolvedType();
	TypeBinding castExpressionResolvedType = castExpression.getResolvedType();
	this.handle(
...
public void unsafeRawFieldAssignment(FieldBinding field, TypeBinding expressionType, ASTNode location) {
	...
	this.handle(
		...
		new String[] { 
		        new String(expressionType.readableName()), new String(field.name), new String(field.declaringClass.readableName()), new String(field.declaringClass.erasure(env).readableName()) },
		new String[] { 
		        new String(expressionType.shortReadableName()), new String(field.name), new String(field.declaringClass.shortReadableName()), new String(field.declaringClass.erasure(env).shortReadableName()) },
		severity,
		nodeSourceStart(field,location),
		nodeSourceEnd(field, location)); 
}
    ...
    if (isConstructor) {
		this.handle(
			...
			new String[] {
				new String(rawMethod.declaringClass.readableName()),
				typesAsString(rawMethod.original().isVarargs(), rawMethod.parameters, false),
				new String(rawMethod.declaringClass.erasure(env).readableName()),
			 }, 
			new String[] {
				new String(rawMethod.declaringClass.shortReadableName()),
				typesAsString(rawMethod.original().isVarargs(), rawMethod.parameters, true),
				new String(rawMethod.declaringClass.erasure(env).shortReadableName()),
			 }, 
			severity,
			location.sourceStart,
			location.sourceEnd);    
...
public void unsafeTypeConversion(Expression expression, TypeBinding expressionType, TypeBinding expectedType) {
	...
	this.handle(
		...
		new String[] { new String(expressionType.readableName()), new String(expectedType.readableName()), new String(expectedType.erasure(env).readableName()) },
		new String[] { new String(expressionType.shortReadableName()), new String(expectedType.shortReadableName()), new String(expectedType.erasure(env).shortReadableName()) },
		severity,
		expression.sourceStart,
		expression.sourceEnd);    
}
...
//////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    private Set<Triple<Integer,Integer,String>> errorsAlreadyReported = 
        new HashSet<Triple<Integer,Integer,String>>();
    
    private void javaGIProblem(ASTNode location, String errorMessage, boolean abort) {
        try {
            this.referenceContext = (ReferenceContext) location;
        } catch (ClassCastException e) {        
        }
        javaGIProblem(location == null ? 0 : location.sourceStart,
                      location == null ? 0 : location.sourceEnd,
                      errorMessage,
                      abort);
        this.referenceContext = null;
    ...
    private void javaGIProblem(InvocationSite invocationSite, String errorMessage, boolean abort) {
        if (this.referenceContext == null) {
            try {
                this.referenceContext = (ReferenceContext) invocationSite;
            } catch (ClassCastException e) {        
            }
        }
        javaGIProblem(invocationSite == null ? 0 : invocationSite.sourceStart(),
    ...
    private void javaGIProblem(int sourceStart, int sourceEnd, String errorMessage, boolean abort) {
        Triple<Integer,Integer,String> triple = new Triple<Integer,Integer,String>(sourceStart, sourceEnd, errorMessage);
        if (errorsAlreadyReported.contains(triple)) {
            GILog.jdebug("error '%s' already reported", errorMessage);
            return;
        }
        errorsAlreadyReported.add(triple);
        GILog.error(errorMessage);
        String[] arguments = new String[] {errorMessage};
        this.handle(
                    IProblem.Unclassified,
                    arguments,
                    arguments,
                    abort ? (ProblemSeverities.Error | ProblemSeverities.Fatal | ProblemSeverities.Abort)
                    : (ProblemSeverities.Error | ProblemSeverities.Fatal),
                    sourceStart,
                    sourceEnd);
    }

    public void javaGIProblem(String errorMessage, Object... args) {
        javaGIProblem((ASTNode) null, errorMessage, args);
    }
    
    public void javaGIFatalProblem(String errorMessage, Object... args) {
        javaGIFatalProblem((ASTNode) null, errorMessage, args);
    }
    
    public void javaGIProblem(ASTNode location, String errorMessage, Object... args) {
        javaGIProblem(location, String.format(errorMessage, args), false);
    }

    public void javaGIFatalProblem(ASTNode location, String errorMessage, Object... args) {
        javaGIProblem(location, String.format(errorMessage, args), true);
    }
    
    public void javaGIProblem(InvocationSite invocationSite, String errorMessage, Object... args) {
        javaGIProblem(invocationSite, String.format(errorMessage, args), false);
    }

    public void javaGIFatalProblem(InvocationSite invocationSite, String errorMessage, Object... args) {
        javaGIProblem(invocationSite, String.format(errorMessage, args), true);
    }
}
