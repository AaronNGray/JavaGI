...
import javagi.eclipse.jdt.internal.compiler.classfmt.ClassFileReader;
...
import javagi.eclipse.jdt.internal.compiler.parser.ScannerHelper;
...
public NameEnvironmentAnswer findClass(char[] typeName, String qualifiedPackageName, String qualifiedBinaryFileName, boolean asBinaryOnly) {
	...
	boolean sourceExists = ((this.mode & SOURCE) != 0) && 
	                        (doesFileExist(fileName + SUFFIX_STRING_java, qualifiedPackageName) || doesFileExist(fileName + SUFFIX_STRING_javagi, qualifiedPackageName));
	if (sourceExists && !asBinaryOnly) {
		String fullSourcePath;
		if (doesFileExist(fileName + SUFFIX_STRING_java, qualifiedPackageName)) 
		    fullSourcePath = this.path + qualifiedBinaryFileName.substring(0, qualifiedBinaryFileName.length() - 6)  + SUFFIX_STRING_java;
		else
		    fullSourcePath = this.path + qualifiedBinaryFileName.substring(0, qualifiedBinaryFileName.length() - 6)  + SUFFIX_STRING_javagi;
		if (!binaryExists)
...
public char[][][] findTypeNames(String qualifiedPackageName) {
	...
	String[] listFiles = dir.list(new FilenameFilter() {
		public boolean accept(File directory, String name) {
			...
			return fileName.endsWith(".class") || fileName.endsWith(".java") || fileName.endsWith(".javagi"); //$NON-NLS-1$ //$NON-NLS-2$
		}
	});
	int length;
