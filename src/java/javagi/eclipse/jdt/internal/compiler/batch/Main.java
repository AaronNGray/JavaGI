...
import javagi.eclipse.jdt.core.compiler.CompilationProgress;
import javagi.eclipse.jdt.core.compiler.IProblem;
import javagi.eclipse.jdt.core.compiler.InvalidInputException;
import javagi.eclipse.jdt.core.compiler.batch.BatchCompiler;
import javagi.eclipse.jdt.internal.compiler.AbstractAnnotationProcessorManager;
...
import javagi.eclipse.jdt.internal.compiler.ICompilerRequestor;
...
import javagi.eclipse.jdt.internal.compiler.batch.FileSystem.Classpath;
...
import javagi.eclipse.jdt.internal.compiler.env.AccessRule;
...
import javagi.eclipse.jdt.internal.compiler.impl.CompilerStats;
...
import javagi.eclipse.jdt.internal.compiler.problem.DefaultProblemFactory;
...
import javagi.eclipse.jdt.internal.compiler.util.GenericXMLWriter;
import javagi.eclipse.jdt.internal.compiler.util.HashtableOfInt;
...
public class Main implements ProblemSeverities, SuffixConstants {
	public static class Logger {
		...
		private void logProblem(CategorizedProblem problem, int localErrorCount,
			...
			} else if ((this.tagBits & Logger.RAW) != 0) {
				String result = problem.getMessage();
				this.printlnErr(result);
			} else {
				if (localErrorCount == 0) {
					this.printlnErr("----------"); //$NON-NLS-1$
		...
		public void logVersion(final boolean printToOut) {
			if (this.log != null && (this.tagBits & Logger.XML) == 0) {
				final String version = Version.versionString + "\nBased on the " + this.main.bind("misc.version", //$NON-NLS-1$
					new String[] {
	...
	public String stopAfterPhase = null;
	private String[] expandedCommandLine;

	private PrintWriter err;

	ArrayList extraProblems;
	public final static String bundleName = "javagi.eclipse.jdt.internal.compiler.batch.messages"; //$NON-NLS-1$
 ...
 * <ul>
 * <li><code>javagi.eclipse.jdt.internal.compiler.classfmt.ClassFileConstants.JDK1_1</code></li>
 * <li><code>javagi.eclipse.jdt.internal.compiler.classfmt.ClassFileConstants.JDK1_2</code></li>
 * <li><code>javagi.eclipse.jdt.internal.compiler.classfmt.ClassFileConstants.JDK1_3</code></li>
 * <li><code>javagi.eclipse.jdt.internal.compiler.classfmt.ClassFileConstants.JDK1_4</code></li>
 * <li><code>javagi.eclipse.jdt.internal.compiler.classfmt.ClassFileConstants.JDK1_5</code></li>
 * <li><code>javagi.eclipse.jdt.internal.compiler.classfmt.ClassFileConstants.JDK1_6</code></li>
 * <li><code>javagi.eclipse.jdt.internal.compiler.classfmt.ClassFileConstants.JDK1_7</code></li>
 * </ul>
...
public void configure(String[] argv) throws InvalidInputException {
	...
	final int INSIDE_GI_LOG = 21;
	final int INSIDE_STOP_AFTER = 100;
		...
		switch(mode) {
			case DEFAULT :
				...
				if (currentArg.endsWith(SuffixConstants.SUFFIX_STRING_java) || 
				    currentArg.endsWith(SuffixConstants.SUFFIX_STRING_javagi)) {
				...
				if (currentArg.equals("-1.3")) { //$NON-NLS-1$
					...
					System.err.println("ignoring option " + currentArg);
				...
				if (currentArg.equals("-Xraw")) {
				    mode = DEFAULT;
				    this.logger.setRaw();
				    continue;
				}
				if (currentArg.equals("-Xlog")) {
				    mode = INSIDE_GI_LOG;
				    continue;
				}
                if (currentArg.equals("-XdumpAST")) { //$NON-NLS-1$
                    this.dumpAST = true;
                    mode = DEFAULT;
                    continue;
                }
                if (currentArg.equals("-XstopAfter")) {
                    mode = INSIDE_STOP_AFTER;
                    continue;
                }
			...
			case INSIDE_SOURCE :
				...
				*/
				System.err.println("ignoring option -source " + currentArg);
				mode = DEFAULT;
				continue;
			...
			case INSIDE_CLASS_NAMES :
				...
				continue;
			case INSIDE_GI_LOG :
			    javagi.compiler.GILog.init(currentArg);
			    mode = DEFAULT;
			    continue;
			case INSIDE_STOP_AFTER :
			    boolean found = false;
			    for (String s : CompilerOptions.VALID_PHASES) {
			        if (s.equals(currentArg)) {
			            found = true;
			            this.stopAfterPhase = s;
			            break;
			        }
			    }
                mode = DEFAULT;
			    if (!found) {
			        throw new InvalidInputException("Invalid compiler phase: " + currentArg +
			                ". Valid phases: " + 
			                java.util.Arrays.toString(CompilerOptions.VALID_PHASES));
			    }
			    continue;
		}
...
protected void initializeAnnotationProcessorManager() {
	try {
		Class c = Class.forName("javagi.eclipse.jdt.internal.compiler.apt.dispatch.BatchAnnotationProcessorManager"); //$NON-NLS-1$
		...
		// should not happen
		throw new javagi.eclipse.jdt.internal.compiler.problem.AbortCompilation();
	} catch (IllegalAccessException e) {
		// should not happen
		throw new javagi.eclipse.jdt.internal.compiler.problem.AbortCompilation();
		...
		if (compilationUnit.destinationPath == null) {
			if (this.destinationPath == null) {
				currentDestinationPath =
					Util.extractDestinationPathFromSourceFile(unitResult);
...
public void performCompilation() throws InvalidInputException {
	...
	this.batchCompiler =
		new Compiler(
			...
			this.out,
			this.progress);
	this.batchCompiler.destinationPath = this.destinationPath;
	...
	this.compilerOptions.dumpAST = this.dumpAST;
	this.compilerOptions.stopAfterPhase = this.stopAfterPhase;
