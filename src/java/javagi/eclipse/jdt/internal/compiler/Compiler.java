...
import javagi.eclipse.jdt.internal.compiler.env.*;
...
import javagi.eclipse.jdt.internal.compiler.util.*;

import java.io.*;
import java.util.*;

import javagi.compiler.GILog;
import javagi.compiler.GIUnsupportedFeature;
import javagi.compiler.ImplementationManager;
import javagi.compiler.ImplementationSerializer;
...
public class Compiler implements ITypeRequestor, ProblemSeverities {
	...
	// SW: changed to final, global variables used for JavaGI may cause problems otherwise
	public final boolean useSingleThread = true; // by default the compiler will not use worker threads to read/process/write
	 ...
	 *
	 *  @param environment javagi.eclipse.jdt.internal.compiler.api.env.INameEnvironment
	 ...
	 *            rules.
	 *
	 *  @param policy javagi.eclipse.jdt.internal.compiler.api.problem.IErrorHandlingPolicy
	 *      Configurable part for problem handling, allowing the compiler client to
	 *      specify the rules for handling problems (stop on first error or accumulate
	 *      them all) and at the same time perform some actions such as opening a dialog
	 *      in UI when compiling interactively.
	 *      @see javagi.eclipse.jdt.internal.compiler.DefaultErrorHandlingPolicies
	 *  
	 *  @param settings java.util.Map
	 *      The settings that control the compiler behavior.
	 *      
	 *  @param requestor javagi.eclipse.jdt.internal.compiler.api.ICompilerRequestor
	 *      Component which will receive and persist all compilation results and is intended
	 *      to consume them as they are produced. Typically, in a batch compiler, it is 
	 *      responsible for writing out the actual .class files to the file system.
	 *      @see javagi.eclipse.jdt.internal.compiler.CompilationResult
	 *
	 *  @param problemFactory javagi.eclipse.jdt.internal.compiler.api.problem.IProblemFactory
	 ...
	 *      @see javagi.eclipse.jdt.internal.compiler.DefaultErrorHandlingPolicies
	 *      
	 *  @param options javagi.eclipse.jdt.internal.compiler.impl.CompilerOptions
...
//		    System.out.println();
	...
	public void compile(ICompilationUnit[] sourceUnits) {
		...
		try {
            ...
            this.stats.parseTime += parseEnd - parseStart;
		        
            if (CompilerOptions.PARSE_PHASE.equals(this.options.stopAfterPhase)) {
                javagi.compiler.GILog.jinfo("Stopping after parsing as requested");
                return;
		    }
            ...
            for (int i = 0; i < this.totalUnits; i++) {
                unit = unitsToProcess[i];
                javagi.compiler.ImplementationManager.addCompilationUnit(unit.scope);
            }
			...
			ImplementationManager.checkRestrictions(lookupEnvironment);
			ImplementationSerializer serializer = new ImplementationSerializer();
			for (int i = 0; i < this.totalUnits; i++) {
				unit = unitsToProcess[i];
				...
				try {
					...
					generateCode(unit, i, serializer);
				} finally {
		    ...
		    // System.err.println("handling abort compilation " + e);
                    this.handleInternalException(e, unit);
		} catch (Error e) {
                    this.handleInternalException(e, unit, null);
                    System.err.println("re-throwing " + e);
                    throw e; // rethrow
	...
	protected void handleInternalException(
		...
		if (result != null) {
			...
			String[] pbArguments;
			if (internalException instanceof GIUnsupportedFeature) {
			    pbArguments = new String[]{internalException.getMessage()};
			} else {
			    pbArguments  = new String[] {
	...
	protected void handleInternalException(
		...
		if (abortException.isSilent) {
			...
			//System.err.println("re-throwing silent exception: " + abortException.silentException);
	...
	public void process(CompilationUnitDeclaration unit, int i) {
	    GILog.jinfo("Processing compilation unit " + new String(unit.getFileName()));
		this.lookupEnvironment.unitBeingCompleted = unit;
        ImplementationManager.setCurrentCompilationUnitScope(unit.scope);
		...
		// type checking
		unit.resolve();
		long afterTycheck = System.currentTimeMillis();
		...
		GILog.jinfo("Finished processing compilation unit " + new String(unit.getFileName()) +
		            ": tycheck= " + (afterTycheck - resolveStart) +  ", analyse=" + 
		            (generateStart - afterTycheck) +
		            ", total=" + (generateStart - resolveStart));
	}
	
    public void generateCode(CompilationUnitDeclaration unit, int i, ImplementationSerializer serializer) {
        GILog.jinfo("Generating code for compilation unit " + new String(unit.getFileName()));
        // code generation
		long generateStart = System.currentTimeMillis();
		unit.generateCode();
		Translation.generateCode(unit, this.destinationPath);
		if (!unit.hasErrors()) {
		    serializer.writeToDisk(unit, this.destinationPath);
		}
		...
		this.stats.generateTime += t - generateStart;
		...
		GILog.jinfo("Finished generating code for compilation unit " + new String(unit.getFileName()) +
                " in " + (t - generateStart) + " ms");
	}
	...
	private void initForJavaGI() {
        javagi.compiler.ImplementationManager.init();
        CaptureBinding.resetCaptureID();
        Naming.init();
        javagi.compiler.TypePool.init();
	}
}
