...
import javagi.compiler.Constants;
import javagi.compiler.Naming;
...
import javagi.eclipse.jdt.internal.compiler.ast.*;
...
import javagi.eclipse.jdt.internal.compiler.parser.diagnose.DiagnoseParser;
import javagi.eclipse.jdt.internal.compiler.problem.AbortCompilation;
import javagi.eclipse.jdt.internal.compiler.problem.AbortCompilationUnit;
...
import javagi.eclipse.jdt.internal.compiler.util.Messages;
...
public class Parser implements  ParserBasicInformation, TerminalTokens, OperatorIds, TypeIds {
	...
	private static final boolean DEBUG_STACK = javagi.compiler.GILog.Parsing().isFinest();
	...
	private final static String READABLE_NAMES_FILE_NAME =
		"javagi.eclipse.jdt.internal.compiler.parser." + READABLE_NAMES_FILE; //$NON-NLS-1$
	...
	public javagi.eclipse.jdt.internal.compiler.ReadManager readManager;
...
public final static void buildFilesFromLPG(String dataFilename, String dataFilename2) {
	...
	//[javagi.eclipse.jdt.internal.compiler.parser.Parser.buildFilesFromLPG("d:/leapfrog/grammar/javadcl.java")]
...
protected void concatGenericsLists() {
	...
	if (DEBUG_STACK) System.out.println("concatenating generics stack: top of genericsLengthStack = " +
	                                    this.genericsLengthStack[this.genericsLengthPtr]);
}
protected void concatNodeLists() {
	...
	if (DEBUG_STACK) {
      System.out.println("concatenating ast stack: top of astLengthStack = " +
                         this.astLengthStack[this.astLengthPtr]);
      int i = this.astPtr;
      while (i >= 0) {
        System.out.println("astStack[" + i + "] = " + this.astStack[i]);
        i--;
      }
      i = this.astLengthPtr;
      while (i >= 0) {
        System.out.println("astLengthStack[" + i + "] = " + this.astLengthStack[i]);
        i--;
      }
    }
}
...
protected void consumeArrayAccess(boolean unspecifiedReference) {
    ...
    System.arraycopy(this.expressionStack, this.expressionPtr + 1, args , 0, length);
    Expression recv;
    if (unspecifiedReference) {
        recv = getUnspecifiedReference();
    } else {
        recv = this.expressionStack[this.expressionPtr--];
        this.expressionLengthPtr--;
    }
    
    if (args.length == 0) {
        this.problemReporter().javaGIProblem(recv, "array access expression with empty index list");
    }

    Expression pos = args[0];
    Expression[] extraPos = new Expression[args.length - 1];
    System.arraycopy(args, 1, extraPos, 0, extraPos.length);
	Expression exp = new ArrayReference(recv, pos, extraPos);
...
protected void consumeInterfaceHeader() {
	...
	// SW: check for missing implementation parameter
	if (typeDecl.implParameters == null) {
	    TypeParameter This = new TypeParameter();
	    This.name = new char[]{'T', 'h', 'i', 's'};
	    typeDecl.implParameters = new TypeParameter[]{This};
	}
	typeDecl.fixSuperInterfaces();
}
...
protected void consumeInternalCompilationUnitWithTypes() {
    ...
    javagi.compiler.GILog.Parsing().jfine("consumeInternalCompilationUnitWithTypes: length=" + length);
    javagi.compiler.GILog.Parsing().jdebug("finished parsing of compilation unit: %s", this.compilationUnit);
    // javagi.compiler.GILog.printStackTrace();
}
...
protected void consumeMethodInvocationName() {
    ...
    m.owningInterface = (NameReference) this.astStack[this.astPtr--];
    this.astLengthPtr--;
...
protected void consumePrimaryNoNewArrayImplementationThis() {
    this.intPtr--; // remove the implementation start position
    pushOnExpressionStack(
            new QualifiedThisReference(
                this.intStack[this.intPtr--],
                this.endPosition));
...
//*BEGIN_INPUT* JavaAction.java
// This method is part of an automatic generation : do NOT edit-modify  
protected void consumeRule(int act) {
  switch ( act ) {
    case 30 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Type ::= PrimitiveType"); }  //$NON-NLS-1$
		    consumePrimitiveType();  
			break;
 
    case 44 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ReferenceType ::= ClassOrInterfaceType"); }  //$NON-NLS-1$
		    consumeReferenceType();   
			break;
 
    case 48 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassOrInterface ::= Name"); }  //$NON-NLS-1$
		    consumeClassOrInterfaceName();   
			break;
 
    case 49 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassOrInterface ::= GenericType DOT Name"); }  //$NON-NLS-1$
		    consumeClassOrInterface();   
			break;
 
    case 50 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: GenericType ::= ClassOrInterface TypeArguments"); }  //$NON-NLS-1$
		    consumeGenericType();   
			break;
 
    case 51 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArrayTypeWithTypeArgumentsName ::= GenericType DOT Name"); }  //$NON-NLS-1$
		    consumeArrayTypeWithTypeArgumentsName();   
			break;
 
    case 52 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArrayType ::= PrimitiveType Dims"); }  //$NON-NLS-1$
		    consumePrimitiveArrayType();   
			break;
 
    case 53 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArrayType ::= Name Dims"); }  //$NON-NLS-1$
		    consumeNameArrayType();   
			break;
 
    case 54 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArrayType ::= ArrayTypeWithTypeArgumentsName Dims"); }  //$NON-NLS-1$
		    consumeGenericTypeNameArrayType();   
			break;
 
    case 55 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArrayType ::= GenericType Dims"); }  //$NON-NLS-1$
		    consumeGenericTypeArrayType();   
			break;
 
    case 60 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: QualifiedName ::= Name DOT SimpleName"); }  //$NON-NLS-1$
		    consumeQualifiedName();  
			break;
 
    case 61 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: CompilationUnit ::= EnterCompilationUnit InternalCompilationUnit"); }  //$NON-NLS-1$
		    consumeCompilationUnit();  
			break;
 
    case 62 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InternalCompilationUnit ::= PackageDeclaration"); }  //$NON-NLS-1$
		    consumeInternalCompilationUnit();  
			break;
 
    case 63 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InternalCompilationUnit ::= PackageDeclaration ImportDeclarations ReduceImports"); }  //$NON-NLS-1$
		    consumeInternalCompilationUnit();  
			break;
 
    case 64 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InternalCompilationUnit ::= PackageDeclaration ImportDeclarations ReduceImports TypeDeclarations"); }  //$NON-NLS-1$
		    consumeInternalCompilationUnitWithTypes();  
			break;
 
    case 65 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InternalCompilationUnit ::= PackageDeclaration TypeDeclarations"); }  //$NON-NLS-1$
		    consumeInternalCompilationUnitWithTypes();  
			break;
 
    case 66 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InternalCompilationUnit ::= ImportDeclarations ReduceImports"); }  //$NON-NLS-1$
		    consumeInternalCompilationUnit();  
			break;
 
    case 67 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InternalCompilationUnit ::= TypeDeclarations"); }  //$NON-NLS-1$
		    consumeInternalCompilationUnitWithTypes();  
			break;
 
    case 68 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InternalCompilationUnit ::= ImportDeclarations ReduceImports TypeDeclarations"); }  //$NON-NLS-1$
		    consumeInternalCompilationUnitWithTypes();  
			break;
 
    case 69 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InternalCompilationUnit ::="); }  //$NON-NLS-1$
		    consumeEmptyInternalCompilationUnit();  
			break;
 
    case 70 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ReduceImports ::="); }  //$NON-NLS-1$
		    consumeReduceImports();  
			break;
 
    case 71 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnterCompilationUnit ::="); }  //$NON-NLS-1$
		    consumeEnterCompilationUnit();  
			break;
 
    case 87 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: CatchHeader ::= catch LPAREN FormalParameter RPAREN LBRACE"); }  //$NON-NLS-1$
		    consumeCatchHeader();  
			break;
 
    case 89 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImportDeclarations ::= ImportDeclarations ImportDeclaration"); }  //$NON-NLS-1$
		    consumeImportDeclarations();  
			break;
 
    case 91 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeDeclarations ::= TypeDeclarations TypeDeclaration"); }  //$NON-NLS-1$
		    consumeTypeDeclarations();  
			break;
 
    case 92 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PackageDeclaration ::= PackageDeclarationName SEMICOLON"); }  //$NON-NLS-1$
		     consumePackageDeclaration();  
			break;
 
    case 93 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PackageDeclarationName ::= Modifiers package PushRealModifiers Name"); }  //$NON-NLS-1$
		     consumePackageDeclarationNameWithModifiers();  
			break;
 
    case 94 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PackageDeclarationName ::= PackageComment package Name"); }  //$NON-NLS-1$
		     consumePackageDeclarationName();  
			break;
 
    case 95 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PackageComment ::="); }  //$NON-NLS-1$
		     consumePackageComment();  
			break;
 
    case 100 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: SingleTypeImportDeclaration ::= SingleTypeImportDeclarationName SEMICOLON"); }  //$NON-NLS-1$
		    consumeImportDeclaration();  
			break;
 
    case 101 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: SingleTypeImportDeclarationName ::= import Name"); }  //$NON-NLS-1$
		    consumeSingleTypeImportDeclarationName();  
			break;
 
    case 102 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeImportOnDemandDeclaration ::= TypeImportOnDemandDeclarationName SEMICOLON"); }  //$NON-NLS-1$
		    consumeImportDeclaration();  
			break;
 
    case 103 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeImportOnDemandDeclarationName ::= import Name DOT MULTIPLY"); }  //$NON-NLS-1$
		    consumeTypeImportOnDemandDeclarationName();  
			break;
 
     case 107 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeDeclaration ::= SEMICOLON"); }  //$NON-NLS-1$
		    consumeEmptyTypeDeclaration();  
			break;
 
    case 111 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Modifiers ::= Modifiers Modifier"); }  //$NON-NLS-1$
		    consumeModifiers2();  
			break;
 
    case 123 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Modifier ::= Annotation"); }  //$NON-NLS-1$
		    consumeAnnotationAsModifier();  
			break;
 
    case 124 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassDeclaration ::= ClassHeader ClassBody"); }  //$NON-NLS-1$
		    consumeClassDeclaration();  
			break;
 
    case 125 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassHeader ::= ClassHeaderName ClassHeaderExtendsopt ClassHeaderImplementsopt..."); }  //$NON-NLS-1$
		    consumeClassHeader();  
			break;
 
    case 126 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassHeaderName ::= ClassHeaderName1 TypeParameters"); }  //$NON-NLS-1$
		    consumeTypeHeaderNameWithTypeParameters();  
			break;
 
    case 128 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassHeaderName1 ::= Modifiersopt class Identifier"); }  //$NON-NLS-1$
		    consumeClassHeaderName1();  
			break;
 
    case 129 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassHeaderExtends ::= extends ClassType"); }  //$NON-NLS-1$
		    consumeClassHeaderExtends();  
			break;
 
    case 130 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassHeaderImplements ::= implements InterfaceTypeList"); }  //$NON-NLS-1$
		    consumeClassHeaderImplements();  
			break;
 
    case 131 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassHeaderConstraints ::= ConstraintClause"); }  //$NON-NLS-1$
		    consumeClassHeaderConstraints();  
			break;
 
    case 133 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceTypeList ::= InterfaceTypeList COMMA InterfaceType"); }  //$NON-NLS-1$
		    consumeInterfaceTypeList();  
			break;
 
    case 134 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceType ::= ClassOrInterfaceType"); }  //$NON-NLS-1$
		    consumeInterfaceType();  
			break;
 
    case 137 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassBodyDeclarations ::= ClassBodyDeclarations ClassBodyDeclaration"); }  //$NON-NLS-1$
		    consumeClassBodyDeclarations();  
			break;
 
    case 141 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassBodyDeclaration ::= Diet NestedMethod CreateInitializer Block"); }  //$NON-NLS-1$
		    consumeClassBodyDeclaration();  
			break;
 
    case 142 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Diet ::="); }  //$NON-NLS-1$
		    consumeDiet();  
			break;

    case 143 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Initializer ::= Diet NestedMethod CreateInitializer Block"); }  //$NON-NLS-1$
		    consumeClassBodyDeclaration();  
			break;
 
    case 144 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: CreateInitializer ::="); }  //$NON-NLS-1$
		    consumeCreateInitializer();  
			break;

    case 151 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassMemberDeclaration ::= SEMICOLON"); }  //$NON-NLS-1$
		    consumeEmptyTypeDeclaration();  
			break;

    case 154 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: FieldDeclaration ::= Modifiersopt Type VariableDeclarators SEMICOLON"); }  //$NON-NLS-1$
		    consumeFieldDeclaration();  
			break;
 
    case 156 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: VariableDeclarators ::= VariableDeclarators COMMA VariableDeclarator"); }  //$NON-NLS-1$
		    consumeVariableDeclarators();  
			break;
 
    case 159 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnterVariable ::="); }  //$NON-NLS-1$
		    consumeEnterVariable();  
			break;
 
    case 160 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExitVariableWithInitialization ::="); }  //$NON-NLS-1$
		    consumeExitVariableWithInitialization();  
			break;
 
    case 161 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExitVariableWithoutInitialization ::="); }  //$NON-NLS-1$
		    consumeExitVariableWithoutInitialization();  
			break;
 
    case 162 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ForceNoDiet ::="); }  //$NON-NLS-1$
		    consumeForceNoDiet();  
			break;
 
    case 163 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: RestoreDiet ::="); }  //$NON-NLS-1$
		    consumeRestoreDiet();  
			break;
 
    case 168 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MethodDeclaration ::= MethodHeader MethodBody"); }  //$NON-NLS-1$
		    // set to true to consume a method with a body
  consumeMethodDeclaration(true);   
			break;
 
    case 169 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AbstractMethodDeclaration ::= MethodHeader SEMICOLON"); }  //$NON-NLS-1$
		    // set to false to consume a method without body
  consumeMethodDeclaration(false);  
			break;
 
    case 170 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MethodHeader ::= MethodHeaderName FormalParameterListopt MethodHeaderRightParen..."); }  //$NON-NLS-1$
		    consumeMethodHeader();  
			break;
 
    case 171 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MethodHeaderName ::= Modifiersopt TypeParameters Type Identifier LPAREN"); }  //$NON-NLS-1$
		    consumeMethodHeaderNameWithTypeParameters(false);  
			break;
 
    case 172 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MethodHeaderName ::= Modifiersopt Type Identifier LPAREN"); }  //$NON-NLS-1$
		    consumeMethodHeaderName(false);  
			break;
 
    case 173 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MethodHeaderRightParen ::= RPAREN"); }  //$NON-NLS-1$
		    consumeMethodHeaderRightParen();  
			break;
 
    case 174 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MethodHeaderExtendedDims ::= Dimsopt"); }  //$NON-NLS-1$
		    consumeMethodHeaderExtendedDims();  
			break;
 
    case 175 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MethodHeaderThrowsClause ::= throws ClassTypeList"); }  //$NON-NLS-1$
		    consumeMethodHeaderThrowsClause();  
			break;
 
    case 176 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MethodHeaderConstraints ::= ConstraintClause"); }  //$NON-NLS-1$
		    consumeMethodHeaderConstraints();  
			break;
 
    case 177 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ConstructorHeader ::= ConstructorHeaderName FormalParameterListopt MethodHeaderRightParen..."); }  //$NON-NLS-1$
		    consumeConstructorHeader();  
			break;
 
    case 178 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ConstructorHeaderName ::= Modifiersopt TypeParameters Identifier LPAREN"); }  //$NON-NLS-1$
		    consumeConstructorHeaderNameWithTypeParameters();  
			break;
 
    case 179 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ConstructorHeaderName ::= Modifiersopt Identifier LPAREN"); }  //$NON-NLS-1$
		    consumeConstructorHeaderName();  
			break;
 
    case 181 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: FormalParameterList ::= FormalParameterList COMMA FormalParameter"); }  //$NON-NLS-1$
		    consumeFormalParameterList();  
			break;
 
    case 182 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: FormalParameter ::= Modifiersopt Type VariableDeclaratorId"); }  //$NON-NLS-1$
		    consumeFormalParameter(false);  
			break;
 
    case 183 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: FormalParameter ::= Modifiersopt Type ELLIPSIS VariableDeclaratorId"); }  //$NON-NLS-1$
		    consumeFormalParameter(true);  
			break;
 
    case 185 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassTypeList ::= ClassTypeList COMMA ClassTypeElt"); }  //$NON-NLS-1$
		    consumeClassTypeList();  
			break;
 
    case 186 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassTypeElt ::= ClassType"); }  //$NON-NLS-1$
		    consumeClassTypeElt();  
			break;
 
    case 187 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MethodBody ::= NestedMethod LBRACE BlockStatementsopt RBRACE"); }  //$NON-NLS-1$
		    consumeMethodBody();  
			break;
 
    case 188 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: NestedMethod ::="); }  //$NON-NLS-1$
		    consumeNestedMethod();  
			break;
 
    case 189 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: StaticInitializer ::= StaticOnly Block"); }  //$NON-NLS-1$
		    consumeStaticInitializer();  
			break;

    case 190 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: StaticOnly ::= static"); }  //$NON-NLS-1$
		    consumeStaticOnly();  
			break;
 
    case 191 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ConstructorDeclaration ::= ConstructorHeader MethodBody"); }  //$NON-NLS-1$
		    consumeConstructorDeclaration() ;  
			break;
 
    case 192 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ConstructorDeclaration ::= ConstructorHeader SEMICOLON"); }  //$NON-NLS-1$
		    consumeInvalidConstructorDeclaration() ;  
			break;
 
    case 193 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExplicitConstructorInvocation ::= this LPAREN ArgumentListopt RPAREN SEMICOLON"); }  //$NON-NLS-1$
		    consumeExplicitConstructorInvocation(0, THIS_CALL);  
			break;
 
    case 194 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExplicitConstructorInvocation ::= OnlyTypeArguments this LPAREN ArgumentListopt RPAREN SEMICOLON"); }  //$NON-NLS-1$
		    consumeExplicitConstructorInvocationWithTypeArguments(0,THIS_CALL);  
			break;
 
    case 195 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExplicitConstructorInvocation ::= super LPAREN ArgumentListopt RPAREN SEMICOLON"); }  //$NON-NLS-1$
		    consumeExplicitConstructorInvocation(0,SUPER_CALL);  
			break;
 
    case 196 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExplicitConstructorInvocation ::= OnlyTypeArguments super LPAREN ArgumentListopt RPAREN..."); }  //$NON-NLS-1$
		    consumeExplicitConstructorInvocationWithTypeArguments(0,SUPER_CALL);  
			break;
 
    case 197 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExplicitConstructorInvocation ::= Primary DOT super LPAREN ArgumentListopt RPAREN SEMICOLON"); }  //$NON-NLS-1$
		    consumeExplicitConstructorInvocation(1, SUPER_CALL);  
			break;
 
    case 198 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExplicitConstructorInvocation ::= Primary DOT OnlyTypeArguments super LPAREN ArgumentListopt..."); }  //$NON-NLS-1$
		    consumeExplicitConstructorInvocationWithTypeArguments(1, SUPER_CALL);  
			break;
 
    case 199 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExplicitConstructorInvocation ::= Name DOT super LPAREN ArgumentListopt RPAREN SEMICOLON"); }  //$NON-NLS-1$
		    consumeExplicitConstructorInvocation(2, SUPER_CALL);  
			break;
 
    case 200 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExplicitConstructorInvocation ::= Name DOT OnlyTypeArguments super LPAREN ArgumentListopt RPAREN"); }  //$NON-NLS-1$
		    consumeExplicitConstructorInvocationWithTypeArguments(2, SUPER_CALL);  
			break;
 
    case 201 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExplicitConstructorInvocation ::= Primary DOT this LPAREN ArgumentListopt RPAREN SEMICOLON"); }  //$NON-NLS-1$
		    consumeExplicitConstructorInvocation(1, THIS_CALL);  
			break;
 
    case 202 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExplicitConstructorInvocation ::= Primary DOT OnlyTypeArguments this LPAREN ArgumentListopt..."); }  //$NON-NLS-1$
		    consumeExplicitConstructorInvocationWithTypeArguments(1, THIS_CALL);  
			break;
 
    case 203 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExplicitConstructorInvocation ::= Name DOT this LPAREN ArgumentListopt RPAREN SEMICOLON"); }  //$NON-NLS-1$
		    consumeExplicitConstructorInvocation(2, THIS_CALL);  
			break;
 
    case 204 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExplicitConstructorInvocation ::= Name DOT OnlyTypeArguments this LPAREN ArgumentListopt RPAREN"); }  //$NON-NLS-1$
		    consumeExplicitConstructorInvocationWithTypeArguments(2, THIS_CALL);  
			break;
 
    case 205 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceDeclaration ::= InterfaceHeader InterfaceBody"); }  //$NON-NLS-1$
		    consumeInterfaceDeclaration();  
			break;
 
    case 206 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceHeader ::= InterfaceHeaderName InterfaceHeaderExtendsopt InterfaceHeaderConstraintsopt"); }  //$NON-NLS-1$
		    consumeInterfaceHeader();  
			break;
 
    case 207 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceHeader ::= InterfaceHeaderName InterfaceHeaderImplClause InterfaceHeaderConstraintsopt"); }  //$NON-NLS-1$
		    consumeInterfaceHeader();  
			break;
 
    case 208 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceHeaderName ::= InterfaceHeaderName1 TypeParameters"); }  //$NON-NLS-1$
		    consumeTypeHeaderNameWithTypeParameters();  
			break;
 
    case 210 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceHeaderName1 ::= Modifiersopt interface Identifier"); }  //$NON-NLS-1$
		    consumeInterfaceHeaderName1();  
			break;
 
    case 212 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceHeaderImplParameters -> InterfaceHeaderImplParameterList"); }  //$NON-NLS-1$
		    consumeInterfaceHeaderImplParameters();  
			break;
 
    case 214 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceHeaderImplParameterList ::= InterfaceHeaderImplParameterList COMMA TypeParameterHeader"); }  //$NON-NLS-1$
		    consumeInterfaceHeaderImplParameterList();  
			break;
 
    case 215 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceHeaderImplConstraints ::= ImplConstraintClause"); }  //$NON-NLS-1$
		    consumeInterfaceHeaderImplConstraints();  
			break;
 
    case 216 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceHeaderExtends ::= extends InterfaceTypeList"); }  //$NON-NLS-1$
		    consumeInterfaceHeaderExtends();  
			break;
 
    case 217 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceHeaderConstraints ::= ConstraintClause"); }  //$NON-NLS-1$
		    consumeInterfaceHeaderConstraints();  
			break;
 
    case 220 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceMemberDeclarations ::= InterfaceMemberDeclarations InterfaceMemberDeclaration"); }  //$NON-NLS-1$
		    consumeInterfaceMemberDeclarations();  
			break;
 
    case 221 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceMemberDeclaration ::= SEMICOLON"); }  //$NON-NLS-1$
		    consumeEmptyTypeDeclaration();  
			break;
 
    case 223 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceMemberDeclaration ::= MethodHeader MethodBody"); }  //$NON-NLS-1$
		    consumeInvalidMethodDeclaration();  
			break;
 
    case 224 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InvalidConstructorDeclaration ::= ConstructorHeader MethodBody"); }  //$NON-NLS-1$
		    consumeInvalidConstructorDeclaration(true);   
			break;
 
    case 225 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InvalidConstructorDeclaration ::= ConstructorHeader SEMICOLON"); }  //$NON-NLS-1$
		    consumeInvalidConstructorDeclaration(false);   
			break;
 
    case 234 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PushLeftBrace ::="); }  //$NON-NLS-1$
		    consumePushLeftBrace();  
			break;
 
    case 235 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArrayInitializer ::= LBRACE PushLeftBrace ,opt RBRACE"); }  //$NON-NLS-1$
		    consumeEmptyArrayInitializer();  
			break;
 
    case 236 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArrayInitializer ::= LBRACE PushLeftBrace VariableInitializers RBRACE"); }  //$NON-NLS-1$
		    consumeArrayInitializer();  
			break;
 
    case 237 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArrayInitializer ::= LBRACE PushLeftBrace VariableInitializers COMMA RBRACE"); }  //$NON-NLS-1$
		    consumeArrayInitializer();  
			break;
 
    case 239 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: VariableInitializers ::= VariableInitializers COMMA VariableInitializer"); }  //$NON-NLS-1$
		    consumeVariableInitializers();  
			break;
 
    case 240 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Block ::= OpenBlock LBRACE BlockStatementsopt RBRACE"); }  //$NON-NLS-1$
		    consumeBlock();  
			break;
 
    case 241 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: OpenBlock ::="); }  //$NON-NLS-1$
		    consumeOpenBlock() ;  
			break;
 
    case 243 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: BlockStatements ::= BlockStatements BlockStatement"); }  //$NON-NLS-1$
		    consumeBlockStatements() ;  
			break;
 
    case 247 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: BlockStatement ::= InterfaceDeclaration"); }  //$NON-NLS-1$
		    consumeInvalidInterfaceDeclaration();  
			break;
 
    case 248 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: BlockStatement ::= AnnotationTypeDeclaration"); }  //$NON-NLS-1$
		    consumeInvalidAnnotationTypeDeclaration();  
			break;
 
    case 249 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: BlockStatement ::= EnumDeclaration"); }  //$NON-NLS-1$
		    consumeInvalidEnumDeclaration();  
			break;
 
    case 250 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: LocalVariableDeclarationStatement ::= LocalVariableDeclaration SEMICOLON"); }  //$NON-NLS-1$
		    consumeLocalVariableDeclarationStatement();  
			break;
 
    case 251 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: LocalVariableDeclaration ::= Type PushModifiers VariableDeclarators"); }  //$NON-NLS-1$
		    consumeLocalVariableDeclaration();  
			break;
 
    case 252 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: LocalVariableDeclaration ::= Modifiers Type PushRealModifiers VariableDeclarators"); }  //$NON-NLS-1$
		    consumeLocalVariableDeclaration();  
			break;
 
    case 253 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PushModifiers ::="); }  //$NON-NLS-1$
		    consumePushModifiers();  
			break;
 
    case 254 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PushModifiersForHeader ::="); }  //$NON-NLS-1$
		    consumePushModifiersForHeader();  
			break;
 
    case 255 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PushRealModifiers ::="); }  //$NON-NLS-1$
		    consumePushRealModifiers();  
			break;
 
    case 281 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EmptyStatement ::= SEMICOLON"); }  //$NON-NLS-1$
		    consumeEmptyStatement();  
			break;
 
    case 282 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: LabeledStatement ::= Label COLON Statement"); }  //$NON-NLS-1$
		    consumeStatementLabel() ;  
			break;
 
    case 283 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: LabeledStatementNoShortIf ::= Label COLON StatementNoShortIf"); }  //$NON-NLS-1$
		    consumeStatementLabel() ;  
			break;
 
    case 284 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Label ::= Identifier"); }  //$NON-NLS-1$
		    consumeLabel() ;  
			break;
 
     case 285 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExpressionStatement ::= StatementExpression SEMICOLON"); }  //$NON-NLS-1$
		    consumeExpressionStatement();  
			break;
 
    case 294 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: IfThenStatement ::= if LPAREN Expression RPAREN Statement"); }  //$NON-NLS-1$
		    consumeStatementIfNoElse();  
			break;
 
    case 295 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: IfThenElseStatement ::= if LPAREN Expression RPAREN StatementNoShortIf else Statement"); }  //$NON-NLS-1$
		    consumeStatementIfWithElse();  
			break;
 
    case 296 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: IfThenElseStatementNoShortIf ::= if LPAREN Expression RPAREN StatementNoShortIf else..."); }  //$NON-NLS-1$
		    consumeStatementIfWithElse();  
			break;
 
    case 297 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: SwitchStatement ::= switch LPAREN Expression RPAREN OpenBlock SwitchBlock"); }  //$NON-NLS-1$
		    consumeStatementSwitch() ;  
			break;
 
    case 298 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: SwitchBlock ::= LBRACE RBRACE"); }  //$NON-NLS-1$
		    consumeEmptySwitchBlock() ;  
			break;
 
    case 301 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: SwitchBlock ::= LBRACE SwitchBlockStatements SwitchLabels RBRACE"); }  //$NON-NLS-1$
		    consumeSwitchBlock() ;  
			break;
 
    case 303 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: SwitchBlockStatements ::= SwitchBlockStatements SwitchBlockStatement"); }  //$NON-NLS-1$
		    consumeSwitchBlockStatements() ;  
			break;
 
    case 304 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: SwitchBlockStatement ::= SwitchLabels BlockStatements"); }  //$NON-NLS-1$
		    consumeSwitchBlockStatement() ;  
			break;
 
    case 306 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: SwitchLabels ::= SwitchLabels SwitchLabel"); }  //$NON-NLS-1$
		    consumeSwitchLabels() ;  
			break;
 
     case 307 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: SwitchLabel ::= case ConstantExpression COLON"); }  //$NON-NLS-1$
		    consumeCaseLabel();  
			break;
 
     case 308 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: SwitchLabel ::= default COLON"); }  //$NON-NLS-1$
		    consumeDefaultLabel();  
			break;
 
    case 309 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: WhileStatement ::= while LPAREN Expression RPAREN Statement"); }  //$NON-NLS-1$
		    consumeStatementWhile() ;  
			break;
 
    case 310 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: WhileStatementNoShortIf ::= while LPAREN Expression RPAREN StatementNoShortIf"); }  //$NON-NLS-1$
		    consumeStatementWhile() ;  
			break;
 
    case 311 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: DoStatement ::= do Statement while LPAREN Expression RPAREN SEMICOLON"); }  //$NON-NLS-1$
		    consumeStatementDo() ;  
			break;
 
    case 312 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ForStatement ::= for LPAREN ForInitopt SEMICOLON Expressionopt SEMICOLON ForUpdateopt RPAREN..."); }  //$NON-NLS-1$
		    consumeStatementFor() ;  
			break;
 
    case 313 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ForStatementNoShortIf ::= for LPAREN ForInitopt SEMICOLON Expressionopt SEMICOLON ForUpdateopt"); }  //$NON-NLS-1$
		    consumeStatementFor() ;  
			break;
 
    case 314 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ForInit ::= StatementExpressionList"); }  //$NON-NLS-1$
		    consumeForInit() ;  
			break;
 
    case 318 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: StatementExpressionList ::= StatementExpressionList COMMA StatementExpression"); }  //$NON-NLS-1$
		    consumeStatementExpressionList() ;  
			break;
 
    case 319 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AssertStatement ::= assert Expression SEMICOLON"); }  //$NON-NLS-1$
		    consumeSimpleAssertStatement() ;  
			break;
 
    case 320 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AssertStatement ::= assert Expression COLON Expression SEMICOLON"); }  //$NON-NLS-1$
		    consumeAssertStatement() ;  
			break;
 
    case 321 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: BreakStatement ::= break SEMICOLON"); }  //$NON-NLS-1$
		    consumeStatementBreak() ;  
			break;
 
    case 322 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: BreakStatement ::= break Identifier SEMICOLON"); }  //$NON-NLS-1$
		    consumeStatementBreakWithLabel() ;  
			break;
 
    case 323 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ContinueStatement ::= continue SEMICOLON"); }  //$NON-NLS-1$
		    consumeStatementContinue() ;  
			break;
 
    case 324 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ContinueStatement ::= continue Identifier SEMICOLON"); }  //$NON-NLS-1$
		    consumeStatementContinueWithLabel() ;  
			break;
 
    case 325 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ReturnStatement ::= return Expressionopt SEMICOLON"); }  //$NON-NLS-1$
		    consumeStatementReturn() ;  
			break;
 
    case 326 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ThrowStatement ::= throw Expression SEMICOLON"); }  //$NON-NLS-1$
		    consumeStatementThrow();  
			break;
 
    case 327 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: SynchronizedStatement ::= OnlySynchronized LPAREN Expression RPAREN Block"); }  //$NON-NLS-1$
		    consumeStatementSynchronized();  
			break;
 
    case 328 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: OnlySynchronized ::= synchronized"); }  //$NON-NLS-1$
		    consumeOnlySynchronized();  
			break;
 
    case 329 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TryStatement ::= try TryBlock Catches"); }  //$NON-NLS-1$
		    consumeStatementTry(false);  
			break;
 
    case 330 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TryStatement ::= try TryBlock Catchesopt Finally"); }  //$NON-NLS-1$
		    consumeStatementTry(true);  
			break;
 
    case 332 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExitTryBlock ::="); }  //$NON-NLS-1$
		    consumeExitTryBlock();  
			break;
 
    case 334 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Catches ::= Catches CatchClause"); }  //$NON-NLS-1$
		    consumeCatches();  
			break;
 
    case 335 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: CatchClause ::= catch LPAREN FormalParameter RPAREN Block"); }  //$NON-NLS-1$
		    consumeStatementCatch() ;  
			break;
 
    case 337 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PushLPAREN ::= LPAREN"); }  //$NON-NLS-1$
		    consumeLeftParen();  
			break;
 
    case 338 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PushRPAREN ::= RPAREN"); }  //$NON-NLS-1$
		    consumeRightParen();  
			break;
 
    case 343 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PrimaryNoNewArray ::= this"); }  //$NON-NLS-1$
		    consumePrimaryNoNewArrayThis();  
			break;
 
    case 344 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PrimaryNoNewArray ::= PushLPAREN Expression_NotName PushRPAREN"); }  //$NON-NLS-1$
		    consumePrimaryNoNewArray();  
			break;
 
    case 345 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PrimaryNoNewArray ::= PushLPAREN Name PushRPAREN"); }  //$NON-NLS-1$
		    consumePrimaryNoNewArrayWithName();  
			break;
 
    case 348 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PrimaryNoNewArray ::= Name DOT this"); }  //$NON-NLS-1$
		    consumePrimaryNoNewArrayNameThis();  
			break;
 
    case 349 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PrimaryNoNewArray ::= Name DOT super"); }  //$NON-NLS-1$
		    consumePrimaryNoNewArrayNameSuper();  
			break;
 
    case 350 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PrimaryNoNewArray ::= implementation DOT this"); }  //$NON-NLS-1$
		    consumePrimaryNoNewArrayImplementationThis();  
			break;
 
    case 351 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PrimaryNoNewArray ::= Name DOT class"); }  //$NON-NLS-1$
		    consumePrimaryNoNewArrayName();  
			break;
 
    case 352 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PrimaryNoNewArray ::= Name Dims DOT class"); }  //$NON-NLS-1$
		    consumePrimaryNoNewArrayArrayType();  
			break;
 
    case 353 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PrimaryNoNewArray ::= PrimitiveType Dims DOT class"); }  //$NON-NLS-1$
		    consumePrimaryNoNewArrayPrimitiveArrayType();  
			break;
 
    case 354 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PrimaryNoNewArray ::= PrimitiveType DOT class"); }  //$NON-NLS-1$
		    consumePrimaryNoNewArrayPrimitiveType();  
			break;
 
    case 357 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AllocationHeader ::= new ClassType LPAREN ArgumentListopt RPAREN"); }  //$NON-NLS-1$
		    consumeAllocationHeader();  
			break;
 
    case 358 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassInstanceCreationExpression ::= new OnlyTypeArguments ClassType LPAREN ArgumentListopt..."); }  //$NON-NLS-1$
		    consumeClassInstanceCreationExpressionWithTypeArguments();  
			break;
 
    case 359 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassInstanceCreationExpression ::= new ClassType LPAREN ArgumentListopt RPAREN ClassBodyopt"); }  //$NON-NLS-1$
		    consumeClassInstanceCreationExpression();  
			break;
 
    case 360 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassInstanceCreationExpression ::= Primary DOT new OnlyTypeArguments ClassType LPAREN..."); }  //$NON-NLS-1$
		    consumeClassInstanceCreationExpressionQualifiedWithTypeArguments() ;  
			break;
 
    case 361 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassInstanceCreationExpression ::= Primary DOT new ClassType LPAREN ArgumentListopt RPAREN..."); }  //$NON-NLS-1$
		    consumeClassInstanceCreationExpressionQualified() ;  
			break;
 
    case 362 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassInstanceCreationExpression ::= ClassInstanceCreationExpressionName new ClassType LPAREN..."); }  //$NON-NLS-1$
		    consumeClassInstanceCreationExpressionQualified() ;  
			break;
 
    case 363 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassInstanceCreationExpression ::= ClassInstanceCreationExpressionName new OnlyTypeArguments..."); }  //$NON-NLS-1$
		    consumeClassInstanceCreationExpressionQualifiedWithTypeArguments() ;  
			break;
 
    case 364 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassInstanceCreationExpressionName ::= Name DOT"); }  //$NON-NLS-1$
		    consumeClassInstanceCreationExpressionName() ;  
			break;
 
    case 365 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassBodyopt ::="); }  //$NON-NLS-1$
		    consumeClassBodyopt();  
			break;
 
    case 367 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnterAnonymousClassBody ::="); }  //$NON-NLS-1$
		    consumeEnterAnonymousClassBody();  
			break;
 
    case 369 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArgumentList ::= ArgumentList COMMA Expression"); }  //$NON-NLS-1$
		    consumeArgumentList();  
			break;
 
    case 370 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArrayCreationHeader ::= new PrimitiveType DimWithOrWithOutExprs"); }  //$NON-NLS-1$
		    consumeArrayCreationHeader();  
			break;
 
    case 371 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArrayCreationHeader ::= new ClassOrInterfaceType DimWithOrWithOutExprs"); }  //$NON-NLS-1$
		    consumeArrayCreationHeader();  
			break;
 
    case 372 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArrayCreationWithoutArrayInitializer ::= new PrimitiveType DimWithOrWithOutExprs"); }  //$NON-NLS-1$
		    consumeArrayCreationExpressionWithoutInitializer();  
			break;
 
    case 373 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArrayCreationWithArrayInitializer ::= new PrimitiveType DimWithOrWithOutExprs ArrayInitializer"); }  //$NON-NLS-1$
		    consumeArrayCreationExpressionWithInitializer();  
			break;
 
    case 374 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArrayCreationWithoutArrayInitializer ::= new ClassOrInterfaceType DimWithOrWithOutExprs"); }  //$NON-NLS-1$
		    consumeArrayCreationExpressionWithoutInitializer();  
			break;
 
    case 375 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArrayCreationWithArrayInitializer ::= new ClassOrInterfaceType DimWithOrWithOutExprs..."); }  //$NON-NLS-1$
		    consumeArrayCreationExpressionWithInitializer();  
			break;
 
    case 377 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: DimWithOrWithOutExprs ::= DimWithOrWithOutExprs DimWithOrWithOutExpr"); }  //$NON-NLS-1$
		    consumeDimWithOrWithOutExprs();  
			break;
 
     case 379 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: DimWithOrWithOutExpr ::= LBRACKET RBRACKET"); }  //$NON-NLS-1$
		    consumeDimWithOrWithOutExpr();  
			break;
 
     case 380 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Dims ::= DimsLoop"); }  //$NON-NLS-1$
		    consumeDims();  
			break;
 
     case 383 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: OneDimLoop ::= LBRACKET RBRACKET"); }  //$NON-NLS-1$
		    consumeOneDimLoop();  
			break;
 
    case 384 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: FieldAccess ::= Primary DOT Identifier"); }  //$NON-NLS-1$
		    consumeFieldAccess(false);  
			break;
 
    case 385 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: FieldAccess ::= super DOT Identifier"); }  //$NON-NLS-1$
		    consumeFieldAccess(true);  
			break;
 
    case 386 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MethodInvocation ::= Name HASH LPAREN ArgumentListopt RPAREN"); }  //$NON-NLS-1$
		    consumeSpecialForm();  
			break;
 
    case 387 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MethodInvocation ::= Name InterfaceSpecifieropt LPAREN ArgumentListopt RPAREN"); }  //$NON-NLS-1$
		    consumeMethodInvocationName();  
			break;
 
    case 388 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MethodInvocation ::= Name DOT OnlyTypeArguments Identifier InterfaceSpecifieropt LPAREN..."); }  //$NON-NLS-1$
		    consumeMethodInvocationNameWithTypeArguments();  
			break;
 
    case 389 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MethodInvocation ::= Primary DOT OnlyTypeArguments Identifier InterfaceSpecifieropt LPAREN..."); }  //$NON-NLS-1$
		    consumeMethodInvocationPrimaryWithTypeArguments();  
			break;
 
    case 390 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MethodInvocation ::= Primary DOT Identifier InterfaceSpecifieropt LPAREN ArgumentListopt RPAREN"); }  //$NON-NLS-1$
		    consumeMethodInvocationPrimary();  
			break;
 
    case 391 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MethodInvocation ::= super DOT OnlyTypeArguments Identifier LPAREN ArgumentListopt RPAREN"); }  //$NON-NLS-1$
		    consumeMethodInvocationSuperWithTypeArguments();  
			break;
 
    case 392 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MethodInvocation ::= super DOT Identifier LPAREN ArgumentListopt RPAREN"); }  //$NON-NLS-1$
		    consumeMethodInvocationSuper();  
			break;
 
     case 393 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceSpecifieropt ::="); }  //$NON-NLS-1$
		    consumeEmptyInterfaceSpecifieropt();  
			break;
 
     case 395 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceSpecifier ::= COLONCOLON Name"); }  //$NON-NLS-1$
		    consumeInterfaceSpecifier();  
			break;
 
    case 396 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArrayAccess ::= Name LBRACKET ArgumentList RBRACKET"); }  //$NON-NLS-1$
		    consumeArrayAccess(true);  
			break;
 
    case 397 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArrayAccess ::= PrimaryNoNewArray LBRACKET ArgumentList RBRACKET"); }  //$NON-NLS-1$
		    consumeArrayAccess(false);  
			break;
 
    case 398 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArrayAccess ::= ArrayCreationWithArrayInitializer LBRACKET ArgumentList RBRACKET"); }  //$NON-NLS-1$
		    consumeArrayAccess(false);  
			break;
 
    case 400 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PostfixExpression ::= Name"); }  //$NON-NLS-1$
		    consumePostfixExpression();  
			break;
 
    case 403 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PostIncrementExpression ::= PostfixExpression PLUS_PLUS"); }  //$NON-NLS-1$
		    consumeUnaryExpression(OperatorIds.PLUS,true);  
			break;
 
    case 404 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PostDecrementExpression ::= PostfixExpression MINUS_MINUS"); }  //$NON-NLS-1$
		    consumeUnaryExpression(OperatorIds.MINUS,true);  
			break;
 
    case 405 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PushPosition ::="); }  //$NON-NLS-1$
		    consumePushPosition();  
			break;
 
    case 408 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: UnaryExpression ::= PLUS PushPosition UnaryExpression"); }  //$NON-NLS-1$
		    consumeUnaryExpression(OperatorIds.PLUS);  
			break;
 
    case 409 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: UnaryExpression ::= MINUS PushPosition UnaryExpression"); }  //$NON-NLS-1$
		    consumeUnaryExpression(OperatorIds.MINUS);  
			break;
 
    case 411 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PreIncrementExpression ::= PLUS_PLUS PushPosition UnaryExpression"); }  //$NON-NLS-1$
		    consumeUnaryExpression(OperatorIds.PLUS,false);  
			break;
 
    case 412 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: PreDecrementExpression ::= MINUS_MINUS PushPosition UnaryExpression"); }  //$NON-NLS-1$
		    consumeUnaryExpression(OperatorIds.MINUS,false);  
			break;
 
    case 414 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: UnaryExpressionNotPlusMinus ::= TWIDDLE PushPosition UnaryExpression"); }  //$NON-NLS-1$
		    consumeUnaryExpression(OperatorIds.TWIDDLE);  
			break;
 
    case 415 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: UnaryExpressionNotPlusMinus ::= NOT PushPosition UnaryExpression"); }  //$NON-NLS-1$
		    consumeUnaryExpression(OperatorIds.NOT);  
			break;
 
    case 417 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: CastExpression ::= PushLPAREN PrimitiveType Dimsopt PushRPAREN InsideCastExpression..."); }  //$NON-NLS-1$
		    consumeCastExpressionWithPrimitiveType();  
			break;
 
    case 418 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: CastExpression ::= PushLPAREN Name OnlyTypeArgumentsForCastExpression Dimsopt PushRPAREN..."); }  //$NON-NLS-1$
		    consumeCastExpressionWithGenericsArray();  
			break;
 
    case 419 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: CastExpression ::= PushLPAREN Name OnlyTypeArgumentsForCastExpression DOT ClassOrInterfaceType"); }  //$NON-NLS-1$
		    consumeCastExpressionWithQualifiedGenericsArray();  
			break;
 
    case 420 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: CastExpression ::= PushLPAREN Name PushRPAREN InsideCastExpressionLL1..."); }  //$NON-NLS-1$
		    consumeCastExpressionLL1();  
			break;
 
    case 421 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: CastExpression ::= PushLPAREN Name Dims PushRPAREN InsideCastExpression..."); }  //$NON-NLS-1$
		    consumeCastExpressionWithNameArray();  
			break;
 
    case 422 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: OnlyTypeArgumentsForCastExpression ::= OnlyTypeArguments"); }  //$NON-NLS-1$
		    consumeOnlyTypeArgumentsForCastExpression();  
			break;
 
    case 423 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InsideCastExpression ::="); }  //$NON-NLS-1$
		    consumeInsideCastExpression();  
			break;
 
    case 424 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InsideCastExpressionLL1 ::="); }  //$NON-NLS-1$
		    consumeInsideCastExpressionLL1();  
			break;
 
    case 425 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InsideCastExpressionWithQualifiedGenerics ::="); }  //$NON-NLS-1$
		    consumeInsideCastExpressionWithQualifiedGenerics();  
			break;
 
    case 427 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MultiplicativeExpression ::= MultiplicativeExpression MULTIPLY UnaryExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.MULTIPLY);  
			break;
 
    case 428 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MultiplicativeExpression ::= MultiplicativeExpression DIVIDE UnaryExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.DIVIDE);  
			break;
 
    case 429 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MultiplicativeExpression ::= MultiplicativeExpression REMAINDER UnaryExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.REMAINDER);  
			break;
 
    case 431 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AdditiveExpression ::= AdditiveExpression PLUS MultiplicativeExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.PLUS);  
			break;
 
    case 432 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AdditiveExpression ::= AdditiveExpression MINUS MultiplicativeExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.MINUS);  
			break;
 
    case 434 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ShiftExpression ::= ShiftExpression LEFT_SHIFT AdditiveExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.LEFT_SHIFT);  
			break;
 
    case 435 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ShiftExpression ::= ShiftExpression RIGHT_SHIFT AdditiveExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.RIGHT_SHIFT);  
			break;
 
    case 436 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ShiftExpression ::= ShiftExpression UNSIGNED_RIGHT_SHIFT AdditiveExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.UNSIGNED_RIGHT_SHIFT);  
			break;
 
    case 438 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: RelationalExpression ::= RelationalExpression LESS ShiftExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.LESS);  
			break;
 
    case 439 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: RelationalExpression ::= RelationalExpression GREATER ShiftExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.GREATER);  
			break;
 
    case 440 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: RelationalExpression ::= RelationalExpression LESS_EQUAL ShiftExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.LESS_EQUAL);  
			break;
 
    case 441 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: RelationalExpression ::= RelationalExpression GREATER_EQUAL ShiftExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.GREATER_EQUAL);  
			break;
 
    case 443 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InstanceofExpression ::= InstanceofExpression instanceof ReferenceType"); }  //$NON-NLS-1$
		    consumeInstanceOfExpression();  
			break;
 
    case 445 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EqualityExpression ::= EqualityExpression EQUAL_EQUAL InstanceofExpression"); }  //$NON-NLS-1$
		    consumeEqualityExpression(OperatorIds.EQUAL_EQUAL);  
			break;
 
    case 446 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EqualityExpression ::= EqualityExpression NOT_EQUAL InstanceofExpression"); }  //$NON-NLS-1$
		    consumeEqualityExpression(OperatorIds.NOT_EQUAL);  
			break;
 
    case 448 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AndExpression ::= AndExpression AND EqualityExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.AND);  
			break;
 
    case 450 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExclusiveOrExpression ::= ExclusiveOrExpression XOR AndExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.XOR);  
			break;
 
    case 452 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InclusiveOrExpression ::= InclusiveOrExpression OR ExclusiveOrExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.OR);  
			break;
 
    case 454 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ConditionalAndExpression ::= ConditionalAndExpression AND_AND InclusiveOrExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.AND_AND);  
			break;
 
    case 456 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ConditionalOrExpression ::= ConditionalOrExpression OR_OR ConditionalAndExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.OR_OR);  
			break;
 
    case 458 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ConditionalExpression ::= ConditionalOrExpression QUESTION Expression COLON..."); }  //$NON-NLS-1$
		    consumeConditionalExpression(OperatorIds.QUESTIONCOLON) ;  
			break;
 
    case 461 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Assignment ::= PostfixExpression AssignmentOperator AssignmentExpression"); }  //$NON-NLS-1$
		    consumeAssignment();  
			break;
 
    case 463 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Assignment ::= InvalidArrayInitializerAssignement"); }  //$NON-NLS-1$
		    ignoreExpressionAssignment(); 
			break;
 
    case 464 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AssignmentOperator ::= EQUAL"); }  //$NON-NLS-1$
		    consumeAssignmentOperator(EQUAL);  
			break;
 
    case 465 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AssignmentOperator ::= MULTIPLY_EQUAL"); }  //$NON-NLS-1$
		    consumeAssignmentOperator(MULTIPLY);  
			break;
 
    case 466 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AssignmentOperator ::= DIVIDE_EQUAL"); }  //$NON-NLS-1$
		    consumeAssignmentOperator(DIVIDE);  
			break;
 
    case 467 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AssignmentOperator ::= REMAINDER_EQUAL"); }  //$NON-NLS-1$
		    consumeAssignmentOperator(REMAINDER);  
			break;
 
    case 468 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AssignmentOperator ::= PLUS_EQUAL"); }  //$NON-NLS-1$
		    consumeAssignmentOperator(PLUS);  
			break;
 
    case 469 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AssignmentOperator ::= MINUS_EQUAL"); }  //$NON-NLS-1$
		    consumeAssignmentOperator(MINUS);  
			break;
 
    case 470 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AssignmentOperator ::= LEFT_SHIFT_EQUAL"); }  //$NON-NLS-1$
		    consumeAssignmentOperator(LEFT_SHIFT);  
			break;
 
    case 471 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AssignmentOperator ::= RIGHT_SHIFT_EQUAL"); }  //$NON-NLS-1$
		    consumeAssignmentOperator(RIGHT_SHIFT);  
			break;
 
    case 472 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AssignmentOperator ::= UNSIGNED_RIGHT_SHIFT_EQUAL"); }  //$NON-NLS-1$
		    consumeAssignmentOperator(UNSIGNED_RIGHT_SHIFT);  
			break;
 
    case 473 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AssignmentOperator ::= AND_EQUAL"); }  //$NON-NLS-1$
		    consumeAssignmentOperator(AND);  
			break;
 
    case 474 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AssignmentOperator ::= XOR_EQUAL"); }  //$NON-NLS-1$
		    consumeAssignmentOperator(XOR);  
			break;
 
    case 475 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AssignmentOperator ::= OR_EQUAL"); }  //$NON-NLS-1$
		    consumeAssignmentOperator(OR);  
			break;
 
    case 479 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Expressionopt ::="); }  //$NON-NLS-1$
		    consumeEmptyExpression();  
			break;
 
    case 484 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassBodyDeclarationsopt ::="); }  //$NON-NLS-1$
		    consumeEmptyClassBodyDeclarationsopt();  
			break;
 
    case 485 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ClassBodyDeclarationsopt ::= NestedType ClassBodyDeclarations"); }  //$NON-NLS-1$
		    consumeClassBodyDeclarationsopt();  
			break;
 
    case 486 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplementationBodyDeclarationsopt ::="); }  //$NON-NLS-1$
		    consumeEmptyImplementationBodyDeclarationsopt();  
			break;
 
    case 487 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplementationBodyDeclarationsopt ::= NestedType ImplementationBodyDeclarations"); }  //$NON-NLS-1$
		    consumeImplementationBodyDeclarationsopt();  
			break;
 
    case 488 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplementationReceiverBodyDeclarationsopt ::="); }  //$NON-NLS-1$
		    consumeEmptyImplementationReceiverBodyDeclarationsopt();  
			break;
 
    case 489 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplementationReceiverBodyDeclarationsopt ::= NestedType ImplementationReceiverBodyDeclarations"); }  //$NON-NLS-1$
		    consumeImplementationReceiverBodyDeclarationsopt();  
			break;
 
     case 490 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Modifiersopt ::="); }  //$NON-NLS-1$
		    consumeDefaultModifiers();  
			break;
 
    case 491 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Modifiersopt ::= Modifiers"); }  //$NON-NLS-1$
		    consumeModifiers();  
			break;
 
    case 492 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: BlockStatementsopt ::="); }  //$NON-NLS-1$
		    consumeEmptyBlockStatementsopt();  
			break;
 
     case 494 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Dimsopt ::="); }  //$NON-NLS-1$
		    consumeEmptyDimsopt();  
			break;
 
     case 496 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ArgumentListopt ::="); }  //$NON-NLS-1$
		    consumeEmptyArgumentListopt();  
			break;
 
    case 502 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: FormalParameterListopt ::="); }  //$NON-NLS-1$
		    consumeFormalParameterListopt();  
			break;
 
     case 518 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceMemberDeclarationsopt ::="); }  //$NON-NLS-1$
		    consumeEmptyInterfaceMemberDeclarationsopt();  
			break;
 
     case 519 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InterfaceMemberDeclarationsopt ::= NestedType InterfaceMemberDeclarations"); }  //$NON-NLS-1$
		    consumeInterfaceMemberDeclarationsopt();  
			break;
 
    case 520 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: NestedType ::="); }  //$NON-NLS-1$
		    consumeNestedType();  
			break;

     case 521 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ForInitopt ::="); }  //$NON-NLS-1$
		    consumeEmptyForInitopt();  
			break;
 
     case 523 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ForUpdateopt ::="); }  //$NON-NLS-1$
		    consumeEmptyForUpdateopt();  
			break;
 
     case 527 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Catchesopt ::="); }  //$NON-NLS-1$
		    consumeEmptyCatchesopt();  
			break;
 
     case 529 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnumDeclaration ::= EnumHeader EnumBody"); }  //$NON-NLS-1$
		    consumeEnumDeclaration();  
			break;
 
     case 530 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnumHeader ::= EnumHeaderName ClassHeaderImplementsopt"); }  //$NON-NLS-1$
		    consumeEnumHeader();  
			break;
 
     case 531 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnumHeaderName ::= Modifiersopt enum Identifier"); }  //$NON-NLS-1$
		    consumeEnumHeaderName();  
			break;
 
     case 532 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnumHeaderName ::= Modifiersopt enum Identifier TypeParameters"); }  //$NON-NLS-1$
		    consumeEnumHeaderNameWithTypeParameters();  
			break;
 
     case 533 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnumBody ::= LBRACE EnumBodyDeclarationsopt RBRACE"); }  //$NON-NLS-1$
		    consumeEnumBodyNoConstants();  
			break;
 
     case 534 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnumBody ::= LBRACE COMMA EnumBodyDeclarationsopt RBRACE"); }  //$NON-NLS-1$
		    consumeEnumBodyNoConstants();  
			break;
 
     case 535 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnumBody ::= LBRACE EnumConstants COMMA EnumBodyDeclarationsopt RBRACE"); }  //$NON-NLS-1$
		    consumeEnumBodyWithConstants();  
			break;
 
     case 536 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnumBody ::= LBRACE EnumConstants EnumBodyDeclarationsopt RBRACE"); }  //$NON-NLS-1$
		    consumeEnumBodyWithConstants();  
			break;
 
    case 538 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnumConstants ::= EnumConstants COMMA EnumConstant"); }  //$NON-NLS-1$
		    consumeEnumConstants();  
			break;
 
    case 539 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnumConstantHeaderName ::= Modifiersopt Identifier"); }  //$NON-NLS-1$
		    consumeEnumConstantHeaderName();  
			break;
 
    case 540 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnumConstantHeader ::= EnumConstantHeaderName ForceNoDiet Argumentsopt RestoreDiet"); }  //$NON-NLS-1$
		    consumeEnumConstantHeader();  
			break;
 
    case 541 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnumConstant ::= EnumConstantHeader ForceNoDiet ClassBody RestoreDiet"); }  //$NON-NLS-1$
		    consumeEnumConstantWithClassBody();  
			break;
 
    case 542 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnumConstant ::= EnumConstantHeader"); }  //$NON-NLS-1$
		    consumeEnumConstantNoClassBody();  
			break;
 
    case 543 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Arguments ::= LPAREN ArgumentListopt RPAREN"); }  //$NON-NLS-1$
		    consumeArguments();  
			break;
 
    case 544 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Argumentsopt ::="); }  //$NON-NLS-1$
		    consumeEmptyArguments();  
			break;
 
    case 546 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnumDeclarations ::= SEMICOLON ClassBodyDeclarationsopt"); }  //$NON-NLS-1$
		    consumeEnumDeclarations();  
			break;
 
    case 547 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnumBodyDeclarationsopt ::="); }  //$NON-NLS-1$
		    consumeEmptyEnumDeclarations();  
			break;
 
    case 549 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnhancedForStatement ::= EnhancedForStatementHeader Statement"); }  //$NON-NLS-1$
		    consumeEnhancedForStatement();  
			break;
 
    case 550 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnhancedForStatementNoShortIf ::= EnhancedForStatementHeader StatementNoShortIf"); }  //$NON-NLS-1$
		    consumeEnhancedForStatement();  
			break;
 
    case 551 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnhancedForStatementHeaderInit ::= for LPAREN Type PushModifiers Identifier Dimsopt"); }  //$NON-NLS-1$
		    consumeEnhancedForStatementHeaderInit(false);  
			break;
 
    case 552 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnhancedForStatementHeaderInit ::= for LPAREN Modifiers Type PushRealModifiers Identifier..."); }  //$NON-NLS-1$
		    consumeEnhancedForStatementHeaderInit(true);  
			break;
 
    case 553 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnhancedForStatementHeader ::= EnhancedForStatementHeaderInit COLON Expression RPAREN"); }  //$NON-NLS-1$
		    consumeEnhancedForStatementHeader();  
			break;
 
    case 554 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: SingleStaticImportDeclaration ::= SingleStaticImportDeclarationName SEMICOLON"); }  //$NON-NLS-1$
		    consumeImportDeclaration();  
			break;
 
    case 555 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: SingleStaticImportDeclarationName ::= import static Name"); }  //$NON-NLS-1$
		    consumeSingleStaticImportDeclarationName();  
			break;
 
    case 556 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: StaticImportOnDemandDeclaration ::= StaticImportOnDemandDeclarationName SEMICOLON"); }  //$NON-NLS-1$
		    consumeImportDeclaration();  
			break;
 
    case 557 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: StaticImportOnDemandDeclarationName ::= import static Name DOT MULTIPLY"); }  //$NON-NLS-1$
		    consumeStaticImportOnDemandDeclarationName();  
			break;
 
    case 558 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeArguments ::= LESS TypeArgumentList1"); }  //$NON-NLS-1$
		    consumeTypeArguments();  
			break;
 
    case 559 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: OnlyTypeArguments ::= LESS TypeArgumentList1"); }  //$NON-NLS-1$
		    consumeOnlyTypeArguments();  
			break;
 
    case 561 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeArgumentList1 ::= TypeArgumentList COMMA TypeArgument1"); }  //$NON-NLS-1$
		    consumeTypeArgumentList1();  
			break;
 
    case 563 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeArgumentList ::= TypeArgumentList COMMA TypeArgument"); }  //$NON-NLS-1$
		    consumeTypeArgumentList();  
			break;
 
    case 564 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeArgument ::= ReferenceType"); }  //$NON-NLS-1$
		    consumeTypeArgument();  
			break;
 
    case 568 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ReferenceType1 ::= ReferenceType GREATER"); }  //$NON-NLS-1$
		    consumeReferenceType1();  
			break;
 
    case 569 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ReferenceType1 ::= ClassOrInterface LESS TypeArgumentList2"); }  //$NON-NLS-1$
		    consumeTypeArgumentReferenceType1();  
			break;
 
    case 571 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeArgumentList2 ::= TypeArgumentList COMMA TypeArgument2"); }  //$NON-NLS-1$
		    consumeTypeArgumentList2();  
			break;
 
    case 574 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ReferenceType2 ::= ReferenceType RIGHT_SHIFT"); }  //$NON-NLS-1$
		    consumeReferenceType2();  
			break;
 
    case 575 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ReferenceType2 ::= ClassOrInterface LESS TypeArgumentList3"); }  //$NON-NLS-1$
		    consumeTypeArgumentReferenceType2();  
			break;
 
    case 577 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeArgumentList3 ::= TypeArgumentList COMMA TypeArgument3"); }  //$NON-NLS-1$
		    consumeTypeArgumentList3();  
			break;
 
    case 580 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ReferenceType3 ::= ReferenceType UNSIGNED_RIGHT_SHIFT"); }  //$NON-NLS-1$
		    consumeReferenceType3();  
			break;
 
    case 581 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Wildcard ::= QUESTION"); }  //$NON-NLS-1$
		    consumeWildcard();  
			break;
 
    case 582 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Wildcard ::= QUESTION WildcardBounds"); }  //$NON-NLS-1$
		    consumeWildcardWithBounds();  
			break;
 
    case 583 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: WildcardBounds ::= extends ReferenceType"); }  //$NON-NLS-1$
		    consumeWildcardBoundsExtends();  
			break;
 
    case 584 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: WildcardBounds ::= super ReferenceType"); }  //$NON-NLS-1$
		    consumeWildcardBoundsSuper();  
			break;
 
    case 585 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: WildcardBounds ::= implements ReferenceType"); }  //$NON-NLS-1$
		    consumeWildcardBoundsImplements();  
			break;
 
    case 586 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Wildcard1 ::= QUESTION GREATER"); }  //$NON-NLS-1$
		    consumeWildcard1();  
			break;
 
    case 587 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Wildcard1 ::= QUESTION WildcardBounds1"); }  //$NON-NLS-1$
		    consumeWildcard1WithBounds();  
			break;
 
    case 588 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: WildcardBounds1 ::= extends ReferenceType1"); }  //$NON-NLS-1$
		    consumeWildcardBounds1Extends();  
			break;
 
    case 589 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: WildcardBounds1 ::= super ReferenceType1"); }  //$NON-NLS-1$
		    consumeWildcardBounds1Super();  
			break;
 
    case 590 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: WildcardBounds1 ::= implements ReferenceType1"); }  //$NON-NLS-1$
		    consumeWildcardBounds1Implements();  
			break;
 
    case 591 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Wildcard2 ::= QUESTION RIGHT_SHIFT"); }  //$NON-NLS-1$
		    consumeWildcard2();  
			break;
 
    case 592 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Wildcard2 ::= QUESTION WildcardBounds2"); }  //$NON-NLS-1$
		    consumeWildcard2WithBounds();  
			break;
 
    case 593 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: WildcardBounds2 ::= extends ReferenceType2"); }  //$NON-NLS-1$
		    consumeWildcardBounds2Extends();  
			break;
 
    case 594 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: WildcardBounds2 ::= super ReferenceType2"); }  //$NON-NLS-1$
		    consumeWildcardBounds2Super();  
			break;
 
    case 595 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: WildcardBounds2 ::= implements ReferenceType2"); }  //$NON-NLS-1$
		    consumeWildcardBounds2Implements();  
			break;
 
    case 596 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Wildcard3 ::= QUESTION UNSIGNED_RIGHT_SHIFT"); }  //$NON-NLS-1$
		    consumeWildcard3();  
			break;
 
    case 597 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Wildcard3 ::= QUESTION WildcardBounds3"); }  //$NON-NLS-1$
		    consumeWildcard3WithBounds();  
			break;
 
    case 598 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: WildcardBounds3 ::= extends ReferenceType3"); }  //$NON-NLS-1$
		    consumeWildcardBounds3Extends();  
			break;
 
    case 599 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: WildcardBounds3 ::= super ReferenceType3"); }  //$NON-NLS-1$
		    consumeWildcardBounds3Super();  
			break;
 
    case 600 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: WildcardBounds3 ::= implements ReferenceType3"); }  //$NON-NLS-1$
		    consumeWildcardBounds3Implements();  
			break;
 
    case 601 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeParameterHeader ::= Identifier"); }  //$NON-NLS-1$
		    consumeTypeParameterHeader();  
			break;
 
    case 602 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeParameters ::= LESS TypeParameterList1"); }  //$NON-NLS-1$
		    consumeTypeParameters();  
			break;
 
    case 604 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeParameterList ::= TypeParameterList COMMA TypeParameter"); }  //$NON-NLS-1$
		    consumeTypeParameterList();  
			break;
 
    case 606 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeParameter ::= TypeParameterHeader extends ReferenceType"); }  //$NON-NLS-1$
		    consumeTypeParameterWithExtends();  
			break;
 
    case 607 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeParameter ::= TypeParameterHeader extends ReferenceType AdditionalBoundList"); }  //$NON-NLS-1$
		    consumeTypeParameterWithExtendsAndBounds();  
			break;
 
    case 608 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeParameter ::= TypeParameterHeader implements ReferenceType"); }  //$NON-NLS-1$
		    consumeTypeParameterWithImplements();  
			break;
 
    case 609 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeParameter ::= TypeParameterHeader implements ReferenceType AdditionalBoundList"); }  //$NON-NLS-1$
		    consumeTypeParameterWithImplementsAndBounds();  
			break;
 
    case 611 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AdditionalBoundList ::= AdditionalBoundList AdditionalBound"); }  //$NON-NLS-1$
		    consumeAdditionalBoundList();  
			break;
 
    case 612 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AdditionalBound ::= AND ReferenceType"); }  //$NON-NLS-1$
		    consumeAdditionalBound();  
			break;
 
    case 614 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeParameterList1 ::= TypeParameterList COMMA TypeParameter1"); }  //$NON-NLS-1$
		    consumeTypeParameterList1();  
			break;
 
    case 615 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeParameter1 ::= TypeParameterHeader GREATER"); }  //$NON-NLS-1$
		    consumeTypeParameter1();  
			break;
 
    case 616 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeParameter1 ::= TypeParameterHeader extends ReferenceType1"); }  //$NON-NLS-1$
		    consumeTypeParameter1WithExtends();  
			break;
 
    case 617 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeParameter1 ::= TypeParameterHeader extends ReferenceType AdditionalBoundList1"); }  //$NON-NLS-1$
		    consumeTypeParameter1WithExtendsAndBounds();  
			break;
 
    case 618 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeParameter1 ::= TypeParameterHeader implements ReferenceType1"); }  //$NON-NLS-1$
		    consumeTypeParameter1WithImplements();  
			break;
 
    case 619 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: TypeParameter1 ::= TypeParameterHeader implements ReferenceType AdditionalBoundList1"); }  //$NON-NLS-1$
		    consumeTypeParameter1WithImplementsAndBounds();  
			break;
 
    case 621 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AdditionalBoundList1 ::= AdditionalBoundList AdditionalBound1"); }  //$NON-NLS-1$
		    consumeAdditionalBoundList1();  
			break;
 
    case 622 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AdditionalBound1 ::= AND ReferenceType1"); }  //$NON-NLS-1$
		    consumeAdditionalBound1();  
			break;
 
    case 628 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: UnaryExpression_NotName ::= PLUS PushPosition UnaryExpression"); }  //$NON-NLS-1$
		    consumeUnaryExpression(OperatorIds.PLUS);  
			break;
 
    case 629 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: UnaryExpression_NotName ::= MINUS PushPosition UnaryExpression"); }  //$NON-NLS-1$
		    consumeUnaryExpression(OperatorIds.MINUS);  
			break;
 
    case 632 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: UnaryExpressionNotPlusMinus_NotName ::= TWIDDLE PushPosition UnaryExpression"); }  //$NON-NLS-1$
		    consumeUnaryExpression(OperatorIds.TWIDDLE);  
			break;
 
    case 633 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: UnaryExpressionNotPlusMinus_NotName ::= NOT PushPosition UnaryExpression"); }  //$NON-NLS-1$
		    consumeUnaryExpression(OperatorIds.NOT);  
			break;
 
    case 636 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MultiplicativeExpression_NotName ::= MultiplicativeExpression_NotName MULTIPLY UnaryExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.MULTIPLY);  
			break;
 
    case 637 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MultiplicativeExpression_NotName ::= Name MULTIPLY UnaryExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpressionWithName(OperatorIds.MULTIPLY);  
			break;
 
    case 638 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MultiplicativeExpression_NotName ::= MultiplicativeExpression_NotName DIVIDE UnaryExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.DIVIDE);  
			break;
 
    case 639 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MultiplicativeExpression_NotName ::= Name DIVIDE UnaryExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpressionWithName(OperatorIds.DIVIDE);  
			break;
 
    case 640 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MultiplicativeExpression_NotName ::= MultiplicativeExpression_NotName REMAINDER UnaryExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.REMAINDER);  
			break;
 
    case 641 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MultiplicativeExpression_NotName ::= Name REMAINDER UnaryExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpressionWithName(OperatorIds.REMAINDER);  
			break;
 
    case 643 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AdditiveExpression_NotName ::= AdditiveExpression_NotName PLUS MultiplicativeExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.PLUS);  
			break;
 
    case 644 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AdditiveExpression_NotName ::= Name PLUS MultiplicativeExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpressionWithName(OperatorIds.PLUS);  
			break;
 
    case 645 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AdditiveExpression_NotName ::= AdditiveExpression_NotName MINUS MultiplicativeExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.MINUS);  
			break;
 
    case 646 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AdditiveExpression_NotName ::= Name MINUS MultiplicativeExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpressionWithName(OperatorIds.MINUS);  
			break;
 
    case 648 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ShiftExpression_NotName ::= ShiftExpression_NotName LEFT_SHIFT AdditiveExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.LEFT_SHIFT);  
			break;
 
    case 649 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ShiftExpression_NotName ::= Name LEFT_SHIFT AdditiveExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpressionWithName(OperatorIds.LEFT_SHIFT);  
			break;
 
    case 650 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ShiftExpression_NotName ::= ShiftExpression_NotName RIGHT_SHIFT AdditiveExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.RIGHT_SHIFT);  
			break;
 
    case 651 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ShiftExpression_NotName ::= Name RIGHT_SHIFT AdditiveExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpressionWithName(OperatorIds.RIGHT_SHIFT);  
			break;
 
    case 652 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ShiftExpression_NotName ::= ShiftExpression_NotName UNSIGNED_RIGHT_SHIFT AdditiveExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.UNSIGNED_RIGHT_SHIFT);  
			break;
 
    case 653 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ShiftExpression_NotName ::= Name UNSIGNED_RIGHT_SHIFT AdditiveExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpressionWithName(OperatorIds.UNSIGNED_RIGHT_SHIFT);  
			break;
 
    case 655 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: RelationalExpression_NotName ::= ShiftExpression_NotName LESS ShiftExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.LESS);  
			break;
 
    case 656 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: RelationalExpression_NotName ::= Name LESS ShiftExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpressionWithName(OperatorIds.LESS);  
			break;
 
    case 657 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: RelationalExpression_NotName ::= ShiftExpression_NotName GREATER ShiftExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.GREATER);  
			break;
 
    case 658 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: RelationalExpression_NotName ::= Name GREATER ShiftExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpressionWithName(OperatorIds.GREATER);  
			break;
 
    case 659 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: RelationalExpression_NotName ::= RelationalExpression_NotName LESS_EQUAL ShiftExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.LESS_EQUAL);  
			break;
 
    case 660 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: RelationalExpression_NotName ::= Name LESS_EQUAL ShiftExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpressionWithName(OperatorIds.LESS_EQUAL);  
			break;
 
    case 661 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: RelationalExpression_NotName ::= RelationalExpression_NotName GREATER_EQUAL ShiftExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.GREATER_EQUAL);  
			break;
 
    case 662 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: RelationalExpression_NotName ::= Name GREATER_EQUAL ShiftExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpressionWithName(OperatorIds.GREATER_EQUAL);  
			break;
 
    case 664 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InstanceofExpression_NotName ::= Name instanceof ReferenceType"); }  //$NON-NLS-1$
		    consumeInstanceOfExpressionWithName();  
			break;
 
    case 665 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InstanceofExpression_NotName ::= InstanceofExpression_NotName instanceof ReferenceType"); }  //$NON-NLS-1$
		    consumeInstanceOfExpression();  
			break;
 
    case 667 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EqualityExpression_NotName ::= EqualityExpression_NotName EQUAL_EQUAL InstanceofExpression"); }  //$NON-NLS-1$
		    consumeEqualityExpression(OperatorIds.EQUAL_EQUAL);  
			break;
 
    case 668 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EqualityExpression_NotName ::= Name EQUAL_EQUAL InstanceofExpression"); }  //$NON-NLS-1$
		    consumeEqualityExpressionWithName(OperatorIds.EQUAL_EQUAL);  
			break;
 
    case 669 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EqualityExpression_NotName ::= EqualityExpression_NotName NOT_EQUAL InstanceofExpression"); }  //$NON-NLS-1$
		    consumeEqualityExpression(OperatorIds.NOT_EQUAL);  
			break;
 
    case 670 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EqualityExpression_NotName ::= Name NOT_EQUAL InstanceofExpression"); }  //$NON-NLS-1$
		    consumeEqualityExpressionWithName(OperatorIds.NOT_EQUAL);  
			break;
 
    case 672 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AndExpression_NotName ::= AndExpression_NotName AND EqualityExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.AND);  
			break;
 
    case 673 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AndExpression_NotName ::= Name AND EqualityExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpressionWithName(OperatorIds.AND);  
			break;
 
    case 675 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExclusiveOrExpression_NotName ::= ExclusiveOrExpression_NotName XOR AndExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.XOR);  
			break;
 
    case 676 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExclusiveOrExpression_NotName ::= Name XOR AndExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpressionWithName(OperatorIds.XOR);  
			break;
 
    case 678 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InclusiveOrExpression_NotName ::= InclusiveOrExpression_NotName OR ExclusiveOrExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.OR);  
			break;
 
    case 679 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: InclusiveOrExpression_NotName ::= Name OR ExclusiveOrExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpressionWithName(OperatorIds.OR);  
			break;
 
    case 681 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ConditionalAndExpression_NotName ::= ConditionalAndExpression_NotName AND_AND..."); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.AND_AND);  
			break;
 
    case 682 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ConditionalAndExpression_NotName ::= Name AND_AND InclusiveOrExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpressionWithName(OperatorIds.AND_AND);  
			break;
 
    case 684 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ConditionalOrExpression_NotName ::= ConditionalOrExpression_NotName OR_OR..."); }  //$NON-NLS-1$
		    consumeBinaryExpression(OperatorIds.OR_OR);  
			break;
 
    case 685 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ConditionalOrExpression_NotName ::= Name OR_OR ConditionalAndExpression"); }  //$NON-NLS-1$
		    consumeBinaryExpressionWithName(OperatorIds.OR_OR);  
			break;
 
    case 687 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ConditionalExpression_NotName ::= ConditionalOrExpression_NotName QUESTION Expression COLON..."); }  //$NON-NLS-1$
		    consumeConditionalExpression(OperatorIds.QUESTIONCOLON) ;  
			break;
 
    case 688 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ConditionalExpression_NotName ::= Name QUESTION Expression COLON ConditionalExpression"); }  //$NON-NLS-1$
		    consumeConditionalExpressionWithName(OperatorIds.QUESTIONCOLON) ;  
			break;
 
    case 692 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AnnotationTypeDeclarationHeaderName ::= Modifiers AT PushRealModifiers interface Identifier"); }  //$NON-NLS-1$
		    consumeAnnotationTypeDeclarationHeaderName() ;  
			break;
 
    case 693 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AnnotationTypeDeclarationHeaderName ::= Modifiers AT PushRealModifiers interface Identifier..."); }  //$NON-NLS-1$
		    consumeAnnotationTypeDeclarationHeaderNameWithTypeParameters() ;  
			break;
 
    case 694 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AnnotationTypeDeclarationHeaderName ::= AT PushModifiersForHeader interface Identifier..."); }  //$NON-NLS-1$
		    consumeAnnotationTypeDeclarationHeaderNameWithTypeParameters() ;  
			break;
 
    case 695 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AnnotationTypeDeclarationHeaderName ::= AT PushModifiersForHeader interface Identifier"); }  //$NON-NLS-1$
		    consumeAnnotationTypeDeclarationHeaderName() ;  
			break;
 
    case 696 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AnnotationTypeDeclarationHeader ::= AnnotationTypeDeclarationHeaderName ClassHeaderExtendsopt..."); }  //$NON-NLS-1$
		    consumeAnnotationTypeDeclarationHeader() ;  
			break;
 
    case 697 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AnnotationTypeDeclaration ::= AnnotationTypeDeclarationHeader AnnotationTypeBody"); }  //$NON-NLS-1$
		    consumeAnnotationTypeDeclaration() ;  
			break;
 
    case 699 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AnnotationTypeMemberDeclarationsopt ::="); }  //$NON-NLS-1$
		    consumeEmptyAnnotationTypeMemberDeclarationsopt() ;  
			break;
 
    case 700 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AnnotationTypeMemberDeclarationsopt ::= NestedType AnnotationTypeMemberDeclarations"); }  //$NON-NLS-1$
		    consumeAnnotationTypeMemberDeclarationsopt() ;  
			break;
 
    case 702 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AnnotationTypeMemberDeclarations ::= AnnotationTypeMemberDeclarations..."); }  //$NON-NLS-1$
		    consumeAnnotationTypeMemberDeclarations() ;  
			break;
 
    case 703 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AnnotationMethodHeaderName ::= Modifiersopt TypeParameters Type Identifier LPAREN"); }  //$NON-NLS-1$
		    consumeMethodHeaderNameWithTypeParameters(true);  
			break;
 
    case 704 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AnnotationMethodHeaderName ::= Modifiersopt Type Identifier LPAREN"); }  //$NON-NLS-1$
		    consumeMethodHeaderName(true);  
			break;
 
    case 705 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AnnotationMethodHeaderDefaultValueopt ::="); }  //$NON-NLS-1$
		    consumeEmptyMethodHeaderDefaultValue() ;  
			break;
 
    case 706 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AnnotationMethodHeaderDefaultValueopt ::= DefaultValue"); }  //$NON-NLS-1$
		    consumeMethodHeaderDefaultValue();  
			break;
 
    case 707 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AnnotationMethodHeader ::= AnnotationMethodHeaderName FormalParameterListopt..."); }  //$NON-NLS-1$
		    consumeMethodHeader();  
			break;
 
    case 708 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AnnotationTypeMemberDeclaration ::= AnnotationMethodHeader SEMICOLON"); }  //$NON-NLS-1$
		    consumeAnnotationTypeMemberDeclaration() ;  
			break;
 
    case 716 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: AnnotationName ::= AT Name"); }  //$NON-NLS-1$
		    consumeAnnotationName() ;  
			break;
 
    case 717 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: NormalAnnotation ::= AnnotationName LPAREN MemberValuePairsopt RPAREN"); }  //$NON-NLS-1$
		    consumeNormalAnnotation() ;  
			break;
 
    case 718 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MemberValuePairsopt ::="); }  //$NON-NLS-1$
		    consumeEmptyMemberValuePairsopt() ;  
			break;
 
    case 721 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MemberValuePairs ::= MemberValuePairs COMMA MemberValuePair"); }  //$NON-NLS-1$
		    consumeMemberValuePairs() ;  
			break;
 
    case 722 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MemberValuePair ::= SimpleName EQUAL EnterMemberValue MemberValue ExitMemberValue"); }  //$NON-NLS-1$
		    consumeMemberValuePair() ;  
			break;
 
    case 723 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnterMemberValue ::="); }  //$NON-NLS-1$
		    consumeEnterMemberValue() ;  
			break;
 
    case 724 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ExitMemberValue ::="); }  //$NON-NLS-1$
		    consumeExitMemberValue() ;  
			break;
 
    case 726 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MemberValue ::= Name"); }  //$NON-NLS-1$
		    consumeMemberValueAsName() ;  
			break;
 
    case 729 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MemberValueArrayInitializer ::= EnterMemberValueArrayInitializer LBRACE PushLeftBrace..."); }  //$NON-NLS-1$
		    consumeMemberValueArrayInitializer() ;  
			break;
 
    case 730 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MemberValueArrayInitializer ::= EnterMemberValueArrayInitializer LBRACE PushLeftBrace..."); }  //$NON-NLS-1$
		    consumeMemberValueArrayInitializer() ;  
			break;
 
    case 731 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MemberValueArrayInitializer ::= EnterMemberValueArrayInitializer LBRACE PushLeftBrace COMMA..."); }  //$NON-NLS-1$
		    consumeEmptyMemberValueArrayInitializer() ;  
			break;
 
    case 732 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MemberValueArrayInitializer ::= EnterMemberValueArrayInitializer LBRACE PushLeftBrace RBRACE"); }  //$NON-NLS-1$
		    consumeEmptyMemberValueArrayInitializer() ;  
			break;
 
    case 733 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: EnterMemberValueArrayInitializer ::="); }  //$NON-NLS-1$
		    consumeEnterMemberValueArrayInitializer() ;  
			break;
 
    case 735 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MemberValues ::= MemberValues COMMA MemberValue"); }  //$NON-NLS-1$
		    consumeMemberValues() ;  
			break;
 
    case 736 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: MarkerAnnotation ::= AnnotationName"); }  //$NON-NLS-1$
		    consumeMarkerAnnotation() ;  
			break;
 
    case 737 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: SingleMemberAnnotationMemberValue ::= MemberValue"); }  //$NON-NLS-1$
		    consumeSingleMemberAnnotationMemberValue() ;  
			break;
 
    case 738 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: SingleMemberAnnotation ::= AnnotationName LPAREN SingleMemberAnnotationMemberValue RPAREN"); }  //$NON-NLS-1$
		    consumeSingleMemberAnnotation() ;  
			break;
 
    case 739 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: RecoveryMethodHeaderName ::= Modifiersopt TypeParameters Type Identifier LPAREN"); }  //$NON-NLS-1$
		    consumeRecoveryMethodHeaderNameWithTypeParameters();  
			break;
 
    case 740 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: RecoveryMethodHeaderName ::= Modifiersopt Type Identifier LPAREN"); }  //$NON-NLS-1$
		    consumeRecoveryMethodHeaderName();  
			break;
 
    case 741 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: RecoveryMethodHeader ::= RecoveryMethodHeaderName FormalParameterListopt MethodHeaderRightParen"); }  //$NON-NLS-1$
		    consumeMethodHeader();  
			break;
 
    case 742 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: RecoveryMethodHeader ::= RecoveryMethodHeaderName FormalParameterListopt MethodHeaderRightParen"); }  //$NON-NLS-1$
		    consumeMethodHeader();  
			break;
 
    case 743 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ConstraintClause ::= where ConstraintList"); }  //$NON-NLS-1$
		    consumeConstraintClause();  
			break;
 
    case 745 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ConstraintList ::= ConstraintList COMMA Constraint"); }  //$NON-NLS-1$
		    consumeConstraintList();  
			break;
 
    case 746 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Constraint ::= ReferenceType extends ReferenceType"); }  //$NON-NLS-1$
		    consumeExtendsConstraint ();  
			break;
 
    case 747 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: Constraint ::= ImplConstraintLeftList implements ReferenceType"); }  //$NON-NLS-1$
		    consumeImplementsConstraint ();  
			break;
 
    case 748 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplConstraintLeft ::= ReferenceType"); }  //$NON-NLS-1$
		    consumeImplConstraintLeft ();  
			break;
 
    case 750 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplConstraintLeftList ::= ImplConstraintLeftList MULTIPLY ImplConstraintLeft"); }  //$NON-NLS-1$
		    consumeImplConstraintLeftList ();  
			break;
 
    case 751 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplConstraintClause ::= where ImplConstraintList"); }  //$NON-NLS-1$
		    consumeConstraintClause();  
			break;
 
    case 753 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplConstraintList ::= ImplConstraintList COMMA ImplConstraint"); }  //$NON-NLS-1$
		    consumeConstraintList();  
			break;
 
    case 754 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplConstraint ::= ImplConstraintLeftList implements ReferenceType"); }  //$NON-NLS-1$
		    consumeImplementsConstraint ();  
			break;
 
    case 755 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ReceiverDeclaration ::= ReceiverDeclarationHeader ReceiverDeclarationBody"); }  //$NON-NLS-1$
		    consumeReceiverDeclaration ();  
			break;
 
    case 756 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ReceiverDeclarationHeader ::= receiver Identifier"); }  //$NON-NLS-1$
		    consumeReceiverDeclarationHeader ();  
			break;
 
     case 758 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ReceiverMemberDeclarationsopt ::="); }  //$NON-NLS-1$
		    consumeEmptyReceiverMemberDeclarationsopt();  
			break;
 
     case 759 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ReceiverMemberDeclarationsopt ::= ReceiverMemberDeclarations"); }  //$NON-NLS-1$
		    consumeReceiverMemberDeclarationsopt();  
			break;
 
    case 761 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ReceiverMemberDeclarations ::= ReceiverMemberDeclarations ReceiverMemberDeclaration"); }  //$NON-NLS-1$
		    consumeReceiverMemberDeclarations();  
			break;
 
    case 763 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplementationDeclaration ::= ImplementationHeader ImplementationBody"); }  //$NON-NLS-1$
		    consumeImplementationDeclaration();  
			break;
 
    case 764 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplementationHeader ::= ImplementationHeader1 ImplementationHeaderHead..."); }  //$NON-NLS-1$
		    consumeImplementationHeader();  
			break;
 
    case 766 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplementationHeader1 ::= ImplementationHeader2 TypeParameters"); }  //$NON-NLS-1$
		    consumeImplementationHeader1WithTypeParameters();  
			break;
 
    case 767 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplementationHeader2 ::= Modifiersopt implementation"); }  //$NON-NLS-1$
		    consumeImplementationHeader2();  
			break;
 
    case 768 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplementationHeaderHead ::= InterfaceType LBRACKET ClassTypeList RBRACKET"); }  //$NON-NLS-1$
		    consumeImplementationHeaderHead();  
			break;
 
    case 769 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplementationHeaderName ::= SimpleName SimpleName"); }  //$NON-NLS-1$
		    consumeImplementationHeaderName();  
			break;
 
    case 770 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplementationHeaderConstraints ::= ConstraintClause"); }  //$NON-NLS-1$
		    consumeImplementationHeaderConstraints();  
			break;
 
    case 771 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplementationHeaderExtends ::= extends InterfaceType LBRACKET ClassTypeList RBRACKET"); }  //$NON-NLS-1$
		    consumeImplementationHeaderExtends();  
			break;
 
    case 772 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplementationHeaderExtends ::= extends InterfaceType"); }  //$NON-NLS-1$
		    consumeImplementationHeaderExtends2();  
			break;
 
    case 775 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplementationBodyDeclarations ::= ImplementationBodyDeclarations ImplementationBodyDeclaration"); }  //$NON-NLS-1$
		    consumeImplementationBodyDeclarations();  
			break;
 
    case 778 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplementationReceiverDeclaration ::= ImplementationReceiverDeclarationHeader LBRACE..."); }  //$NON-NLS-1$
		    consumeImplementationReceiverDeclaration();  
			break;
 
    case 779 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplementationReceiverDeclarationHeader ::= receiver ReferenceType"); }  //$NON-NLS-1$
		    consumeImplementationReceiverDeclarationHeader();  
			break;
 
    case 781 : if (javagi.compiler.GILog.Parsing().isFine()) { javagi.compiler.GILog.Parsing().jfine("RULE: ImplementationReceiverBodyDeclarations ::= ImplementationReceiverBodyDeclarations..."); }  //$NON-NLS-1$
		    consumeImplementationReceiverBodyDeclarations();  
			break;
 
	}
}
//*END_INPUT* JavaAction.java
...
//			this.scanner.commentPtr = -1;
//	}
	if (javagi.compiler.GILog.Parsing().isFine()) {
	    javagi.compiler.GILog.Parsing().jfine("TOKEN: " + this.scanner.toStringAction(type));
	}
	switch (type) {
...
protected void consumeTypeParameter1WithBound(int boundKind) {
	...
	typeParameter.boundKind = boundKind;
}
protected void consumeTypeParameter1WithBoundAndBounds(int boundKind) {
...
protected void consumeTypeParameter1WithExtends() {
    consumeTypeParameter1WithBound(TypeParameter.EXTENDS_BOUND);
}
protected void consumeTypeParameter1WithExtendsAndBounds() {
    consumeTypeParameter1WithBoundAndBounds(TypeParameter.EXTENDS_BOUND);
}
protected void consumeTypeParameter1WithImplements() {
    consumeTypeParameter1WithBound(TypeParameter.IMPLEMENTS_BOUND);   
}
protected void consumeTypeParameter1WithImplementsAndBounds() {
    consumeTypeParameter1WithBoundAndBounds(TypeParameter.IMPLEMENTS_BOUND);
}
...
protected void consumeTypeParameterWithBound(int boundKind) {
...
protected void consumeTypeParameterWithBoundAndBounds(int boundKind) {
...
protected void consumeTypeParameterWithExtends() {
    consumeTypeParameterWithBound(TypeParameter.EXTENDS_BOUND);
}
protected void consumeTypeParameterWithExtendsAndBounds() {
    consumeTypeParameterWithBoundAndBounds(TypeParameter.EXTENDS_BOUND);
}
protected void consumeTypeParameterWithImplements() {
    consumeTypeParameterWithBound(TypeParameter.IMPLEMENTS_BOUND);   
}
protected void consumeTypeParameterWithImplementsAndBounds() {
    consumeTypeParameterWithBoundAndBounds(TypeParameter.IMPLEMENTS_BOUND);
}
...
// SW
protected void consumeWildcardBounds1Implements() {
    Wildcard wildcard = new Wildcard(Wildcard.IMPLEMENTS);
    wildcard.bound = (TypeReference) this.genericsStack[this.genericsPtr];
    wildcard.sourceEnd = wildcard.bound.sourceEnd;
    this.intPtr--; // remove end position of the '?'
    wildcard.sourceStart = this.intStack[this.intPtr--];
...
// SW
protected void consumeWildcardBounds2Implements() {
    Wildcard wildcard = new Wildcard(Wildcard.IMPLEMENTS);
    wildcard.bound = (TypeReference) this.genericsStack[this.genericsPtr];
    wildcard.sourceEnd = wildcard.bound.sourceEnd;
    this.intPtr--; // remove end position of the '?'
...
// SW
protected void consumeWildcardBounds3Implements() {
    Wildcard wildcard = new Wildcard(Wildcard.IMPLEMENTS);
    wildcard.bound = (TypeReference) this.genericsStack[this.genericsPtr];
    wildcard.sourceEnd = wildcard.bound.sourceEnd;
    this.intPtr--; // remove end position of the '?'
...
// SW
protected void consumeWildcardBoundsImplements() {
    Wildcard wildcard = new Wildcard(Wildcard.IMPLEMENTS);
    wildcard.bound = getTypeReference(this.intStack[this.intPtr--]);
    wildcard.sourceEnd = wildcard.bound.sourceEnd;
    this.intPtr--; // remove end position of the '?'
...
protected TypeReference getTypeReference(int dim) {
	...
	//System.out.println("getTypeReference(" + dim + ") = " + ref);
	return ref;
}
...
/*
protected Reference getUnspecifiedReference() {
	// build a (unspecified) NameReference which may be qualified
	...
	if ((length = this.identifierLengthStack[this.identifierLengthPtr--]) == 1)
		...
		Expression recv = new SingleNameReference(tokens[0], positions[0]);
		FieldReference fr = null;
		for (int i = 1; i < tokens.length; i++) {
            fr = new FieldReference(tokens[i], positions[i]);
            fr.receiver = recv;
            fr.sourceStart = fr.receiver.sourceStart;
            recv = fr;
        }
...
*/
protected Reference getUnspecifiedReferenceOptimized() {
	/* build a (unspecified) NameReference which may be qualified
...
protected void pushIdentifier() {
	...
	if (DEBUG_STACK) System.out.println("Pushing " + new String(this.identifierStack[this.identifierPtr]) + 
	                                    " on identifierStack");
	this.identifierPositionStack[this.identifierPtr] =
	...
	if (DEBUG_STACK) System.out.println("Pushing 1 on identifierLengthStack");
}
protected void pushIdentifier(int flag) {
	...
	if (DEBUG_STACK) System.out.println("Pushing " + this.identifierLengthStack[this.identifierLengthPtr] + 
                                         " on identifierStack");
}
protected void pushOnAstLengthStack(int pos) {
	...
	if (DEBUG_STACK) System.out.println("Pushing " + this.astLengthStack[this.astLengthPtr] +
	                                    " on astLengthStack");
}
protected void pushOnAstStack(ASTNode node) {
	...
	if (DEBUG_STACK) {
      System.out.println("Pushing " + this.astStack[this.astPtr] + " on astStack");
      int i = this.astPtr;
      while (i >= 0) {
        System.out.println("astStack[" + i + "] = " + this.astStack[i]);
        i--;
	...
	this.astLengthStack[this.astLengthPtr] = 1;
	if (DEBUG_STACK) { 
      System.out.println("Pushing 1 on astLengthStack");
...
protected void pushOnGenericsStack(ASTNode node) {
	...
	if (DEBUG_STACK) {
	    System.out.println("Pushing " + this.genericsStack[this.genericsPtr] +
	                                    " on genericsStack");
	    int i = this.genericsPtr;
	      while (i >= 0) {
	        System.out.println("genericsStack[" + i + "] = " + this.genericsStack[i]);
	        i--;
	      }    
	}
	...
	if (DEBUG_STACK) {
	    System.out.println("Pushing 1 on genericsLengthStack");
	       int i = this.genericsLengthPtr;
	          while (i >= 0) {
	            System.out.println("genericsLengthStack[" + i + "] = " + this.genericsLengthStack[i]);
	            i--;
	          }  
	}
}
protected void pushOnGenericsIdentifiersLengthStack(int pos) {
	...
	if (DEBUG_STACK) {
	    System.out.println("Pushing " + this.genericsIdentifiersLengthStack[this.genericsIdentifiersLengthPtr]
	                                    + " on genericsIdentifierLengthStack");
        int i = this.genericsIdentifiersLengthPtr;
        while (i >= 0) {
          System.out.println("genericsIdentifiersLengthStack[" + i + "] = " + this.genericsIdentifiersLengthStack[i]);
          i--;
        }  
	}
}
protected void pushOnGenericsLengthStack(int pos) {
	...
	if (DEBUG_STACK) {
	    System.out.println("Pushing " + this.genericsLengthStack[this.genericsLengthPtr] +
	                                    " on genericsLengthStack");
        int i = this.genericsLengthPtr;
        while (i >= 0) {
          System.out.println("genericsLengthStack[" + i + "] = " + this.genericsLengthStack[i]);
          i--;
...
protected void pushOnIntStack(int pos) {
	...
	if (DEBUG_STACK) System.out.println("Pushing " + this.intStack[this.intPtr] + " on intStack");
}
...
protected void reportSyntaxErrors(boolean isDietParse, int oldFirstToken) {
	...
	if(isDietParse) {
		...
		int[][] intervalToSkip = javagi.eclipse.jdt.internal.compiler.parser.diagnose.RangeUtil.computeDietRange(types);
...
protected void consumeHeaderConstraints() {
    int len = this.astLengthStack[this.astLengthPtr--];
    this.astPtr = this.astPtr - len;
    TypeDeclaration typeDecl = (TypeDeclaration) this.astStack[this.astPtr];
    typeDecl.constraints = new Constraint[len];
    System.arraycopy(this.astStack, this.astPtr+1,
            typeDecl.constraints, 0, len);
}
protected void consumeClassHeaderConstraints() {
    javagi.compiler.GILog.Parsing().jfine("consumeClassHeaderConstraints");
    consumeHeaderConstraints();
}

protected void consumeInterfaceHeaderConstraints() {
    javagi.compiler.GILog.Parsing().jfine("consumeInterfaceHeaderConstraints");
    consumeHeaderConstraints();
}

protected void consumeMethodHeaderConstraints() {
    javagi.compiler.GILog.Parsing().jfine("consumeMethodHeaderConstraints");
    int len = this.astLengthStack[this.astLengthPtr--];
    this.astPtr = this.astPtr - len;
    AbstractMethodDeclaration methodDecl = (AbstractMethodDeclaration) this.astStack[this.astPtr];
    methodDecl.constraints = new Constraint[len];
    System.arraycopy(this.astStack, this.astPtr+1,
            methodDecl.constraints, 0, len);
    // System.out.println("==> " + methodDecl);
}

protected void consumeConstraintList() {
    javagi.compiler.GILog.Parsing().jfine("consumeConstraintList");
    concatNodeLists();
}
protected void consumeConstraintClause() {
    javagi.compiler.GILog.Parsing().jfine("consumeConstraintClause");
    this.intPtr--; // start position of where
    //Constraint c = (Constraint) this.astStack[this.astPtr];
    //c.sourceStart = this.intStack[this.intPtr--]; // start position of 'where'
    // System.out.println("==> " + c + ", start: " + c.sourceStart + ", end: " + c.sourceEnd);
}
protected void consumeExtendsConstraint() {
    javagi.compiler.GILog.Parsing().jfine("consumeExtendsConstraint");
    TypeReference rhs = getTypeReference(this.intStack[this.intPtr--]);
    TypeReference lhs = getTypeReference(this.intStack[this.intPtr--]);
    Constraint c = new Constraint(Constraint.EXTENDS_CONSTRAINT);
    // if rhs is a generic type, then the sourceEnd is wrong; don't know how to fix this
    c.sourceStart = lhs.sourceStart();
    c.sourceEnd = rhs.sourceEnd();
    c.constrainedTypes = new TypeReference[]{lhs};
    c.constrainingType = rhs;
    pushOnAstStack(c);
}

protected void consumeImplementsConstraint() {
    javagi.compiler.GILog.Parsing().jfine("consumeImplementsConstraint");
    Constraint c = new Constraint(Constraint.IMPLEMENTS_CONSTRAINT);
    c.constrainingType = getTypeReference(this.intStack[this.intPtr--]);
    int len = this.genericsLengthStack[this.genericsLengthPtr--];
    c.constrainedTypes = new TypeReference[len];
    this.genericsPtr = this.genericsPtr - len;
    System.arraycopy(this.genericsStack, this.genericsPtr+1, 
            c.constrainedTypes, 0, len);
    c.sourceStart = c.constrainedTypes[0].sourceStart();
    //if c.constrainingType is a generic type, then the sourceEnd is wrong; don't know how to fix this
    c.sourceEnd = c.constrainingType.sourceEnd();
    pushOnAstStack(c);
}

protected void consumeImplConstraintLeft() {
    javagi.compiler.GILog.Parsing().jfine("consumeImplConstraintLeft");
    pushOnGenericsStack(getTypeReference(this.intStack[this.intPtr--]));
}

protected void consumeImplConstraintLeftList() {
    javagi.compiler.GILog.Parsing().jfine("consumeImplConstraintLeftList");
    concatGenericsLists();
}

/*
protected void consumeMonoConstraint() {
    javagi.compiler.GILog.Parsing().jfine("consumeMonoConstraint");
    Constraint c = new Constraint(Constraint.MONO_CONSTRAINT);
    c.sourceEnd = this.intStack[this.intPtr--]; // end position of 'mono'
    TypeReference t = getTypeReference(this.intStack[this.intPtr--]);
    c.sourceStart = t.sourceStart();
    c.constrainedTypes = new TypeReference[]{t};
    pushOnAstStack(c);
}
*/

protected void consumeInterfaceHeaderImplConstraints() {
    javagi.compiler.GILog.Parsing().jfine("consumeInterfaceHeaderImplConstraints");
    int len = this.astLengthStack[this.astLengthPtr--];
    this.astPtr = this.astPtr - len;
    TypeDeclaration typeDecl = (TypeDeclaration) this.astStack[this.astPtr];
    typeDecl.implConstraints = new Constraint[len];
    System.arraycopy(this.astStack, this.astPtr+1,
                     typeDecl.implConstraints, 0, len);
}
protected void consumeInterfaceHeaderImplParameterList() {
    javagi.compiler.GILog.Parsing().jfine("consumeInterfaceHeaderImplParameterList");
    concatGenericsLists();
}
protected void consumeInterfaceHeaderImplParameters() {
    javagi.compiler.GILog.Parsing().jfine("consumeInterfaceHeaderImplParameters");
    TypeDeclaration typeDecl = (TypeDeclaration) this.astStack[this.astPtr];
    int length = this.genericsLengthStack[this.genericsLengthPtr--];
    this.genericsPtr -= length;
    typeDecl.implParameters = new TypeParameter[length];
    System.arraycopy(this.genericsStack, this.genericsPtr + 1, typeDecl.implParameters, 0, length);
    typeDecl.bodyStart = typeDecl.implParameters[length-1].declarationSourceEnd + 1;
}

protected void consumeReceiverDeclarationHeader() {
    javagi.compiler.GILog.Parsing().jfine("consumeReceiverDeclarationHeader");
    TypeDeclaration typeDecl = new TypeDeclaration(this.compilationUnit.compilationResult);
    typeDecl.modifiers = ClassFileConstants.AccReceiver;
    //highlight the name of the type
    long pos = this.identifierPositionStack[this.identifierPtr];
    typeDecl.sourceEnd = (int) pos;
    typeDecl.sourceStart = (int) (pos >>> 32);
    
    // fix the name: we cannot use the type variable is the name of the
    // receiver declaration, because receiver declarations are modelled as member types,
    // which introduce a new name. Hence, we create a new, artificial name and
    // record the original name in a magic field. (We cannot refer directly to the type
    // variable as a type because of the error
    // "Cannot make a static reference to the non-static type X".)
    char[] realName = this.identifierStack[this.identifierPtr--];
    FieldDeclaration magicField = new FieldDeclaration(javagi.compiler.Constants.MAGIC_RECEIVER_FIELD_NAME, 0, 0);
    magicField.type = new SingleTypeReference(realName, typeDecl.sourceStart);
    if (typeDecl.fields == null) {
        typeDecl.fields = new FieldDeclaration[]{magicField};
    } else {
        FieldDeclaration[] fs = new FieldDeclaration[1 + typeDecl.fields.length];
        fs[0] = magicField;
        System.arraycopy(typeDecl.fields, 0, fs, 1, typeDecl.fields.length);
        typeDecl.fields = fs;
    }
    char[] prefix = javagi.compiler.Constants.RECEIVER_VAR_PREFIX;
    char[] suffix = javagi.compiler.Constants.RECEIVER_VAR_SUFFIX; 
    char[] fixedName = new char[prefix.length + realName.length + suffix.length];
    System.arraycopy(prefix, 0, fixedName, 0, prefix.length);
    System.arraycopy(realName, 0, fixedName, prefix.length, realName.length);
    System.arraycopy(suffix, 0, fixedName, prefix.length + realName.length, suffix.length);
    typeDecl.receiverVariable = realName;
    typeDecl.name = fixedName;
...
protected void consumeReceiverDeclaration() { 
    javagi.compiler.GILog.Parsing().jfine("consumeReceiverDeclaration");
    int length = this.astLengthStack[this.astLengthPtr--];
    this.astPtr -= length;
    TypeDeclaration typeDecl = (TypeDeclaration) this.astStack[this.astPtr];
    System.arraycopy(this.astStack, this.astPtr + 1, typeDecl.methods = new AbstractMethodDeclaration[length], 0, length);
    for (AbstractMethodDeclaration m: typeDecl.methods) {
        m.modifiers |= ClassFileConstants.AccAbstract | ClassFileConstants.AccPublic;
    }
    typeDecl.modifiers |= ClassFileConstants.AccAbstract;
    typeDecl.bodyEnd = this.endStatementPosition;
    typeDecl.declarationSourceEnd = flushCommentsDefinedPriorTo(this.endStatementPosition); 
}
protected void consumeEmptyReceiverMemberDeclarationsopt() {
    javagi.compiler.GILog.Parsing().jfine("consumeEmptyReceiverMemberDeclarationsopt");
    pushOnAstLengthStack(0);
}
protected void consumeReceiverMemberDeclarationsopt() {
    javagi.compiler.GILog.Parsing().jfine("consumeReceiverMemberDeclarationsopt");
}
protected void consumeReceiverMemberDeclarations() {
    javagi.compiler.GILog.Parsing().jfine("consumeReceiverMemberDeclarations");
    concatNodeLists();
}

protected void consumeImplementationHeader2() {
    javagi.compiler.GILog.Parsing().jfine("consumeImplementationHeader2");
    // copied from consumeClassHeaderName1
    TypeDeclaration typeDecl = new TypeDeclaration(this.compilationUnit.compilationResult);
    if (this.nestedMethod[this.nestedType] == 0) {
        if (this.nestedType != 0) {
            typeDecl.bits |= ASTNode.IsMemberType;
    ...
    //compute the declaration source too
    // 'implementation' pushes two int positions: the beginning of the class token and its end.
    // we want to keep the beginning position but get rid of the end position.
    typeDecl.declarationSourceStart = this.intStack[this.intPtr--]; 
    this.intPtr--; // remove the end position of the class token

    typeDecl.modifiersSourceStart = this.intStack[this.intPtr--];
    typeDecl.modifiers = ClassFileConstants.AccImplementation | this.intStack[this.intPtr--];
    if (typeDecl.modifiersSourceStart >= 0) {
        typeDecl.declarationSourceStart = typeDecl.modifiersSourceStart;
    }
...
protected void consumeImplementationHeader1WithTypeParameters() {
    javagi.compiler.GILog.Parsing().jfine("consumeImplementationHeader1WithTypeParameters");
    int length = this.genericsLengthStack[this.genericsLengthPtr--];
    TypeDeclaration typeDecl = (TypeDeclaration) this.astStack[this.astPtr];
    typeDecl.typeParameters = new TypeParameter[length];
    this.genericsPtr = this.genericsPtr - length;
    System.arraycopy(this.genericsStack, this.genericsPtr + 1, typeDecl.typeParameters, 0, length);
}
protected void consumeImplementationHeaderHead() {
    javagi.compiler.GILog.Parsing().jfine("consumeImplementationHeaderHead");
    int length = this.astLengthStack[this.astLengthPtr--]; // consume implementing types
    int implStart = this.astPtr - length + 1;
    int iface = this.astPtr - length;
    this.astPtr = this.astPtr - length - 1;
    this.astLengthPtr--; // consume iface
    if (DEBUG_STACK) {
      int i = this.astPtr;
      while (i >= 0) {
        System.out.println("astStack[" + i + "] = " + this.astStack[i]);
    ...
    TypeDeclaration typeDecl = (TypeDeclaration) this.astStack[this.astPtr]; 
    typeDecl.interfaceType = (TypeReference) this.astStack[iface];
    System.arraycopy(this.astStack, implStart, typeDecl.implTypes = new TypeReference[length], 0, length);
    // set positions
    typeDecl.sourceStart = typeDecl.interfaceType.sourceStart;
    typeDecl.sourceEnd = this.endPosition; // position of ']'
    // compute the name of the dictionary
    if (typeDecl.name == null) {
        typeDecl.name = Naming.parserGeneratedDictionaryName(typeDecl).toCharArray();
    }
}
protected void consumeImplementationHeader() {
    javagi.compiler.GILog.Parsing().jfine("consumeImplementationHeader");
    TypeDeclaration typeDecl = (TypeDeclaration) this.astStack[this.astPtr];    
    if (this.currentToken == TokenNameLBRACE) { 
        typeDecl.bodyStart = this.scanner.currentPosition;
    }
...
protected void consumeImplementationHeaderExtends() {
    javagi.compiler.GILog.Parsing().jfine("consumeImplementationHeaderExtends");
    int length = this.astLengthStack[this.astLengthPtr--]; // consume implementing types
    int implStart = this.astPtr - length + 1;
    int iface = this.astPtr - length;
    this.astPtr = this.astPtr - length - 1;
    ...
    Constraint extendsConstraint = new Constraint(Constraint.IMPLEMENTS_CONSTRAINT);
    extendsConstraint.constrainingType = (TypeReference) this.astStack[iface];
    System.arraycopy(this.astStack, implStart, extendsConstraint.constrainedTypes = new TypeReference[length], 0, length);
    TypeDeclaration typeDecl = (TypeDeclaration) this.astStack[this.astPtr];
    typeDecl.extendsConstraint = extendsConstraint;
}
protected void consumeImplementationHeaderExtends2() {
    javagi.compiler.GILog.Parsing().jfine("consumeImplementationHeaderExtends2");
    TypeReference sup = (TypeReference) this.astStack[this.astPtr--];
    this.astLengthPtr--;
    TypeDeclaration typeDecl = (TypeDeclaration) this.astStack[this.astPtr];
    typeDecl.extendsImplementation = sup;
}
protected void consumeImplementationHeaderName() {
    javagi.compiler.GILog.Parsing().jfine("consumeImplementationHeaderName");
    TypeDeclaration typeDecl = (TypeDeclaration) this.astStack[this.astPtr];
    typeDecl.name = this.identifierStack[this.identifierPtr--];
    typeDecl.explicitImplementationName = true;
    this.identifierLengthPtr--;
    
    String as = new String(this.identifierStack[this.identifierPtr--]);
    this.identifierLengthPtr--;
    if (! "as".equals(as)) {
        this.problemReporter().javaGIProblem(typeDecl, "token 'as' expected in front of the name of the implementation");
    }
}
protected void consumeImplementationHeaderConstraints() {
    javagi.compiler.GILog.Parsing().jfine("consumeImplementationHeaderConstraints");
    consumeClassHeaderConstraints();
}
protected void consumeImplementationBodyDeclarations() {
    javagi.compiler.GILog.Parsing().jfine("consumeImplementationBodyDeclarations");
    concatNodeLists();
}
protected void consumeEmptyImplementationBodyDeclarationsopt() {
    javagi.compiler.GILog.Parsing().jfine("consumeEmptyImplementationBodyDeclarationsopt");
    pushOnAstLengthStack(0);
}
protected void consumeImplementationBodyDeclarationsopt() {
    javagi.compiler.GILog.Parsing().jfine("consumeImplementationBodyDeclarationsopt");
    this.nestedType--;
}
protected void consumeImplementationDeclaration() {
    javagi.compiler.GILog.Parsing().jfine("consumeImplementationDeclaration");
    int length;
    if ((length = this.astLengthStack[this.astLengthPtr--]) != 0) {
        //there are length declarations
        //dispatch according to the type of the declarations
    ...
    ConstructorDeclaration constrDecl = typeDecl.createDefaultConstructor(false, true, true);
    constrDecl.modifiers |= ClassFileConstants.AccPublic;
    typeDecl.bodyEnd = this.endStatementPosition;
    if (length == 0 && !containsComment(typeDecl.bodyStart, typeDecl.bodyEnd)) {
        typeDecl.bits |= ASTNode.UndocumentedEmptyBlock;
    }
...
protected void consumeImplementationReceiverDeclarationHeader() {
    javagi.compiler.GILog.Parsing().jfine("consumeImplementationReceiverDeclarationHeader");
    TypeDeclaration typeDecl = new TypeDeclaration(this.compilationUnit.compilationResult);
    typeDecl.modifiers = ClassFileConstants.AccImplementationReceiver;

    // receiver type
    TypeReference recvType = getTypeReference(this.intStack[this.intPtr--]);
    typeDecl.receiverType = recvType;
    // receiver declaration, because receiver declarations are modelled as member types,
    // which introduce a new name. Hence, we create a new, artificial name.
    char[] realName = CharOperation.toString(recvType.getParameterizedTypeName()).toCharArray();
    char[] prefix = javagi.compiler.Constants.RECEIVER_VAR_PREFIX;
    char[] suffix = javagi.compiler.Constants.RECEIVER_VAR_SUFFIX; 
    char[] fixedName = new char[prefix.length + realName.length + suffix.length];
    System.arraycopy(prefix, 0, fixedName, 0, prefix.length);
...
protected void consumeImplementationReceiverDeclaration() {
    javagi.compiler.GILog.Parsing().jfine("consumeImplementationReceiverDeclaration");
    int length = this.astLengthStack[this.astLengthPtr--];
    this.astPtr -= length;
    TypeDeclaration typeDecl = (TypeDeclaration) this.astStack[this.astPtr];
    System.arraycopy(this.astStack, this.astPtr + 1, typeDecl.methods = new AbstractMethodDeclaration[length], 0, length);
...
protected void consumeImplementationReceiverBodyDeclarations() {
    javagi.compiler.GILog.Parsing().jfine("consumeImplementationReceiverBodyDeclarations");
    concatNodeLists();
}

protected void consumeEmptyImplementationReceiverBodyDeclarationsopt() {
    javagi.compiler.GILog.Parsing().jfine("consumeEmptyImplementationReceiverBodyDeclarationsopt");
    pushOnAstLengthStack(0);
}
protected void consumeImplementationReceiverBodyDeclarationsopt() {
    javagi.compiler.GILog.Parsing().jfine("consumeImplementationReceiverBodyDeclarationsopt");
    this.nestedType--;
}

protected void consumeMethodInvocationStaticInterface(boolean hasTyargs) {
    MessageSend m = newMessageSend();
    m.sourceEnd = this.rParenPos;
    m.sourceStart = 
        (int) ((m.nameSourcePosition = this.identifierPositionStack[this.identifierPtr]) >>> 32); 
    ...
    // handle implementing types
    int length = this.astLengthStack[this.astLengthPtr--];
    this.astPtr -= length;
    System.arraycopy(this.astStack, this.astPtr + 1, m.implTypes = new TypeReference[length], 0, length);
    
    m.receiver = getUnspecifiedReference();
    m.sourceStart = m.receiver.sourceStart;     
    pushOnExpressionStack(m);
}
protected void consumeMethodInvocationStaticInterface() {
    javagi.compiler.GILog.Parsing().jfine("consumeMethodInvocationStaticInterface");
    consumeMethodInvocationStaticInterface(false);
}
protected void consumeMethodInvocationStaticInterfaceWithTypeArguments() {
    javagi.compiler.GILog.Parsing().jfine("consumeMethodInvocationStaticInterfaceWithTypeArguments");
    consumeMethodInvocationStaticInterface(true);
}

protected void consumeInterfaceSpecifier() {
    pushOnAstStack(getUnspecifiedReference());
}

protected void consumeEmptyInterfaceSpecifieropt() {
    pushOnAstStack(null);
}

protected void consumeSpecialForm() {
    // handle arguments
    int length;
    Expression[] args;
    if ((length = this.expressionLengthStack[this.expressionLengthPtr--]) != 0) {
    ...
    // setup positions
    ExplicitCoercion c = new ExplicitCoercion();
    c.sourceEnd = this.rParenPos;
    c.sourceStart = 
        (int) ((c.nameSourcePosition = this.identifierPositionStack[this.identifierPtr]) >>> 32); 
    
    // handle name
    String name = new String(this.identifierStack[this.identifierPtr--]);
    if (! "coerce".equals(name)) {
        this.problemReporter().javaGIProblem(c, "unknown special form '" + name + "'");
    } else {
        c.setArguments(args, this.problemReporter());
    }
    ...
    if (this.identifierLengthStack[this.identifierLengthPtr] == 1) {
        ...
        this.identifierLengthStack[this.identifierLengthPtr]--;
        NameReference r = getUnspecifiedReference();
        c.sourceStart = r.sourceStart;     
        this.problemReporter().javaGIProblem(c, "special forms do not support receivers");
    }
    pushOnExpressionStack(c);   
}
/*
protected void consumeWrapperCreationExpression() {
    classInstanceCreation(false);
    AllocationExpression exp = (AllocationExpression) this.expressionStack[this.expressionPtr];
    String wrapper = new String(this.identifierStack[this.identifierPtr--]);
    this.identifierLengthPtr--;
    if (! "wrapper".equals(wrapper)) {
        this.problemReporter().javaGIProblem((Expression) exp, "token 'wrapper' after 'new'");
    }
    exp.isWrapperAllocation = true;
    //System.out.println("consumeWrapperCreationExpression: " + exp.getClass());
}
protected void consumeImplementationCreationExpression() {
    classInstanceCreation(false);
    AllocationExpression exp = (AllocationExpression) this.expressionStack[this.expressionPtr];
    //System.out.println("consumeImplementationCreationExpression: exp = " + exp.getClass());
    TypeReference iface = getTypeReference(0);
    this.intPtr--; // consume 'implementation' start position
    exp.isImplementationAllocation = true;
    exp.interfaceType = iface;
    //System.out.println("consumeImplementationCreationExpression: iface = " + iface);
}
protected void consumeImplementationCreationExpression2() {
    classInstanceCreation(false);
    AllocationExpression exp = (AllocationExpression) this.expressionStack[this.expressionPtr];   
    this.intPtr--; // consume 'implementation' start position
    exp.isImplementationAllocation = true;
    //System.out.println("consumeImplementationCreationExpression2: exp = " + exp.getClass());
}
*/
}