...
public class CompilerOptions {
	...
	// changed by SW
	public final long complianceLevel = ClassFileConstants.JDK1_6; // by default be compliant with 1.4
	public final long sourceLevel = ClassFileConstants.JDK1_6; //1.3 source behavior by default
	public final long targetJDK = ClassFileConstants.JDK1_6; // default generates for JVM1.2
	...
	public void set(Map optionsMap) {
		...
		if ((optionValue = optionsMap.get(OPTION_Compliance)) != null) {
			...
			if (level != this.complianceLevel) {
			    System.err.println("ignoring change of compliance level");
			}
			//if (level != 0) this.complianceLevel = level;
		}
		if ((optionValue = optionsMap.get(OPTION_Source)) != null) {
            long level = versionToJdkLevel(optionValue);
	        if (level != this.sourceLevel) {
	            System.err.println("ignoring change of source level");
	        }
			//if (level != 0) this.sourceLevel = level;
		}
		if ((optionValue = optionsMap.get(OPTION_TargetPlatform)) != null) {
			long level = versionToJdkLevel(optionValue);
	         if (level != this.targetJDK) {
	                System.err.println("ignoring change of target level");
	         }
			/*
			if (level != 0) {
    ...
    public String toString() {
		...
		buf.append("\n\t- dumpAST : ").append(this.dumpAST ? "ON" : "OFF"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		buf.append("\n\t- stopAfterPhase : ").append(this.stopAfterPhase); //$NON-NLS-1$ 
    ...
    public static final String PARSE_PHASE = "parsing";
    public static final String[] VALID_PHASES = new String[]{PARSE_PHASE};
