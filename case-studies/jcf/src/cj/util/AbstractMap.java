...
public abstract class AbstractMap<K,V,M> implements Map<K,V,M> {
    /**
	...
	Iterator<Entry<K,V,Object>,Object> i = entrySet().iterator();
	if (value==null) {
	    while (i.hasNext()) {
		Entry<K,V,?> e = i.next();
		if (e.getValue()==null)
	...
	Iterator<Map.Entry<K,V,Object>,Object> i = entrySet().iterator();
	if (key==null) {
	    while (i.hasNext()) {
		Entry<K,V,?> e = i.next();
		if (e.getKey()==null)
	...
	Iterator<Entry<K,V,M>, Shrinkable> i = shrinkableEntrySet().iterator();
	Entry<K,V,M> correctEntry = null;
	if (key==null) {
	    while (correctEntry==null && i.hasNext()) {
		Entry<K,V,M> e = i.next();
		if (e.getKey()==null)
		    correctEntry = e;
	    }
	} else {
     ...
     */
    transient volatile Set<K,Shrinkable>        keySet = null;
    transient volatile Collection<V,Shrinkable> values = null;
	...
	if (keySet == null) {
	    keySet = new AbstractSet<K,Shrinkable>() {
		public Iterator<K,Shrinkable> iterator() {
		    return new Iterator<K,Shrinkable>() {
			private Iterator<Entry<K,V,M>,Shrinkable> i = 
	...
	if (values == null) {
	    values = new AbstractCollection<V,Shrinkable>() {
		public Iterator<V,Shrinkable> iterator() {
		    return new Iterator<V,Shrinkable>() {
			private Iterator<Entry<K,V,M>,Shrinkable> i = 
                        (Iterator) entrySet().iterator();
		...
		public  boolean add(V o) where M extends Resizable {
     ...
     */
    //    public abstract Set<Entry<K,V,M>,M> entrySet();
    ...
    protected Object clone() throws CloneNotSupportedException {
        AbstractMap<K,V,M> result = (AbstractMap<K,V,M>)super.clone();
        result.keySet = null;
        result.values = null;
        return result;
    }
     ...
     */
    static class SimpleEntry<K,V,M> implements Entry<K,V,M> {
	K key;
	V value;
